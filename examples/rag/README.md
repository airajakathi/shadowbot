...
This directory contains comprehensive examples demonstrating RAG (Retrieval Augmented Generation) with ShadowBot Agents.
...
```bash
# Index documents
shadowbot rag index ./documents --collection my_docs

# Query
shadowbot rag query "What is the main finding?" --collection my_docs

# Interactive chat
shadowbot rag chat --collection my_docs

# Evaluate
shadowbot rag eval golden_queries.json --collection my_docs
```
...
Use with:
```bash
shadowbot rag query "Question" --config rag_config.yaml
```
