...
Demonstrates how ShadowBot Agents uses lazy imports to achieve
...
def example_measure_memory():
    ...
    # Clear modules
    for key in list(sys.modules.keys()):
        if key.startswith('shadowbotagents'):
            del sys.modules[key]
...
def example_specific_imports():
    ...
    print("  from shadowbotagents import Agent, Task")
    
    # Avoid - star imports load everything
    print("\nAvoid - star imports load more:")
    print("  from shadowbotagents import *")
...
if __name__ == "__main__":
    print("ShadowBot Agents - Lazy Imports Example")
    print("=" * 60)
