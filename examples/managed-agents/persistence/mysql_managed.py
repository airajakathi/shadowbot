...
Prerequisites:
    pip install shadowbot shadowbotagents mysql-connector-python
...
from shadowbot import ManagedAgent, LocalManagedConfig, DB
from shadowbotagents import Agent

MYSQL_URL = os.getenv("MYSQL_URL", "mysql://root:praisontest@localhost:3307/shadowbot")
...
conn = mysql.connector.connect(host="localhost", port=3307, user="root", password="praisontest", database="shadowbot")
cursor = conn.cursor()
cursor.execute("SELECT table_name FROM information_schema.tables WHERE table_schema='shadowbot' AND table_name LIKE '%message%'")
