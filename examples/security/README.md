...
This example demonstrates ShadowBot security features including SBOM generation, lockfile validation, dependency auditing, and PII redaction.
...
shadowbot recipe sbom ./my-recipe --format cyclonedx -o sbom.json

# Generate SPDX SBOM
shadowbot recipe sbom ./my-recipe --format spdx -o sbom.json
```
...
```bash
# Audit dependencies
shadowbot recipe audit ./my-recipe

# Strict audit (fail on issues)
shadowbot recipe audit ./my-recipe --strict
```
...
shadowbot recipe validate ./my-recipe --require-lockfile
```
