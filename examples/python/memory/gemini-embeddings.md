# Gemini Embedding Support in ShadowBot

This document describes how to use Google's Gemini embedding models with ShadowBot's memory system.

## Overview

ShadowBot now supports configurable embedding models through LiteLLM, including Google's Gemini embedding models. This allows you to use models like `text-embedding-004` or `gemini-embedding-exp-03-07` for semantic search and memory storage.

## Configuration

### Default Configuration (OpenAI)

By default, ShadowBot uses OpenAI's `text-embedding-3-small` model:

```python
from shadowbotagents import ShadowBotAgents

agents = ShadowBotAgents(
    agents=[...],
    tasks=[...],
...
2. **Dependencies**: Ensure you have the memory dependencies installed:
   ```bash
   pip install "shadowbotagents[memory]"
   ```
...
```python
import os
from shadowbotagents import Agent, Task, ShadowBotAgents
