#!/usr/bin/env python3
"""
Remote Sandbox Examples for ShadowBot
...
from shadowbot.sandbox import SSHSandbox, ModalSandbox, DaytonaSandbox
from shadowbotagents.sandbox import ResourceLimits
...
async def ssh_example():
    ...
    try:
        sandbox = SSHSandbox(
            ...
            user=ssh_user,
            key_file=ssh_key,
            working_dir="/tmp/shadowbot_demo"
        )
        
        # Check availability
        if not sandbox.is_available:
            print("❌ SSH backend not available. Install with: pip install shadowbot[ssh]")
            return
...
print("Hello from remote SSH server! 🚀")
        ...
        # Write and read file
        print("\n📝 File operations...")
        await sandbox.write_file("/tmp/shadowbot_demo/hello.py", "print('Hello from file!')")
        content = await sandbox.read_file("/tmp/shadowbot_demo/hello.py")
        print(f"File content: {content}")
        
        # Execute the file
        result = await sandbox.execute_file("/tmp/shadowbot_demo/hello.py")
...
async def modal_example():
    ...
    try:
        ...
        # Check availability
        if not sandbox.is_available:
            print("❌ Modal backend not available. Install with: pip install shadowbot[modal]")
            return
...
async def daytona_example():
    ...
    try:
        ...
        # Check availability
        if not sandbox.is_available:
            print("❌ Daytona backend not available. Install with: pip install shadowbot[daytona]")
            return
...
async def main():
    """Run all examples."""
    print("🚀 ShadowBot Remote Sandbox Examples")
    print("=" * 70)
    ...
    print("4. Use these sandboxes in your ShadowBot agents!")
