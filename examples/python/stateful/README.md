...
This directory contains comprehensive examples demonstrating ShadowBot's stateful agents capabilities, including session management, memory systems, and persistent workflows.
...
**Key Features Shown:**
```python
workflow = ShadowBotAgents(agents=[...], memory=True, process="workflow")
...
1. Install ShadowBot with memory support:
```bash
...
### With Existing ShadowBot Features
...
workflow = ShadowBotAgents(..., verbose=1)
```
...
These examples provide a comprehensive foundation for building sophisticated stateful agent applications with ShadowBot.