...
⚠️ **Note**: Knowledge base functionality is temporarily disabled due to a compatibility issue with the current version of ShadowBot Agents.
...
with st.sidebar:
    ...
    new_url = st.text_input(
        "Add URL",
        placeholder="https://docs.shadowbot.com/introduction",
...
if openai_key:
    ...
    def load_knowledge() -> list:
        ...
        return ["https://docs.shadowbot.com/introduction/agents.md"]  # Default URL
    ...
    def load_agent(_knowledge: list) -> Agent:
        ...
        # TODO: Re-enable when ShadowBot Agents knowledge issue is resolved
        return Agent(
    ...
    with col1:
        if st.button("What is ShadowBot?", use_container_width=True):
            st.session_state.query = "What is ShadowBot and how do Agents work?"
    with col2:
        if st.button("Teams in ShadowBot", use_container_width=True):
            st.session_state.query = "What are Teams in ShadowBot and how do they work?"
    with col3:
...
with st.expander("📖 How This Works"):
    st.markdown("""
    **This app uses the ShadowBot Agents framework to create an intelligent Q&A system:**
    
    ⚠️ **Current Status**: Knowledge base functionality is temporarily disabled due to compatibility issues with the current version of ShadowBot Agents (v0.0.157).
    ...
    **What Works Now:**
    ...
    3. **Agent Framework**: ShadowBot Agents for orchestration
    ...
    **Key Components:**
    - `Agent`: ShadowBot Agents framework for creating intelligent agents
    - `llm`: OpenAI GPT-5-nano for generating responses
    
    **Why ShadowBot Agents?**
    ...
    **Next Steps:**
    - Monitor ShadowBot Agents updates for knowledge base fixes
