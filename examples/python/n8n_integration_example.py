"""Complete n8n Integration Example for ShadowBot

This example demonstrates the complete bidirectional n8n ↔ ShadowBot integration:

1. ShadowBot → n8n: Agents executing n8n workflows (using ShadowBot-Tools)
2. n8n → ShadowBot: n8n workflows invoking ShadowBot agents (using API endpoint)

Prerequisites:
1. Install dependencies:
   pip install "shadowbot-tools[n8n]" fastapi uvicorn
...
def setup_shadowbot_agents():
    """Set up ShadowBot agents with n8n tools."""
    print("🚀 Setting up ShadowBot agents with n8n capabilities...")
    
    try:
        from shadowbot_tools.n8n import n8n_workflow, n8n_list_workflows
        ...
        print("💡 Install with: pip install 'shadowbot-tools[n8n]'")
        return {}


def setup_api_server_with_agents(agents):
    """Set up FastAPI server with agent invoke endpoints."""
    print("🌐 Setting up API server for n8n → ShadowBot integration...")
    
    from shadowbot.api.agent_invoke import register_agent, router
    
    # Create FastAPI app
    app = FastAPI(
        title="ShadowBot n8n Integration Server",
        description="API server for n8n to invoke ShadowBot agents",
        version="1.0.0"
    )
    ...
    async def root():
        ...
        return {
            "message": "ShadowBot n8n Integration Server",
...
async def demo_shadowbot_to_n8n(agents):
    """Demonstrate ShadowBot agents calling n8n workflows."""
    print("\n🔄 Demo: ShadowBot → n8n Integration")
    print("=" * 50)
...
async def demo_n8n_to_shadowbot():
    """Demonstrate n8n invoking ShadowBot agents via HTTP."""
    print("\n🔄 Demo: n8n → ShadowBot Integration")
    print("=" * 50)
...
def create_n8n_workflow_examples():
    ...
    print("""
🔹 HTTP Request Node (n8n → ShadowBot):
   Method: POST
...
🔹 Webhook Trigger for ShadowBot response:
   URL: http://localhost:8000/webhook/shadowbot-response
   Method: POST
...
def run_server(port=8000):
    ...
    print(f"\n🚀 Starting ShadowBot n8n Integration Server on port {port}...")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="ShadowBot n8n Integration Demo")
    ...
    if args.server:
        ...
        # Run demos
        await demo_shadowbot_to_n8n(agents)
        create_n8n_workflow_examples()
        print("\n💡 To test n8n → ShadowBot, start the server:")
        ...
        print("🔗 ShadowBot ↔ n8n Bidirectional Integration")
        print("=" * 50)
        
        print("\n📋 Integration Components:")
        print("1. 🔧 ShadowBot-Tools: n8n workflow execution tools")
        print("2. 🌐 API Endpoint: Agent invoke endpoint for n8n")
        print("3. 🤖 Agents: ShadowBot agents with n8n capabilities")
