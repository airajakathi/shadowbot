...
ShadowBot now supports intelligent multi-provider and multi-model capabilities, allowing agents to automatically select the most appropriate AI model based on task requirements, cost considerations, and performance needs.
...
```python
from shadowbotagents import Task, ShadowBotAgents
...
```python
from shadowbotagents.llm import ModelRouter
...
```python
from shadowbotagents.llm import ModelProfile, TaskComplexity
...
```python
from shadowbotagents.agents import AutoAgents
