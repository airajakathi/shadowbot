#!/usr/bin/env python3
"""
Example: Fetch and search ShadowBot templates from the catalog.
...
def generate_cli_command(template: Dict, action: str = "run") -> str:
    ...
    if action == "run":
        return f"shadowbot templates run {name}"
    elif action == "info":
        return f"shadowbot templates info {name}"
    elif action == "init":
        return f"shadowbot templates init my-project --template {name}"
    return f"shadowbot templates {action} {name}"
...
def main():
    print("ShadowBot Template Catalog - Python Example")
    print("-" * 45)
    ...
    if not templates:
        ...
        print("   Try building locally: shadowbot templates catalog build")
        return
