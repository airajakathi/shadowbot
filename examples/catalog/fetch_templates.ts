...
const CATALOG_URL = "https://mervinpraison.github.io/shadowbot-template-catalog/data/templates.json";
...
function generateCLICommand(template: Template, action: string = "run"): string {
  ...
  switch (action) {
    case "run":
      return `shadowbot templates run ${name}`;
    case "info":
      return `shadowbot templates info ${name}`;
    case "init":
      return `shadowbot templates init my-project --template ${name}`;
    default:
      return `shadowbot templates ${action} ${name}`;
  }
}
...
async function main(): Promise<void> {
  console.log("ShadowBot Template Catalog - TypeScript Example");
