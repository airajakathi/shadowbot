...
CATALOG_URL = "https://mervinpraison.github.io/shadowbot-template-catalog/data/templates.json"
    ...
    for t in templates:
        if (query_lower in t.get("name", "").lower() or
            ...
            results.append({
                ...
                "tags": t.get("tags", []),
                "cli_command": f"shadowbot templates run {t.get('name')}"
            })
    ...
    for t in templates:
        if t.get("name") == name:
            return json.dumps({
                ...
                "cli_commands": {
                    "run": f"shadowbot templates run {name}",
                    "info": f"shadowbot templates info {name}",
                    "init": f"shadowbot templates init my-project --template {name}"
                }
            })
...
def main():
    print("ShadowBot Template Finder Agent")
    print("=" * 40)
    ...
    try:
        ...
        print("shadowbotagents not installed. Install with: pip install shadowbotagents")
        return
    
    # Create the Template Finder Agent
    agent = Agent(
        name="Template Finder",
        role="ShadowBot Template Expert",
        goal="Help users find the right template for their needs",
        backstory="""You are an expert on ShadowBot templates. You help users 
