...
def run_server():
    ...
    try:
        ...
        print("Install with: pip install 'shadowbotagents[api]'")
        sys.exit(1)
    ...
    # Create FastAPI app
    app = FastAPI(title="ShadowBot A2A Server")
...
def run_client():
    ...
    try:
        req = urllib.request.Request(f"{base_url}/__shadowbot__/discovery")
        with urllib.request.urlopen(req, timeout=5) as resp:
