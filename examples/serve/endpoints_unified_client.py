...
Demonstrates using the shadowbot endpoints CLI to interact with any ShadowBot server.
...
def run_cli(args: list) -> tuple:
    """Run shadowbot endpoints CLI command."""
    cmd = ["python3", "-m", "shadowbot.cli.main", "endpoints"] + args
...
def main():
    print("=" * 60)
    print("ShadowBot Unified Endpoints Client Example")
    print("=" * 60)
    ...
    if code == 0:
        ...
        print("  shadowbot serve unified --port 8765")
        print("  shadowbot serve agents --file agents.yaml --port 8765")
        print("  shadowbot serve a2a --port 8765")
        return 1
    else:
