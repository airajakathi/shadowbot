...
from shadowbot.recipe.server import create_wsgi_app
...
def main():
    ...
    try:
        ...
        result = subprocess.run(
            ["python3", "-m", "shadowbot", "recipe", "publish", str(bundle_path),
             "--registry", f"http://127.0.0.1:{port}", "--json"],
            capture_output=True, text=True,
            cwd="/Users/praison/shadowbot-package/src/shadowbot"
        )
        ...
        result = subprocess.run(
            ["python3", "-m", "shadowbot", "recipe", "list",
             "--registry", f"http://127.0.0.1:{port}", "--json"],
            capture_output=True, text=True,
            cwd="/Users/praison/shadowbot-package/src/shadowbot"
        )
        ...
        result = subprocess.run(
            ["python3", "-m", "shadowbot", "recipe", "search", "agent",
             "--registry", f"http://127.0.0.1:{port}", "--json"],
            capture_output=True, text=True,
            cwd="/Users/praison/shadowbot-package/src/shadowbot"
        )
        ...
        result = subprocess.run(
            ["python3", "-m", "shadowbot", "recipe", "pull", "sample-agent",
