# AGENTS.md - ShadowBot TypeScript SDK Comprehensive Guide

> **For AI Agents and Developers**: This document provides the complete context needed to work with the ShadowBot TypeScript SDK, including design principles, architecture, repository structure, and implementation guidelines.

---

## 1. What is ShadowBot TypeScript?

ShadowBot TypeScript is a **high-performance, agentic AI framework** for Node.js/JavaScript, designed for building production-ready AI agents and multi-agent workflows.
...
├── src/
│   ├── shadowbot-agents/          # Python SDK (shadowbotagents)
│   │   ├── shadowbotagents/       # Python package
│   │   ├── tests/                 # Unit & integration tests
│   │   └── examples/              # Package-level examples
│   │
│   ├── shadowbot-ts/              # TypeScript SDK (this package)
│   │   ├── src/                   # Source code
│   │   ├── tests/                 # Unit & integration tests
│   │   ├── examples/              # TypeScript examples
│   │   └── package.json           # Package config
│   │
│   └── shadowbot/                 # Wrapper (shadowbot CLI)
│
...
```typescript
import { AgentTeam, Agent } from 'shadowbot';
...
export type ShadowBotAgents = AgentTeam;
```
...
```typescript
import { Task, Agent } from 'shadowbot';
...
```typescript
import { tool, FunctionTool } from 'shadowbot';
...
ShadowBot TypeScript provides first-class support for Vercel AI SDK:
...
```typescript
import { Agent, tools } from 'shadowbot';
...
```typescript
import { Memory, FileMemory, AutoMemory } from 'shadowbot';
...
```typescript
import { Agent, db } from 'shadowbot';
...
```typescript
import { Agent, AgentCallbacks } from 'shadowbot';
...
```typescript
import { WorkflowHooks, createLoggingWorkflowHooks } from 'shadowbot';
...
```typescript
import { Task, Workflow, createContext } from 'shadowbot';
...
```typescript
import { Loop, loop } from 'shadowbot';
...
```typescript
import { Repeat, repeat } from 'shadowbot';
...
```typescript
import { MCPClient, createMCPClient } from 'shadowbot';
...
```typescript
import { MCPServer, createMCPServer } from 'shadowbot';
...
```typescript
import { createLangfuseAdapter, createLangSmithAdapter } from 'shadowbot';
...
```typescript
import { createGuardrail, GuardrailAction } from 'shadowbot';
...
```typescript
import { AudioAgent, createAudioAgent } from 'shadowbot';
...
```bash
# Install globally
npm install -g shadowbot

# Run agent
shadowbot-ts agent run --prompt "Hello"

# Multi-agent
shadowbot-ts agents run --config agents.yaml

# Start server
shadowbot-ts serve --port 8000
```
...
```typescript
// Most common
import { Agent, AgentTeam, Task, tool, tools } from 'shadowbot';

// Memory
import { Memory, FileMemory, AutoMemory, db } from 'shadowbot';

// Hooks
import { HookManager, AgentCallbacks, WorkflowHooks } from 'shadowbot';

// MCP
import { MCPClient, MCPServer, createMCPClient } from 'shadowbot';

// Observability
import { createLangfuseAdapter, createLangSmithAdapter } from 'shadowbot';

// Specialized agents
import { AudioAgent, VideoAgent, VisionAgent, CodeAgent } from 'shadowbot';

// Workflows
import { Workflow, Loop, Repeat, createContext } from 'shadowbot';

// RAG
import { RAG, createRAG, Knowledge } from 'shadowbot';

// Eval
import { Evaluator, EvalSuite, createEvaluator } from 'shadowbot';
```
...
// CLI: shadowbot-ts memory clear --session <id>
```

| Feature | API | CLI Equivalent |
|---------|-----|----------------|
| Chat | `agent.chat(prompt)` | `shadowbot-ts agent chat` |
| Multi-agent | `team.start(task)` | `shadowbot-ts agents run` |
| Server | `createServer()` | `shadowbot-ts serve` |
...
> Make ShadowBot the **best TypeScript agent framework in the world**
...
*This document is the source of truth for the ShadowBot TypeScript SDK architecture and design principles.*
