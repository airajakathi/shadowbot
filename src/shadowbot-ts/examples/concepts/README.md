# ShadowBot Concept Examples

This directory contains examples demonstrating core concepts of the ShadowBot package.
...
> **Note**: These examples assume you have installed the `shadowbot` package from npm. If you're running these examples from within the package source code, the imports will automatically use the local package code.
...
```typescript
import { Agent, Task, ShadowBotAgents } from 'shadowbot';
...
// Run the agent
const system = new ShadowBotAgents({
    agents: [agent],
    tasks: [task],
    verbose: true
});
