# ShadowBot Tool Registration Examples

This document demonstrates the three different ways to register tool functions in ShadowBot.
...
import { registerFunction } from 'shadowbot';
...
ShadowBot can use remote tools exposed via the Model Context Protocol over Server-Sent Events.

1. Start the Python SSE server in another terminal:
```bash
python ../../shadowbot-agents/tests/mcp-sse-direct-server.py --host 127.0.0.1 --port 8080
```
