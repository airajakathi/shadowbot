...
const CACHE_DIR = path.join(os.homedir(), '.shadowbot', 'cache');
...
async function showHelp(outputFormat: string): Promise<void> {
  ...
  if (outputFormat === 'json') {
    ...
    await pretty.dim('  shadowbot-ts cache set mykey "Hello World"');
    await pretty.dim('  shadowbot-ts cache get mykey');
    await pretty.dim('  shadowbot-ts cache list');
  }
}
