...
async function approveRequest(requestId: string, outputFormat: string): Promise<void> {
  if (!requestId) {
    throw new Error('Request ID required. Usage: shadowbot approval approve <requestId>');
  }
...
async function denyRequest(requestId: string, outputFormat: string): Promise<void> {
  if (!requestId) {
    throw new Error('Request ID required. Usage: shadowbot approval deny <requestId>');
  }
...
async function cancelRequest(requestId: string, outputFormat: string): Promise<void> {
  if (!requestId) {
    throw new Error('Request ID required. Usage: shadowbot approval cancel <requestId>');
  }
...
async function addAutoApprove(args: string[], outputFormat: string): Promise<void> {
  ...
  if (!toolName) {
    throw new Error('Tool name required. Usage: shadowbot approval auto-approve <toolName>');
  }
...
async function addAutoDeny(args: string[], outputFormat: string): Promise<void> {
  const toolName = args[0];
  if (!toolName) {
    throw new Error('Tool name required. Usage: shadowbot approval auto-deny <toolName>');
  }
...
async function showHelp(outputFormat: string): Promise<void> {
  const help = {
    ...
    examples: [
      'shadowbot approval status',
      'shadowbot approval pending',
      'shadowbot approval approve abc123',
      'shadowbot approval auto-approve readFile',
      'shadowbot approval interactive',
    ],
  };
