...
async function listTools(options: ToolsOptions, outputFormat: string): Promise<void> {
  ...
  if (outputFormat === 'json') {
    ...
    await pretty.dim('\nUse "shadowbot-ts tools info <id>" for details');
    await pretty.dim('Use "shadowbot-ts tools doctor" to check dependencies');
  }
}
...
async function toolInfo(args: string[], options: ToolsOptions, outputFormat: string): Promise<void> {
  ...
  if (!metadata && !registeredTool) {
    if (outputFormat === 'json') {
      ...
      await pretty.dim('Use "shadowbot-ts tools list" to see available tools');
    }
...
async function toolAdd(args: string[], options: ToolsOptions, outputFormat: string): Promise<void> {
  ...
  if (!packageOrPath) {
    if (outputFormat === 'json') {
      ...
      await pretty.dim('Usage: shadowbot-ts tools add <package-name>');
      await pretty.dim('       shadowbot-ts tools add ./path/to/tool.js');
    }
    process.exit(EXIT_CODES.INVALID_ARGUMENTS);
  }
