...
async function showHelp(outputFormat: string): Promise<void> {
  ...
  if (outputFormat === 'json') {
    ...
    await pretty.dim('  shadowbot-ts observability list');
    await pretty.dim('  shadowbot-ts observability doctor langfuse');
    await pretty.dim('  shadowbot-ts observability test memory');
  }
}
