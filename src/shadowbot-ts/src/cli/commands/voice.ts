...
async function showHelp(outputFormat: string): Promise<void> {
  ...
  if (outputFormat === 'json') {
    ...
    await pretty.dim('  shadowbot-ts voice speak "Hello World" --voice nova');
    await pretty.dim('  shadowbot-ts voice transcribe audio.mp3');
    await pretty.dim('  shadowbot-ts voice voices');
  }
}
