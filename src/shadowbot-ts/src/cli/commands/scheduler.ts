...
async function handleCreate(args: string[], options: Record<string, unknown>, isJson: boolean): Promise<void> {
  ...
  if (!name) {
    ...
    if (isJson) {
      ...
      console.log('Usage: shadowbot-ts scheduler create <name> --cron "* * * * *"');
    }
    process.exit(1);
  }
...
function showHelp(isJson: boolean): void {
  const help = {
    ...
    examples: [
      'shadowbot-ts scheduler create my-task --cron "*/5 * * * *"',
      'shadowbot-ts scheduler create backup --interval 3600',
      'shadowbot-ts scheduler list',
      'shadowbot-ts scheduler run <task-id>',
      'shadowbot-ts scheduler patterns'
    ]
  };
