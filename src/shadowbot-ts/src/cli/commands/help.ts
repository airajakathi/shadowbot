...
async function showGeneralHelp(outputFormat: string): Promise<void> {
  ...
  if (outputFormat === 'json') {
    ...
    await pretty.plain(`
ShadowBot TypeScript CLI v${CLI_SPEC_VERSION}

Usage:
  shadowbot-ts <command> [options]
...
Examples:
  shadowbot-ts chat "Hello, how are you?"
  shadowbot-ts chat "Write a poem" --stream
  shadowbot-ts run "Analyze this data" --agent my-agent.yaml
  shadowbot-ts providers
  shadowbot-ts help chat
`);
  }
}
...
async function showCommandHelp(command: string, outputFormat: string): Promise<void> {
  ...
  if (!spec) {
    ...
    await pretty.info('Run "shadowbot-ts help" to see available commands');
    return;
  }
...
Usage:
  shadowbot-ts ${command}${args.length ? ' ' + args.map(a => a.required ? `<${a.name}>` : `[${a.name}]`).join(' ') : ''} [options]`);
