...
export async function execute(args: string[], options: Record<string, unknown>): Promise<void> {
  ...
  const tui = createInteractiveTUI({
    ...
    historyFile,
    welcomeMessage: `ShadowBot Interactive Mode
Model: ${model || 'default'}
...
Usage:
  shadowbot-ts interactive [options]
...
Examples:
  shadowbot-ts interactive
  shadowbot-ts interactive --model gpt-4o
  shadowbot-ts interactive --history ~/.shadowbot_history
`);
}
