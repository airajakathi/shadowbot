 ...
 * - ShadowBotAgents → AgentTeam (with Agents, ShadowBotAgents as aliases)
...
describe('AgentTeam Rename', () => {
  ...
  describe('Silent Aliases', () => {
    it('should export ShadowBotAgents as alias for AgentTeam', async () => {
      const { AgentTeam, ShadowBotAgents } = await import('../../../src/agent/simple');
      expect(ShadowBotAgents).toBe(AgentTeam);
    });
  ...
  describe('Main Package Exports', () => {
    ...
    it('should export all aliases from main index', async () => {
      ...
      expect(pkg.AgentTeam).toBe(pkg.ShadowBotAgents);
    });
  });
});
...
describe('Backward Compatibility', () => {
  it('should allow instantiation with old name ShadowBotAgents', async () => {
    const { ShadowBotAgents, Agent } = await import('../../../src/agent/simple');
    const agent = new Agent({ instructions: 'Test agent' });
    const team = new ShadowBotAgents([agent]);
