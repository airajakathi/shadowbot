# AGENTS.md - ShadowBot SDK Comprehensive Guide

> **For AI Agents and Developers**: This document provides the complete context needed to work with the ShadowBot ecosystem, including design principles, architecture, repository structure, and implementation guidelines.

---

## 1. What is ShadowBot?

ShadowBot is a **high-performance, agentic AI framework** designed for building production-ready AI agents and multi-agent workflows.
...
├── src/
│   ├── shadowbot-agents/          # Core SDK (shadowbotagents)
│   │   ├── shadowbotagents/       # Python package
│   │   ├── tests/                 # Unit & integration tests
│   │   ├── examples/              # Package-level examples
│   │   └── pyproject.toml         # Package config
│   │
│   ├── shadowbot/                 # Wrapper (shadowbot)
│   │   ├── shadowbot/             # CLI, integrations, heavy impls
│   │   └── pyproject.toml
│   │
│   └── shadowbot-ts/              # TypeScript SDK
│       └── src/
│
...
│
/Users/praison/ShadowBot-tools/    # External tools/plugins
/Users/praison/ShadowBotDocs/      # Documentation (Mintlify)
    └── docs/
        ├── agents/
        ├── api/
        ├── capabilities/
...
├─────────────────────────────────────────────────────────────────┤
│                    shadowbotagents (Core SDK)                   │
│  Protocols • Hooks • Adapters • Base Classes • Decorators       │
├─────────────────────────────────────────────────────────────────┤
│                   shadowbot-tools (External)                    │
...
## 3. Core SDK Architecture (shadowbotagents)
...
```bash
grep -r "class.*Protocol" shadowbotagents/ --include="*.py"
```
...
```
Core SDK (shadowbotagents)          Wrapper (shadowbot)
━━━━━━━━━━━━━━━━━━━━━━━━━━━         ━━━━━━━━━━━━━━━━━━━
...
def use_chromadb():
    try:
        ...
        raise ImportError("Install with: pip install shadowbotagents[memory]")
...
from shadowbotagents import Agent, tool
...
from shadowbotagents import Agent, MemoryConfig, HooksConfig
...
from shadowbotagents import Agent, ExecutionConfig, MemoryConfig
...
from shadowbotagents import tool
...
from shadowbotagents.tools import BaseTool
...
**Base files:**
- `shadowbotagents/tools/base.py` - BaseTool class
- `shadowbotagents/tools/decorator.py` - @tool decorator
- `shadowbotagents/tools/registry.py` - Tool registry
...
```python
from shadowbotagents.hooks import add_hook, before_tool, after_tool
...
```python
from shadowbotagents.memory.protocols import MemoryProtocol
...
```python
from shadowbotagents import Agent
from shadowbotagents.tools import schedule_add, schedule_list, schedule_remove
...
- `scheduler/store.py` — `FileScheduleStore` (thread-safe JSON persistence at `~/.shadowbot/schedules/jobs.json`)
...
```python
# Base files in shadowbotagents/db/
from shadowbotagents.db import DbAdapter, AsyncDbAdapter
...
```bash
# Basic agent
shadowbot agent run --name "assistant" --prompt "Hello"

# With tools
shadowbot agent run --tools web_search --prompt "Search for Python"

# Multi-agent workflow
shadowbot workflow run --file workflow.yaml

# Server mode
shadowbot serve --port 8000
```
...
# With coverage
pytest --cov=shadowbotagents tests/
```
...
```python
# Most common
from shadowbotagents import Agent, Agents, Task, tool, Tools

# Configuration
from shadowbotagents import (
...
# Event Bus
from shadowbotagents.bus import EventBus, get_default_bus

# Memory
from shadowbotagents import Memory, Session

# Workflows
from shadowbotagents import Workflow, Route, Parallel, Loop
```
...
| What | Where |
|------|-------|
| Agent class | `shadowbotagents/agent/agent.py` |
| Tool decorator | `shadowbotagents/tools/decorator.py` |
| Base tool | `shadowbotagents/tools/base.py` |
| Hook system | `shadowbotagents/hooks/` |
| Memory protocols | `shadowbotagents/memory/protocols.py` |
| Event bus | `shadowbotagents/bus/bus.py` |
| Workflow engine | `shadowbotagents/workflows/workflows.py` |
| Policy engine | `shadowbotagents/policy/engine.py` |
| Scheduler module | `shadowbotagents/scheduler/` |
| Schedule tools | `shadowbotagents/tools/schedule_tools.py` |
...
BotOS is the multi-platform bot orchestration layer for ShadowBot. It follows the same protocol-driven, agent-centric philosophy as the rest of the SDK.
...
|-------|---------|---------|
| `BotOSProtocol` | `shadowbotagents` (Core SDK) | Interface contract — lightweight |
| `BotOSConfig` | `shadowbotagents` (Core SDK) | Configuration dataclass |
| `Bot` | `shadowbot` (Wrapper) | Single-platform bot wrapper |
| `BotOS` | `shadowbot` (Wrapper) | Multi-platform orchestrator |
| `_registry` | `shadowbot` (Wrapper) | Platform adapter registry |

### 12.3 Usage Patterns

```python
# === Level 1: Single bot, 3 lines ===
from shadowbot.bots import Bot
from shadowbotagents import Agent
...
from shadowbot.bots import BotOS
from shadowbotagents import Agent
...
from shadowbot.bots import BotOS, Bot
from shadowbotagents import Agent, AgentTeam, Task
...
from shadowbot.bots._registry import register_platform
...
| What | Where |
|------|-------|
| BotOSProtocol | `shadowbotagents/bots/protocols.py` |
| BotOSConfig | `shadowbotagents/bots/config.py` |
| Bot class | `shadowbot/bots/bot.py` |
| BotOS class | `shadowbot/bots/botos.py` |
| Platform registry | `shadowbot/bots/_registry.py` |
...
```python
from shadowbot.bots._registry import register_platform, list_platforms
...
> Make ShadowBot the **best agent framework in the world**
...
*This document is the source of truth for the ShadowBot SDK architecture and design principles.*
