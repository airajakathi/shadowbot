# Centralized Logging in ShadowBot Agents
...
Enhanced the existing `shadowbotagents/_logging.py` with:

1. **Consistent naming convention**: All loggers follow `shadowbotagents.<module>` pattern
...
from shadowbotagents import get_logger
...
```python
import os
from shadowbotagents import get_logger, configure_structured_logging
...
```json
{
  ...
  "level": "INFO", 
  "logger": "shadowbotagents.agent.agent",
...
# Configure in Python:
logging.getLogger('shadowbotagents.memory').setLevel(logging.DEBUG)
```
...
|-------|--------------------|
| `"my_module"` | `"shadowbotagents.my_module"` |
| `"shadowbotagents.existing"` | `"shadowbotagents.existing"` (unchanged) |
| `"shadowbot.other"` | `"shadowbotagents.other"` |
| `"__main__"` | `"shadowbotagents.main"` |
| `None` (auto-detect) | `"shadowbotagents.<current_module>"` |
...
```python
import os
from shadowbotagents import configure_structured_logging
...
```python
import logging
from shadowbotagents import get_logger, StructuredFormatter
...
jq 'select(.logger | startswith("shadowbotagents.agent"))' logs.json
...
- `shadowbotagents/plugins/builtin/logging_plugin.py` - Logging plugin with extra data
- `shadowbotagents/memory/memory.py` - Memory module with subsystem tagging

## Benefits

1. **Unified format**: All ShadowBot logs follow the same format
