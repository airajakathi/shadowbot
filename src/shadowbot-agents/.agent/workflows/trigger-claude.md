...
PROMPT=$(cat << 'EOF'
@claude
You are working on the ShadowBot SDK. Follow AGENTS.md strictly.
...
- CORE vs WRAPPER vs TOOLS ROUTING:
  1. Core SDK (shadowbotagents/): Only core protocols, base classes, decorators. No heavy implementations.
  2. Wrapper (shadowbot/): CLI, heavy implementations, optional dependencies.
  3. Tools (ShadowBot-Tools): Specific integrations, external tools (e.g. SurrealDB, Slack), and community extensions MUST go to the `MervinPraison/ShadowBot-Tools` repository. If the feature is a tool, clone `https://github.com/MervinPraison/ShadowBot-Tools`, implement it there, and create the PR using `gh pr create -R MervinPraison/ShadowBot-Tools`.
...
STEP 4 — TEST:
- Run: `cd src/shadowbot-agents && PYTHONPATH=. python -m pytest tests/ -x -q --timeout=30`
