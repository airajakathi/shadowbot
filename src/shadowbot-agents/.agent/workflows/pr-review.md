...
```
https://github.com/MervinPraison/ShadowBot/pull/<NUMBER>   → PR
https://github.com/MervinPraison/ShadowBot/issues/<NUMBER> → Issue
```
...
// turbo
```bash
cd /Users/praison/shadowbot-package && gh pr diff <NUMBER>
```
...
For each changed file, evaluate against **ShadowBot engineering principles** (from AGENTS.md):

| Principle | What to Check |
|-----------|---------------|
| **Protocol-driven core** | No heavy implementations in `shadowbotagents/` (core SDK) |
...
// turbo
```bash
cd /Users/praison/shadowbot-package && gh pr checkout <NUMBER>
```
...
python -m pytest src/shadowbot/tests/unit/test_cwe78_env_injection.py -v
...
```bash
cd /Users/praison/shadowbot-package && \
gh pr review <NUMBER> --approve --body "LGTM — reviewed diff, tests pass, follows ShadowBot engineering principles."
```
...
// turbo
```bash
cd /Users/praison/shadowbot-package && gh pr view <NUMBER> --json state,mergedAt
```
...
| Run tests | `cd src/shadowbot-agents && python -m pytest tests/ -q --tb=no` |
