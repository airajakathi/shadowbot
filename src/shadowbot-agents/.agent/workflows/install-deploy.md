---
description: Update install.sh and deploy ShadowBot to Kubernetes via Helm (CI/CD Pipeline)
---

# Install.sh Update & Helm Deployment Workflow

Complete guide for updating the ShadowBot install script and deploying to Kubernetes via the full CI/CD pipeline.
...
|---------------|---------------|----------|
| **install.sh** | `/Users/praison/shadowbot-package/src/shadowbot/scripts/install.sh` | Main bash installer |
| **install.ps1** | `/Users/praison/shadowbot-package/src/shadowbot/scripts/install.ps1` | PowerShell installer |

### CI/CD Pipeline (GitHub → ACR → Helm → K8s)
| File/Resource | Absolute Path | Purpose |
|---------------|---------------|----------|
| **GitHub Workflow** | `/Users/praison/Sites/localhost/shadowbot/.github/workflows/azure-pipeline.yaml` | CI/CD pipeline triggered on git push |
| **Dockerfile.php** | `/Users/praison/Sites/localhost/shadowbot/Dockerfile.php` | PHP container build |
| **Dockerfile.nginx** | `/Users/praison/Sites/localhost/shadowbot/Dockerfile.nginx` | Nginx container build |
| **Dockerfile.init** | `/Users/praison/Sites/localhost/shadowbot/Dockerfile.init` | Init container build |

### Helm Deployment
| File/Resource | Absolute Path | Purpose |
|---------------|---------------|----------|
| **Helm Charts** | `/Users/praison/repos/person/helmcharts/` | All Kubernetes deployments |
| **wb_azure Chart** | `/Users/praison/repos/person/helmcharts/charts/wb_azure/` | ShadowBot WordPress deployment |
| **Production Values** | `/Users/praison/Sites/localhost/shadowbot/values-shadowbot-production.yaml` | Production Helm config |
| **Local Values** | `/Users/praison/Sites/localhost/shadowbot/values-local.yaml` | Local development config |
| **Deployment Docs** | `/Users/praison/Sites/localhost/shadowbot/DEPLOYMENT.md` | Deployment documentation |
| **Deployment Makefile** | `/Users/praison/Sites/localhost/shadowbot/Makefile` | Deployment automation |
...
**File**: `/Users/praison/Sites/localhost/shadowbot/.github/workflows/azure-pipeline.yaml`
...
**What it does:**
1. **Syncs install.sh** from ShadowBot repository (only if changed!)
   - Checks out `MervinPraison/ShadowBot`
...
**In shadowbot site repository** (`https://github.com/MervinPraison/praison.ai`):
| Secret | Purpose |
|--------|---------|
...
```bash
cd /Users/praison/Sites/localhost/shadowbot

# Get the short SHA from the latest commit (matches the Docker tag)
SHORT_SHA=$(git rev-parse --short HEAD)

# Update values file with new image tag
helm upgrade shadowbot /Users/praison/repos/person/helmcharts/charts/wb_azure \
  -f values-shadowbot-production.yaml \
...
```bash
# Edit values-shadowbot-production.yaml to set image.tag
sed -i "s/tag: \".*\"/tag: \"$SHORT_SHA\"/" values-shadowbot-production.yaml

# Then commit and push the updated values
git add values-shadowbot-production.yaml
...
SHORT_SHA=$(git rev-parse --short HEAD)
NAMESPACE=default
RELEASE_NAME=shadowbot
CHART_PATH=/Users/praison/repos/person/helmcharts/charts/wb_azure
VALUES_FILE=/Users/praison/Sites/localhost/shadowbot/values-shadowbot-production.yaml

echo "🚀 Deploying ShadowBot with image tag: $SHORT_SHA"
...
**File to edit**: `/Users/praison/Sites/localhost/shadowbot/.github/workflows/azure-pipeline.yaml`
...
```yaml
  deploy:
    ...
    - name: Deploy with Helm
      run: |
        SHORT_SHA=$(git rev-parse --short HEAD)
        helm upgrade --install shadowbot \
    ...
    - name: Verify deployment
      run: |
        ...
        helm status shadowbot -n default
```
...
### When to Update
- New ShadowBot version released
...
cat /Users/praison/shadowbot-package/src/shadowbot/scripts/install.sh
```
...
- **Lines 451-550**: ShadowBot package installation
...
cd /Users/praison/shadowbot-package/src/shadowbot/scripts
bash install.sh --dry-run

# Test in Docker (full test)
bash /Users/praison/shadowbot-package/src/shadowbot/scripts/test-install-smoke.sh
```
...
bash -n /Users/praison/shadowbot-package/src/shadowbot/scripts/install.sh

# Check install.ps1 syntax (if pwsh available)
pwsh -Command "Get-Command /Users/praison/shadowbot-package/src/shadowbot/scripts/install.ps1"
```
...
helm upgrade --install shadowbot /Users/praison/repos/person/helmcharts/charts/wb_azure \
  -f values-shadowbot-production.yaml \
  --namespace default \
  --timeout 10m \
  --atomic
...
vim values-shadowbot-production.yaml
...
```yaml
# /Users/praison/Sites/localhost/shadowbot/values-shadowbot-production.yaml
...
# Manual checks
helm status shadowbot -n default
kubectl get pods -l app.kubernetes.io/instance=shadowbot -n default
kubectl get svc -l app.kubernetes.io/instance=shadowbot -n default
```
...
kubectl get pods -n default | grep shadowbot

# Services should be Active
kubectl get svc -n default | grep shadowbot

# Ingress should exist
kubectl get ingress -n default | grep shadowbot
...
# Or manually:
helm rollback shadowbot 1 -n default
```
...
```bash
cd /Users/praison/Sites/localhost/shadowbot
./scripts/adopt-resources.sh shadowbot default
```
...
kubectl annotate <resource-type> <name> \
  meta.helm.sh/release-name=shadowbot \
...
# Check pod logs
kubectl logs -l app.kubernetes.io/instance=shadowbot -n default
...
1. `/Users/praison/ShadowBotDocs/docs/installation.mdx` — Installation docs
2. `/Users/praison/shadowbot-package/src/shadowbot/scripts/README.md` — Script docs (if exists)
...
```bash
# Watch pods
kubectl get pods -l app.kubernetes.io/instance=shadowbot -n default -w

# Check resource usage
kubectl top pods -l app.kubernetes.io/instance=shadowbot -n default
```
...
1. Check `/Users/praison/Sites/localhost/shadowbot/DEPLOYMENT.md` for detailed docs
2. Check `/Users/praison/Sites/localhost/shadowbot/INFRASTRUCTURE_AS_CODE_SUMMARY.md` for quick reference
3. Run `helm status shadowbot -n default` for deployment status
...
**Key Point**: The workflow automatically syncs `install.sh` from `MervinPraison/ShadowBot` repository **only if changed**:

```yaml
- name: Checkout ShadowBot for install.sh
  uses: actions/checkout@v2
  with:
    repository: MervinPraison/ShadowBot
    path: shadowbot-package

- name: Sync install.sh to web folder (if changed)
  run: |
    # Only sync if files have actually changed
    if ! diff -q shadowbot-package/src/shadowbot/scripts/install.sh ./web/install.sh; then
      cp shadowbot-package/src/shadowbot/scripts/install.sh ./web/install.sh
    fi
```
...
| To Change | Edit File |
|-----------|-----------|
| Application code | `/Users/praison/Sites/localhost/shadowbot/` (PHP files) |
| Docker images | `Dockerfile.php`, `Dockerfile.nginx`, `Dockerfile.init` |
| CI/CD pipeline | `.github/workflows/azure-pipeline.yaml` |
| Helm deployment config | `values-shadowbot-production.yaml` |
| Install script | `/Users/praison/shadowbot-package/src/shadowbot/scripts/install.sh` |
...
```bash
# 1. Update install.sh
vim /Users/praison/shadowbot-package/src/shadowbot/scripts/install.sh

# 2. Test install.sh
bash /Users/praison/shadowbot-package/src/shadowbot/scripts/install.sh --dry-run

# 3. Commit and push
git add /Users/praison/shadowbot-package/src/shadowbot/scripts/install.sh
...
cd /Users/praison/Sites/localhost/shadowbot
make rollback
# Or: helm rollback shadowbot 1 -n default
```
