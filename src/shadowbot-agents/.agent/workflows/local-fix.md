...
// turbo
```bash
cd /Users/praison/shadowbot-package && git checkout main && git pull origin main
```
...
|-------|---------------|----------------------|
| **Core SDK** (`shadowbotagents/`) | Protocols, hooks, adapters, base classes, decorators, dataclasses | Heavy implementations, optional deps at module level |
| **Wrapper** (`shadowbot/`) | CLI commands, integrations, heavy impls, DB adapters, UI | Core logic |
| **Tools** (`ShadowBot-tools/`) | Pluggable tools, community extensions | Core or wrapper logic |
...
| Docs | `/Users/praison/ShadowBotDocs/` (read AGENTS.md first) |
...
python -c "import time; t=time.time(); import shadowbotagents; print(f'{(time.time()-t)*1000:.0f}ms')"
```
...
// turbo
```bash
cd /Users/praison/shadowbot-package && gh pr view --json state,title,url | cat
```
...
```bash
# Core SDK
cd /Users/praison/shadowbot-package/src/shadowbot-agents && shadowbot publish pypi
