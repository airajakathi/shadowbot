...
Research → Clone & Analyze → ShadowBot Analysis → Draft → Create Issue.
...
## Phase 3 — Analyze ShadowBot Codebase

### 3a. Identify relevant components

```bash
ls /Users/praison/shadowbot-package/src/shadowbot-agents/shadowbotagents/
ls /Users/praison/shadowbot-package/src/shadowbot-agents/shadowbotagents/tools/
ls /Users/praison/shadowbot-package/src/shadowbot-agents/shadowbotagents/agent/
```
...
|------|-----------------|
| `shadowbotagents/tools/shell_tools.py` | `execute_command`, approval decorators |
| `shadowbotagents/agent/agent.py` | `Agent.start()`, `Agent.chat()`, token tracking |
| `shadowbotagents/approval/` | `@require_approval`, auto-approval backend |
| `shadowbotagents/eval/` | Evaluation framework hooks |
| `shadowbotagents/agents/agents.py` | `AgentTeam` multi-agent |
| `pyproject.toml` | Current version, optional dependencies |

### 3b. Map integration points

For each ShadowBot component, document:
- What it does
...
## ShadowBot Architecture (Relevant to This Integration)
Table: Component | Location | Relevance

### ShadowBot Execution Model
How ShadowBot currently handles the relevant operations, and what must be adapted.
...
gh issue create \
  ...
  --title "Feature: Integrate ShadowBot Agents with Terminal-Bench 2.0 (Harbor Framework)" \
  --label "enhancement" \
  --body "..."
```
...
- [ ] ShadowBot codebase analyzed for integration points
