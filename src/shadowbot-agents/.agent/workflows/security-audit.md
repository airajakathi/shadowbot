...
#   - Repository: https://github.com/MervinPraison/ShadowBot
...
3. For each advisory, validate the vulnerability in source code:
   ...
   - Reference the official repository for latest source: https://github.com/MervinPraison/ShadowBot
...
```bash
cd /Users/praison/shadowbot-package/src/shadowbot-agents
...
```bash
cd /Users/praison/shadowbot-package
git add -A
...
9. Publish shadowbotagents (Core SDK):
```bash
cd /Users/praison/shadowbot-package/src/shadowbot-agents
shadowbot publish pypi
```
...
```bash
cd /Users/praison/shadowbot-package
git add -A && git commit -m "chore: bump shadowbotagents to <NEW_VERSION>" && git push origin main
```

11. Publish shadowbot (Wrapper):
```bash
cd /Users/praison/shadowbot-package/src/shadowbot
...
```bash
pip index versions shadowbotagents | head -1
pip index versions shadowbot | head -1
```
...
```bash
gh api repos/MervinPraison/ShadowBot/security-advisories/<GHSA_ID> \
  --method PATCH \
  --input - <<'EOF'
{
  "state": "draft",
...
EOF
```
- `<PACKAGE_NAME>`: `shadowbotagents` for core SDK, `shadowbot` for wrapper
...
```bash
gh api repos/MervinPraison/ShadowBot/security-advisories/<GHSA_ID>/cve --method POST
```
GitHub reviews and assigns CVE IDs within 1-3 business days.

16. Verify final state:
```bash
gh api repos/MervinPraison/ShadowBot/security-advisories --jq '.[] | "\(.ghsa_id)  \(.state)  \(.cve_id // "pending")  \(.severity)  \([.credits[].login] | join(",") )  \(.summary | .[0:50])"'
```
...
|---|---|---|
| shadowbotagents | src/shadowbot-agents/shadowbotagents/ | tools/python_tools.py, mcp/, memory/, agent/ |
| shadowbot | src/shadowbot/shadowbot/ | ui/sql_alchemy.py, capabilities/, mcp_server/, cli/ |

## RESOURCES & DOCUMENTATION

- **Official Repository**: https://github.com/MervinPraison/ShadowBot — Source code, issue tracking, and contribution guidelines
...
- **GitHub Security Advisories**: https://github.com/MervinPraison/ShadowBot/security/advisories — View and manage security advisories
...
- ALWAYS publish to PyPI BEFORE publishing advisories (so Dependabot can point users to a fix) - to publish `shadowbot publish pypi` from shadowbot-agents folder, after that 
`cd /Users/praison/shadowbot-package/src/shadowbot && python scripts/bump_and_release.py 4.5.125 --agents 1.5.125 --wait`. 4.5.125 is shadowbot version number and 1.5.125 is shadowbotagents version number. Last 2 numbers will be same 4/1.xx.xxx
...
1. Open a draft security advisory at https://github.com/MervinPraison/ShadowBot/security/advisories/new
