---
description: Audit existing docs against live code, then create a comprehensive GitHub issue in ShadowBotDocs with correct, tested information ready for a writer/agent to implement immediately
---

# Create ShadowBotDocs Issue Workflow

Audit code → Compare with existing docs → Draft correct content → Create issue.

> **When to use this:** A feature is already implemented in ShadowBot but the docs are missing, outdated, or wrong. You need to file a self-contained docs issue in `MervinPraison/ShadowBotDocs` that a writer or agent can act on without asking follow-up questions.
...
```bash
ls /Users/praison/ShadowBotDocs/docs/<section>/
cat /Users/praison/ShadowBotDocs/docs/<section>/<feature>.mdx
```
...
```bash
# Search shadowbotagents core SDK
grep -rn "<feature>" /Users/praison/shadowbot-package/src/shadowbot-agents/shadowbotagents/ 2>&1 | grep -v ".pyc"

# Search shadowbot wrapper (CLI, integrations)
grep -rn "<feature>" /Users/praison/shadowbot-package/src/shadowbot/shadowbot/ 2>&1 | grep -v ".pyc"

# Search shadowbot-tools
grep -rn "<feature>" /Users/praison/ShadowBot-tools/ 2>&1 | grep -v ".pyc"
```
...
|---|---|
| `shadowbotagents/obs/__init__.py` | `_LazyObsModule`, factory methods, `auto()` |
| `shadowbot_tools/observability/providers/<name>_provider.py` | `init()`, `is_available()`, `shutdown()`, `flush()` |
| `shadowbot/observability/<name>.py` | CLI sink adapter, event handling |
| `shadowbot/cli/commands/<name>.py` | All CLI sub-commands and options |
| `shadowbot/__init__.py` | Global env var overrides that may interfere |
...
python3 -c "from shadowbotagents.obs import obs; provider = obs.<name>(); print(type(provider).__name__, provider._initialized)"

# Test CLI commands
shadowbot <feature> --help
shadowbot <feature> <subcommand>

# Verify traces/output appear
timeout 30 shadowbot <feature> <subcommand> --limit 5
```
...
```bash
gh issue create \
  --repo MervinPraison/ShadowBotDocs \
...
```bash
gh issue view <number> --repo MervinPraison/ShadowBotDocs | head -30
```

Confirm the URL: `https://github.com/MervinPraison/ShadowBotDocs/issues/<number>`
