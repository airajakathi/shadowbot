---
description: create and publish a post on mer.vin using shadowbotwp
---

## Config

- Server: `default` → `https://mer.vin`
- Config: `~/.shadowbotwp/config.yaml`
- Category for ShadowBot features: `ShadowBot`
...
`shadowbotwp media upload` correctly SFTPs the file to `/tmp/` on the server, but
`_execute_wp()` in `~/shadowbotwp-cli/shadowbotwp/core/wp_client.py` never appends
...
```bash
shadowbotwp update <POST_ID> --category "ShadowBot"
```
...
```bash
shadowbotwp create "Feature Name: What It Does" \
  --status publish \
...
<!-- wp:code -->
<pre class="wp-block-code"><code>from shadowbotagents import Agent
agent = Agent(name="example")
result = agent.start("task")</code></pre>
<!-- /wp:code -->'

# Then set category:
shadowbotwp update <ID> --category "ShadowBot"
```
