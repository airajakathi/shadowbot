#!/usr/bin/env python3
"""
ShadowBot Agents - Detailed Profiling Benchmark

Provides function-level profiling for ShadowBot agent execution.
...
Usage:
    ...
    python benchmarks/detailed_profiling_benchmark.py --framework shadowbot
...
def profile_shadowbot_detailed(model, prompt):
    """Profile ShadowBot agent execution in detail."""
    print("\n" + "=" * 80)
    print("DETAILED PROFILING: ShadowBot")
    print("=" * 80)
    ...
    print("-" * 80)
    breakdown = profiler.get_detailed_breakdown(filter_pattern='shadowbot', limit=20)
    ...
    print("-" * 80)
    breakdown = profiler2.get_detailed_breakdown(filter_pattern='shadowbot', limit=30)
    print(f"{'Function':<40} {'File':<50} {'Calls':>8} {'CumTime':>10}")
    print("-" * 80)
    for item in breakdown:
        func = item['function'][:38]
    ...
    # ShadowBot-specific functions
    print("\n--- ShadowBot-Specific Functions ---")
    print("-" * 80)
    shadowbot_funcs = profiler3.get_detailed_breakdown(filter_pattern='shadowbot', limit=50)
    print(f"{'Function':<40} {'File':<50} {'Calls':>8} {'CumTime':>10}")
    print("-" * 80)
    for item in shadowbot_funcs:
        func = item['function'][:38]
        file = item['file'].split('/')[-1][:48] if '/' in item['file'] else item['file'][:48]
        print(f"{func:<40} {file:<50} {item['calls']:>8} {item['cumulative_time']*1000:>9.2f}ms")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='ShadowBot Agents - Detailed Profiling Benchmark')
    parser.add_argument('--framework', '-f', type=str, default='all', 
                        choices=['all', 'shadowbot', 'agno', 'crewai'],
    ...
    print("=" * 80)
    print("ShadowBot Agents - Detailed Profiling Benchmark")
    print("=" * 80)
    print(f"\nModel: {args.model}")
    ...
    if args.framework in ['all', 'shadowbot']:
        try:
            results['ShadowBot'] = profile_shadowbot_detailed(args.model, args.prompt)
        except Exception as e:
            print(f"ShadowBot error: {e}")
            import traceback
            traceback.print_exc()
