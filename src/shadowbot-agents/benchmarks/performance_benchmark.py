"""
ShadowBot Agents - Comprehensive Performance Benchmark
...
def instantiate_shadowbot_agent():
    """Instantiate a ShadowBot agent with one tool."""
    return PraisonAgent(
        name="Test Agent",
...
def instantiate_shadowbot_litellm_agent():
    """Instantiate a ShadowBot agent with LiteLLM backend."""
    return PraisonAgent(
        name="Test Agent",
        instructions="Be concise, reply with one sentence.",
...
def save_benchmark_results(results: dict, baseline):
    ...
    display_names = {
        'agno': 'Agno',
        'shadowbot': 'ShadowBot',
        'shadowbot_litellm': 'ShadowBot (LiteLLM)',
        'pydantic': 'PydanticAI',
    ...
    with open(filepath, 'w') as f:
        f.write('# ShadowBot Agents - Benchmark Results\n\n')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='ShadowBot Agents - Comprehensive Performance Benchmark')
    parser.add_argument('--save', action='store_true', help='Save results to file')
    args = parser.parse_args()
    
    print("="*70)
    print("ShadowBot Agents - Comprehensive Performance Benchmark")
    print("="*70)
    ...
    # Test ShadowBot first
    try:
        print("\n" + "="*70)
        print("Testing ShadowBot Agents")
        print("="*70)
        results['shadowbot'] = run_benchmark(
            instantiate_shadowbot_agent,
            "ShadowBot",
            iterations=1000,
            warmup=10
        )
    except Exception as e:
        print(f"Error testing ShadowBot: {e}")
    
    try:
        print("\n" + "="*70)
        print("Testing ShadowBot Agents (LiteLLM)")
        print("="*70)
        results['shadowbot_litellm'] = run_benchmark(
            instantiate_shadowbot_litellm_agent,
            "ShadowBot (LiteLLM)",
            iterations=1000,
            warmup=10
        )
    except Exception as e:
        print(f"Error testing ShadowBot (LiteLLM): {e}")
    ...
    if 'shadowbot' in results:
        baseline = results['shadowbot']
