#!/usr/bin/env python3
"""
ShadowBot Agents - Real Execution Benchmark
...
def benchmark_shadowbot(model, iterations, prompt):
    """Benchmark ShadowBot agent execution using agent.start()"""
...
def benchmark_shadowbot_litellm(model, iterations, prompt):
    """Benchmark ShadowBot with LiteLLM backend using agent.start()"""
    from shadowbotagents import Agent
...
def save_results(results: dict, model: str, iterations: int, prompt: str):
    ...
    with open(filepath, 'w') as f:
        f.write('# ShadowBot Agents - Real Execution Benchmark\n\n')
        ...
        methods = {
            'ShadowBot': 'agent.start()',
            'ShadowBot (LiteLLM)': 'agent.start()',
            'Agno': 'agent.run()',
            'CrewAI': 'crew.kickoff()',
        }
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='ShadowBot Agents - Real Execution Benchmark')
    ...
    print('=' * 60)
    print('ShadowBot Agents - Real Execution Benchmark')
    print('=' * 60)
    print(f'\nModel: {model}')
    ...
    # Run benchmarks
    try:
        results['ShadowBot'] = benchmark_shadowbot(model, iterations, prompt)
    except Exception as e:
        print(f"ShadowBot error: {e}")
    
    try:
        results['ShadowBot (LiteLLM)'] = benchmark_shadowbot_litellm(model, iterations, prompt)
    except Exception as e:
        print(f"ShadowBot (LiteLLM) error: {e}")
