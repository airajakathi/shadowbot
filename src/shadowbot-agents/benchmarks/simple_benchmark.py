#!/usr/bin/env python3
"""
ShadowBot Agents - Performance Benchmark
...
def run_benchmark():
    ...
    print('=' * 60)
    print('ShadowBot Agents - Performance Benchmark')
    print('=' * 60)
    print(f'\nIterations: {ITERATIONS}')
    print('Metric: Agent instantiation time (microseconds)\n')
    
    # ShadowBot (without tools)
    print("Testing ShadowBot...")
    from shadowbotagents import Agent as PraisonAgent
    
    results['ShadowBot'] = measure_instantiation(
        lambda: PraisonAgent(name='Test', llm='gpt-4o-mini', output="silent")
    )
    
    results['ShadowBot (LiteLLM)'] = measure_instantiation(
    ...
    baseline = results.get('ShadowBot', 1)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='ShadowBot Agents - Performance Benchmark')
