"""
ShadowBot Agents - Response Time Benchmark
...
def benchmark_shadowbot_response(iterations: int = 5) -> ResponseResult:
    """Benchmark ShadowBot agent response time"""
    from shadowbotagents import Agent
    
    result = ResponseResult()
    
    for i in range(iterations):
        print(f"  ShadowBot iteration {i+1}/{iterations}...")
...
def benchmark_shadowbot_with_tool(iterations: int = 5) -> ResponseResult:
    """Benchmark ShadowBot agent response time with tool calling"""
    from shadowbotagents import Agent
    
    result = ResponseResult()
    
    for i in range(iterations):
        print(f"  ShadowBot (with tool) iteration {i+1}/{iterations}...")
...
if __name__ == "__main__":
    print("="*70)
    print("ShadowBot Agents - Response Time Benchmark")
    print("="*70)
    print("\nModel: gpt-4o-mini")
    ...
    print("\nBenchmarking ShadowBot...")
    try:
        results['shadowbot_simple'] = benchmark_shadowbot_response(iterations)
    ...
    print("\nBenchmarking ShadowBot with tool...")
    try:
        results['shadowbot_tool'] = benchmark_shadowbot_with_tool(iterations)
    except Exception as e:
        print(f"Error: {e}")
        import traceback
        traceback.print_exc()
    ...
    print(f"\n{'Test':<30} {'ShadowBot (avg)':<20} {'Agno (avg)':<20} {'Ratio':<15}")
    print("-" * 85)
    
    if 'shadowbot_simple' in results and 'agno_simple' in results:
        p_time = results['shadowbot_simple'].avg_time
    ...
    if 'shadowbot_tool' in results and 'agno_tool' in results:
        p_time = results['shadowbot_tool'].avg_time
