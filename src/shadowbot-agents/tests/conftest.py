"""
Pytest configuration for ShadowBot Agents tests.
...
def _reset_circuit_breaker_registry():
    ...
    yield
    try:
        from shadowbotagents.llm.circuit_breaker import CircuitBreaker
...
def _reset_display_callbacks():
    ...
    """
    try:
        import shadowbotagents.main as _main
...
def _reset_module_shadowing():
    """Remove submodules from shadowbotagents namespace after tests to restore __getattr__.
    
    If 'from shadowbotagents.embedding import xyz' is called, Python adds the 'embedding'
    module object to shadowbotagents.__dict__, which overrides the __getattr__ proxy for
    ...
    try:
        ...
        if hasattr(shadowbotagents, 'embedding') and isinstance(shadowbotagents.embedding, type(sys)):
            delattr(shadowbotagents, 'embedding')
    except (ImportError, AttributeError):
        pass
