...
Tests save/load/list/format for cost reports in ~/.shadowbot/costs/.
"""
...
class TestSaveCostReport:
    ...
    def test_save_creates_file(self, tmp_path):
        from shadowbotagents.agent.cost_persistence import save_cost_report
        agent = MagicMock()
        agent.name = "test_agent"
        agent.cost_summary = {"tokens_in": 100, "tokens_out": 50, "cost": 0.001, "llm_calls": 1}
        with patch("shadowbotagents.agent.cost_persistence.COST_DIR", tmp_path):
...
class TestLoadCostReport:
    ...
    def test_load_existing(self, tmp_path):
        from shadowbotagents.agent.cost_persistence import load_cost_report
...
class TestListCostReports:
    ...
    def test_list_empty(self, tmp_path):
        from shadowbotagents.agent.cost_persistence import list_cost_reports
        with patch("shadowbotagents.agent.cost_persistence.COST_DIR", tmp_path):
...
class TestFormatCostTable:
    ...
    def test_format_empty(self):
        from shadowbotagents.agent.cost_persistence import format_cost_table
