"""
Tests for namespace package guard in shadowbotagents.

These tests verify that shadowbotagents properly detects when it's loaded
...
class TestNamespaceGuard:
    ...
    def test_shadowbotagents_has_file_attribute(self):
        """Verify shadowbotagents.__file__ is not None (not a namespace package)."""
        import shadowbotagents
        assert shadowbotagents.__file__ is not None, (
            "shadowbotagents.__file__ is None - this indicates it's loaded as a "
            "namespace package. Remove stale shadowbotagents directory from site-packages."
        )
    
    def test_shadowbotagents_has_init_py(self):
        """Verify shadowbotagents has a proper __init__.py."""
        import shadowbotagents
        import os
        
        # __file__ should point to __init__.py
        if shadowbotagents.__file__:
            assert shadowbotagents.__file__.endswith("__init__.py"), (
                f"Expected __init__.py, got {shadowbotagents.__file__}"
            )
            assert os.path.exists(shadowbotagents.__file__), (
                f"__init__.py does not exist at {shadowbotagents.__file__}"
            )
    
    def test_agent_import_succeeds(self):
        """Verify Agent can be imported from shadowbotagents."""
