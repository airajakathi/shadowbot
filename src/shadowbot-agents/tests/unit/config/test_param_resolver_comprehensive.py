...
class TestDictStrictValidation:
    ...
    def test_dict_validation_non_dataclass_config(self):
        ...
        from shadowbotagents.config.param_resolver import _get_valid_keys
...
class TestAgentConsolidatedParams:
    ...
    def test_agent_hooks_via_resolver(self):
        ...
        from shadowbotagents.config.feature_configs import HooksConfig
        from shadowbotagents.config.param_resolver import resolve, ArrayMode
    ...
    def test_agent_skills_via_resolver(self):
        ...
        from shadowbotagents.config.feature_configs import SkillsConfig
        from shadowbotagents.config.param_resolver import resolve
    ...
    def test_agent_knowledge_via_resolver(self):
        ...
        from shadowbotagents.config.feature_configs import KnowledgeConfig
        from shadowbotagents.config.param_resolver import resolve, ArrayMode
    ...
    def test_agent_guardrails_via_resolver(self):
        ...
        from shadowbotagents.config.feature_configs import GuardrailConfig
        from shadowbotagents.config.param_resolver import resolve, ArrayMode
        from shadowbotagents.config.presets import GUARDRAIL_PRESETS
...
class TestNamingAlias:
    """Tests for Agents vs ShadowBotAgents naming.
    
    v4.0.0 Updates:
    - Agents is now a SILENT alias for AgentManager (no deprecation warning)
    - ShadowBotAgents has been REMOVED entirely (raises ImportError)
    """
    
    def test_agents_is_silent_alias(self):
        """Agents should be available as silent alias for AgentManager."""
        from shadowbotagents import AgentManager, Agents
        # Agents is now a silent alias
        assert Agents is AgentManager
    
    def test_shadowbotagents_removed_v4(self):
        """ShadowBotAgents was removed in v4 - should raise ImportError."""
        import pytest
        with pytest.raises(ImportError):
            from shadowbotagents import ShadowBotAgents
