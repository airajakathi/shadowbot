...
class TestTokenUsageSinkProtocol:
    ...
    def test_protocol_exists(self):
        from shadowbotagents.telemetry.protocols import TokenUsageSinkProtocol
        assert TokenUsageSinkProtocol is not None

    def test_noop_satisfies_protocol(self):
        from shadowbotagents.telemetry.protocols import TokenUsageSinkProtocol, NoOpTokenUsageSink
        sink = NoOpTokenUsageSink()
        assert isinstance(sink, TokenUsageSinkProtocol)

    def test_inmemory_satisfies_protocol(self):
        from shadowbotagents.telemetry.protocols import TokenUsageSinkProtocol, InMemoryTokenUsageSink
...
class TestNoOpTokenUsageSink:
    ...
    def test_persist_is_noop(self):
        from shadowbotagents.telemetry.protocols import NoOpTokenUsageSink
        from shadowbotagents.telemetry.token_collector import TokenMetrics
        sink = NoOpTokenUsageSink()
        # Should not raise
        sink.persist(
            task_id="t1",
...
class TestInMemoryTokenUsageSink:
    ...
    def test_persist_stores_record(self):
        from shadowbotagents.telemetry.protocols import InMemoryTokenUsageSink
        from shadowbotagents.telemetry.token_collector import TokenMetrics
        sink = InMemoryTokenUsageSink()
        sink.persist(
            task_id="t1",
...
class TestTokenCollectorSinkIntegration:
    ...
    def test_collector_with_sink(self):
        from shadowbotagents.telemetry.token_collector import TokenCollector, TokenMetrics
        from shadowbotagents.telemetry.protocols import InMemoryTokenUsageSink
        sink = InMemoryTokenUsageSink()
