...
class TestLearningToolImports:
    ...
    def test_import_from_tools_module(self):
        from shadowbotagents.tools import store_learning, search_learning
        assert callable(store_learning)
        assert callable(search_learning)

    def test_import_directly(self):
        from shadowbotagents.tools.learning import store_learning, search_learning
        assert callable(store_learning)
        assert callable(search_learning)

    def test_no_collision_with_learn_package(self):
        """tools.learning and memory.learn are separate Python paths."""
        from shadowbotagents.tools.learning import store_learning
        from shadowbotagents.memory.learn.manager import LearnManager
...
class TestStoreLearning:
    ...
    def _make_state(self, learn_manager=None):
        from shadowbotagents.tools.injected import AgentState
        return AgentState(
            agent_id="test",
            run_id="run-1",
            session_id="sess-1",
    ...
    def test_store_no_state(self):
        ...
        from shadowbotagents.tools.learning import store_learning
        with patch("shadowbotagents.tools.learning.get_current_state", return_value=None):
...
class TestSearchLearning:
    ...
    def test_search_with_results(self):
        from shadowbotagents.tools.learning import search_learning
        mgr = MagicMock()
