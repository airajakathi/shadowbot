...
from shadowbotagents.auth.subscription.claude_code import (
    ClaudeCodeAuth,
...
def test_file_path_reads_credentials(monkeypatch):
    ...
    try:
        with patch('shadowbotagents.auth.subscription.claude_code.Path') as mock_path:
...
def test_resolve_credentials_no_auth_raises_error():
    ...
    with patch.dict(os.environ, {}, clear=True):
        with patch('shadowbotagents.auth.subscription.claude_code._read_keychain_credentials', return_value=None):
            with patch('shadowbotagents.auth.subscription.claude_code._read_file_credentials', return_value=None):
