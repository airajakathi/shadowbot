...
class TestTaskMessageSinkProtocol:
    ...
    def test_protocol_exists(self):
        from shadowbotagents.task.message_sink import TaskMessageSinkProtocol
        assert TaskMessageSinkProtocol is not None

    def test_noop_satisfies_protocol(self):
        from shadowbotagents.task.message_sink import TaskMessageSinkProtocol, NoOpTaskMessageSink
        sink = NoOpTaskMessageSink()
        assert isinstance(sink, TaskMessageSinkProtocol)

    def test_inmemory_satisfies_protocol(self):
        from shadowbotagents.task.message_sink import TaskMessageSinkProtocol, InMemoryTaskMessageSink
...
class TestTaskMessage:
    ...
    def test_message_creation(self):
        from shadowbotagents.task.message_sink import TaskMessage
        msg = TaskMessage(
            task_id="t1",
            seq_num=1,
...
class TestNoOpTaskMessageSink:
    ...
    def test_emit_is_noop(self):
        from shadowbotagents.task.message_sink import NoOpTaskMessageSink, TaskMessage
        sink = NoOpTaskMessageSink()
    ...
    def test_replay_returns_empty(self):
        from shadowbotagents.task.message_sink import NoOpTaskMessageSink
        sink = NoOpTaskMessageSink()
...
class TestInMemoryTaskMessageSink:
    ...
    def test_emit_stores_message(self):
        from shadowbotagents.task.message_sink import InMemoryTaskMessageSink, TaskMessage
        sink = InMemoryTaskMessageSink()
...
class TestTaskMessageEmitter:
    ...
    def test_emitter_auto_sequences(self):
        from shadowbotagents.task.message_sink import InMemoryTaskMessageSink, TaskMessageEmitter
        sink = InMemoryTaskMessageSink()
    ...
    def test_emitter_with_noop_sink(self):
        ...
        from shadowbotagents.task.message_sink import NoOpTaskMessageSink, TaskMessageEmitter
        sink = NoOpTaskMessageSink()
        emitter = TaskMessageEmitter(task_id="t1", sink=sink)
