...
class TestAdapterRegistry:
    ...
    def _make_registry(self):
        from shadowbotagents.utils.adapter_registry import AdapterRegistry
...
class TestMemoryAdapterRegistry:
    def test_imports(self):
        from shadowbotagents.memory.adapters.registry import (
            MemoryAdapterRegistry,
    ...
    def test_core_adapters_registered(self):
        from shadowbotagents.memory.adapters import list_memory_adapters
        adapters = list_memory_adapters()
        assert "sqlite" in adapters
        assert "in_memory" in adapters

    def test_get_in_memory_adapter(self):
        from shadowbotagents.memory.adapters import get_memory_adapter
        adapter = get_memory_adapter("in_memory")
        assert adapter is not None

    def test_get_first_available_defaults(self):
        from shadowbotagents.memory.adapters import get_first_available_memory_adapter
    ...
    def test_custom_registry_isolation(self):
        ...
        from shadowbotagents.memory.adapters.registry import MemoryAdapterRegistry
        ...
        from shadowbotagents.memory.adapters.registry import list_memory_adapters
...
class TestKnowledgeAdapterRegistry:
    def test_imports(self):
        from shadowbotagents.knowledge.adapters.registry import (
            KnowledgeAdapterRegistry,
...
class TestInMemoryAdapter:
    def _adapter(self):
        from shadowbotagents.memory.adapters.in_memory_adapter import InMemoryAdapter
...
class TestLLMProviderAdapters:
    def test_imports(self):
        from shadowbotagents.llm.adapters import (
            DefaultAdapter,
            OllamaAdapter,
            AnthropicAdapter,
            GeminiAdapter,
    ...
    def test_default_adapter_implements_protocol(self):
        from shadowbotagents.llm.adapters import DefaultAdapter
        from shadowbotagents.llm.protocols import LLMProviderAdapterProtocol
        adapter = DefaultAdapter()
        assert isinstance(adapter, LLMProviderAdapterProtocol)

    def test_ollama_adapter_implements_protocol(self):
        from shadowbotagents.llm.adapters import OllamaAdapter
        from shadowbotagents.llm.protocols import LLMProviderAdapterProtocol
        adapter = OllamaAdapter()
        assert isinstance(adapter, LLMProviderAdapterProtocol)

    def test_anthropic_adapter_prompt_caching(self):
        from shadowbotagents.llm.adapters import AnthropicAdapter
    ...
    def test_ollama_threshold_matches_production(self):
        ...
        from shadowbotagents.llm.adapters import OllamaAdapter
        from shadowbotagents.llm.llm import LLM
        adapter = OllamaAdapter()
    ...
    def test_gemini_adapter_formats_internal_tools(self):
        ...
        from shadowbotagents.llm.adapters import GeminiAdapter
        from shadowbotagents.llm.model_capabilities import GEMINI_INTERNAL_TOOLS
...
class TestGetProviderAdapter:
    def test_claude_model_returns_anthropic_adapter(self):
        from shadowbotagents.llm.adapters import get_provider_adapter, AnthropicAdapter
    ...
    def test_ollama_prefix_returns_ollama_adapter(self):
        from shadowbotagents.llm.adapters import get_provider_adapter, OllamaAdapter
    ...
    def test_gemini_prefix_returns_gemini_adapter(self):
        from shadowbotagents.llm.adapters import get_provider_adapter, GeminiAdapter
    ...
    def test_unknown_model_returns_default_adapter(self):
        from shadowbotagents.llm.adapters import get_provider_adapter, DefaultAdapter
