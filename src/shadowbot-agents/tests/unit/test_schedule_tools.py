...
class TestScheduleModels:
    ...
    def test_schedule_every_creation(self):
        from shadowbotagents.scheduler.models import Schedule
    ...
    def test_schedule_job_creation(self):
        from shadowbotagents.scheduler.models import ScheduleJob, Schedule
...
class TestFileScheduleStore:
    ...
    def _make_store(self, tmp_path):
        from shadowbotagents.scheduler.store import FileScheduleStore
...
class TestScheduleParser:
    ...
    def test_parse_hourly(self):
        from shadowbotagents.scheduler.parser import parse_schedule
...
class TestScheduleTools:
    ...
    def test_schedule_add_returns_confirmation(self, tmp_path):
        from shadowbotagents.tools.schedule_tools import schedule_add
        with patch('shadowbotagents.tools.schedule_tools._get_store') as mock_gs:
            from shadowbotagents.scheduler.store import FileScheduleStore
    ...
    def test_schedule_list_returns_jobs(self, tmp_path):
        from shadowbotagents.tools.schedule_tools import schedule_list, schedule_add
        with patch('shadowbotagents.tools.schedule_tools._get_store') as mock_gs:
            from shadowbotagents.scheduler.store import FileScheduleStore
    ...
    def test_schedule_remove_returns_confirmation(self, tmp_path):
        from shadowbotagents.tools.schedule_tools import schedule_add, schedule_remove
        with patch('shadowbotagents.tools.schedule_tools._get_store') as mock_gs:
            from shadowbotagents.scheduler.store import FileScheduleStore
            store = FileScheduleStore(store_dir=str(tmp_path))
            mock_gs.return_value = store
    ...
    def test_schedule_remove_nonexistent(self, tmp_path):
        from shadowbotagents.tools.schedule_tools import schedule_remove
        with patch('shadowbotagents.tools.schedule_tools._get_store') as mock_gs:
            from shadowbotagents.scheduler.store import FileScheduleStore
            mock_gs.return_value = FileScheduleStore(store_dir=str(tmp_path))
    ...
    def test_schedule_list_empty(self, tmp_path):
        from shadowbotagents.tools.schedule_tools import schedule_list
        with patch('shadowbotagents.tools.schedule_tools._get_store') as mock_gs:
            from shadowbotagents.scheduler.store import FileScheduleStore
            mock_gs.return_value = FileScheduleStore(store_dir=str(tmp_path))
            result = schedule_list()
...
class TestScheduleRunner:
    ...
    def test_runner_creation(self, tmp_path):
        from shadowbotagents.scheduler.runner import ScheduleRunner
        from shadowbotagents.scheduler.store import FileScheduleStore
        store = FileScheduleStore(store_dir=str(tmp_path))
...
class TestScheduleHookEvents:
    ...
    def test_schedule_hook_events_exist(self):
        from shadowbotagents.hooks.types import HookEvent
...
class TestLazyImport:
    ...
    def test_shadowbotagents_import_no_scheduler(self):
        """Importing shadowbotagents should NOT load scheduler module."""
...
class TestScheduleLoop:
    ...
    def test_schedule_loop_fires_due_jobs(self, tmp_path):
        ...
        from shadowbotagents.scheduler.loop import ScheduleLoop
        from shadowbotagents.scheduler.store import FileScheduleStore
        from shadowbotagents.scheduler.models import ScheduleJob, Schedule
        import threading
    ...
    def test_schedule_loop_lazy_import(self):
        ...
        from shadowbotagents.scheduler import ScheduleLoop
