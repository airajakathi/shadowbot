...
NOTE: These tests require the shadowbot package to be installed.
They are skipped if shadowbot is not available.
"""

import pytest
from unittest.mock import patch, MagicMock

# Skip entire module if shadowbot is not installed
pytest.importorskip("shadowbot", reason="shadowbot package not installed")
...
class TestBotCapabilities:
    ...
    def test_bot_capabilities_defaults(self):
        ...
        from shadowbot.cli.features.bots_cli import BotCapabilities
...
class TestBotHandler:
    ...
    def test_bot_handler_init(self):
        ...
        from shadowbot.cli.features.bots_cli import BotHandler
        
        handler = BotHandler()
        assert handler is not None
    
    @patch('shadowbot.cli.features.bots_cli.BotHandler._load_agent')
    def test_build_tools_browser(self, mock_load_agent):
        """Test _build_tools with browser enabled."""
        from shadowbot.cli.features.bots_cli import BotHandler, BotCapabilities
        
        handler = BotHandler()
        caps = BotCapabilities(browser=True)
        
        # Mock the browser tool import
        with patch.dict('sys.modules', {'shadowbot_tools': MagicMock()}):
...
class TestHandleBotCommand:
    ...
    def test_handle_bot_command_no_platform(self, capsys):
        ...
        from shadowbot.cli.features.bots_cli import handle_bot_command
...
class TestBuildCapabilitiesFromArgs:
    ...
    def test_build_capabilities_from_args_defaults(self):
        ...
        from shadowbot.cli.features.bots_cli import _build_capabilities_from_args
...
class TestAddCapabilityArgs:
    ...
    def test_add_capability_args(self):
        ...
        import argparse
        from shadowbot.cli.features.bots_cli import _add_capability_args
