...
class TestOpenAIClientResponseFormat:
    ...
    def test_create_completion_accepts_response_format(self):
        ...
        """
        from shadowbotagents.llm.openai_client import OpenAIClient
        
        with patch('shadowbotagents.llm.openai_client._get_openai') as mock_get_openai:
...
class TestAgentNativeStructuredOutput:
    ...
    def test_agent_chat_uses_response_format_for_supported_model(self):
        ...
        # Mock the LLM call
        with patch('shadowbotagents.agent.agent.Agent._chat_completion') as mock_chat:
            mock_response = MagicMock()
            mock_response.choices = [MagicMock()]
            mock_response.choices[0].message.content = '{"topics": ["AI", "ML"]}'
            mock_chat.return_value = mock_response
            
            with patch('shadowbotagents.llm.model_capabilities.supports_structured_outputs', return_value=True):
                agent = Agent(name="test", llm="gpt-4o-mini")
...
class TestFallbackBehavior:
    ...
    def test_text_injection_for_unsupported_model(self):
        ...
        with patch('shadowbotagents.llm.model_capabilities.supports_structured_outputs', return_value=False):
...
class TestSchemaEchoExtraction:
    ...
    def test_parse_json_output_extracts_data(self):
        ...
        from shadowbotagents.workflows.workflows import _parse_json_output
...
class TestPerformanceImpact:
    ...
    def test_no_heavy_imports_in_agent_module(self):
        ...
        if 'shadowbotagents.agent.agent' in sys.modules:
            del sys.modules['shadowbotagents.agent.agent']
    ...
    def test_model_capabilities_lazy_import(self):
        ...
        from shadowbotagents.llm import model_capabilities
