...
class TestPraisonaiHomeOverride:
    ...
    def test_all_global_paths_honor_override(self):
        ...
        from shadowbotagents.paths import (
            _clear_cache,
            get_data_dir,
            get_sessions_dir,
            get_skills_dir,
        ...
        custom_home = tempfile.mkdtemp(prefix="shadowbot_test_")
        try:
            _clear_cache()
    ...
    def test_retrieval_config_defaults_to_project_local(self):
        """RetrievalConfig.persist_path should default to project-local .shadowbot."""
    ...
    def test_summarizer_defaults_to_project_local(self):
        """HierarchicalSummarizer._persist_path should default to project-local .shadowbot/summaries."""
...
class TestRealAgenticCall:
    ...
    def test_agent_with_shadowbot_home_override(self):
        """
        Run agent with PRAISONAI_HOME override and verify data paths work.
        """
        from shadowbotagents.paths import _clear_cache

        custom_home = tempfile.mkdtemp(prefix="shadowbot_agent_test_")
        try:
            _clear_cache()
...
class TestNoHardcodedPaths:
    ...
    def test_no_expanduser_in_autonomy(self):
        ...
        assert 'expanduser("~"), ".shadowbot", "snapshots"' not in source
    ...
    def test_no_expanduser_in_scheduler_store(self):
        ...
        assert 'expanduser("~"), ".shadowbot", "schedules"' not in source

    def test_no_hardcoded_string_in_backends(self):
        """storage/backends.py should not have hardcoded ~/.shadowbot/storage.db default."""
        import inspect
