...
pytest.importorskip("pydantic", reason="requires shadowbotagents[llm]")
...
def test_agent_accepts_auth_kwarg(mock_subscription_auth):
    ...
    with patch('shadowbotagents.auth.resolve_subscription_credentials', mock_subscription_auth):
        agent = Agent(
            name="test-agent", 
