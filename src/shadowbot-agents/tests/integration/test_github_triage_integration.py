...
These tests validate the ShadowBot GitHub triage command:
...
Usage:
    ...
    GITHUB_TOKEN=$(gh auth token) GITHUB_REPOSITORY=MervinPraison/ShadowBot \
        PRAISONAI_AUTO_APPROVE=true shadowbot github triage \
        --issue 1347 --agent-file .github/shadowbot-issue-triage.yaml
...
class TestGitHubTriageIntegration:
    ...
    def test_triage_yaml_structure(self):
        ...
        yaml_path = "/Users/praison/shadowbot-package/.github/shadowbot-issue-triage.yaml"
    ...
    def test_sticky_comment_has_branch_and_pr_links(self):
        ...
        from shadowbot.cli.commands.github import StickyComment
        ...
        # Set branch
        sticky.set_branch("shadowbot/issue-123")
        ...
        assert "shadowbot/issue-123" in body
        assert "https://github.com/owner/repo/tree/shadowbot/issue-123" in body
        
        # Should have "Create PR →" link to compare URL (not an actual PR URL)
        assert "Create PR →" in body
        assert "compare/main...shadowbot/issue-123" in body
...
class TestGitHubTriageManual:
    ...
    Run with:
        ...
        OPENAI_API_KEY=sk-... \
        GITHUB_REPOSITORY=MervinPraison/ShadowBot \
        PRAISONAI_AUTO_APPROVE=true \
        shadowbot github triage --issue <ISSUE_NUM> \
            --agent-file .github/shadowbot-issue-triage.yaml
    """
    ...
    def test_no_pr_created_without_explicit_request(self):
        ...
        4. Verify NO PR exists: gh pr list --head shadowbot/issue-XXX
    ...
    def test_pr_created_when_explicitly_requested(self):
        ...
        4. Verify PR IS created: gh pr list --head shadowbot/issue-XXX
    ...
    def test_triage_command_full_workflow(self):
        ...
        GITHUB_REPOSITORY=MervinPraison/ShadowBot 
        PRAISONAI_AUTO_APPROVE=true 
        shadowbot github triage 
            --issue 1347 
            --agent-file .github/shadowbot-issue-triage.yaml 2>&1 | 
        tee /tmp/triage-1347b.log
        
        Then verify:
        - Exit code is 0
        - Branch shadowbot/issue-1347 exists remotely
