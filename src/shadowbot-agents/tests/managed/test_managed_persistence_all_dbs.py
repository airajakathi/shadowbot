...
def _make_local_managed(**kwargs):
    ...
    from shadowbot.integrations.managed_local import LocalManagedAgent, LocalManagedConfig
...
def _run_full_lifecycle(store_factory, cleanup=None):
    ...
    with patch("shadowbotagents.Agent", return_value=mock_inner):
...
class TestSqliteConversationStore:
    ...
    def test_sqlite_store_operations(self, tmp_path):
        ...
        from shadowbot.persistence.conversation.sqlite import SQLiteConversationStore
        from shadowbot.persistence.conversation.base import ConversationSession, ConversationMessage
    ...
    def test_sqlite_managed_roundtrip(self, tmp_path):
        ...
        from shadowbotagents.session.store import DefaultSessionStore
...
class TestPostgresConversationStore:
    ...
    def _make_pg_store(self, prefix=None):
        from shadowbot.persistence.conversation.postgres import PostgresConversationStore
    ...
    def test_postgres_managed_roundtrip(self):
        ...
        """
        from shadowbot.persistence.conversation.base import ConversationMessage, ConversationSession
        from shadowbot.persistence.conversation.postgres import PostgresConversationStore
        from shadowbot.integrations.db_session_adapter import DbSessionAdapter
...
class TestMySQLConversationStore:
    ...
    def check_mysql(self):
        ...
        try:
            ...
            c = mysql.connector.connect(host="localhost", port=3307, user="root", password="praisontest", database="shadowbot", connect_timeout=3)
            c.close()
        except Exception:
            pytest.skip("MySQL not available on localhost:3307")

    def _make_mysql_store(self, prefix=None):
        from shadowbot.persistence.conversation.mysql import MySQLConversationStore
        prefix = prefix or f"test_{uuid.uuid4().hex[:6]}_"
    ...
    def _cleanup_mysql(self, prefix):
        import mysql.connector
        c = mysql.connector.connect(host="localhost", port=3307, user="root", password="praisontest", database="shadowbot")
        cur = c.cursor()
    ...
    def test_mysql_data_verified_in_db(self):
        ...
        store = MySQLConversationStore(
            ...
            database="shadowbot", table_prefix=prefix,
        )
...
class TestRedisStateStore:
    ...
    def test_redis_state_operations(self):
        ...
        from shadowbot.persistence.state.redis import RedisStateStore
...
class TestMongoDBStateStore:
    ...
    def test_mongodb_state_operations(self):
        ...
        from shadowbot.persistence.state.mongodb import MongoDBStateStore

        collection = f"test_{uuid.uuid4().hex[:8]}"
        store = MongoDBStateStore(
            url="mongodb://localhost:27017",
            database="shadowbot_test",
        ...
        try:
            ...
            c["shadowbot_test"].drop_collection(collection)
            c.close()
    ...
    def test_mongodb_data_verified_in_db(self):
        ...
        try:
            ...
            import pymongo
            c = pymongo.MongoClient("mongodb://localhost:27017/")
            doc = c["shadowbot_test"][collection].find_one({"_id": "verify_key"})
