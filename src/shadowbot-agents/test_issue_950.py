#!/usr/bin/env python3
"""
Test for issue #950: Convert shadowbotagents to shadowbot imports
The goal is to enable: from ShadowBot import Agent instead of from Agents import Agent
"""
...
def test_issue_950_goal():
    ...
    print("Testing issue #950 goal: from ShadowBot import Agent")
    
    try:
        # This is what the issue wants to achieve
        # Note: In Python, package names are case-sensitive, so we import from 'shadowbot'
        # but the user goal is conceptually "from ShadowBot import Agent"
        from shadowbot import Agent
        print("✅ SUCCESS: `from shadowbot import Agent` works!")
        ...
        from shadowbot import Task, AgentManager
        print("✅ SUCCESS: `from shadowbot import Task, AgentManager` works!")
...
def test_backward_compatibility():
    ...
    try:
        ...
        print("✅ SUCCESS: `from shadowbotagents import Agent` still works!")
...
def test_package_name_case_sensitivity():
    ...
    # The actual package name is 'shadowbot' (lowercase)
    # but the issue mentions 'ShadowBot' (capitalized)
    
    try:
        # This should work (lowercase)
        from shadowbot import Agent as AgentLowercase
        print("✅ SUCCESS: `from shadowbot import Agent` works (lowercase package name)")
        
        # Try with uppercase (this should fail in most cases unless there's a ShadowBot package)
        try:
            from ShadowBot import Agent as AgentUppercase
            print("✅ SUCCESS: `from ShadowBot import Agent` works (uppercase package name)")
            ...
            print("   The working pattern is: from shadowbot import Agent")
...
if __name__ == "__main__":
    print("Testing Issue #950: Convert shadowbotagents to shadowbot imports")
    print("=" * 60)
    ...
    if all(results):
        ...
        print("✅ Users can now use: from shadowbot import Agent")
        print("✅ Backward compatibility maintained: from shadowbotagents import Agent")
        print("\nNote: In Python, package names are case-sensitive.")
        print("The correct import is: from shadowbot import Agent (lowercase)")
        sys.exit(0)
    else:
