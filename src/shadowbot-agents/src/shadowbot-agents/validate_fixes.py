...
def test_imports():
    ...
    try:
        from shadowbotagents.auth.subscription.protocols import SubscriptionCredentials, AuthError
        print("✅ Protocol imports work")
        
        from shadowbotagents.auth.subscription.claude_code import ClaudeCodeAuth
        print("✅ Claude Code auth imports work")
        
        from shadowbotagents.auth.subscription.registry import resolve_subscription_credentials
...
def test_codex_experimental():
    ...
    try:
        from shadowbotagents.auth.subscription.codex import CodexAuth
        auth = CodexAuth()
        try:
...
def test_gemini_experimental():
    ...
    try:
        from shadowbotagents.auth.subscription.gemini_cli import GeminiCliAuth
        auth = GeminiCliAuth()
        try:
            auth.resolve_credentials()
