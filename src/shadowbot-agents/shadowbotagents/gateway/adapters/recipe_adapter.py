...
from shadowbotagents.streaming import StreamEventEmitter
...
class RecipeBotAdapter:
    ...
    def _get_runtime(self):
        ...
        if self._runtime is None:
            try:
                from shadowbot.recipe.core import RecipeRuntime
                self._runtime = RecipeRuntime(recipe_name=self.recipe_name, **self.config)
            except ImportError as e:
                raise ImportError(
                    f"Failed to load ShadowBot recipe components: {e}. "
                    "Make sure you have shadowbot installed."
                ) from e
