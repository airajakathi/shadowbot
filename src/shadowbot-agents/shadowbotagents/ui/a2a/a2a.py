...
Exposes ShadowBot Agents via the A2A (Agent2Agent) protocol.
"""
...
from shadowbotagents.ui.a2a.types import AgentCard
from shadowbotagents.ui.a2a.agent_card import generate_agent_card
from shadowbotagents.ui.a2a.task_store import TaskStore
...
class A2A:
    """
    A2A Interface for ShadowBot Agents.
    
    Exposes a ShadowBot Agent or Agents workflow via the A2A protocol,
    enabling agent-to-agent communication with other A2A-compatible systems.
    
    Usage:
        from shadowbotagents import Agent
        from shadowbotagents.ui.a2a import A2A
    ...
    Args:
        agent: Single ShadowBot Agent instance
    ...
    def __init__(
        ...
        if description:
            ...
            self.description = "ShadowBot Agent via A2A"
    ...
    async def _handle_message_send(self, request_id, params: dict):
        ...
        from shadowbotagents.ui.a2a.types import (
            Message, TextPart, TaskState,
        )
        from shadowbotagents.ui.a2a.conversion import (
            extract_user_input,
            shadowbot_to_a2a_message,
            create_artifact,
        )
        ...
        if return_immediately:
            ...
            async def _background_process(task_id, user_input_text):
                try:
                    ...
                    response_msg = shadowbot_to_a2a_message(str(response), task_id=task_id)
        ...
        try:
            ...
            response_msg = shadowbot_to_a2a_message(
    ...
    async def _handle_message_stream(self, request_id, params: dict):
        ...
        from shadowbotagents.ui.a2a.streaming import stream_agent_response
        from shadowbotagents.ui.a2a.conversion import extract_user_input
