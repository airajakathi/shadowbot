"""
Observability interface for ShadowBot Agents.

This module provides the protocol and types for observability/tracing.
Implementations are provided by the wrapper layer (shadowbot_tools.observability).

Usage (simplest - recommended):
    from shadowbotagents import Agent, obs
...
class _LazyObsModule:
    ...
    Allows `from shadowbotagents import obs` without importing heavy deps.
    Actual provider classes are loaded only when accessed.
    """
    
    _PROVIDERS = {
        "langfuse": "shadowbot_tools.observability.providers.langfuse_provider",
        "langsmith": "shadowbot_tools.observability.providers.langsmith_provider",
        "agentops": "shadowbot_tools.observability.providers.agentops_provider",
        "arize": "shadowbot_tools.observability.providers.arize_phoenix_provider",
        "phoenix": "shadowbot_tools.observability.providers.arize_phoenix_provider",
        "datadog": "shadowbot_tools.observability.providers.datadog_provider",
        "mlflow": "shadowbot_tools.observability.providers.mlflow_provider",
        "openlit": "shadowbot_tools.observability.providers.openlit_provider",
        "opik": "shadowbot_tools.observability.providers.opik_provider",
        "portkey": "shadowbot_tools.observability.providers.portkey_provider",
        "traceloop": "shadowbot_tools.observability.providers.traceloop_provider",
        "weave": "shadowbot_tools.observability.providers.weave_provider",
        "braintrust": "shadowbot_tools.observability.providers.braintrust_provider",
        "langtrace": "shadowbot_tools.observability.providers.langtrace_provider",
        "langwatch": "shadowbot_tools.observability.providers.langwatch_provider",
        "maxim": "shadowbot_tools.observability.providers.maxim_provider",
    }
    ...
    def _create_provider_factory(self, provider_name: str):
        ...
        def factory(**kwargs):
            try:
                ...
                raise ImportError(
                    f"Observability provider '{provider_name}' requires shadowbot-tools. "
                    f"Install with: pip install shadowbot-tools\n"
                    f"Original error: {e}"
                ) from e
        return factory
    ...
    def auto(self, **kwargs):
        ...
        Also loads configuration from ~/.shadowbot/langfuse.env if it exists.
        Returns None if no provider is detected.
        """
        import os
        from pathlib import Path
        
        # First, try to load from shadowbot config file if present
    ...
    def __repr__(self):
        return "<module 'shadowbotagents.obs' (lazy observability backends)>"
