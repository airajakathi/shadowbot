"""
Schedule Store Protocol for ShadowBot Agents.
...
class ScheduleStoreProtocol(Protocol):
    ...
    Implementations:
        - ``ConfigYamlScheduleStore`` — YAML config (``~/.shadowbot/config.yaml``) [default]
        - ``FileScheduleStore`` — JSON file (``~/.shadowbot/schedules/jobs.json``) [legacy]
        - ``_InMemoryScheduleStore`` (in ShadowBotUI) — YAML config.yaml
