"""
Configuration Loader for ShadowBot Agents.

Loads configuration from multiple sources with precedence:
1. Explicit parameters (highest)
2. Environment variables
3. Config file (.shadowbot/config.toml or shadowbot.toml)
4. Defaults (lowest)
...
Usage:
    from shadowbotagents.config.loader import get_config, get_default
...
@dataclass
class PraisonConfig:
    """Root configuration for ShadowBot Agents."""
...
def _find_config_file() -> Optional[Path]:
    ...
    Search order:
    1. .shadowbot/config.toml (project-local)
    2. shadowbot.toml (project root)
    3. ~/.shadowbot/config.toml (user global)
