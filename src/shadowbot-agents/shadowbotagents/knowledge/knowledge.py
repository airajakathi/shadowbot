...
class Knowledge:
    ...
    def _deps(self):
        ...
        try:
            ...
            raise ImportError(
                ...
                'pip install "shadowbotagents[knowledge]"'
            )
    ...
    def config(self):
        ...
        default_collection = "shadowbot_knowledge"
                    ...
                    if self._config["vector_store"]["provider"] == "mongodb":
                        base_config["vector_store"] = {
                            ...
                            "config": {
                                ...
                                "database": self._config["vector_store"]["config"].get("database", "shadowbot"),
    ...
    def _prepare_mem0_config(self, config):
        """Prepare config for mem0 by removing ShadowBot-specific fields.
        
        mem0's RerankerConfig only accepts 'provider' and 'config' fields.
        ShadowBot adds 'enabled' and 'default_rerank' for internal use.
        """
        mem0_config = config.copy()
        
        # Strip ShadowBot-specific reranker fields that mem0 doesn't accept
    ...
    def _init_legacy_memory(self):
        ...
        # Prepare config for mem0 (strip ShadowBot-specific fields)
