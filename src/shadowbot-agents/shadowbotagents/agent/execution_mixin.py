...
class ExecutionMixin:
    ...
    def _delegate_to_backend(self, prompt: str, **kwargs) -> Optional[str]:
        ...
        ShadowBot's session management (SessionStore, auto_save) stays consistent
            ...
            if port not in _registered_agents:
                ...
                if _shared_apps.get(port) is None:
                    _shared_apps[port] = FastAPI(
                        title=f"ShadowBot Agents API (Port {port})",
                        description="API for interacting with ShadowBot Agents"
                    )
