...
class Agent(UnifiedExecutionMixin, ToolExecutionMixin, ChatHandlerMixin, SessionManagerMixin, ChatMixin, ExecutionMixin, MemoryMixin, AsyncMemoryMixin):
    ...
    def from_template(
        ...
        **kwargs
        ...
        try:
            ...
            from shadowbot.templates.loader import create_agent_from_template
            return create_agent_from_template(uri, config=config, offline=offline, **kwargs)
        except ImportError:
            raise ImportError(
                "Template support requires the 'shadowbot' package. "
    ...
    def __init__(
        ...
        if memory is not None:
            try:
                ...
                raise ImportError(
                    ...
                    "Please install with: pip install \"shadowbotagents[memory]\""
                )
        ...
        if _history_enabled and session_id is None and _history_session_id is not None:
            ...
            import os as _os
            _session_dir = _os.path.join(_os.path.expanduser("~"), ".shadowbot", "sessions")
        ...
        if base_url:
            try:
                ...
                raise ImportError(
                    ...
                    "Please install with: pip install \"shadowbotagents[llm]\""
                ) from e
...
Summary:"""
    ...
    def _add_skill_tools(self):
        ...
        Uses lazy imports from shadowbotagents.tools to avoid performance impact
    ...
    def _init_rules_manager(self):
        ...
        - ~/.shadowbot/rules/ (global)
        - .shadowbot/rules/ (workspace)
        - Subdirectory rules
        ...
        if self._rag_instance is None:
            ...
            if self.knowledge:
                try:
                    ...
                    logging.warning("RAG module not available. Install with: pip install 'shadowbotagents[rag]'")
                    return None
    ...
    def _get_knowledge_context(self, query: str, use_rag: bool = False) -> tuple:
        ...
        try:
            from shadowbotagents.rag.context import build_context
    ...
    def retrieve(self, query: str, **kwargs) -> "ContextPack":
        ...
        if not self.rag:
            raise ImportError("RAG module not available. Install with: pip install 'shadowbotagents[rag]'")
    ...
    def _resolve_cli_backend(self, cli_backend):
        ...
        try:
            ...
            raise ImportError(
                ...
                "This should not happen in a properly installed shadowbotagents package."
            )
        ...
        if isinstance(cli_backend, str):
            raise TypeError(
                ...
                "in the wrapper layer. Use: shadowbot.Agent(cli_backend='claude-code') "
                "or manually resolve: agent = shadowbotagents.Agent(cli_backend=resolve_cli_backend('claude-code'))"
            )
