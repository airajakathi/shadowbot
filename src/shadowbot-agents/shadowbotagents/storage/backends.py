"""
Storage Backend Implementations for ShadowBot Agents.
...
import tempfile
import logging
from shadowbotagents._logging import get_logger
...
class FileBackend:
    ...
    Example:
        ```python
        backend = FileBackend(storage_dir="~/.shadowbot/storage")
...
class SQLiteBackend:
    ...
    Example:
        ```python
        backend = SQLiteBackend(db_path="~/.shadowbot/data.db")
        backend.save("session_123", {"messages": []})
        data = backend.load("session_123")
        ```
    """
...
class _LazyStorageModule:
    ...
    _BACKENDS = {
        "RedisBackend": "shadowbot.storage",
        "MongoDBBackend": "shadowbot.storage", 
        "PostgreSQLBackend": "shadowbot.storage",
        "DynamoDBBackend": "shadowbot.storage",
        # Legacy names for compatibility
        "RedisStorageAdapter": "shadowbot.storage",
        "MongoDBStorageAdapter": "shadowbot.storage",
        "PostgreSQLStorageAdapter": "shadowbot.storage", 
        "DynamoDBStorageAdapter": "shadowbot.storage",
    }
    ...
    def __getattr__(self, name: str):
        if name in self._BACKENDS:
            try:
                ...
                raise ImportError(
                    f"Storage backend '{name}' requires the shadowbot package. "
                    f"Install with: pip install shadowbot\n"
                    f"For specific backends, use: pip install 'shadowbot[redis]', 'shadowbot[mongodb]', etc.\n"
                    f"Original error: {e}"
                ) from e
        raise AttributeError(f"module 'storage' has no attribute {name!r}")
    
    def __repr__(self):
        return "<module 'shadowbotagents.storage' (lazy heavy backends)>"
...
def get_backend(
    ...
    **kwargs
    ...
    Example:
        ...
        backend = get_backend("file", storage_dir="~/.shadowbot/data")
        
        # SQLite backend - lightweight, in core SDK
        backend = get_backend("sqlite", db_path="~/.shadowbot/data.db")
        
        # Heavy backends - from wrapper, require shadowbot package
        backend = get_backend("redis", url="redis://localhost:6379")
        backend = get_backend("mongodb", url="mongodb://localhost:27017/")
        backend = get_backend("postgresql", host="localhost", database="shadowbot")
        backend = get_backend("dynamodb", table_name="shadowbot-storage")
        ```
    """
...
def __getattr__(name: str):
    ...
    This allows `from shadowbotagents.storage.backends import RedisBackend` to work
