...
Concrete implementations live in the `shadowbot` wrapper. This module
re-exports them lazily so user code can keep importing from
``shadowbotagents.push``.
"""
import importlib
...
_WRAPPER_MODULE = "shadowbot.push"
...
def __getattr__(name: str):
    ...
    if name in _WRAPPER_NAMES:
        try:
            ...
            raise ImportError(
                f"{name} requires the shadowbot wrapper package. "
