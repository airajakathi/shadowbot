...
1. Project: ./.shadowbot/plugins/
2. User: ~/.shadowbot/plugins/

Usage:
    from shadowbotagents.plugins.discovery import discover_plugins, load_plugin
...
def ensure_plugin_dir() -> Path:
    ...
    Creates ~/.shadowbot/plugins/ if it doesn't exist.
    Uses centralized paths.py for consistent path management.
...
def get_plugin_template(name: str, description: str = "", author: str = "") -> str:
    ...
    return f'''"""
Plugin Name: {name}
Description: {description or "A ShadowBot plugin"}
Version: 1.0.0
