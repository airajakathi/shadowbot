"""
Plugin Manager for ShadowBot Agents.
...
class PluginManager:
    ...
    def auto_discover_plugins(self) -> int:
        ...
        Scans:
        - ./.shadowbot/plugins/ (project-level)
        - ~/.shadowbot/plugins/ (user-level)
    ...
    def discover_entry_points(self) -> int:
        ...
        Looks for entry_points in the "shadowbotagents.plugins" group.
        ...
        try:
            ...
            eps = get_ep(group="shadowbotagents.plugins")
        except TypeError:
            # Python 3.9 fallback - call then get group
            try:
                all_eps = get_ep()
                eps = all_eps.get("shadowbotagents.plugins", [])
            except Exception:
                eps = []
    ...
    def discover_entry_points(self) -> int:
        ...
        that register in the 'shadowbot.plugins' entry-point group.
        ...
        try:
            ...
            # Python 3.10+
            entry_points = importlib.metadata.entry_points(group="shadowbot.plugins")
        except ImportError:
            ...
            try:
                entry_points = importlib.metadata.entry_points().get("shadowbot.plugins", [])
            except Exception as e:
