"""
Centralized logging configuration for ShadowBot Agents.
...
def _get_all_noisy_loggers() -> List[str]:
    ...
    return [
        ...
        "shadowbotagents.telemetry.telemetry",
    ]
...
def configure_structured_logging():
    ...
    Call this function to enable structured logging across all ShadowBot modules.
    Useful for log aggregation systems like ELK, Splunk, or CloudWatch.
    
    Example:
        import os
        from shadowbotagents._logging import configure_structured_logging
    ...
    if os.environ.get('PRAISONAI_STRUCTURED_LOGS', '').lower() == 'true':
        ...
        # Also reconfigure any shadowbotagents loggers that have their own handlers
        for logger_name in logging.Logger.manager.loggerDict:
            if logger_name.startswith('shadowbotagents.'):
...
def get_logger(
    ...
    *,
    ...
    """Get a logger with consistent naming convention for ShadowBot modules.
    
    This function ensures all loggers follow the 'shadowbotagents.<module>' pattern
    and provides optional structured logging support.
    
    Args:
        name: Logger name. If None, uses the calling module's __name__.
              If it doesn't start with 'shadowbotagents.', the prefix is added.
    ...
    if not name.startswith('shadowbotagents.'):
        if name == '__main__':
            name = 'shadowbotagents.main'
        elif name == 'shadowbot' or name.startswith('shadowbot.'):
            # Handle cases like 'shadowbot.something' -> 'shadowbotagents.something'
            name = 'shadowbotagents' + name[len('shadowbot'):]
        else:
            # Add prefix for non-shadowbot modules
            name = f'shadowbotagents.{name}'
...
get_shadowbot_logger = get_logger  # Descriptive alias
