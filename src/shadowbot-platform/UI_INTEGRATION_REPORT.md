# ShadowBot Platform — Comprehensive UI Integration Report

> **Purpose**: This document provides everything a UI provider needs to build a frontend layer on top of the ShadowBot Platform backend. It covers every API endpoint, data model, UI component requirement, and analysis against similar tools in the space.
...
ShadowBot Platform is a **multi-tenant project management and issue tracking backend** designed for **human + AI agent teams**. It sits on top of the `shadowbotagents` Core SDK and provides:
...
| `__main__.py` | CLI entry point: `python -m shadowbot_platform` (uvicorn server) |
...
| Aspect | ShadowBot | Tool A | Tool B |
|--------|-----------|---------|-----------|
...
```python
from shadowbot_platform import PlatformClient
...
```bash
# CLI entry point
python -m shadowbot_platform --host 0.0.0.0 --port 8000 --reload
```
...
| Feature | ShadowBot Platform | Tool A | Tool B |
|---------|-------------------|---------|-----------|
...
| **Search** | ❌ | ✅ | ❌ |
| **CLI** | ✅ `python -m shadowbot_platform` | ✅ Full CLI | ✅ npx CLI |
...
| ✅ | **CLI entry point** | `python -m shadowbot_platform` runs uvicorn with `--host`, `--port`, `--reload` options. |
...
| ✅ | **Package exports** | `from shadowbot_platform import PlatformClient, create_app` works directly. |
...
The ShadowBot Platform provides a **solid, production-ready backend** with 40+ API endpoints covering authentication, multi-tenancy, project management, issue tracking, agent management, labels, dependencies, comments, and activity logging.
...
- **CLI entry point** — `python -m shadowbot_platform` for easy server startup
