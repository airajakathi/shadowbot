...
print_banner() {
    ...
    echo "╔═══════════════════════════════════════════════════════════╗"
    echo "║                                                           ║"
    echo "║   ${CYAN}ShadowBot${NC}${BOLD} - AI Agents Made Simple                      ║"
    echo "║                                                           ║"
...
create_venv() {
    local python_cmd="$1"
    local venv_dir="${2:-$HOME/.shadowbot/venv}"
...
install_shadowbot() {
    ...
    # Build install command - default is shadowbot[all] for full experience
    local install_pkg="shadowbot[all]"
    if [[ "$VERSION" != "latest" ]]; then
        install_pkg="shadowbot[all]==$VERSION"
    fi
    
    # Override extras if specified
    if [[ -n "$EXTRAS" ]]; then
        install_pkg="shadowbot[$EXTRAS]"
        if [[ "$VERSION" != "latest" ]]; then
            install_pkg="shadowbot[$EXTRAS]==$VERSION"
        fi
    fi
    ...
    if [[ "$EXTRAS" == *"ui"* ]] || [[ "$EXTRAS" == *"chat"* ]] || [[ "$EXTRAS" == *"code"* ]]; then
        local wrapper_pkg="shadowbot[$EXTRAS]"
        if [[ "$VERSION" != "latest" ]]; then
            wrapper_pkg="shadowbot[$EXTRAS]==$VERSION"
        fi
        $pip_cmd install --upgrade "$wrapper_pkg"
    fi
    
    log_success "ShadowBot installed successfully!"
}
...
setup_shell_path() {
    ...
    if grep -q "$bin_dir" "$shell_rc" 2>/dev/null; then
        ...
        echo "" >> "$shell_rc"
        echo "# ShadowBot" >> "$shell_rc"
        echo "$path_line" >> "$shell_rc"
        log_success "Added ShadowBot to PATH in $shell_rc"
    fi
...
verify_installation() {
    ...
    # Check import
    if $python_cmd -c "from shadowbotagents import Agent; print('Import successful')" 2>/dev/null; then
        log_success "ShadowBot agents package verified"
    else
        log_error "Failed to import shadowbotagents"
        return 1
    fi
    
    # Check CLI if wrapper installed
    if command_exists shadowbot; then
        local cli_version
        cli_version=$(shadowbot --version 2>/dev/null | head -n1 || echo "unknown")
        log_success "CLI available: shadowbot $cli_version"
    fi
}
...
print_next_steps() {
    ...
    if [[ -n "$venv_dir" ]] && [[ "$SKIP_VENV" != "1" ]]; then
        ...
        echo ""
        echo "  2. Or restart your terminal to use ShadowBot globally"
        echo ""
    fi
    
    echo "  Set up a messaging bot (Telegram / Discord / Slack / WhatsApp):"
    echo -e "     ${CYAN}shadowbot onboard${NC}"
    echo ""
    echo "  Open the dashboard UI (localhost only):"
    echo -e "     ${CYAN}shadowbot claw${NC}    ${BOLD}→${NC} http://127.0.0.1:8082"
    echo ""
    echo "  Quick start:"
    echo -e "     ${CYAN}python -c \"from shadowbotagents import Agent; Agent(name='test').start('Hello!')\"${NC}"
    echo ""
    echo "  Documentation:"
...
run_onboarding() {
    ...
    if ! [ -e /dev/tty ] || ! [ -t 1 ]; then
        log_info "No TTY available — skipping onboarding. Run 'shadowbot setup' later."
        return 0
    fi
    ...
    if "$py" -m shadowbot setup < /dev/tty > /dev/tty 2> /dev/tty; then
        ...
        echo -e "  ${CYAN}shadowbot setup${NC}"
        echo ""
    fi
}
...
maybe_offer_bot_onboarding() {
    ...
    if [[ "$NO_PROMPT" == "1" ]]; then
        log_info "Skipping bot onboarding (non-interactive mode) — run 'shadowbot onboard' later"
        return 0
    fi
    if [[ "$DRY_RUN" == "1" ]]; then
    ...
    if ! [ -e /dev/tty ]; then
        log_info "No TTY available — skipping bot onboarding. Run 'shadowbot onboard' later."
        return 0
    fi
    ...
    case "$yn" in
        [nN]*)
            log_info "Skipped — run 'shadowbot onboard' anytime to set up a bot."
            ;;
        *)
            if "$py" -m shadowbot onboard < /dev/tty > /dev/tty 2> /dev/tty; then
                log_success "Bot onboarding completed!"
                BOT_ONBOARDED=1
            else
                log_warn "Bot onboarding skipped or failed — you can retry with 'shadowbot onboard'."
            fi
            ;;
    esac
}
...
main() {
    ...
    # Install ShadowBot
    install_shadowbot "$python_cmd" "$venv_dir"
