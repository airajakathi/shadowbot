#!/usr/bin/env python3
"""
Combined bump version and release script for ShadowBot package.

Usage:
    # Basic: bump shadowbot to 3.8.2 with agents 0.11.8
...
2. Optionally waits for shadowbotagents to be available on PyPI
...
def get_shadowbot_dir() -> Path:
    """Get the shadowbot package directory."""
    return get_project_root() / "src/shadowbot"
...
def bump_version(new_version: str, agents_version: Optional[str] = None):
    ...
    root = get_project_root()
    shadowbot_dir = get_shadowbot_dir()
...
def validate_dependencies(max_retries: int = 3, retry_interval: int = 60, use_frozen: bool = False) -> bool:
    ...
    for attempt in range(max_retries):
        ...
        if not use_frozen:
            run(["uv", "cache", "clean"], cwd=shadowbot_dir, silent=True)
        
        result = run(lock_cmd, cwd=shadowbot_dir, check=False)
...
def release(version: str, use_frozen_lock: bool = False, no_add_all: bool = False):
    ...
    if not use_frozen_lock:
        print("\n🧹 Clearing uv cache...")
        run(["uv", "cache", "clean"], cwd=shadowbot_dir)
    
    print("\n📦 Running uv lock...")
    run(["uv", "lock", "--frozen"] if use_frozen_lock else ["uv", "lock"], cwd=shadowbot_dir)
    
    # 3. uv build
    print("\n🔨 Running uv build...")
    run(["uv", "build"], cwd=shadowbot_dir)
    
    # 4. Git add and commit
    print("\n📝 Committing changes...")
    
    release_files = [
        "src/shadowbot/shadowbot/version.py",
        "src/shadowbot/shadowbot/deploy.py",
        "docker/Dockerfile", 
        "docker/Dockerfile.chat",
        "docker/Dockerfile.dev", 
        "docker/Dockerfile.ui",
        "src/shadowbot/shadowbot.rb",
        "src/shadowbot/pyproject.toml",
        "src/shadowbot/uv.lock",
        "src/shadowbot/README.md",
        "src/shadowbot-agents/pyproject.toml",
        "src/shadowbot-agents/uv.lock"
    ]
    ...
    run([
        ...
        "--title", f"ShadowBot {tag}",
        "--notes", f"Release {tag}",
        "--latest"
    ], cwd=root)
    
    print(f"\n✅ Released ShadowBot {tag}")
    print("\nNext step:")
    print("  cd src/shadowbot && uv publish")


def main():
    parser = argparse.ArgumentParser(
        description="Bump version and release ShadowBot package",
...
"""
    ...
    parser.add_argument(
        "--agents", "-a",
        help="Update shadowbotagents dependency version (e.g., 0.11.8)",
        default=None
    )
    parser.add_argument(
        "--auto",
        action="store_true",
        help="Auto-detect latest shadowbotagents version from PyPI"
    )
    parser.add_argument(
        "--wait", "-w",
        action="store_true",
    ...
    # Handle --auto flag
    if args.auto:
        print("\n🔍 Auto-detecting latest shadowbotagents version from PyPI...")
        agents_version = get_pypi_version("shadowbotagents")
        if not agents_version:
            print("❌ Failed to auto-detect shadowbotagents version")
            sys.exit(1)
    ...
    if args.wait and args.agents:
        if not wait_for_pypi_version("shadowbotagents", args.agents, max_wait=args.max_wait):
