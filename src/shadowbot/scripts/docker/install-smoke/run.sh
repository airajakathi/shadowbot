...
echo "==> ShadowBot Install Smoke Test"
...
if [[ -z "$EXPECTED_VERSION" ]]; then
  ...
  EXPECTED_VERSION=$(pip index versions shadowbotagents 2>/dev/null | head -1 | grep -oP '\(\K[^)]+' || echo "")
  if [[ -z "$EXPECTED_VERSION" ]]; then
...
VENV_DIR="$HOME/.shadowbot/venv"
if [[ -d "$VENV_DIR" ]]; then
  source "$VENV_DIR/bin/activate"
fi

# Check import works
python3 -c "import shadowbotagents; print(f'Installed version: {shadowbotagents.__version__}')"

INSTALLED_VERSION=$(python3 -c "import shadowbotagents; print(shadowbotagents.__version__)" 2>/dev/null)
