#!/usr/bin/env python3
"""
CI script to check version sync between shadowbot-agents and shadowbot packages.
...
def load_dependencies(pyproject_path: Path) -> Dict[str, str]:
    ...
    for extra, extra_deps in data.get("project", {}).get("optional-dependencies", {}).items():
        for dep in extra_deps:
            if not dep.startswith("shadowbot"):  # Skip self-references
                name, version = parse_version_spec(dep)
                if name not in deps:
                    deps[name] = version
...
def main():
    ...
    agents_pyproject = repo_root / "src" / "shadowbot-agents" / "pyproject.toml"
    wrapper_pyproject = repo_root / "src" / "shadowbot" / "pyproject.toml"
    ...
    print(f"  shadowbot-agents: {len(agents_deps)} dependencies")
    print(f"  shadowbot: {len(wrapper_deps)} dependencies")
    ...
    if conflicts:
        ...
        for dep, v1, v2 in conflicts:
            print(f"   {dep}:")
            print(f"      shadowbot-agents: {v1}")
            print(f"      shadowbot: {v2}")
