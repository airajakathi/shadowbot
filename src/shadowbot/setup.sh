...
run_ui_interface() {
    ...
    if [[ $QUICK == true ]]; then
        ...
        if [[ $use_chainlit =~ ^[Yy]$ ]]; then
            ...
            if [[ $use_ag2 =~ ^[Yy]$ ]]; then
                shadowbot --framework autogen --init "$prompt"
            else
                shadowbot --init "$prompt"
            fi
        fi
    fi
}
...
if [[ $CICD == false ]]; then
    print_message "Cloning ShadowBot repository and checking out ${TAG}..."
    if [ ! -d "ShadowBot" ]; then
        git clone https://github.com/MervinPraison/ShadowBot.git
        cd ShadowBot
        git checkout ${TAG}
    else
        cd ShadowBot
...
if [[ $UI == true || $CHAT == true || $CODE == true || $REALTIME == true ]]; then
    ...
    # Check if shadowbot is already installed
    if ! pip show shadowbot &> /dev/null; then
        print_message "Installing ShadowBot components..."
        ...
        for component in "${components[@]}"; do
            pip install "shadowbot[$component]"
        done
    else
        print_message "ShadowBot is already installed. Skipping installation."
    fi
