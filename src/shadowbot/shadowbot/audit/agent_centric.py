...
AGENT_IMPORTS = [
    r"from\s+shadowbotagents\s+import\s+.*\bAgent\b",
    r"from\s+shadowbotagents\s+import\s+.*\bShadowBotAgents\b",
    r"from\s+shadowbotagents\s+import\s+.*\bWorkflow\b",
    r"from\s+shadowbotagents\.agent\s+import\s+Agent",
    r"from\s+shadowbotagents\.agents\s+import\s+ShadowBotAgents",
    r"from\s+shadowbotagents\.workflow\s+import\s+Workflow",
]

# Instantiation patterns
INSTANTIATION_PATTERNS = [
    r"\bAgent\s*\(",
    r"\bShadowBotAgents\s*\(",
    r"\bWorkflow\s*\(",
]

# Low-level imports that should come AFTER agent-centric code
LOW_LEVEL_IMPORTS = [
    r"from\s+shadowbotagents\.(memory|knowledge|tools|context)",
    r"from\s+shadowbotagents\.mcp",
    r"from\s+shadowbotagents\.hooks",
    r"from\s+shadowbotagents\.middleware",
]
...
class AgentCentricAuditor:
    ...
    def _method_2_ast_import_detector(self, content: str) -> Tuple[bool, str]:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ImportFrom):
                if node.module and "shadowbotagents" in node.module:
                    for alias in node.names:
                        if alias.name in ("Agent", "ShadowBotAgents", "Workflow"):
    ...
    def generate_quickstart_block(self, feature_name: str = "Demo") -> str:
        ...
        return f'''from shadowbotagents import Agent
