"""
Langextract TraceSinkProtocol Implementation for ShadowBot.
...
Architecture:
- Core SDK (shadowbotagents): Defines TraceSinkProtocol (unchanged)
- Wrapper (shadowbot): Implements LangextractSink adapter (this file)
...
from shadowbotagents.trace.protocol import (
    ActionEvent,
    ActionEventType,
    TraceSinkProtocol,
)
...
class LangextractSinkConfig:
    ...
    output_path: str = "shadowbot-trace.html"
    jsonl_path: Optional[str] = None           # derived from output_path if None
    document_id: str = "shadowbot-run"
...
class LangextractSink:
    ...
    def context_sink(self) -> "_ContextToActionBridge":
        ...
        ``shadowbotagents.trace.context_events.set_context_emitter`` (or
    ...
    def _render(self) -> None:
        ...
        try:
            ...
            raise ImportError(
                "langextract is not installed. Install with: pip install 'shadowbot[langextract]'"
            ) from err
