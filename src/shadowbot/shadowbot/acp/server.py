"""
ACP Server implementation for ShadowBot.

This module implements the Agent Client Protocol server that allows
IDEs/editors to communicate with ShadowBot agents.
"""
...
def _setup_stderr_logging(debug: bool = False) -> None:
    ...
    root_logger = logging.getLogger("shadowbot.acp")
...
class ACPServer:
    """
    ACP Server that exposes ShadowBot agents to IDE clients.
    ...
    def _get_agent(self):
        ...
        try:
            ...
            logger.error("shadowbotagents not installed")
            raise RuntimeError("shadowbotagents package required")
...
async def _run_server(config: ACPConfig) -> None:
    ...
    try:
        ...
        logger.error(
            ...
            "Install with: pip install shadowbot[acp]\n"
    ...
    logger.info("Starting ShadowBot ACP server...")
...
def serve(
    ...
    **kwargs: Any,
    ...
    This is the main entry point for running ShadowBot as an ACP agent.
