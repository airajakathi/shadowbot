...
def generate_api_server_code(agents_file: str, config: Optional[APIConfig] = None) -> str:
    ...
    code = f'''"""
Auto-generated API server for ShadowBot agents.
"""
...
if AUTH_ENABLED and not AUTH_TOKEN:
    ...
    print(
        f"[shadowbot-api] generated API token (set PRAISONAI_API_TOKEN to override): {{AUTH_TOKEN}}",
        file=sys.stderr,
        flush=True,
    )
...
def health():
    ...
    return jsonify({{"status": "ok", "service": "shadowbot-api"}})
...
def chat():
    ...
    try:
        shadowbot = ShadowBot(agent_file="{agents_file}")
...
def start_api_server(
    ...
    try:
        ...
        import tempfile
        server_file = os.path.join(tempfile.gettempdir(), f"shadowbot_api_server.py")
