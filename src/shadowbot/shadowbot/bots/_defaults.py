...
def apply_bot_smart_defaults(agent: Any, config: Optional[Any] = None, session_key: str = "default") -> Any:
    ...
    try:
        from shadowbotagents import Agent as _Agent
    except ImportError:
        return agent
    ...
    if config and getattr(config, 'auto_approve_tools', False):
        if getattr(agent, '_approval_backend', None) is None:
            try:
                ...
                logger.warning("AutoApproveBackend not available - install shadowbotagents[approval]")
    ...
    workspace = None
    try:
        from shadowbotagents.workspace import Workspace
...
def _resolve_tool_names_with_workspace(tool_names: List[str], workspace=None) -> list:
    ...
    try:
        from shadowbotagents.tools.profiles import resolve_profiles
        from shadowbotagents.tools import ToolResolver
...
def _create_workspace_tools(tool_names: List[str], workspace) -> list:
    ...
    try:
        ...
        if any(name in ["read_file", "write_file", "edit_file", "list_files", "search_files"] for name in tool_names):
            from shadowbotagents.tools.file_tools import create_file_tools
        ...
        if any(name in ["edit_file", "search_files"] for name in tool_names):
            from shadowbotagents.tools.edit_tools import create_edit_tools
        ...
        if any(name in ["skill_manage", "skills_list", "skill_view"] for name in tool_names):
            from shadowbotagents.tools.skill_tools import create_skill_tools
        ...
        if any(name in ["todo_add", "todo_list", "todo_update"] for name in tool_names):
            from shadowbotagents.tools.todo_tools import create_todo_tools
        ...
        if "session_search" in tool_names:
            from shadowbotagents.tools.session_tools import create_session_tools
        ...
        if "delegate_task" in tool_names:
            from shadowbotagents.tools.delegation_tools import create_delegation_tools
...
def _get_fallback_tools_with_workspace(workspace=None) -> list:
    ...
    try:
        from shadowbotagents.tools import search_web
        fallback_tools.insert(0, search_web)
    except (ImportError, AttributeError):
        pass
    
    try:
        from shadowbotagents.tools import store_memory, search_memory
