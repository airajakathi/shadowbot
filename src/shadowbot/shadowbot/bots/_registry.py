"""
Platform registry for ShadowBot BotOS.
...
def _telegram_loader():
    import importlib
    mod = importlib.import_module("shadowbot.bots.telegram")
    return getattr(mod, "TelegramBot")

def _discord_loader():
    import importlib
    mod = importlib.import_module("shadowbot.bots.discord")
    return getattr(mod, "DiscordBot")

def _slack_loader():
    import importlib
    mod = importlib.import_module("shadowbot.bots.slack")
    return getattr(mod, "SlackBot")

def _whatsapp_loader():
    import importlib
    mod = importlib.import_module("shadowbot.bots.whatsapp")
    return getattr(mod, "WhatsAppBot")

def _linear_loader():
    import importlib
    mod = importlib.import_module("shadowbot.bots.linear")
    return getattr(mod, "LinearBot")

def _email_loader():
    import importlib
    mod = importlib.import_module("shadowbot.bots.email")
    return getattr(mod, "EmailBot")

def _agentmail_loader():
    import importlib
    mod = importlib.import_module("shadowbot.bots.agentmail")
...
class BotPlatformRegistry(PluginRegistry):
    ...
    def __init__(self):
        super().__init__(
            entry_point_group="shadowbot.bots",
