...
class EmailBot(ChatCommandMixin, MessageHookMixin):
    """Email bot runtime for ShadowBot agents.
    ...
    async def send_message(
        ...
        if isinstance(content, dict):
            subject = content.get("subject", "Message from ShadowBot")
            body = content.get("body", "")
            html = content.get("html")
        else:
            subject = "Message from ShadowBot"
            body = str(content)
            html = None
