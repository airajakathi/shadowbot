...
class BotOS:
    ...
    async def _run_schedule_loop(self) -> None:
        ...
        When the UI scheduler is already running (ShadowBotUI started),
        this loop defers to it to avoid double-firing jobs.
        """
        try:
            from shadowbotagents.tools.schedule_tools import _get_store
        ...
        while True:
            try:
                ...
                ui_running = False
                try:
                    from shadowbotui.features.schedules import _scheduler_running
            ...
            if tool_names and isinstance(tool_names, list):
                ...
                for tname in tool_names:
                    if isinstance(tname, str):
                        try:
                            import importlib
                            mod = importlib.import_module("shadowbotagents.tools")
                            fn = getattr(mod, tname, None)
                            if fn:
                                resolved_tools.append(fn)
                            else:
                                logger.warning(f"BotOS: tool '{tname}' not found in shadowbotagents.tools")
                        except ImportError:
