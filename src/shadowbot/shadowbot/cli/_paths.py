"""Canonical filesystem paths for ShadowBot user state.
...
def praison_home() -> Path:
    """Return the canonical ShadowBot home directory.

    Honours ``PRAISONAI_HOME`` so packaging systems (Nix, Docker) can
    relocate state. Defaults to ``~/.shadowbot``.
    """
    override = os.environ.get("PRAISONAI_HOME")
    return Path(override).expanduser() if override else Path.home() / ".shadowbot"
...
def default_bot_config_path() -> Path:
    """Return the path where ``shadowbot onboard`` writes ``bot.yaml``.
...
def resolve_bot_config_path(cli_value: str = "bot.yaml") -> str:
    ...
    3. ``~/.shadowbot/bot.yaml`` (or ``PRAISONAI_BOT_CONFIG``) — the
       canonical location written by ``shadowbot onboard``.
