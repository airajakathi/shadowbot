...
class StickyComment:
    ...
    def _build_body(self, final: Optional[bool] = None) -> str:
        """Render the full Markdown comment body (ShadowBot enhanced style)."""
        if final is True:
        ...
        header = f"**🤖 ShadowBot {verb} #{self._issue}** {status_icon}"
        ...
        if final is not None:
            ...
            lines.append("")
            lines.append("*Powered by [ShadowBot](https://praison.ai) 🤖*")
...
def _make_context_sink(sticky: StickyComment):
    ...
    try:
        from shadowbotagents.trace.context_events import ContextTraceSinkProtocol
    except ImportError:
        return None
...
def github_triage(
    ...
    print(f"[cyan]🚀 ShadowBot triage for {repo_name}#{issue}…[/cyan]")
    ...
    try:
        ...
        sys.argv = ["shadowbot", "workflow", "run", agent_file, "--var", f"ISSUE_NUMBER={issue}"]
