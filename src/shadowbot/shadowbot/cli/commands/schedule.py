"""
Schedule command group for ShadowBot CLI.
...
def schedule_add_cmd(
    ...
    Examples:
        shadowbot schedule add "morning-hello" -s "cron:0 9 * * *" -m "say hello"
        shadowbot schedule add "tg-reminder" -s daily -m "check email" --agent support --channel telegram --channel-id 12345
    """
    output = get_output_controller()
    try:
