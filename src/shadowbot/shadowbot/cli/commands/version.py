"""
Version command group for ShadowBot CLI.
...
def version_show():
    ...
    versions = {
        "shadowbot": __version__,
    }
    
    # Check shadowbotagents version
    try:
        import shadowbotagents
        versions["shadowbotagents"] = getattr(shadowbotagents, "__version__", "unknown")
    except ImportError:
        versions["shadowbotagents"] = "not installed"
    ...
    output.print_panel(
        f"ShadowBot: {versions['shadowbot']}\n"
        f"ShadowBot Agents: {versions['shadowbotagents']}\n"
...
def version_check():
    ...
    try:
        ...
        url = "https://pypi.org/pypi/shadowbot/json"
        ...
        if latest != __version__:
            output.print_warning(
                ...
                f"Run: pip install --upgrade shadowbot"
            )
        else:
