"""ShadowBot Flow — Visual workflow builder powered by Langflow.

Provides `shadowbot flow` CLI command to launch Langflow with
ShadowBot custom components (Agent, Agents, Task) pre-loaded.
"""
...
def flow_start(
    ...
    """Start ShadowBot Flow — visual workflow builder.

    Launches Langflow with ShadowBot Agent, AgentTeam, and Task components
    pre-loaded in the sidebar. Build complex AI workflows visually.

    Install: pip install shadowbot[flow]

    Examples:
        shadowbot flow
        shadowbot flow --port 8080
        shadowbot flow --host 0.0.0.0 --backend-only
    """
    ...
    try:
        ...
        console.print(
            ...
            "[yellow]Install with: pip install shadowbot[flow][/yellow]"
        )
        raise typer.Abort()

    # Resolve ShadowBot components directory (absolute path)
    ...
    console.print()
    console.print("[bold green]🚀 Starting ShadowBot Flow[/bold green]")
    console.print(f"[dim]Langflow + ShadowBot components on {host}:{port}[/dim]")
    console.print(
    ...
    try:
        ...
        console.print("\n[yellow]ShadowBot Flow stopped.[/yellow]")
...
def flow_import(
    ...
    Converts ShadowBot YAML workflow to Langflow JSON format and uploads
    to a running Langflow instance for visual editing.
    
    Examples:
        shadowbot flow import workflow.yaml
        shadowbot flow import workflow.yaml --dry-run
        shadowbot flow import workflow.yaml --output flow.json
        shadowbot flow import workflow.yaml --url http://localhost:8080
    """
    from pathlib import Path
    from rich.console import Console
    ...
    try:
        from shadowbot.flow.converter import yaml_to_langflow_json
        ...
        # Upload to Langflow
        from shadowbot.flow.client import create_client
        ...
        if health["status"] != "healthy":
            ...
            console.print("[yellow]Make sure Langflow is running: shadowbot flow[/yellow]")
            raise typer.Abort()
        ...
        console.print("[yellow]Install with: pip install 'shadowbot[flow]'[/yellow]")
        raise typer.Abort()
    except Exception as e:
...
def flow_export(
    ...
    Downloads a flow from Langflow and converts it back to ShadowBot 
    YAML format for use with the CLI.
    
    Examples:
        shadowbot flow export abc-123-def
        shadowbot flow export abc-123-def --output my_workflow.yaml
        shadowbot flow export abc-123-def --format json
    """
    from pathlib import Path
    from rich.console import Console
    ...
    try:
        ...
        if format == "json":
            ...
            # Convert to YAML
            from shadowbot.flow.converter import langflow_json_to_yaml
...
def flow_list(
    ...
    Examples:
        shadowbot flow list
        shadowbot flow list --search research
        shadowbot flow list --url http://localhost:8080
    """
    from rich.console import Console
