"""
ShadowBot Examples CLI Command.

Run and manage example files with reporting and diagnostics.

Usage:
    shadowbot examples run                    # Run all examples
    shadowbot examples run --path ./examples  # Custom path
    shadowbot examples list                   # List discovered examples
"""
...
def _get_default_examples_path() -> Path:
    ...
    candidates = [
        Path.home() / "shadowbot-package" / "examples",
        Path("/Users/praison/shadowbot-package/examples"),
        Path.cwd() / "examples",
...
def run(
    ...
    Examples:
        shadowbot examples run
        shadowbot examples run --path ./examples --timeout 120
        shadowbot examples run --group python --group mcp --max-items 5
        shadowbot examples run --include "context/*" --exclude "*_wow.py"
        shadowbot examples run --fail-fast --no-stream
    """
    # Lazy import to avoid loading at CLI startup
    from shadowbot.suite_runner import ExamplesSource, SuiteExecutor, RunResult
    ...
    if not quiet:
        typer.echo("=" * 60)
        typer.echo("ShadowBot Examples Runner")
        typer.echo("=" * 60)
...
def list_examples(
    ...
    Examples:
        shadowbot examples list
        shadowbot examples list --groups
        shadowbot examples list --group python --metadata
    """
    from shadowbot.suite_runner import ExamplesSource
...
def find_examples(
    ...
    Examples:
        shadowbot examples find rag
        shadowbot examples find mcp --group python
        shadowbot examples find gemini --limit 20
    """
    from shadowbot.suite_runner import ExamplesSource
...
def info(
    ...
    Examples:
        shadowbot examples info ./examples/context/01_basic.py
    """
    from shadowbot.suite_runner import ExamplesSource
...
def examples_stats(
    ...
    Examples:
        shadowbot examples stats
        shadowbot examples stats --group python --group mcp
    """
...
def examples_run_all(
    ...
    Examples:
        shadowbot examples run-all
        shadowbot examples run-all --sequential
        shadowbot examples run-all --workers 8 --timeout 120
    """
    import json
...
def examples_report(
    ...
    Examples:
        shadowbot examples report                    # View latest report
        shadowbot examples report ./my-report        # View specific report
        shadowbot examples report --wide --limit 0   # Full details
        shadowbot examples report --match failed     # Only failures
        shadowbot examples report --format json      # JSON output
    """
    # Lazy import to avoid loading at CLI startup
