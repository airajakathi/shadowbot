"""
Train command group for ShadowBot CLI.

Provides two types of training:
1. LLM Fine-tuning: shadowbot train dataset.json
2. Agent Training: shadowbot train agents [OPTIONS]
"""
...
def train_callback():
    ...
    For LLM fine-tuning:
        shadowbot train llm dataset.json
    
    For agent training (iterative improvement):
        shadowbot train agents --input "What is Python?"
        shadowbot train agents --input "Explain AI" --human
    """
    pass
...
def train_llm(
    ...
    Examples:
        shadowbot train llm dataset.json
        shadowbot train llm --model llama-3.1 dataset.json
    """
...
def train_agents(
    ...
    Examples:
        ...
        shadowbot train agents --input "What is 2+2?" --expected "4"
        
        # With scenarios file
        shadowbot train agents --scenarios scenarios.json
        
        # Human feedback mode
        shadowbot train agents --input "Explain AI" --human
        
        # More iterations
        shadowbot train agents --input "Hello" --iterations 5
        
        # With agent file
        shadowbot train agents my_agent.yaml --scenarios scenarios.json
    """
    ...
    try:
        from shadowbot.train.agents import AgentTrainer, TrainingScenario
    ...
    if agent_file:
        ...
        try:
            ...
            output.print_error(
                "shadowbotagents not installed",
                remediation="pip install shadowbotagents"
            )
            raise typer.Exit(1)
    ...
    from shadowbot.train.agents.grader import TrainingGrader
...
def train_list(
    ...
    try:
        from shadowbot.train.agents.storage import list_training_sessions
    except ImportError:
...
def train_show(
    ...
    Examples:
        ...
        shadowbot train show train-abc123
        
        # Show with detailed iterations
        shadowbot train show train-abc123 --iterations
        
        # Output as JSON
        shadowbot train show train-abc123 --json
    """
    from ..output.console import get_output_controller
    ...
    try:
        from shadowbot.train.agents.storage import TrainingStorage
    except ImportError:
        output.print_error("Training module not available")
        raise typer.Exit(1)
...
def train_apply(
    ...
    Examples:
        ...
        shadowbot train apply train-abc123
        
        # Apply specific iteration
        shadowbot train apply train-abc123 --iteration 2
        
        # Apply to agent from YAML file
        shadowbot train apply train-abc123 --agent my_agent.yaml
        
        # Apply and run immediately
        shadowbot train apply train-abc123 --run "Hello, how are you?"
    """
    from ..output.console import get_output_controller
    ...
    try:
        from shadowbot.train.agents import apply_training, get_training_profile
        from shadowbot.train.agents.storage import TrainingStorage
    except ImportError as e:
    ...
    if run_prompt:
        ...
        output.print_info(
            ...
            f"  shadowbot train apply {session_id} --run 'Your prompt here'\n"
            "\nOr in Python:\n"
            f"  from shadowbot.train.agents import apply_training\n"
...
def _load_agent_from_file(file_path: str, output) -> Optional[object]:
    ...
    if path.suffix in ('.yaml', '.yml'):
        try:
            from shadowbot.cli.main import ShadowBot
            praison = ShadowBot(agent_file=str(path))
