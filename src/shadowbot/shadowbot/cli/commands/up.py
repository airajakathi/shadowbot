"""ShadowBot Up — Unified startup command.

Starts Langfuse + Langflow + observability in one command for
the complete ShadowBot × Langfuse × Langflow experience.
"""
...
app = typer.Typer(name="up", help="Start unified ShadowBot stack (Langfuse + Langflow + tracing)")
...
def up_start(
    ...
    """Start unified ShadowBot stack.
    
    Launches Langfuse (observability) + Langflow (visual builder) with
    automatic observability wiring. The complete ShadowBot experience.
    
    Examples:
        shadowbot up
        shadowbot up --no-langfuse  # Langflow only
        shadowbot up --no-langflow  # Langfuse only
        shadowbot up --host 0.0.0.0 --langfuse-port 3001
    """
    ...
    console.print(Panel.fit(
        "[bold green]🚀 ShadowBot Stack Starting[/bold green]\n\n"
    ...
    try:
        ...
        if not no_langfuse:
            try:
                # Check if shadowbot langfuse command exists
                langfuse_cmd = [
                    sys.executable, "-m", "shadowbot.cli.main", "langfuse",
                ...
                console.print("[yellow]Try: pip install 'shadowbot[langfuse]'[/yellow]")
                if not no_langflow:
        ...
        if not no_langflow:
            try:
                # Get path to shadowbot flow command
                langflow_cmd = [
                    sys.executable, "-m", "shadowbot.cli.main", "flow",
                ...
                console.print("[yellow]Try: pip install 'shadowbot[flow]'[/yellow]")
        ...
        console.print(Panel(
            ...
            "2. Use ShadowBot Agent/AgentTeam components from the sidebar\n"  
            "3. Run workflows with: [cyan]shadowbot run workflow.yaml --observe langfuse[/cyan]\n"
...
def up_status(
    ...
    """Check status of ShadowBot services."""
    console = Console()
...
def up_logs(
    ...
    """Show logs from ShadowBot services."""
    console = Console()
