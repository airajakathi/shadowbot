"""
Docs command group for ShadowBot CLI.

Provides documentation commands including code execution validation.

Usage:
    shadowbot docs run                    # Run all Python blocks from docs
    shadowbot docs list                   # List discovered code blocks
    shadowbot docs run --dry-run          # Extract only, no execution
    shadowbot docs cli run-all            # Validate all CLI commands from docs
"""
...
def _get_default_docs_path() -> Path:
    ...
    candidates = [
        Path.home() / "ShadowBotDocs" / "docs",
        Path("/Users/praison/ShadowBotDocs/docs"),
        Path.cwd() / "docs",
...
def docs_run(
    ...
    Examples:
        shadowbot docs run
        shadowbot docs run --docs-path /path/to/docs --timeout 120
        shadowbot docs run --group models --group tools --max-snippets 10
        shadowbot docs run --include "quickstart*" --fail-fast
    """
    # Lazy import to avoid loading at CLI startup
    from shadowbot.suite_runner import DocsSource, SuiteExecutor, RunResult
    ...
    if not quiet:
        typer.echo("=" * 60)
        typer.echo("ShadowBot Docs Code Execution")
        typer.echo("=" * 60)
...
def docs_list(
    ...
    Examples:
        shadowbot docs list
        shadowbot docs list --groups
        shadowbot docs list --group models --code
    """
    from shadowbot.suite_runner import DocsSource
...
def docs_stats(
    ...
    Examples:
        shadowbot docs stats
        shadowbot docs stats --group models --group tools
    """
...
def docs_run_all(
    ...
    Examples:
        shadowbot docs run-all
        shadowbot docs run-all --sequential
        shadowbot docs run-all --workers 8 --timeout 120
    """
    import json
    ...
    from shadowbot.suite_runner import RunReport, RunResult, SuiteReporter
...
def docs_generate(
    package: Optional[Path] = typer.Option(
        None,
        "--package", "-p",
        help="Path to shadowbotagents package (default: auto-detect)",
    ),
    output: Optional[Path] = typer.Option(
        None,
        "--output", "-o",
        help="Output directory for MDX files (default: ~/ShadowBotDocs/docs/sdk/reference/shadowbotagents)",
    ),
    ...
    Parses shadowbotagents package using griffe and generates Mintlify-compatible
    MDX documentation for all public symbols in _LAZY_IMPORTS.
    
    Examples:
        shadowbot docs generate                    # Generate all reference docs
        shadowbot docs generate --dry-run          # Preview without writing
        shadowbot docs generate --module agent     # Generate only agent module
        shadowbot docs generate --update-nav       # Also update docs.json
    """
    import subprocess
    import time
    ...
    # Default paths
    default_package = Path("/Users/praison/shadowbot-package/src/shadowbot-agents/shadowbotagents")
    default_output = Path("/Users/praison/ShadowBotDocs/docs/sdk/reference/shadowbotagents")
    generator_script = Path("/Users/praison/ShadowBot-tools/scripts/generate-sdk-docs.py")
    ...
    typer.echo("=" * 60)
    typer.echo("ShadowBot SDK Documentation Generator")
    typer.echo("=" * 60)
...
def docs_report(
    ...
    Examples:
        shadowbot docs report                    # View latest report
        shadowbot docs report ./my-report        # View specific report
        shadowbot docs report --wide --limit 0   # Full details
        shadowbot docs report --match failed     # Only failures
        shadowbot docs report --format json      # JSON output
    """
    # Lazy import to avoid loading at CLI startup
    from shadowbot.suite_runner.report_viewer import view_report
...
def docs_api_md(
    ...
    covering all public exports from shadowbotagents, shadowbot, CLI, and TypeScript.
    
    Examples:
        shadowbot docs api-md              # Generate api.md
        shadowbot docs api-md --check      # Check if api.md is up to date
        shadowbot docs api-md --stdout     # Print to stdout
    """
    from shadowbot._dev.api_md import generate_api_md
...
def cli_run_all(
    docs_path: Optional[Path] = typer.Option(
        None,
        "--docs-path", "-p",
        help="Path to documentation directory (default: ~/ShadowBotDocs/docs/cli)",
    ),
    timeout: int = typer.Option(
        10,
        "--timeout", "-t",
    ...
    Discovers shadowbot CLI commands in bash code blocks and validates
    them by running with --help to ensure they exist and work.
    
    Examples:
        shadowbot docs cli run-all
        shadowbot docs cli run-all --max-items 10
        shadowbot docs cli run-all --group cli --workers 8
        shadowbot docs cli run-all --ci --timeout 5
    """
    import subprocess
    import shlex
    from concurrent.futures import ThreadPoolExecutor, as_completed
    from shadowbot.suite_runner import CLIDocsSource, RunResult, RunReport, SuiteReporter
    ...
    if docs_path:
        ...
        candidates = [
            Path.home() / "ShadowBotDocs" / "docs" / "cli",
            Path("/Users/praison/ShadowBotDocs/docs/cli"),
            Path.cwd() / "docs" / "cli",
        ]
        docs = None
        for candidate in candidates:
    ...
    if ci:
        print("=" * 60)
        print("ShadowBot CLI Docs Validator")
        print("=" * 60)
...
def cli_list(
    ...
    Examples:
        shadowbot docs cli list
        shadowbot docs cli list --groups
        shadowbot docs cli list --runnable
    """
    from shadowbot.suite_runner import CLIDocsSource
...
def cli_report(
    ...
    Examples:
        shadowbot docs cli report
        shadowbot docs cli report ./my-report
        shadowbot docs cli report --format json
    """
    from shadowbot.suite_runner.report_viewer import view_report
