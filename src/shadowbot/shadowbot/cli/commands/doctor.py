"""
Doctor command group for ShadowBot CLI.
...
def doctor_cleanup(
    ...
    This removes stale shadowbotagents directories from site-packages that
    lack an __init__.py file (namespace package artifacts).
    
    Examples:
        shadowbot doctor cleanup           # Show what would be removed
        shadowbot doctor cleanup --execute # Actually remove stale artifacts
    """
    import os
    import site
    import shutil
    ...
    output.print_warning(f"Found {len(stale_dirs)} stale shadowbotagents directory(ies):")
    for d in stale_dirs:
    ...
    if removed == len(stale_dirs):
        ...
        output.console.print("[dim]Run 'pip install shadowbotagents' to reinstall if needed.[/dim]")
    else:
...
def doctor_troubleshoot():
    ...
    - ImportError: cannot import name 'Agent' from 'shadowbotagents'
    - Namespace package shadowing issues
    - Stale package artifacts
    """
    output = get_output_controller()
    
    output.console.print("\n[bold cyan]ShadowBot Import Troubleshooting[/bold cyan]\n")
    
    # Check current state
    output.console.print("[bold]1. Checking shadowbotagents package state...[/bold]")
    
    try:
        import shadowbotagents
        file_attr = shadowbotagents.__file__
        spec_origin = shadowbotagents.__spec__.origin if shadowbotagents.__spec__ else None
        
        if file_attr is None:
            output.print_error("  ✗ shadowbotagents.__file__ is None (namespace package detected)")
            output.console.print("    [yellow]This indicates stale artifacts in site-packages.[/yellow]")
        else:
            output.print_success(f"  ✓ shadowbotagents.__file__: {file_attr}")
        
        if spec_origin is None:
            output.print_error("  ✗ shadowbotagents.__spec__.origin is None")
        else:
            output.print_success(f"  ✓ shadowbotagents.__spec__.origin: {spec_origin}")
        ...
        output.print_error(f"  ✗ Cannot import shadowbotagents: {e}")
    ...
    for sp in site_packages:
        ...
        if os.path.isdir(shadowbot_dir):
            init_path = os.path.join(shadowbot_dir, '__init__.py')
            if not os.path.exists(init_path):
                output.print_error(f"  ✗ Stale directory found: {shadowbot_dir}")
                stale_found = True
            else:
                output.print_success(f"  ✓ Valid package at: {shadowbot_dir}")
...
1. Run cleanup to remove stale artifacts:
   [green]shadowbot doctor cleanup --execute[/green]

2. Reinstall shadowbotagents:
   [green]pip install --force-reinstall shadowbotagents[/green]

3. Or manually remove stale directory:
   [green]rm -rf $(python -c "import site; print(site.getsitepackages()[0])")/shadowbotagents/[/green]

[cyan]For editable installs:[/cyan]
   [green]pip install -e /path/to/shadowbot-agents[/green]
""")
