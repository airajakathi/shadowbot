...
- `shadowbot index` - Index documents into knowledge base
- `shadowbot query` - Query with structured answer and citations
- `shadowbot chat` - Conversational interface with knowledge (handled by chat.py)
...
def index_command(
    ...
    Use `shadowbot query` to get answers with citations from the indexed knowledge.
    
    Examples:
        shadowbot index ./docs
        shadowbot index paper.pdf --collection research
        shadowbot index ./data --profile --profile-out ./profile.json
    """
    from rich.console import Console
...
def query_command(
    ...
    Examples:
        shadowbot query "What is the main finding?"
        shadowbot query "Summarize the document" --collection research
        shadowbot query "Key points?" --top-k 10 --no-citations
        shadowbot query "Summary?" --hybrid --rerank
    """
    from rich.console import Console
    ...
    with retrieval_profiler(profile, profile_out, profile_top) as profile_data:
        try:
            ...
            from shadowbotagents.rag.retrieval_config import RetrievalConfig, RetrievalPolicy, CitationsMode
...
def search_command(
    ...
    For answers with citations, use `shadowbot query`.
    
    Examples:
        shadowbot search "capital of France"
        shadowbot search "main findings" --collection research --top-k 10
    """
    from rich.console import Console
