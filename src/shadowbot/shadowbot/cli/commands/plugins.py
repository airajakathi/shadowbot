"""
Plugins command group for ShadowBot CLI.
...
def plugins_list(
    ...
    Examples:
        ...
        shadowbot plugins list --enabled
        shadowbot plugins list --json
    """
    try:
...
def plugins_info(
    ...
    Examples:
        shadowbot plugins info memory-core
        shadowbot plugins info browser-tool
    """
    try:
        plugins = _get_available_plugins()
...
def plugins_enable(
    ...
    Examples:
        shadowbot plugins enable memory-core
        shadowbot plugins enable browser-tool
    """
    try:
...
def plugins_disable(
    ...
    Examples:
        shadowbot plugins disable memory-core
        shadowbot plugins disable browser-tool
    """
    try:
        config_path = _get_config_path()
        config = _load_config(config_path)
...
def plugins_create(
    ...
    description: str = typer.Option("A ShadowBot plugin", "--description", "-d", help="Plugin description"),
    ...
    Examples:
        shadowbot plugins create weather
        shadowbot plugins create weather --description "Weather API tools"
        shadowbot plugins create weather -o ./my_plugins/
    """
    try:
        from pathlib import Path
        from shadowbotagents.plugins.discovery import get_plugin_template, ensure_plugin_dir
        ...
        typer.echo(f"  2. Run 'shadowbot plugins discover' to verify")
        typer.echo(f"  3. Use tools with Agent(tools=['example_tool'])")
        
    except ImportError as e:
        typer.echo(f"Error: shadowbotagents package not found. Install with: pip install shadowbotagents", err=True)
        raise typer.Exit(1)
    except Exception as e:
...
def plugins_install(
    ...
    Examples:
        shadowbot plugins install ./my_weather_plugin.py
        shadowbot plugins install ./my_weather_plugin.py --force
    """
    try:
        from pathlib import Path
        import shutil
        from shadowbotagents.plugins.discovery import ensure_plugin_dir
        from shadowbotagents.plugins.parser import parse_plugin_header_from_file
        ...
        typer.echo(f"Error: shadowbotagents package not found.", err=True)
        raise typer.Exit(1)
    except typer.Exit:
        raise
    except Exception as e:
...
def plugins_discover(
    ...
    Examples:
        shadowbot plugins discover
        shadowbot plugins discover --load
        shadowbot plugins discover --json
    """
    try:
        from shadowbotagents.plugins.discovery import discover_plugins, discover_and_load_plugins
        ...
        if json_output:
            ...
            if not plugins:
                ...
                console.print("\nCreate one with: shadowbot plugins init <name>")
                return
...
def plugins_remove(
    ...
    Examples:
        shadowbot plugins remove weather
        shadowbot plugins remove weather_plugin.py --force
    """
    try:
        from pathlib import Path
        from shadowbotagents.plugins.discovery import get_default_plugin_dirs
        ...
        # Fallback without shadowbotagents
        from pathlib import Path
        user_dir = Path.home() / ".praison" / "plugins"
        if not name.endswith(".py"):
            name = f"{name}.py"
...
def _get_available_plugins():
    ...
    try:
        from shadowbotagents.plugins import get_plugin_registry
...
def plugins_callback(ctx: typer.Context):
    ...
    if ctx.invoked_subcommand is None:
        help_text = """
[bold cyan]ShadowBot Plugins - Plugin Management[/bold cyan]

Manage plugins with: shadowbot plugins <command>
