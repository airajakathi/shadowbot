"""
Skills command group for ShadowBot CLI.
...
def skills_install(
    ...
    dest: str = typer.Option(None, "--dest", "-d", help="Install destination (defaults to ~/.shadowbot/skills)"),
):
    """Install a skill from a local path or git URL.

    Local path:  ``shadowbot skills install ./my-skill``
    Git URL:     ``shadowbot skills install https://github.com/org/repo``
    """
    import shutil
    import subprocess
    import tempfile
    ...
    from shadowbotagents.skills import validate as validate_skill
...
def skills_search(
    ...
    try:
        from shadowbotagents.skills import discover_skills
        import re
...
def skills_info(
    ...
    try:
        from shadowbotagents.skills import discover_skills, SkillLoader
        from pathlib import Path
...
def skills_check(
    ...
    Examples:
        shadowbot skills check
        shadowbot skills check --verbose
    """
    try:
        from shadowbotagents.skills import discover_skills
        import os
...
def skills_eligible(
    ...
    Examples:
        shadowbot skills eligible
        shadowbot skills eligible --json
    """
    try:
        from shadowbotagents.skills import discover_skills
        import os
