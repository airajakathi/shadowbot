"""
Eval command group for ShadowBot CLI.
...
def eval_judge(
    ...
    Examples:
        shadowbot eval judge --output "4" --expected "4"
        shadowbot eval judge --output "Hello!" --criteria "Response is helpful"
        shadowbot eval judge --file output.txt --criteria "Code is correct"
    """
    try:
        from shadowbotagents.eval import Judge, JudgeConfig
    except ImportError:
        typer.echo("Error: shadowbotagents package required. Install with: pip install shadowbotagents")
        raise typer.Exit(1)
