"""
Run command group for ShadowBot CLI.
...
def run_main(
    ...
    framework: Optional[str] = typer.Option(None, "--framework", "-f", help="Framework: shadowbot, crewai, autogen"),
    ...
    Examples:
        shadowbot run agents.yaml
        shadowbot run "What is the weather?"
        shadowbot run agents.yaml --interactive
        shadowbot run "What is 2+2?" --profile
    """
    ...
    if not target:
        output.print_panel(
            ...
            "  --framework, -f   Framework (shadowbot, crewai, autogen)\n"
...
def _run_from_file(
    ...
    try:
        # Use existing ShadowBot class
        from shadowbot.cli.main import ShadowBot
        
        praison = ShadowBot(
            agent_file=file_path,
            framework=framework or "shadowbot",
        )
...
def _run_prompt(
    ...
    try:
        ...
        if output_mode == "actions":
            ...
            if approval:
                from shadowbot.cli.features.approval import resolve_approval_config
