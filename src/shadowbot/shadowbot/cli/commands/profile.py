"""
Profile command group for ShadowBot CLI.
...
def profile_query(
    ...
    Examples:
        ...
        shadowbot profile query "Hello" --limit 20
        shadowbot profile query "Test" --deep --show-callers --show-callees
        shadowbot profile query "Test" --save ./profile_results
    """
    ...
    try:
        ...
        typer.echo("Install shadowbotagents: pip install shadowbotagents", err=True)
        raise typer.Exit(1)
...
def profile_imports():
    ...
    try:
        result = subprocess.run(
            [sys.executable, "-X", "importtime", "-c", "import shadowbotagents"],
            capture_output=True,
            text=True,
            timeout=60,
        )
...
def profile_startup():
    ...
    subprocess.run(
        [sys.executable, "-c", "import shadowbot; import shadowbot.cli"],
        capture_output=True,
        text=True,
    )
...
@app.command("suite")
def profile_suite(
    output_dir: Optional[str] = typer.Option("/tmp/shadowbot_profile_suite", "--output", "-o", help="Output directory for results"),
    ...
    Examples:
        shadowbot profile suite
        shadowbot profile suite --output ./my_profile_results
...
def profile_snapshot(
    ...
    Examples:
        ...
        shadowbot profile snapshot v2.0 --format json
    """
    try:
    ...
    if compare:
        ...
        if not baseline_snap:
            typer.echo("❌ No baseline found. Create one with: shadowbot profile snapshot --baseline", err=True)
            raise typer.Exit(1)
        ...
        result = run_profile_suite(
            output_dir="/tmp/shadowbot_snapshot_compare",
            scenarios=scenarios,
            output="minimal",
        )
        ...
        result = run_profile_suite(
            output_dir="/tmp/shadowbot_snapshot",
            scenarios=scenarios,
            verbose=True,
        )
...
def profile_optimize(
    ...
    Examples:
        ...
        shadowbot profile optimize --prewarm
        shadowbot profile optimize --lite
    """
    try:
        from ..features.profiler import (
            PrewarmManager,
