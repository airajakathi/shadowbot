"""
Sandbox command group for ShadowBot CLI.
...
def sandbox_status(
    ...
    Examples:
        shadowbot sandbox status
        shadowbot sandbox status --json
    """
    try:
...
def sandbox_explain(
    ...
    Examples:
        shadowbot sandbox explain
        shadowbot sandbox explain --session main
        shadowbot sandbox explain --agent work
    """
    try:
        from rich.console import Console
...
def sandbox_list(
    ...
    Examples:
        shadowbot sandbox list
        shadowbot sandbox list --browser
        shadowbot sandbox list --json
    """
    try:
...
def sandbox_recreate(
    ...
    Examples:
        shadowbot sandbox recreate --all
        shadowbot sandbox recreate --session main
        shadowbot sandbox recreate --agent mybot
        shadowbot sandbox recreate --browser --force
    """
...
def _get_sandbox_info():
    ...
    info = {
        ...
        "image": "shadowbot-sandbox:latest",
        "containers": [],
    }
    ...
    if info["docker_available"]:
        ...
        try:
            ...
            result = subprocess.run(
                ["docker", "ps", "--filter", "name=shadowbot-sbx-", "--format", "{{.Names}}\t{{.Status}}"],
                capture_output=True,
                text=True,
                timeout=5,
            )
...
def _get_sandbox_containers(browser_only: bool = False):
    ...
    try:
        ...
        filter_name = "shadowbot-browser-" if browser_only else "shadowbot-sbx-"
        result = subprocess.run(
...
def sandbox_callback(ctx: typer.Context):
    ...
    if ctx.invoked_subcommand is None:
        help_text = """
[bold cyan]ShadowBot Sandbox - Container Management[/bold cyan]

Manage sandbox containers with: shadowbot sandbox <command>
...
[bold]Examples:[/bold]
  ...
  shadowbot sandbox explain --agent work
  shadowbot sandbox list --browser
  shadowbot sandbox recreate --all --force
"""
        try:
