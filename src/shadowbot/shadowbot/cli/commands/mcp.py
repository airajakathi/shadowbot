"""
MCP command group for ShadowBot CLI.
...
def mcp_list(
    ...
    if not servers:
        ...
        output.print("\nAdd a server with: shadowbot mcp add <name> <command>")
        return
...
def mcp_sync(
    ...
    for srv in servers_to_sync:
        ...
        try:
            ...
            output.print_error("  MCP package not installed. Install with: pip install shadowbotagents[mcp]")
...
def mcp_tools(
    ...
    try:
        from shadowbot.mcp_server.tool_index import MCPToolIndex
        ...
        if server:
            ...
            if not tools:
                ...
                output.print("\nRun 'shadowbot mcp sync' to index tools")
                return
        else:
...
def mcp_run(
    ...
    DEPRECATED: Use `shadowbot serve mcp --name <server>` instead.
    ...
    Examples:
        shadowbot mcp run my-server
        shadowbot mcp run my-server --transport sse --port 8080
    """
    import sys
    
    # Print deprecation warning
    print("\n\033[93m⚠ DEPRECATION WARNING:\033[0m", file=sys.stderr)
    print("\033[93m'shadowbot mcp run' is deprecated and will be removed in a future version.\033[0m", file=sys.stderr)
    print("\033[93mPlease use 'shadowbot serve mcp --name <server>' instead.\033[0m\n", file=sys.stderr)
    ...
    try:
        ...
        if transport == "stdio":
            ...
            # Try to use shadowbotagents MCP server wrapper
            try:
                from shadowbotagents.mcp import MCP
...
def mcp_auth(
    ...
    Examples:
        shadowbot mcp auth github
        shadowbot mcp auth tavily --timeout 60
    """
    output = get_output_controller()
    loader = get_config_loader()
    ...
    try:
        from shadowbotagents.mcp import (
            MCPAuthStorage,
...
def mcp_logout(
    ...
    Examples:
        shadowbot mcp logout github
        shadowbot mcp logout tavily --yes
    """
    output = get_output_controller()
    
    try:
        from shadowbotagents.mcp import MCPAuthStorage
