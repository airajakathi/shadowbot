"""
RAG command group for ShadowBot CLI.
...
shadowbot profile infrastructure.
"""
...
def rag_profiler(enabled: bool, profile_out: Optional[Path], profile_top: int = 20):
    ...
    Uses the existing shadowbot.profiler infrastructure when available,
    ...
    profiler = None
    try:
        from shadowbot.profiler import Profiler
        Profiler.enable()
        Profiler.clear()
        profiler = Profiler
    except ImportError:
...
def rag_index(
    ...
    Equivalent to: shadowbot knowledge index
    
    Examples:
        shadowbot rag index ./docs
        shadowbot rag index paper.pdf --collection research
        shadowbot rag index ./data --chunking semantic --chunk-size 256
        shadowbot rag index ./docs --profile --profile-out ./profile.json
    """
    from rich.console import Console
    ...
    with rag_profiler(profile, profile_out, profile_top) as profile_data:
        try:
            ...
            console.print("Install with: pip install 'shadowbotagents[knowledge]'")
            raise typer.Exit(1)
        except Exception as e:
...
def rag_query(
    ...
    Examples:
        shadowbot rag query "What is the main finding?"
        shadowbot rag query "Summarize the document" --collection research
        shadowbot rag query "Key points?" --top-k 10 --no-citations
        shadowbot rag query "Summary?" --hybrid --rerank
        shadowbot rag query "Summary?" --profile --profile-out ./profile.json
    """
    from rich.console import Console
    ...
    with rag_profiler(profile, profile_out, profile_top) as profile_data:
        try:
            from shadowbotagents.knowledge import Knowledge
            from shadowbotagents.rag import RAG, RAGConfig
            from shadowbotagents.rag.models import RetrievalStrategy
...
def rag_chat(
    ...
    Examples:
        shadowbot rag chat
        shadowbot rag chat --collection research
        shadowbot rag chat --hybrid --rerank
        shadowbot rag chat --no-stream
    """
    from rich.console import Console
    from rich.panel import Panel
    from rich.markdown import Markdown
...
def rag_eval(
    ...
    Examples:
        shadowbot rag eval golden_queries.json
        shadowbot rag eval tests.json --collection research --output results.json
        shadowbot rag eval tests.json --profile --profile-out ./profile.json
    """
    import json
    from rich.console import Console
...
def rag_serve(
    ...
    DEPRECATED: Use `shadowbot serve rag` instead.
    ...
    Examples:
        shadowbot rag serve
        shadowbot rag serve --collection research --port 9000
        shadowbot rag serve --hybrid --rerank
        shadowbot rag serve --openai-compat --port 8080
        shadowbot rag serve --profile --profile-out ./profile.json
    """
    import sys
    
    # Print deprecation warning
    print("\n\033[93m⚠ DEPRECATION WARNING:\033[0m", file=sys.stderr)
    print("\033[93m'shadowbot rag serve' is deprecated and will be removed in a future version.\033[0m", file=sys.stderr)
    print("\033[93mPlease use 'shadowbot serve rag' instead.\033[0m\n", file=sys.stderr)
    ...
    with rag_profiler(profile, profile_out, profile_top) as profile_data:
        try:
            ...
            console.print("Install with: pip install 'shadowbot[rag-api]'")
            raise typer.Exit(1)
