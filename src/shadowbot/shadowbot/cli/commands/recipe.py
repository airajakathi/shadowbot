"""
Recipe command group for ShadowBot CLI.
...
def recipe_create(
    ...
    Examples:
        shadowbot recipe create "Build a web scraper for news"
        shadowbot recipe create "Research AI trends" --no-optimize
        shadowbot recipe create "Research AI" --agents "researcher:role=AI Researcher,goal=Find papers"
        shadowbot recipe create "Research AI" --tools "researcher:internet_search,arxiv"
    """
...
def recipe_run(
    ...
    Examples:
        shadowbot recipe run ai-url-blog-generator --var url="https://example.com/article"
        shadowbot recipe run ai-dynamic-blog-generator --var topic="LangGraph 0.3" --var style="coding"
        shadowbot recipe run ai-wordpress-post-generator --save --verbose
        shadowbot recipe run ai-topic-gatherer --save --name my-test-run --var topic="AI"
    """
    from shadowbot.cli.main import ShadowBot
    import sys
...
def recipe_info(
    ...
    Examples:
        shadowbot recipe info image-to-blog-generator
        shadowbot recipe info url-to-blog-generator --verbose
    """
    ...
    try:
        from shadowbot.recipe.core import get_template_search_paths
    ...
    if not recipe_path:
        ...
        console.print("[dim]Hint: Run 'shadowbot recipe list' to see available recipes[/dim]")
        raise typer.Exit(1)
...
def recipe_judge(
    ...
    Examples:
        shadowbot recipe judge run-abc123
        shadowbot recipe judge run-abc123 --goal "Analyze image and create blog post"
        shadowbot recipe judge run-abc123 --memory
        shadowbot recipe judge run-abc123 --knowledge
        shadowbot recipe judge run-abc123 --yaml agents.yaml --output plan.yaml
        shadowbot recipe judge run-abc123 --dry-run
    """
    ...
    try:
        ...
        if yaml_file:
            ...
            if not dry_run:
                ...
                print(f"   Run 'shadowbot recipe apply {output_file}' to apply fixes")
        else:
...
def recipe_optimize(
    ...
    Examples:
        shadowbot recipe optimize my-recipe
        shadowbot recipe optimize my-recipe "improve error handling"
        shadowbot recipe optimize my-recipe --iterations 5 --threshold 9.0
        shadowbot recipe optimize my-recipe --input '{"query": "test"}'
        shadowbot recipe optimize my-recipe --criteria "Water flow is optimal: no leaks, pressure within range"
    """
    from pathlib import Path
    import logging
    ...
    try:
        from shadowbot.cli.features.recipe_optimizer import RecipeOptimizer
        
        # Create custom judge if criteria provided
        custom_judge = None
        if criteria:
            try:
                from shadowbotagents.eval import Judge
                custom_judge = Judge(criteria=criteria)
                print(f"   Custom criteria: {criteria[:50]}{'...' if len(criteria) > 50 else ''}")
            except ImportError:
                print("   ⚠️ Custom criteria requires shadowbotagents. Using default judge.")
...
def recipe_serve(
    ...
    DEPRECATED: Use `shadowbot serve recipe` instead.
    
    Serves recipes as REST API endpoints.
    
    Examples:
        shadowbot recipe serve
        shadowbot recipe serve my-recipe --port 8080
        shadowbot recipe serve --host 0.0.0.0 --port 8000
        shadowbot recipe serve --config serve.yaml
        shadowbot recipe serve --api-key my-secret-key
        shadowbot recipe serve --workers 4
    """
    import sys
    
    # Print deprecation warning
    print("\n\033[93m⚠ DEPRECATION WARNING:\033[0m", file=sys.stderr)
    print("\033[93m'shadowbot recipe serve' is deprecated and will be removed in a future version.\033[0m", file=sys.stderr)
    print("\033[93mPlease use 'shadowbot serve recipe' instead.\033[0m\n", file=sys.stderr)
    ...
    try:
        from shadowbot.recipe.serve import serve, load_config
...
def recipe_apply(
    ...
    Examples:
        shadowbot recipe apply judge_plan.yaml
        shadowbot recipe apply judge_plan.yaml --confirm
        shadowbot recipe apply judge_plan.yaml --fix-ids fix_001,fix_002
        shadowbot recipe apply judge_plan.yaml --dry-run
    """
    from shadowbot.replay import JudgePlan, PlanApplier
    from pathlib import Path
