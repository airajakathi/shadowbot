"""
Chat command group for ShadowBot CLI.
...
def chat_main(
    ...
    debug: bool = typer.Option(False, "--debug", help="Enable debug logging to ~/.shadowbot/async_tui_debug.log"),
    ...
    For browser-based chat UI, use: shadowbot ui chat
    
    Examples:
        shadowbot chat
        shadowbot chat "Hello, how are you?"
        shadowbot chat --model gpt-4o --memory
        shadowbot chat --continue  # Resume last session
        shadowbot chat "Summarize this" --file README.md
        shadowbot chat "What is 2+2?" --profile
    """
    import os
    ...
    if (profile or profile_deep) and not prompt:
        ...
        typer.echo("   Use: shadowbot chat \"your prompt\" --profile", err=True)
    ...
    if approval:
        from shadowbot.cli.features.approval import resolve_approval_backend
        try:
...
def _run_profiled_chat(
    ...
    from shadowbot.cli.features.cli_profiler import (
        CLIProfileConfig,
        CLIProfiler,
    )
