"""
Config command group for ShadowBot CLI.
...
def config_list(
    ...
    output.print_panel("Configuration", title="ShadowBot Config")
...
def config_env(
    ...
    try:
        from shadowbotagents.config import ConfigRegistry, ConfigScope, validate_config
    except ImportError:
    ...
    if validate:
        ...
        if validation_results["unknown_vars"]:
            output.print("\n[bold red]Unknown ShadowBot-related environment variables:[/bold red]")
    ...
    output.print_panel(f"Environment Variables{scope_title}", title="ShadowBot Configuration")
...
def config_doctor():
    ...
    try:
        from shadowbotagents.config import validate_config, ConfigRegistry
    except ImportError:
        output.print_error("Unified configuration resolver not available")
        raise typer.Exit(1)
    
    output.print_panel("Configuration Doctor", title="Diagnosing ShadowBot Configuration")
    ...
    output.print(f"[bold cyan]ShadowBot variables:[/bold cyan] {praison_vars}")
    ...
    # Performance check
    from shadowbotagents.config.resolver import ConfigRegistry
