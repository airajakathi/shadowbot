...
def export(
    ...
    """Export ShadowBot YAML workflow to n8n JSON format.
    
    Example:
        shadowbot n8n export my-workflow.yaml --output workflow.json
    """
    if format != 'n8n':
    ...
    try:
        from shadowbot.n8n import YAMLToN8nConverter
        ...
        typer.echo(f"💡 Import this file into n8n or use 'shadowbot n8n preview {yaml_path}' to open directly")
        
    except ImportError:
        typer.echo("Error: n8n dependencies not installed. Run: pip install 'shadowbot[n8n]'", err=True)
        raise typer.Exit(1)
...
def import_workflow(
    ...
    """Import n8n JSON workflow to ShadowBot YAML format.
    
    Example:
        shadowbot n8n import workflow.json --output my-workflow.yaml
    """
    if format != 'n8n':
        typer.echo(f"Error: Unsupported format '{format}'. Only 'n8n' is supported.", err=True)
        raise typer.Exit(1)
    ...
    try:
        from shadowbot.n8n import N8nToYAMLConverter
        import yaml as yaml_lib
        import json
        ...
        typer.echo(f"💡 Run 'shadowbot run {output}' to execute the workflow")
...
def preview(
    ...
    """Preview ShadowBot workflow in n8n visual editor.
    
    This command converts your YAML workflow to n8n format and opens it
    in the n8n visual editor for preview and editing.
    
    Example:
        shadowbot n8n preview my-workflow.yaml
        shadowbot n8n preview my-workflow.yaml --n8n-url http://n8n.example.com
    """
    try:
        from shadowbot.n8n import preview_workflow
...
def pull(
    ...
    Example:
        shadowbot n8n pull abc123 my-workflow.yaml
    """
    try:
        from shadowbot.n8n import export_from_n8n
        ...
        typer.echo(f"💡 Run 'shadowbot run {output_path}' to execute the workflow")
...
def push(
    ...
    Example:
        shadowbot n8n push my-workflow.yaml abc123
    """
    try:
        from shadowbot.n8n import sync_workflow
...
def test_connection(
    ...
    Example:
        shadowbot n8n test
        shadowbot n8n test --n8n-url http://n8n.example.com
    """
    client = None
    try:
        from shadowbot.n8n import N8nClient
