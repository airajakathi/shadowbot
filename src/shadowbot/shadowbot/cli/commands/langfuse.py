"""ShadowBot Langfuse — Observability platform for LLM apps.

Provides `shadowbot langfuse` CLI commands to manage local (Docker) and remote
...
def langfuse_start(
    ...
    data_dir: str = typer.Option(
        None, "--data-dir", help="Data directory (default: ~/.shadowbot/langfuse)"
    ),
    ...
    Examples:
        shadowbot langfuse
        shadowbot langfuse --port 8080
        shadowbot langfuse --no-detach
        shadowbot langfuse --email admin@example.com --password mypassword123
    """
    ...
    if data_dir is None:
        data_dir = os.path.expanduser("~/.shadowbot/langfuse")
    ...
    try:
        if detach:
            ...
            console.print("3. Configure ShadowBot:")
            console.print(f"   [green]shadowbot langfuse config --public-key pk-... --secret-key sk-...[/green]")
            console.print("4. Test: [green]shadowbot langfuse test[/green]")
        ...
        from shadowbot.cli.commands.port import _is_port_conflict_error, _extract_port_from_error, _show_port_conflict_help
        if _is_port_conflict_error(error_output):
            ...
            if port:
                ...
                console.print("[yellow]Run [cyan]shadowbot port list[/cyan] to see ports in use[/yellow]")
        else:
...
def langfuse_init(
    ...
    Examples:
        shadowbot langfuse init
        shadowbot langfuse init --email admin@example.com --password mypass123
        shadowbot langfuse init --email user@company.com --port 8080
    """
    console = Console()
...
def langfuse_status(
    ...
    if not repo_path.exists():
        console.print("[yellow]⚠️  Langfuse repository not found[/yellow]")
        console.print("Run 'shadowbot langfuse' to set up Langfuse")
        return
...
def langfuse_config(
    ...
    config_dir = Path.home() / ".shadowbot"
    ...
    if not any([public_key, secret_key, host]):
        ...
        console.print()
        console.print("2. [bold cyan]Configure ShadowBot:[/bold cyan]")
        console.print("   [dim]shadowbot langfuse config[/dim]")
        ...
        console.print("   [green]shadowbot langfuse test[/green]")
        console.print()
...
def langfuse_test():
    ...
    try:
        from shadowbot.observability.langfuse import LangfuseSink, LangfuseSinkConfig
        from shadowbotagents.trace.protocol import ActionEvent, ActionEventType, TraceEmitter
        import time
    except ImportError as e:
        console.print(f"[red]❌ Import error: {e}[/red]")
        console.print("[yellow]Install with: pip install shadowbot[langfuse][/yellow]")
        raise typer.Abort()
    
    # Load configuration from env vars and ~/.shadowbot/langfuse.env if present
    config_file = Path.home() / ".shadowbot" / "langfuse.env"
    if config_file.exists():
    ...
    if not config.enabled:
        ...
        console.print()
        console.print("[bold]Then configure ShadowBot:[/bold]")
        console.print("  [green]shadowbot langfuse config --public-key=pk-... --secret-key=sk-...[/green]")
        console.print()
    ...
    try:
        ...
        # Agent start
        emitter.agent_start(agent_name, metadata={"test": True, "cli": "shadowbot langfuse test"})
...
def langfuse_version():
    ...
    try:
        ...
        console.print("[blue]Install with: pip install shadowbot[langfuse][/blue]")
...
def langfuse_traces(
    ...
    Examples:
        shadowbot langfuse traces
        shadowbot langfuse traces --limit 10
        shadowbot langfuse traces --session abc-123
        shadowbot langfuse traces --agent MyAgent
    """
    console = Console()
    ...
    try:
        ...
        console.print("[yellow]Run 'shadowbot langfuse config --public-key ... --secret-key ...' first[/yellow]")
        raise typer.Abort()
    ...
    try:
        ...
        if traces:
            console.print(f"[dim]View details: shadowbot langfuse show {traces[0].get('id')}[/dim]")
        ...
        if "401" in str(e):
            ...
            console.print("[yellow]Ensure Langfuse is running: shadowbot langfuse status[/yellow]")
        raise typer.Abort()
    except Exception as e:
        console.print(f"[red]❌ Error: {e}[/red]")
        raise typer.Abort()
...
def langfuse_sessions(
    ...
    Examples:
        shadowbot langfuse sessions
        shadowbot langfuse sessions --limit 10
    """
    console = Console()
    ...
    try:
        ...
        # Show hint
        if sessions:
            console.print(f"[dim]View traces in session: shadowbot langfuse traces --session {sessions[0].get('id')}[/dim]")
...
def langfuse_show(
    ...
    Examples:
        shadowbot langfuse show trace-abc-123
        shadowbot langfuse show --help
    """
    console = Console()
