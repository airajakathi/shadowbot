"""Port management CLI command for ShadowBot.

Provides commands to manage port usage and resolve port conflicts:
- shadowbot port list: List processes using ports
- shadowbot port check <port>: Check if a port is in use
- shadowbot port kill <port>: Kill process using a port

Examples:
    shadowbot port list
    shadowbot port check 6379
    shadowbot port kill 6379
    shadowbot port kill 6379 --force
"""
...
def _show_port_conflict_help(port: int, console: Optional[Console] = None) -> None:
    ...
    table.add_row(
        "1. Kill process",
        f"shadowbot port kill {port}",
        "Kill the process using this port"
    )
    table.add_row(
        "2. Force kill",
        f"shadowbot port kill {port} --force",
        "Force kill if graceful kill fails"
    )
    table.add_row(
        "3. Check details",
        f"shadowbot port check {port}",
...
def port_callback(ctx: typer.Context):
    ...
    if ctx.invoked_subcommand is None:
        ...
        console.print("Use [cyan]shadowbot port list[/cyan] to see all ports in use")
        console.print("Use [cyan]shadowbot port check <port>[/cyan] to check a specific port")
        console.print("Use [cyan]shadowbot port kill <port>[/cyan] to free a port\n")
        raise typer.Exit()
...
def port_list(
    ...
    console.print(table)
    console.print()
    console.print(f"[dim]Tip: Use [cyan]shadowbot port kill <port>[/cyan] to free a port[/dim]")
...
def port_check(
    ...
    if proc:
        ...
        console.print(table)
        console.print()
        console.print(f"To free this port: [cyan]shadowbot port kill {port}[/cyan]")
        raise typer.Exit(1)
    else:
...
def port_kill(
    ...
    if success:
        ...
        if json_output:
            ...
            if not force:
                console.print("[yellow]Try: shadowbot port kill {port} --force[/yellow]")
        raise typer.Exit(1)
