"""
Serve command group for ShadowBot CLI.

Provides unified server management for ALL ShadowBot server types.
All server commands are consolidated under `shadowbot serve <type>`.
...
app = typer.Typer(
    help="Unified server management - start any ShadowBot server",
...
def serve_callback(ctx: typer.Context):
    ...
    if ctx.invoked_subcommand is None:
        help_text = """
[bold cyan]ShadowBot Serve - Unified Server Management[/bold cyan]

Start any ShadowBot server with: shadowbot serve <type>
...
[bold]Examples:[/bold]
  shadowbot serve agents --port 8000
  shadowbot serve gateway --port 8765
  shadowbot serve mcp --transport sse
  shadowbot serve unified

[bold]Discovery:[/bold]
  All HTTP servers expose /__shadowbot__/discovery for endpoint discovery.
"""
        try:
...
# =============================================================================
# UNIFIED SERVER COMMANDS - All server types under `shadowbot serve <type>`
# =============================================================================
...
def serve_agents(
    ...
    Examples:
        shadowbot serve agents
        shadowbot serve agents --file my-agents.yaml --port 8000
    """
    try:
        from ..features.serve import handle_serve_command
...
def serve_gateway(
    ...
    Examples:
        shadowbot serve gateway
        shadowbot serve gateway --port 8765 --agents agents.yaml
    """
    output = get_output_controller()
    ...
    try:
        ...
        output.print("Install with: pip install shadowbot[api]")
        raise typer.Exit(4)
...
def serve_mcp(
    ...
    Examples:
        shadowbot serve mcp --transport stdio
        shadowbot serve mcp --transport sse --port 8080
        shadowbot serve mcp --name my-server
    """
    output = get_output_controller()
    ...
    if name:
        ...
        try:
            ...
            if transport == "stdio":
                ...
                from shadowbotagents.mcp import MCP, ToolsMCPServer
                cmd_string = " ".join(cmd)
            ...
            output.print("Install with: pip install shadowbotagents[mcp]")
            raise typer.Exit(4)
    else:
        # Start default MCP server
        try:
            from shadowbotagents.mcp import ToolsMCPServer
            output.print_info(f"Starting MCP server on {host}:{port}")
            output.print(f"  Transport: {transport}")
            
            mcp_server = ToolsMCPServer(name="shadowbot-mcp")
            if transport == "stdio":
                mcp_server.run()
            else:
                mcp_server.run_sse(host=host, port=port)
...
def serve_acp(
    ...
    Examples:
        shadowbot serve acp
        shadowbot serve acp --workspace ./my-project
        shadowbot serve acp --model gpt-4o
    """
    output = get_output_controller()
    ...
    try:
        ...
        output.print("Install with: pip install shadowbot[acp]")
        raise typer.Exit(4)
...
def serve_lsp(
    ...
    Examples:
        shadowbot serve lsp
        shadowbot serve lsp --language python
    """
    output = get_output_controller()
...
def serve_ui(
    ...
    Examples:
        shadowbot serve ui
        shadowbot serve ui --type chat --port 8084
        shadowbot serve ui --type code --port 8086
    """
    output = get_output_controller()
    ...
    try:
        ...
        output.print("Install with: pip install shadowbot[ui]")
        raise typer.Exit(4)
...
def serve_rag(
    ...
    Examples:
        shadowbot serve rag
        shadowbot serve rag --collection research --port 9000
    """
    output = get_output_controller()
...
def serve_registry(
    ...
    Examples:
        shadowbot serve registry
        shadowbot serve registry --port 7777 --token mysecret
    """
    output = get_output_controller()
...
def serve_docs(
    ...
    Examples:
        shadowbot serve docs
        shadowbot serve docs --port 3000 --path ./docs
    """
    output = get_output_controller()
...
def serve_scheduler(
    ...
    Examples:
        shadowbot serve scheduler
        shadowbot serve scheduler --config schedule.yaml --daemon
    """
    output = get_output_controller()
...
def serve_recipe(
    ...
    Examples:
        shadowbot serve recipe
        shadowbot serve recipe --port 8765 --reload
    """
    try:
        from ..features.serve import handle_serve_command
...
def serve_unified(
    ...
    Examples:
        shadowbot serve unified
        shadowbot serve unified --port 8765 --reload
    """
    try:
        from ..features.serve import handle_serve_command
