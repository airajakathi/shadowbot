"""
Gateway command group for ShadowBot CLI.
...
app = typer.Typer(
    help="Manage the ShadowBot Gateway server",
...
def gateway_start(
    ...
    Examples:
        shadowbot gateway start
        shadowbot gateway start --config gateway.yaml
        shadowbot gateway start --agents agents.yaml --port 9000
    """
...
def gateway_status(
    ...
    Examples:
        shadowbot gateway status
        shadowbot gateway status --port 9000
        shadowbot gateway status --daemon-only
    """
    from ..features.gateway import GatewayHandler
...
def gateway_channels(
    ...
    Examples:
        shadowbot gateway channels
        shadowbot gateway channels --config my-gateway.yaml --json
    """
    import os
    import yaml
...
def gateway_install(
    ...
    Examples:
        shadowbot gateway install
        shadowbot gateway install --config my-bot.yaml --no-start
    """
...
def gateway_uninstall():
    ...
    Examples:
        shadowbot gateway uninstall
    """
    from shadowbot.daemon import uninstall_daemon
    from ..output.console import get_output_controller
...
def gateway_mint_link(
    ...
    Examples:
        shadowbot gateway mint-link
        shadowbot gateway mint-link --ttl 300  # 5 minutes
        shadowbot gateway mint-link --port 9000
    """
    ...
    try:
        ...
        output.print_info("Link saved to ~/.shadowbot/last-link.txt")
...
def gateway_logs(
    ...
    Examples:
        shadowbot gateway logs
        shadowbot gateway logs -n 100
    """
    ...
    try:
        if plat == "systemd":
            from shadowbot.daemon.systemd import get_logs
            ...
            from shadowbot.daemon.launchd import get_logs
            logs = get_logs(lines=lines)
            if logs:
                print(logs)
            else:
            ...
            from shadowbot.daemon.windows import get_logs
            logs = get_logs(lines=lines)
            if logs:
                print(logs)
            else:
...
def gateway_send(
    ...
    Examples:
        shadowbot gateway send --config gateway.yaml --channel telegram --channel-id 12345 -m "Hello"
    """
    import os
    import asyncio
    import yaml
    ...
    async def _send():
        try:
            from shadowbot.gateway.server import WebSocketGateway
...
def gateway_callback(ctx: typer.Context):
    ...
    if ctx.invoked_subcommand is None:
        help_text = """
[bold cyan]ShadowBot Gateway - Multi-Bot WebSocket Server[/bold cyan]

Manage the gateway server: shadowbot gateway <command>
...
[bold]Channel Management:[/bold]
  shadowbot gateway channels --config gateway.yaml
  shadowbot gateway send --config gateway.yaml --channel telegram --channel-id 12345 -m "test"
"""
        try:
