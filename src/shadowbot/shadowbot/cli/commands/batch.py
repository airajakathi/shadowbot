"""
ShadowBot Batch CLI Command.

Run all Python files with ShadowBot imports in the current directory.
Designed for quick debugging - running multiple local scripts at once.

Usage:
    shadowbot batch                    # Run all ShadowBot scripts in current dir
    shadowbot batch --sub              # Include subdirectories
    shadowbot batch --sub --depth 2    # Limit recursion depth
    shadowbot batch --parallel         # Run in parallel with async reporting
    shadowbot batch list               # List discovered files
    shadowbot batch stats              # Show statistics
    shadowbot batch report             # View latest report
"""
...
app = typer.Typer(
    help="Run all ShadowBot scripts in current folder for quick debugging",
...
def batch_run(
    ...
    """
    Run all ShadowBot scripts in the current folder.
    
    Quick debugging tool for running multiple Python files that use
    shadowbotagents or shadowbot. Unlike 'shadowbot run' (which runs
    agent configs) or 'shadowbot examples' (which runs repo examples),
    this command finds and executes local scripts for rapid testing.
    
    Only files containing 'from shadowbotagents' or 'from shadowbot'
    are executed. Test files (test_*.py) are excluded by default.
    
    Examples:
        shadowbot batch                    # Run all in current dir
        shadowbot batch --sub              # Include subdirectories
        shadowbot batch --sub --depth 2    # Limit depth
        shadowbot batch --parallel         # Run in parallel
        shadowbot batch --timeout 120      # Custom timeout
    """
    ...
    from shadowbot.suite_runner import BatchSource, SuiteExecutor, RunResult
    ...
    if not items:
        if ci:
            print(f"INFO: No ShadowBot scripts found in {search_path}")
        else:
            typer.echo(f"ℹ️ No ShadowBot scripts found in {search_path}")
            if not recursive:
    ...
    if not quiet:
        if ci:
            ...
            typer.echo("=" * 60)
            typer.echo("ShadowBot Batch Runner")
            typer.echo("=" * 60)
...
def _run_parallel(
    ...
    from shadowbot.suite_runner import RunResult, RunReport, SuiteReporter
    from datetime import timezone
    
    if ci:
        print("=" * 60)
        print("ShadowBot Batch Runner (Parallel Mode)")
        print("=" * 60)
        print(f"Path: {search_path}")
        ...
        typer.echo("=" * 60)
        typer.echo("ShadowBot Batch Runner (Parallel Mode)")
        typer.echo("=" * 60)
        typer.echo(f"Path: {search_path}")
...
def batch_list(
    ...
    """
    List discovered ShadowBot scripts without running them.
    
    Examples:
        shadowbot batch list
        shadowbot batch list --sub
        shadowbot batch list --groups
    """
    from shadowbot.suite_runner import BatchSource
    ...
    if show_groups:
        ...
        if not groups:
            typer.echo(f"No ShadowBot scripts found in {search_path}")
            return
    ...
    typer.echo(f"Found {len(items)} ShadowBot scripts in {search_path}\n")
...
def batch_stats(
    ...
    """
    Show statistics for ShadowBot scripts.
    
    Displays counts by group, runnable status, and agent-centric usage.
    
    Examples:
        shadowbot batch stats
        shadowbot batch stats --sub
    """
...
def batch_report(
    ...
    Examples:
        shadowbot batch report
        shadowbot batch report --format failures
        shadowbot batch report --dir ~/Downloads/reports/batch/20240115_120000
    """
    import json
    ...
    if not target_dir or not target_dir.exists():
        ...
        typer.echo("   Run 'shadowbot batch' first to generate a report.")
        raise typer.Exit(1)
