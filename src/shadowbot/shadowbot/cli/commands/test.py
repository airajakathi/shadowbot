"""
ShadowBot Test CLI Command

Provides a unified interface for running tests with tier and provider options.

Usage:
    shadowbot test                      # Run main tier (default)
    shadowbot test --tier smoke         # Run smoke tests only
    shadowbot test --tier extended      # Run extended tests
    shadowbot test --provider openai    # Run OpenAI provider tests
    shadowbot test --live               # Enable live API tests
    shadowbot test --parallel auto      # Run with parallelization
    
    # Interactive mode tests
    shadowbot test interactive --csv tests.csv
    shadowbot test interactive --suite smoke
    shadowbot test interactive --list
    shadowbot test interactive --generate-template
"""
...
app = typer.Typer(help="Run ShadowBot test suite with tier and provider options")
...
def run(
    ...
    """
    Run the ShadowBot test suite.
    
    Examples:
        shadowbot test                      # Run main tier
        shadowbot test --tier smoke         # Fast smoke tests
        shadowbot test --tier extended      # Extended tests
        shadowbot test --provider openai --live  # OpenAI live tests
        shadowbot test --parallel auto      # Parallel execution
    """
    # Validate tier
    ...
    # Try current directory first, then src/shadowbot
    test_dirs = [
        os.getcwd(),
        os.path.join(os.getcwd(), "src", "shadowbot"),
...
def _get_builtin_suite_path(suite: str):
    ...
    package_dir = Path(__file__).parent.parent.parent.parent  # Up to src/shadowbot
...
def interactive(
    ...
    Examples:
        shadowbot test interactive --csv tests.csv
        shadowbot test interactive --suite smoke
        shadowbot test interactive --suite tools --keep-artifacts
        shadowbot test interactive --list
        shadowbot test interactive --generate-template
    """
    from pathlib import Path
    ...
    if list_suites:
        ...
        typer.echo("")
        typer.echo("Usage: shadowbot test interactive --suite <name>")
        typer.echo("       PRAISONAI_LIVE_INTERACTIVE=1 shadowbot test interactive --suite advanced")
        return
    
    # Generate template
    if generate_template:
        # Lazy import
        from shadowbot.cli.features.csv_test_runner import generate_csv_template
    ...
    if not csv and not suite:
        ...
        typer.echo("")
        typer.echo("Examples:")
        typer.echo("  shadowbot test interactive --csv tests.csv")
        typer.echo("  shadowbot test interactive --suite smoke")
        typer.echo("  shadowbot test interactive --suite advanced")
        typer.echo("  shadowbot test interactive --list")
        raise typer.Exit(1)
    ...
    # Lazy import runner
    from shadowbot.cli.features.csv_test_runner import CSVTestRunner
...
def info():
    ...
    typer.echo("📊 ShadowBot Test Suite Information")
    typer.echo("=" * 50)
    typer.echo("")
