"""
Unified Dashboard — launch all ShadowBot UIs from a single interface.

Usage:
    shadowbot unified                 # Dashboard on :3000
    shadowbot unified --port 9000     # Custom port
"""
...
def _spawn_service(service_name: str, service_port: int, host: str, log_handle):
    ...
    argv = [sys.executable, "-m", "shadowbot", command, "--port", str(service_port), "--host", host] + extra_flags
...
def _auto_start_services(console, host: str) -> bool:
    """Auto-start ShadowBot services with proper readiness checks."""
    services = [
        ("flow", 7860),
        ("claw", 8082),
        ("ui", 8081)
    ...
    for service_name, service_port in services:
        ...
        try:
            ...
            log_dir = Path.home() / ".shadowbot" / "unified" / "logs"
...
def _run_aiui_dashboard(port: int, host: str, console) -> bool:
    ...
    try:
        ...
        result = subprocess.run([
            sys.executable, "-c", "import shadowbotui"
...
import shadowbotui as aiui
from shadowbot.ui._aiui_datastore import ShadowBotSessionDataStore

# Set up datastore bridge
aiui.set_datastore(ShadowBotSessionDataStore())

# Configure dashboard style
aiui.set_style("dashboard")
aiui.set_branding(title="ShadowBot Unified Dashboard", logo="🌟")
...
async def on_welcome():
    await aiui.say("🌟 Welcome to ShadowBot Unified Dashboard!")
...
'''
        ...
        try:
            ...
            try:
                ...
                # Fallback to python -m shadowbotui
                result = subprocess.run([
                    sys.executable, "-m", "shadowbotui.cli", "run", 
...
<head>
    ...
    <title>ShadowBot Unified Dashboard</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div id="dashboard-view">
        <div class="header">
            <h1>🌟 ShadowBot Unified Dashboard</h1>
        ...
        <div class="footer">
            <p>🚀 ShadowBot - Making AI Agent Development Simple</p>
        </div>
    </div>
...
def unified(
    ...
    """
    Launch the ShadowBot Unified Dashboard.
    ...
    1. Auto-starts all services by default (like 'shadowbot up')
    ...
    Examples:
        shadowbot dashboard                           # Auto-start all services
        shadowbot dashboard --no-auto-start          # Dashboard only (no auto-start)
        shadowbot dashboard --port 9000 --host 0.0.0.0
        shadowbot dashboard --aiui                   # Use aiui interface (experimental)
    """
    ...
    if auto_start:
        console.print("[bold green]🚀 Auto-starting ShadowBot services...[/bold green]")
    ...
    try:
        ...
        console.print(f"[yellow]Install with: pip install 'shadowbot[api]'[/yellow]")
    ...
    # Create FastAPI app
    fastapi_app = FastAPI(title="ShadowBot Unified Dashboard")
    ...
    async def start_service(service: str):
        """Start a ShadowBot service with proper startup verification."""
    ...
    console.print()
    console.print("[bold green]🌟 Starting ShadowBot Unified Dashboard[/bold green]")
