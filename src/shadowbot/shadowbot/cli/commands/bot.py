"""
Bot command group for ShadowBot CLI.
...
def bot_start(
    config: str = typer.Option(
        "bot.yaml", "--config", "-c",
        help="Path to bot YAML config file (defaults to ./bot.yaml if present, else ~/.shadowbot/bot.yaml)",
    ),
):
    ...
    ``./bot.yaml`` (back-compat) → ``~/.shadowbot/bot.yaml`` (canonical).

    Examples:
        shadowbot bot start                              # auto-detect
        shadowbot bot start --config ~/.shadowbot/bot.yaml
        shadowbot bot start -c my-telegram-bot.yaml
    """
...
def bot_telegram(
    ...
    Examples:
        shadowbot bot telegram --token $TELEGRAM_BOT_TOKEN
        shadowbot bot telegram --agent agents.yaml --browser --web
        shadowbot bot telegram --tools DuckDuckGoTool WikipediaTool --memory
    """
...
def bot_discord(
    ...
    Examples:
        shadowbot bot discord --token $DISCORD_BOT_TOKEN
        shadowbot bot discord --agent agents.yaml --browser --web
        shadowbot bot discord --tools DuckDuckGoTool --memory
    """
    from ..features.bots_cli import BotHandler, BotCapabilities
...
def bot_slack(
    ...
    Examples:
        shadowbot bot slack --token $SLACK_BOT_TOKEN
        shadowbot bot slack --agent agents.yaml --browser --web
        shadowbot bot slack --tools DuckDuckGoTool --memory --app-token $SLACK_APP_TOKEN
    """
    from ..features.bots_cli import BotHandler, BotCapabilities
...
def bot_whatsapp(
    ...
    Examples:
        shadowbot bot whatsapp --token $WHATSAPP_ACCESS_TOKEN --phone-id $WHATSAPP_PHONE_NUMBER_ID
        shadowbot bot whatsapp --mode web
        shadowbot bot whatsapp --mode web --creds-dir ~/.myapp/wa-creds
        shadowbot bot whatsapp --agent agents.yaml --memory --web
    """
    from ..features.bots_cli import BotHandler, BotCapabilities
...
def bot_linear(
    ...
    Examples:
        shadowbot bot linear --token $LINEAR_OAUTH_TOKEN --signing-secret $LINEAR_WEBHOOK_SECRET
        shadowbot bot linear --agent agents.yaml --memory --web --tools linear_tools
    """
    from ..features.bots_cli import BotHandler, BotCapabilities
...
def bot_email(
    ...
    Examples:
        shadowbot bot email --token $EMAIL_APP_PASSWORD --email user@gmail.com
        shadowbot bot email --agent agents.yaml --memory
    """
    from ..features.bots_cli import BotHandler, BotCapabilities
...
def bot_agentmail(
    ...
    Examples:
        shadowbot bot agentmail --token $AGENTMAIL_API_KEY
        shadowbot bot agentmail --inbox praison@agentmail.to
        shadowbot bot agentmail --agent agents.yaml --memory
    """
    from ..features.bots_cli import BotHandler, BotCapabilities
...
def bot_install_daemon(
    config: str = typer.Option(
        "bot.yaml", "--config",
        help="Path to bot.yaml (defaults to ./bot.yaml → ~/.shadowbot/bot.yaml)",
    ),
    start: bool = typer.Option(True, "--start/--no-start", help="Start after install"),
):
    """Install bot as OS daemon service (alias for 'shadowbot gateway install').
    
    Examples:
        shadowbot bot install-daemon
        shadowbot bot install-daemon --config my-bot.yaml --no-start
    """
    from shadowbot.daemon import install_daemon
    from .._paths import resolve_bot_config_path
    ...
    try:
        ...
        if result.get("ok"):
            ...
            if start:
                ...
                from shadowbot.daemon import get_daemon_status
...
def _resolve_dlq_path(path: Optional[str]) -> str:
    ...
    return os.path.expanduser(
        os.environ.get("PRAISONAI_DLQ_PATH", "~/.shadowbot/dlq.sqlite")
    )
...
def dlq_list(
    ...
    from shadowbot.bots import InboundDLQ
...
def dlq_replay(
    ...
    """
    import asyncio
    from shadowbot.bots import InboundDLQ, BotSessionManager
    from shadowbot.cli.features.bots_cli import _load_bot_config, _build_agent
...
def bot_callback(ctx: typer.Context):
    ...
    if ctx.invoked_subcommand is None:
        help_text = """
[bold cyan]ShadowBot Bot - Messaging Bots with Full Agent Capabilities[/bold cyan]

Start a bot on any platform with: shadowbot bot <platform>
...
[bold]Zero-Code Mode (YAML config):[/bold]
  shadowbot bot start --config bot.yaml

[bold]Examples:[/bold]
  shadowbot bot telegram --token $TELEGRAM_BOT_TOKEN
  shadowbot bot slack --agent agents.yaml --browser --web
  shadowbot bot discord --tools DuckDuckGoTool --memory --model gpt-4o
  shadowbot bot whatsapp --token $WHATSAPP_ACCESS_TOKEN --phone-id $WHATSAPP_PHONE_NUMBER_ID
  shadowbot bot whatsapp --mode web
  shadowbot bot linear --token $LINEAR_OAUTH_TOKEN --signing-secret $LINEAR_WEBHOOK_SECRET
"""
