"""
ShadowBot Async TUI Application.
...
def _init_debug_logging():
    ...
    _debug_log_file = os.path.expanduser("~/.shadowbot/async_tui_debug.log")
...
from shadowbot.cli.branding import get_logo, get_version
...
class AsyncTUIConfig:
    ...
    debug: bool = False  # Enable debug logging to file (~/.shadowbot/async_tui_debug.log)
...
class AsyncTUI:
        ...
        if self._agent is None:
            ...
            try:
                ...
                agent_config = {
                    ...
                    "instructions": "You are ShadowBot, a helpful assistant. Be concise and helpful. You have access to tools for file operations, code execution, and web search.",
                ...
                logger.error(f"Failed to import shadowbotagents: {e}")
                raise RuntimeError(f"Failed to import shadowbotagents: {e}")
    ...
    async def _start_runtime(self):
        ...
        try:
            from shadowbot.cli.features.interactive_runtime import create_runtime
    ...
    def _load_tools(self):
        ...
        try:
            from shadowbot.cli.features.interactive_tools import get_interactive_tools
        ...
        # Fallback: try to load basic tools directly from shadowbotagents
        logger.debug("Falling back to direct shadowbotagents.tools import...")
        try:
  ...
  /debug           Toggle debug logging to ~/.shadowbot/async_tui_debug.log
  ...
  Use --debug flag or /debug command to enable debug logging"""
            ...
            try:
                with open(filename, "w") as f:
                    f.write("# ShadowBot Conversation\n")
            ...
            log_file = os.path.expanduser("~/.shadowbot/async_tui_debug.log")
            self.messages.append(ChatMessage(
                role="system", 
...
Example: /handoff code "refactor the auth module" """
    ...
    def _execute_in_background(self, prompt: str):
        ...
        try:
            from shadowbotagents import register_display_callback, sync_display_callbacks
        ...
        def run():
            ...
            self._status_text = "ShadowBot is thinking..."
            self._update_output()
            ...
            while not done[0]:
                ...
                self._status_text = f"{spinner[idx]} ShadowBot is thinking... ({elapsed:.1f}s){queue_info}"
                self._update_output()
