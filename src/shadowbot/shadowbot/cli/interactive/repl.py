"""
Interactive REPL for ShadowBot.
...
class InteractiveREPL:
    """
    Interactive REPL for ShadowBot chat.
    ...
    def _get_agent(self):
        ...
        if self._agent is None:
            try:
                ...
                self.io.tool_error(f"Failed to import shadowbotagents: {e}")
                raise
...
def start_interactive(
    ...
    **kwargs
    ...
    This is the main entry point for `shadowbot chat` and `shadowbot` (no args).
    """
    config = REPLConfig(
        model=model,
        verbose=verbose,
