...
def install_warning_filters() -> None:
    """Install ShadowBot's noise filters. Idempotent. CLI-only."""
...
# This warning is harmless and occurs when running as `python -m shadowbot.cli.main`
...
from shadowbot.version import __version__
import yaml
import time
...
# from shadowbot.auto import AutoGenerator  # Lazy: imported in auto/init commands
# from shadowbot.agents_generator import AgentsGenerator  # Lazy: imported when running agents
# REMOVED: from shadowbot.inbuilt_tools import * - causes ~3200ms crewai import
# REMOVED: from shadowbot.inc.config import generate_config - causes ~3500ms langchain import
...
try:
    CALL_MODULE_AVAILABLE = importlib.util.find_spec("shadowbot.api.call") is not None
...
PRAISONAI_AVAILABLE = importlib.util.find_spec("shadowbotagents") is not None
...
def _get_call_module():
    ...
    Raises:
        ImportError: If shadowbot.api.call is not installed
    """
    if not CALL_MODULE_AVAILABLE:
        raise ImportError(
            "Call feature is not installed. Install with: pip install \"shadowbot[call]\""
        )
    from shadowbot.api import call as call_module
    return call_module
...
def _get_autogen():
    ...
    if not AUTOGEN_AVAILABLE:
        raise ImportError(
            "AutoGen is not installed. Install with: pip install \"shadowbot[autogen]\""
        )
    import autogen
    return autogen
...
# Suppress shadowbotagents INFO logs unless LOGLEVEL is explicitly set to debug/info
if os.environ.get('LOGLEVEL', '').upper() not in ('DEBUG', 'INFO'):
    logging.getLogger('shadowbotagents').setLevel(logging.WARNING)
    logging.getLogger('shadowbotagents.llm').setLevel(logging.WARNING)
    logging.getLogger('shadowbotagents.llm.llm').setLevel(logging.WARNING)
...
class ShadowBot:
    def __init__(self, agent_file="agents.yaml", framework="", auto=False, init=False, agent_yaml=None, tools=None):
        """
        Initialize the ShadowBot object with default parameters.
        """
        # Initialize telemetry defaults (moved from lazy __getattr__ hook)
        from shadowbot import _ensure_telemetry_defaults
        ...
        from shadowbot.llm.env import resolve_llm_endpoint
        ...
        if self.framework:
            from shadowbot.framework_adapters.validators import assert_framework_available
    ...
    def main(self):
        """
        The main function of the ShadowBot object. It parses the command-line arguments,
        ...
        # Telemetry defaults now handled in ShadowBot.__init__ with Langfuse awareness
        ...
        self.args = args
        invocation_cmd = "shadowbot"
        version_string = f"ShadowBot version {__version__}"
        ...
        if args.command:
            ...
            if args.command == "persistence":
                from shadowbot.cli.features.persistence import handle_persistence_command
                handle_persistence_command(unknown_args)
                return None
            
            # Handle schedule command
            if args.command == "schedule":
                from shadowbot.cli.features.agent_scheduler import AgentSchedulerHandler
            ...
            # Handle backends command
                ...
                if subcommand == "list" or subcommand is None:
                    ...
                    try:
                        from shadowbot.cli_backends import list_cli_backends
        ...
        if args.deploy:
            if args.schedule or args.schedule_config:
                ...
                from shadowbot.scheduler import create_scheduler
                ...
                from shadowbot.cli.features.deploy import DeployHandler
        ...
        if getattr(args, 'realtime', False):
            try:
                from shadowbot.cli.commands.ui import _launch_aiui_app
                ...
                print('Install with: pip install "shadowbot[ui]"')
                sys.exit(1)
            return
        ...
        if args.command == 'train':
            if not TRAIN_AVAILABLE:
                ...
                print("\npip install \"shadowbot[train]\"")
                print("Or run: shadowbot train init\n")
                sys.exit(1)
            ...
            if 'init' in sys.argv:
                from shadowbot.setup.setup_conda_env import main as setup_conda_main
                setup_conda_main()
        ...
        if args.ui:
            ...
            if save_replay:
                try:
                    from shadowbot.replay import ContextTraceWriter
                    from shadowbotagents.trace.context_events import ContextTraceEmitter, set_context_emitter
            ...
            try:
                ...
                if trace_emitter_token:
                    from shadowbotagents.trace.context_events import reset_context_emitter
    ...
    def parse_args(self):
        """
        Parse the command-line arguments for the ShadowBot CLI.
        """
        ...
        # Check if we're being used as a library (not from shadowbot CLI)
        # Skip CLI parsing to avoid conflicts with applications like Fabric
        is_library_usage = (
            'shadowbot' not in sys.argv[0] and
            not in_test_env
        )
        ...
        parser = argparse.ArgumentParser(prog="shadowbot", description="praisonAI command-line interface")
        parser.add_argument("--framework", choices=["crewai", "autogen", "shadowbot"], help="Specify the framework")
        ...
        parser.add_argument("--call", action="store_true", help="Start the ShadowBot Call server")
        ...
        parser.add_argument("--api-url", type=str, help="ShadowBot API URL for n8n to call (default: auto-detected; for Docker Desktop on macOS/Windows with N8N_DOCKER=1 use http://host.docker.internal:8005)")
        ...
        backend_group.add_argument("--cli-backend", type=str, choices=cli_backend_choices,
                          help="Delegate agent turns to a CLI backend (see shadowbot backends list)")
        ...
        if args.command == 'api:app' or args.command == '/app/api:app':
            args.command = 'agents.yaml'
        if args.command == 'ui':
            # UI command — routes to Typer CLI for clean chat UI (shadowbotui)
            pass
        # chat and code commands are now terminal-native (handled by Typer commands)
        ...
        if args.command == 'replay':
            ...
            register_commands()
            import sys as _sys
            _sys.argv = ['shadowbot', 'replay'] + unknown_args
            try:
                typer_app()
        ...
        if args.command in special_commands:
            ...
            if args.command == 'chat':
                ...
                # UI command — Clean Chat UI (shadowbotui)
                # Routes to Typer CLI for ui command (launches clean chat)
                from .app import app as typer_app, register_commands
                register_commands()
                import sys as _sys
                _sys.argv = ['shadowbot', 'ui'] + unknown_args
                try:
                    typer_app()
                except SystemExit as e:
                    sys.exit(e.code if e.code else 0)
                ...
                if not PRAISONAI_AVAILABLE:
                    print("[red]ERROR: ShadowBot Agents is not installed. Install with:[/red]")
                    print("\npip install shadowbotagents\n")
                    sys.exit(1)
                
                if not args.url:
                    print("[red]ERROR: --url is required for context command[/red]")
                    print("Usage: shadowbot context --url <repository_url> --goal <goal> [--auto-analyze]")
                    sys.exit(1)
                ...
                if not research_query:
                    ...
                    print("Usage: shadowbot research \"Your research query\"")
                    print("       shadowbot research --model deep-research-pro \"Your query\"")
                    sys.exit(1)
                ...
                skills_parser = argparse.ArgumentParser(prog="shadowbot skills")
                ...
                import sys as _sys
                _sys.argv = ['shadowbot', 'profile'] + unknown_args
                typer_app()
                sys.exit(0)
                ...
                if unknown_args and unknown_args[0] == 'agent-centric':
                    ...
                    print("Usage: shadowbot audit agent-centric [--scan|--fix|--check] PATH")
                    print("\nOptions:")
                ...
                # Serve command - launch ShadowBot servers
                ...
                agents_parser = argparse.ArgumentParser(prog="shadowbot agents")
                ...
                if unknown_args:
                    ...
                    print("  shadowbot deploy doctor [--all] [--provider aws|azure|gcp] [--json]")
                    print("  shadowbot deploy init [--file FILE] [--type api|docker|cloud]")
                    print("  shadowbot deploy validate [--file FILE] [--json]")
                    print("  shadowbot deploy plan [--file FILE] [--json]")
                    print("  shadowbot deploy status [--file FILE] [--json] [--verbose]")
                    print("  shadowbot deploy destroy [--file FILE] [--yes] [--force] [--json]")
                    print("  shadowbot deploy run [--file FILE] [--type api|docker|cloud] [--provider aws|azure|gcp]")
                    print("  shadowbot deploy api [--file FILE]")
                    print("  shadowbot deploy docker [--file FILE]")
                    print("  shadowbot deploy cloud --provider aws|azure|gcp [--file FILE]")
                sys.exit(0)
            ...
            # Capabilities CLI commands
                ...
                from shadowbot.browser.cli import app as browser_app
                import sys as _sys
                _sys.argv = ['shadowbot', 'browser'] + unknown_args
                try:
                    browser_app()
                except SystemExit as e:
                    sys.exit(e.code if e.code else 0)
                ...
                # Claw command - 🦞 ShadowBot Dashboard (full UI)
                # Routes to Typer CLI for claw command (launches AIUI dashboard)
                from .app import app as typer_app, register_commands
                register_commands()
                import sys as _sys
                _sys.argv = ['shadowbot', 'claw'] + unknown_args
                try:
                    typer_app()
                except SystemExit as e:
                    sys.exit(e.code if e.code else 0)
                ...
                register_commands()
                import sys as _sys
                _sys.argv = ['shadowbot', args.command] + unknown_args
                try:
                    typer_app()
                except SystemExit as e:
                    sys.exit(e.code if e.code else 0)
                ...
                register_commands()
                import sys as _sys
                _sys.argv = ['shadowbot', 'paths'] + unknown_args
                try:
                    typer_app()
                except SystemExit as e:
                    sys.exit(e.code if e.code else 0)
        ...
        if not args.command and (args.init or args.auto or args.framework):
            if not CREWAI_AVAILABLE and not AUTOGEN_AVAILABLE and not PRAISONAI_AVAILABLE:
                ...
                print("\npip install \"shadowbot\\[crewai]\"  # For CrewAI")
                print("pip install \"shadowbot\\[autogen]\"  # For AutoGen")
                print("pip install \"shadowbot\\[crewai,autogen]\"  # For both frameworks\n")
                print("pip install shadowbotagents # For Agents\n")  
                sys.exit(1)
    ...
    def _rewrite_query(self, query: str, rewrite_tools: str = None, verbose: bool = False) -> str:
        ...
        """
        try:
            from shadowbotagents import QueryRewriterAgent, RewriteStrategy
            from rich import print
            ...
            if rewrite_tools:
                if os.path.isfile(rewrite_tools):
                    ...
                    try:
                        ...
                        import shadowbotagents.tools as tools_module
    ...
    def _expand_prompt(self, prompt: str, expand_tools: str = None, verbose: bool = False) -> str:
        ...
        """
        try:
            from shadowbotagents import PromptExpanderAgent, ExpandStrategy
            from rich import print
    ...
    def handle_memory_command(self, action: str, action_args: list, user_id: str = None):
        ...
        """
        try:
            from shadowbotagents.memory import FileMemory
            from rich import print
            ...
            if action == 'show':
                ...
                if not action_args:
                    print("[red]ERROR: Content required. Usage: shadowbot memory add \"Your memory content\"[/red]")
                    return
                ...
                if not action_args:
                    print("[red]ERROR: Query required. Usage: shadowbot memory search \"query\"[/red]")
                    return
                ...
                if not action_args:
                    print("[red]ERROR: Session name required. Usage: shadowbot memory save <session_name>[/red]")
                    return
                ...
                if not action_args:
                    print("[red]ERROR: Session name required. Usage: shadowbot memory resume <session_name>[/red]")
                    return
                session_name = action_args[0]
                ...
                if not action_args:
                    print("[red]ERROR: Checkpoint ID required. Usage: shadowbot memory restore <checkpoint_id>[/red]")
                    return
                ...
                print("  shadowbot memory show                    - Show memory statistics")
                print("  shadowbot memory add <content>           - Add to long-term memory")
                print("  shadowbot memory search <query>          - Search memories")
                print("  shadowbot memory clear [short|all]       - Clear memory")
                print("  shadowbot memory save <session_name>     - Save session")
                print("  shadowbot memory resume <session_name>   - Resume session")
                print("  shadowbot memory sessions                - List saved sessions")
                print("  shadowbot memory compress                - Compress short-term memory")
                print("  shadowbot memory checkpoint [name]       - Create checkpoint")
                print("  shadowbot memory restore <checkpoint_id> - Restore checkpoint")
                print("  shadowbot memory checkpoints             - List checkpoints")
                print("\n[bold]Options:[/bold]")
                print("  --user-id <id>                           - User ID for memory isolation")
            else:
                print(f"[red]Unknown memory action: {action}[/red]")
                print("Use 'shadowbot memory help' for available commands")
                
        except ImportError as e:
            print(f"[red]ERROR: Failed to import memory module: {e}[/red]")
            print("Make sure shadowbotagents is installed: pip install shadowbotagents")
        except Exception as e:
    ...
    def handle_rules_command(self, action: str, action_args: list):
        ...
        """
        try:
            from shadowbotagents.memory import RulesManager
            from rich import print
            from rich.table import Table
            from rich.console import Console
            ...
            if action == 'list':
                ...
                if not action_args:
                    print("[red]ERROR: Rule name required. Usage: shadowbot rules show <name>[/red]")
                    return
                ...
                if len(action_args) < 2:
                    print("[red]ERROR: Name and content required. Usage: shadowbot rules create <name> <content>[/red]")
                    return
                rule_name = action_args[0]
                ...
                if not action_args:
                    print("[red]ERROR: Rule name required. Usage: shadowbot rules delete <name>[/red]")
                    return
                rule_name = action_args[0]
                ...
                print("  shadowbot rules list                     - List all loaded rules")
                print("  shadowbot rules show <name>              - Show specific rule details")
                print("  shadowbot rules create <name> <content>  - Create a new rule")
                print("  shadowbot rules delete <name>            - Delete a rule")
                print("  shadowbot rules stats                    - Show rules statistics")
                ...
                print("Use 'shadowbot rules help' for available commands")
    ...
    def handle_workflow_command(self, action: str, action_args: list, variables: dict = None, args=None):
        ...
        """
        try:
            from shadowbotagents import Agent as PraisonAgent
            from shadowbotagents.memory import WorkflowManager
            from rich import print
            from rich.table import Table
            from rich.console import Console
            ...
            if action == 'list':
                ...
                if not action_args:
                    print("[red]ERROR: Workflow name required. Usage: shadowbot workflow run <name>[/red]")
                    return
                ...
                if not action_args:
                    print("[red]ERROR: Workflow name required. Usage: shadowbot workflow show <name>[/red]")
                    return
                workflow_name = action_args[0]
                ...
                if not action_args:
                    print("[red]ERROR: Workflow name required. Usage: shadowbot workflow create <name>[/red]")
                    return
                workflow_name = action_args[0]
                ...
                if not action_args:
                    print("[red]ERROR: YAML file required. Usage: shadowbot workflow validate <file.yaml>[/red]")
                    return
                ...
                print("  shadowbot workflow list                  - List available workflows")
                print("  shadowbot workflow run <name>            - Execute a workflow")
                print("  shadowbot workflow run <file.yaml>       - Execute a YAML workflow")
                print("  shadowbot workflow show <name>           - Show workflow details")
                print("  shadowbot workflow create <name>         - Create a new workflow")
                print("  shadowbot workflow validate <file.yaml>  - Validate a YAML workflow")
                print("  shadowbot workflow template <name>       - Create from template")
                print('  shadowbot workflow auto "topic"          - Auto-generate workflow')
                print("\n[bold]Templates:[/bold]")
                ...
                print("\n[bold]Examples:[/bold]")
                print("  shadowbot workflow run 'Research Blog' --tools tavily --save")
                print("  shadowbot workflow run research.yaml --var topic='AI trends'")
                print("  shadowbot workflow template routing --output my_workflow.yaml")
            else:
                print(f"[red]Unknown workflow action: {action}[/red]")
                print("Use 'shadowbot workflow help' for available commands")
    ...
    def _run_yaml_workflow(self, yaml_file: str, action_args: list, variables: dict = None, args=None):
        ...
        try:
            from shadowbotagents.workflows import WorkflowManager
            from rich import print
            from rich.table import Table
            from rich.console import Console
            import uuid
            ...
            if context_auto_compact is True or context_strategy or context_threshold:
                ...
                try:
                    from shadowbotagents.context import ManagerConfig
                    config_kwargs = {"auto_compact": True}
                    if context_strategy:
                        from shadowbotagents.context import OptimizerStrategy
                        strategy_map = {
    ...
    def _validate_yaml_workflow(self, yaml_file: str):
        ...
        """
        try:
            from shadowbotagents.workflows import YAMLWorkflowParser
            from rich import print
            from rich.table import Table
            from rich.console import Console
            import yaml
    ...
    def _create_workflow_from_template(self, template_name: str = None, output_file: str = None):
        ...
        print(f"[cyan]Run with: shadowbot workflow run {output_file}[/cyan]")
    ...
    def _auto_generate_workflow(self, action_args: list):
        ...
        if not topic:
            print('[red]Usage: shadowbot workflow auto "topic" --pattern <pattern>[/red]')
        ...
        try:
            from shadowbot.auto import WorkflowAutoGenerator
    ...
    def handle_hooks_command(self, action: str):
        ...
        """
        try:
            from shadowbotagents.memory import HooksManager
            from rich import print
            from rich.table import Table
            from rich.console import Console
            import json
            ...
            if action == 'list':
                ...
                print("  shadowbot hooks list                     - List configured hooks")
                print("  shadowbot hooks stats                    - Show hooks statistics")
                print("  shadowbot hooks init                     - Create hooks.json template")
                ...
                print("Use 'shadowbot hooks help' for available commands")
    ...
    def handle_docs_command(self, action: str, action_args: list):
        ...
        if action in code_validation_actions:
            from shadowbot.cli.commands.docs import app as docs_app
            import typer
        ...
        try:
            from shadowbotagents.memory import DocsManager
            from rich import print
            from rich.table import Table
            from rich.console import Console
            ...
            if action == 'list':
                ...
                if not action_args:
                    print("[red]ERROR: Doc name required. Usage: shadowbot docs show <name>[/red]")
                    return
                ...
                if len(action_args) < 2:
                    print("[red]ERROR: Name and content required. Usage: shadowbot docs create <name> <content>[/red]")
                    return
                doc_name = action_args[0]
                content = ' '.join(action_args[1:])
                docs.create_doc(
                ...
                if not action_args:
                    print("[red]ERROR: Doc name required. Usage: shadowbot docs delete <name>[/red]")
                    return
                doc_name = action_args[0]
                ...
                print("  shadowbot docs list                     - List all docs")
                print("  shadowbot docs show <name>              - Show specific doc")
                print("  shadowbot docs create <name> <content>  - Create a new doc")
                print("  shadowbot docs delete <name>            - Delete a doc")
                print("\n[bold]Doc Location:[/bold]")
                print("  .praison/docs/*.md, ~/.praison/docs/*.md")
            else:
                print(f"[red]Unknown docs action: {action}[/red]")
                print("Use 'shadowbot docs help' for available commands")
    ...
    def handle_mcp_command(self, action: str, action_args: list):
        ...
        """
        try:
            from shadowbotagents.memory import MCPConfigManager
            from rich import print
            from rich.table import Table
            from rich.console import Console
            ...
            if action == 'list':
                ...
                if not action_args:
                    print("[red]ERROR: Config name required. Usage: shadowbot mcp show <name>[/red]")
                    return
                ...
                if len(action_args) < 2:
                    print("[red]ERROR: Name and command required. Usage: shadowbot mcp create <name> <command> [args...][/red]")
                    return
                config_name = action_args[0]
                ...
                if not action_args:
                    print("[red]ERROR: Config name required. Usage: shadowbot mcp delete <name>[/red]")
                    return
                config_name = action_args[0]
                ...
                if not action_args:
                    print("[red]ERROR: Config name required. Usage: shadowbot mcp enable <name>[/red]")
                    return
                config_name = action_args[0]
                ...
                if not action_args:
                    print("[red]ERROR: Config name required. Usage: shadowbot mcp disable <name>[/red]")
                    return
                config_name = action_args[0]
                ...
                # Start MCP server
                from shadowbot.mcp_server.cli import handle_mcp_command as mcp_serve_handler
                ...
                print("  shadowbot mcp serve                             - Start MCP server (STDIO)")
                print("  shadowbot mcp serve --transport http-stream     - Start HTTP Stream server")
                print("  shadowbot mcp list-tools                        - List available MCP tools")
                print("  shadowbot mcp config-generate --client claude-desktop  - Generate client config")
                print("\n[bold cyan]Config Management:[/bold cyan]")
                print("  shadowbot mcp list                              - List all MCP configs")
                print("  shadowbot mcp show <name>                       - Show specific config")
                print("  shadowbot mcp create <name> <cmd> [args...]     - Create a new config")
                print("  shadowbot mcp delete <name>                     - Delete a config")
                print("  shadowbot mcp enable <name>                     - Enable a config")
                print("  shadowbot mcp disable <name>                    - Disable a config")
                print("\n[bold]Config Location:[/bold]")
                print("  .praison/mcp/*.json, ~/.praison/mcp/*.json")
                print("\n[bold]Examples:[/bold]")
                print("  shadowbot mcp serve --transport stdio")
                print("  shadowbot mcp serve --transport http-stream --port 8080")
                print("  shadowbot mcp create filesystem npx -y @modelcontextprotocol/server-filesystem .")
            else:
                print(f"[red]Unknown mcp action: {action}[/red]")
                print("Use 'shadowbot mcp help' for available commands")
                    ...
                    if sec_choice == 'a':
                        ...
                        if sensitive_file_paths:
                            with open('.gitignore', 'a') as gi:
                                gi.write('\n# Auto-added by shadowbot commit\n')
...
Do NOT add any explanations or formatting."""
    ...
    def _execute_agent_with_budget_handling(self, agent, method_name, *args, **kwargs):
        ...
        """
        from shadowbotagents.errors import BudgetExceededError
        try:
    ...
    def handle_direct_prompt(self, prompt):
        ...
        mention_context = ""
        try:
            from shadowbotagents.tools.mentions import MentionsParser
        ...
        if PRAISONAI_AVAILABLE:
            ...
            if hasattr(self, 'args'):
                if getattr(self.args, 'web_search', False) or getattr(self.args, 'web_fetch', False):
                    from shadowbotagents.config.web_config import WebConfig
                ...
                if getattr(self.args, 'prompt_caching', False):
                    from shadowbotagents import CachingConfig
                ...
                if getattr(self.args, 'planning', False):
                    from shadowbotagents import PlanningConfig
                    planning_kwargs = {}
                ...
                if getattr(self.args, 'memory', False):
                    ...
                    if memory_kwargs:
                        from shadowbotagents import MemoryConfig
                ...
                if getattr(self.args, 'trust', False) or getattr(self.args, 'approve_level', None):
                    from shadowbotagents.approval import set_approval_callback, ApprovalDecision
                    ...
                    if getattr(self.args, 'trust', False):
                        ...
                        def level_based_approve(function_name, arguments, risk_level):
                            ...
                            if tool_level_value <= max_level_value:
                                ...
                                from shadowbotagents.approval import console_approval_callback
                ...
                if getattr(self.args, 'metrics', False):
                    from shadowbotagents import OutputConfig
            ...
            if hasattr(self, 'args') and getattr(self.args, 'cli_backend', None):
                try:
                    from shadowbot.cli_backends import resolve_cli_backend
            ...
            if hasattr(self, 'args') and getattr(self.args, 'auto_rag', False):
                from shadowbotagents import AutoRagAgent
                ...
                if display_mode == 'silent':
                    ...
                    try:
                        from shadowbotagents.output.status import enable_status_output, disable_status_output
                    ...
                    try:
                        from shadowbotagents.output.trace import enable_trace_output, disable_trace_output
                    ...
                    from shadowbotagents.main import register_display_callback as _reg_cb
                    ...
                    try:
                        from shadowbotagents.flow_display import track_workflow
                        flow = track_workflow()
                        flow.start()
                        if hasattr(agent, 'start'):
                            result = self._execute_agent_with_budget_handling(agent, 'start', prompt)
                    ...
                    from shadowbotagents.output.editor import enable_editor_output, disable_editor_output
...
Now, {final_instruction.lower()}:"""
    ...
    def _handle_serve_command(self, args, unknown_args):
        ...
        Usage:
            shadowbot serve agents.yaml
            shadowbot serve agents.yaml --port 8005 --host 0.0.0.0
            shadowbot agents.yaml --serve
        """
        import yaml
        from shadowbotagents import Agent, AgentTeam
        ...
        # Check if command itself is a yaml file (shadowbot agents.yaml --serve)
        ...
        if not os.path.exists(yaml_file):
            print(f"[red]ERROR: File not found: {yaml_file}[/red]")
            print("\nUsage:")
            print("  shadowbot serve agents.yaml")
            print("  shadowbot serve agents.yaml --port 8005")
            print("  shadowbot agents.yaml --serve")
            sys.exit(1)
        ...
        if steps:
            from shadowbotagents import Task
    ...
    def create_aiui_agents_interface(self):
        ...
        Routes to the new `shadowbot ui agents` subcommand.
        """
        try:
            from shadowbot.cli.commands.ui import _launch_aiui_app
            print("🤖 Launching ShadowBot Agents Dashboard (aiui)...")
            _launch_aiui_app(
                app_dir="ui_agents",
            ...
            print("ERROR: ShadowBot UI (aiui) is not installed. Please install it with 'pip install \"shadowbot[ui]\"' to use the agents dashboard.")
    ...
    def handle_context_command(self, url: str, goal: str, auto_analyze: bool = False) -> str:
        ...
        """
        try:
            from shadowbotagents import ContextAgent
    ...
    def handle_research_command(self, query: str, model: str = None, verbose: bool = False, save: bool = False, query_rewrite: bool = False, tools_path: str = None, rewrite_tools: str = None) -> str:
        ...
        """
        try:
            from shadowbotagents import DeepResearchAgent
    ...
    def _start_interactive_mode(self, args):
        ...
        try:
            ...
            from shadowbot.cli.features.message_queue import (
            ...
            from shadowbot.cli.session import get_session_store
            ...
            try:
                ...
                from shadowbot.cli.features.at_mentions import CombinedCompleter
            ...
            console.print("\n[bold cyan]ShadowBot Interactive Mode[/bold cyan]")
            ...
            while running:
                try:
                    ...
                    if worker_state.get('waiting_for_approval_input'):
                        try:
                            from shadowbotagents.approval import ApprovalDecision
    ...
    def _load_basic_tools_fallback(self):
        ...
        tools_list = []
        try:
            from shadowbotagents.tools import (
            ...
            try:
                from shadowbotagents.tools import read_file as tool_read_file
                tools_list.append(tool_read_file)
            except ImportError:
                pass
            try:
                from shadowbotagents.tools import write_file as tool_write_file
                tools_list.append(tool_write_file)
            except ImportError:
                pass
            try:
                from shadowbotagents.tools import list_files as tool_list_files
                tools_list.append(tool_list_files)
            except ImportError:
                pass
            try:
                from shadowbotagents.tools import execute_command
    ...
    def _handle_stats_command(self, console, session_state):
        ...
        from shadowbot.cli.features.cost_tracker import get_pricing
...
Provide a concise summary (max 200 words):"""
    ...
    def _handle_context_command(self, console, args, session_state):
        ...
        """
        try:
            from shadowbot.cli.features.context_manager import (
    ...
    def _run_chat_mode(self, prompt, args):
        ...
        Usage: shadowbot "your prompt" --chat
               shadowbot "your prompt" --chat-mode  (alias)
    ...
    def _start_execution_worker(self, tools_list, console, session_state):
        ...
        import queue as queue_module
        from rich.panel import Panel
        from shadowbot.cli.features.message_queue import ProcessingState
