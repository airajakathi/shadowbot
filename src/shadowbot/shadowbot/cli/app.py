"""
ShadowBot CLI Typer Application.
...
def _setup_langfuse_observability(*, verbose: bool = False) -> None:
    ...
    try:
        from shadowbot.observability.langfuse import LangfuseSink
...
def _setup_langextract_observability(*, verbose: bool = False) -> None:
    ...
    try:
        ...
        if importlib.util.find_spec('langextract') is None:
            if verbose:
                typer.echo("Warning: langextract is not installed. Install with: pip install 'shadowbot[langextract]'", err=True)
            return
        
        from shadowbot.observability.langextract import LangextractSink, LangextractSinkConfig
        from shadowbotagents.trace.protocol import TraceEmitter, set_default_emitter
        import os
        import atexit
        ...
        config = LangextractSinkConfig(
            output_path=os.getenv("PRAISONAI_LANGEXTRACT_OUTPUT", "shadowbot-trace.html"),
...
app = typer.Typer(
    name="shadowbot",
    help="ShadowBot - AI Agents Framework CLI",
...
def version_callback(value: bool):
    ...
    if value:
        ...
        typer.echo(f"ShadowBot version {__version__}")
        raise typer.Exit()
...
def main_callback(
    ...
    """
    ShadowBot - AI Agents Framework CLI.
    ...
    if ctx.invoked_subcommand is None:
        from shadowbot.cli.interactive.async_tui import AsyncTUI, AsyncTUIConfig
...
def register_commands():
    ...
    app.add_typer(ui_app, name="ui", help="🤖 Clean Chat UI (shadowbotui)")
    ...
    app.add_typer(batch_app, name="batch", help="Run all ShadowBot scripts in current folder")
    ...
    def app_cmd(
        ...
        name: str = typer.Option("ShadowBot App", "--name", "-n", help="Application name"),
    ):
        """
        ...
        try:
            ...
            from shadowbotagents import AgentOSConfig
        except ImportError as e:
            console.print(f"[red]Error importing AgentOS: {e}[/red]")
            console.print("[yellow]Install with: pip install shadowbot[api][/yellow]")
            raise typer.Abort()
    ...
    app.add_typer(claw_app, name="claw", help="🦞 ShadowBot Dashboard (full UI)")
    ...
    app.add_typer(up_app, name="up", help="🚀 Start unified ShadowBot stack (Langfuse + Langflow)")
