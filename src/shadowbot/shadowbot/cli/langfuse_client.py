"""
Langfuse API Client for ShadowBot CLI.

Provides a simple HTTP client for fetching traces, sessions, and observations
from the Langfuse API. Used by the `shadowbot langfuse` CLI commands to enable
...
3. ~/.shadowbot/langfuse.env file (created by `shadowbot langfuse config`)
...
class LangfuseClient:
    ...
    def from_config_file(cls, config_path: Optional[Path] = None) -> "LangfuseClient":
        """
        Create client from shadowbot config file.
        
        Reads configuration from ~/.shadowbot/langfuse.env file which is
        created by `shadowbot langfuse config` command.
        
        Args:
            config_path: Path to config file (default: ~/.shadowbot/langfuse.env)
        ...
        if config_path is None:
            config_path = Path.home() / ".shadowbot" / "langfuse.env"
        
        if not config_path.exists():
            raise FileNotFoundError(
                f"Langfuse config not found at {config_path}. "
                f"Run 'shadowbot langfuse config --public-key ... --secret-key ...' to set up."
            )
    ...
    def _make_request(self, endpoint: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            raise LangfuseAPIError(
                ...
                f"Ensure the server is running (try 'shadowbot langfuse status')."
            )
