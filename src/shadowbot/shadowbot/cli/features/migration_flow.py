...
agent frameworks to ShadowBot format.
...
class MigrationFlow:
    ...
    code migration from various agent frameworks to ShadowBot.
...
""",
        ...
        converter = Agent(
            name="Converter",
            role="Code Converter",
            goal="Convert source code to ShadowBot format",
            instructions=f"""You are a code conversion expert. Convert the provided source code to ShadowBot format.

Use this specification for conversion:
{spec_prompt}

Reference for ShadowBot code:
{PRAISONAI_REFERENCE}

Rules:
1. Preserve all logic and behavior
2. Convert imports to shadowbotagents
...
"""
                ...
                # Step 2: Convert
                convert_prompt = f"""Convert this code to ShadowBot format.
