...
Usage: shadowbot session list
       shadowbot session show <name>
       shadowbot session resume <name>
       shadowbot session delete <name>
"""
...
class SessionHandler(CommandHandler):
    ...
    Example:
        shadowbot session list
        shadowbot session show my-project
        shadowbot session resume my-project
        shadowbot session delete my-project
    """
    ...
    def get_help_text(self) -> str:
        return """
Session Commands:
  shadowbot session list                 - List all saved sessions
  shadowbot session show <name>          - Show session details
  shadowbot session resume <name>        - Resume a session (load into memory)
  shadowbot session delete <name>        - Delete a session
...
"""
    ...
    def _get_memory(self):
        ...
        if self._memory is None:
            try:
                from shadowbotagents.memory.file_memory import FileMemory
                self._memory = FileMemory(user_id=self.user_id)
            except ImportError:
                self.print_status(
                    "Session requires shadowbotagents. Install with: pip install shadowbotagents",
                    "error"
                )
                return None
        return self._memory
    ...
    def action_show(self, args: List[str], **kwargs) -> Dict[str, Any]:
        ...
        """
        if not args:
            self.print_status("Usage: shadowbot session show <name>", "error")
            return {}
    ...
    def action_resume(self, args: List[str], **kwargs) -> Dict[str, Any]:
        ...
        """
        if not args:
            self.print_status("Usage: shadowbot session resume <name>", "error")
            return {}
    ...
    def action_delete(self, args: List[str], **kwargs) -> bool:
        ...
        """
        if not args:
            self.print_status("Usage: shadowbot session delete <name>", "error")
            return False
