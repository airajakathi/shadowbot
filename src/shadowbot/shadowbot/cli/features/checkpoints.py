"""
Checkpoints CLI Feature for ShadowBot.

Provides CLI commands for managing file-level checkpoints.

Commands:
- shadowbot checkpoint save <message>     # Save a checkpoint
- shadowbot checkpoint restore <id>       # Restore to a checkpoint
- shadowbot checkpoint list               # List all checkpoints
- shadowbot checkpoint diff [from] [to]   # Show diff between checkpoints
- shadowbot checkpoint delete             # Delete all checkpoints
"""
...
class CheckpointsHandler:
    ...
    async def _get_service(self):
        ...
        if self._service is None:
            from shadowbotagents.checkpoints import CheckpointService
...
def handle_checkpoint_command(args: List[str], workspace_dir: Optional[str] = None, verbose: bool = False):
    ...
    Usage:
        shadowbot checkpoint save <message>     # Save a checkpoint
        shadowbot checkpoint restore <id>       # Restore to a checkpoint
        shadowbot checkpoint list [--limit N]   # List checkpoints
        shadowbot checkpoint diff [from] [to]   # Show diff
        shadowbot checkpoint delete             # Delete all checkpoints
    """
    handler = CheckpointsHandler(workspace_dir=workspace_dir, verbose=verbose)
    
    if not args:
        print("Usage: shadowbot checkpoint <command> [options]")
        print("\nCommands:")
    ...
    if command == "save":
        if len(args) < 2:
            print("Usage: shadowbot checkpoint save <message>")
            return
        ...
        if len(args) < 2:
            print("Usage: shadowbot checkpoint restore <checkpoint_id>")
            return
