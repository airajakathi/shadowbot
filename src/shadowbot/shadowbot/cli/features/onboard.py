"""
Onboarding wizard for ShadowBot bots.
...
def _is_non_interactive() -> bool:
    ...
    them later and re-run ``shadowbot onboard --yes`` to finalise setup.
    """
...
def _praison_home() -> Path:
    """Return ``$PRAISONAI_HOME`` or ``~/.shadowbot``."""
    override = os.environ.get("PRAISONAI_HOME")
    return Path(override) if override else Path.home() / ".shadowbot"


def _save_env_vars(env_vars: Dict[str, Optional[str]]) -> Optional[Path]:
    """Atomically merge ``env_vars`` into ``~/.shadowbot/.env`` (chmod 600).
    ...
    body = ["# ShadowBot configuration",
            "# Managed by shadowbot onboard / setup", ""]
...
def _read_env_value(key: str) -> Optional[str]:
    """Read a single value from ``~/.shadowbot/.env`` (if present)."""
    env_file = _praison_home() / ".env"
...
def _generate_bot_yaml(platforms: List[str], agent_name: str = "assistant", agent_instructions: str = "") -> str:
    """Generate bot.yaml content compatible with BOTH ``shadowbot bot start``
    and ``shadowbot gateway start``.
    ...
    lines = ["# Generated by `shadowbot onboard`. Works with both",
             "# `shadowbot bot start` (single-bot) and `shadowbot gateway start`",
...
class OnboardWizard:
    ...
    def run(self) -> None:
        ...
        # Step 2: Token configuration (password-hidden; persisted to ~/.shadowbot/.env)
        ...
        # Persist everything collected above to ~/.shadowbot/.env
        ...
        # one value to ~/.shadowbot/.env keeps clicks working across
        # daemon restarts.
        ...
        # canonical path is discoverable via ``shadowbot doctor`` and is
        # honoured by both ``shadowbot bot start`` and ``shadowbot gateway
        ...
        from shadowbot.cli._paths import default_bot_config_path
        ...
        if missing_tokens:
            ...
            console.print(
                "  [dim]Re-run [cyan]shadowbot onboard[/cyan] once you have the token(s) and "
            ...
            # run `shadowbot gateway uninstall` afterwards. This keeps the
        ...
        try:
            from shadowbot.cli.commands.mint_link import mint_fresh_link
            _magic_link_url = mint_fresh_link(ttl=600)  # 10 minutes
        except Exception:
            pass
        
        # Done panel. Commands referenced here must exist in `shadowbot --help`.
        ...
        # while ``shadowbot-bot`` is used for systemd.
        ...
        if _os == "darwin":
            ...
            _restart_cmd = "systemctl --user restart shadowbot-bot"
        elif _os == "windows":
            _restart_cmd = "schtasks /End /TN ShadowBotGateway && schtasks /Run /TN ShadowBotGateway"
        else:
            _restart_cmd = "shadowbot gateway install  # re-run installer"
        ...
        console.print(Panel(
            ...
            f"  [cyan]shadowbot claw[/cyan]          [dim]→ http://127.0.0.1:8082[/dim]\n\n"
            ...
            f"  [dim]Token stored in ~/.shadowbot/.env as GATEWAY_AUTH_TOKEN[/dim]\n\n"
            f"[bold]Generate fresh magic links:[/bold]\n"
            f"  [cyan]shadowbot gateway mint-link[/cyan]  [dim]# get a new magic link[/dim]\n\n"
            f"[bold]Manage the daemon:[/bold]\n"
            f"  [cyan]shadowbot gateway status[/cyan]     [dim]# is it running?[/dim]\n"
            f"  [cyan]shadowbot gateway logs[/cyan]       [dim]# tail the logs[/dim]\n"
            f"  [cyan]{_restart_cmd}[/cyan]\n"
            f"  [cyan]shadowbot gateway uninstall[/cyan]  [dim]# remove the daemon[/dim]\n\n"
            f"[bold]Re-run or reconfigure:[/bold]\n"
            f"  [cyan]shadowbot onboard[/cyan]            [dim]# change tokens / add platforms[/dim]\n"
            f"  [cyan]shadowbot gateway start[/cyan]      [dim]# run in foreground (skip the daemon)[/dim]\n"
            f"  [cyan]shadowbot doctor[/cyan]             [dim]# diagnose the whole stack[/dim]",
            title="✅ Done",
            border_style="green",
        ))
    ...
    def _install_daemon_with_feedback(self, print_fn, config_path: str) -> bool:
        ...
        try:
            ...
            from shadowbot.daemon import get_daemon_status, install_daemon
    ...
    def _run_plain(self) -> None:
        ...
        print("\nNext steps:")
        print("  shadowbot gateway status     # check if the daemon is running")
        print("  shadowbot gateway logs       # see what the bot is doing")
        print("  shadowbot onboard            # change tokens / add platforms")
        print(f"  shadowbot gateway start --config {cfg_path}   # foreground run (no daemon)")


def run_onboard() -> None:
    """Entry point for 'shadowbot onboard' command."""
