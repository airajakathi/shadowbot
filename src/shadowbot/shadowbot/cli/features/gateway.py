"""
Gateway CLI commands for ShadowBot.
...
def _load_shadowbot_env_file() -> Dict[str, str]:
    """Load ``~/.shadowbot/.env`` into ``os.environ`` (without overwriting).
    ...
    ``shadowbot onboard`` (e.g. ``TELEGRAM_BOT_TOKEN``) are missing when
...
class GatewayHandler:
    ...
    def start(
        ...
        # Load ~/.shadowbot/.env BEFORE any config parsing or ${VAR}
        # substitution — daemons don't inherit shell env.
        _load_shadowbot_env_file()
        logger.info(
        ...
        try:
            ...
            from shadowbotagents.gateway import GatewayConfig
        except ImportError as e:
            print(f"Error: Gateway requires additional dependencies. {e}")
            print("Install with: pip install shadowbot[api]")
            return
...
def handle_gateway_command(args) -> int:
    ...
    if isinstance(args, list):
        parser = argparse.ArgumentParser(
            prog="shadowbot gateway",
            description="Manage the ShadowBot Gateway server",
        )
