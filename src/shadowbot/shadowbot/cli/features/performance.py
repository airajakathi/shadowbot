...
Usage: shadowbot perf [benchmark|check|import-time|memory]
"""
...
class PerformanceHandler(FlagHandler):
    ...
    Commands:
        shadowbot perf benchmark     - Run full performance benchmark
        shadowbot perf import-time   - Measure import time only
        shadowbot perf memory        - Measure memory usage only
        shadowbot perf check         - Run regression check against targets
        shadowbot perf lazy-check    - Verify lazy imports are working
    """
    ...
    def measure_import_time(self, runs: int = 5) -> Dict[str, Any]:
        """
        Measure import time for shadowbotagents.
...
"""
    ...
    def measure_memory(self) -> Dict[str, Any]:
        """
        Measure memory usage after importing shadowbotagents.
