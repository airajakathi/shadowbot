...
@register_check(
    id="shadowbot_package",
    title="ShadowBot Package",
    description="Check shadowbot package is installed",
    category=CheckCategory.ENVIRONMENT,
    severity=CheckSeverity.CRITICAL,
)
def check_shadowbot_package(config: DoctorConfig) -> CheckResult:
    """Check shadowbot package is installed."""
    try:
        ...
        return CheckResult(
            ...
            category=CheckCategory.ENVIRONMENT,
            status=CheckStatus.PASS,
            message=f"shadowbot {__version__} installed",
        ...
        return CheckResult(
            ...
            category=CheckCategory.ENVIRONMENT,
            status=CheckStatus.FAIL,
            message="shadowbot package not found",
            details=str(e),
            remediation="Install with: pip install shadowbot",
            severity=CheckSeverity.CRITICAL,
        )


@register_check(
    id="shadowbotagents_package",
    title="ShadowBot Agents Package",
    description="Check shadowbotagents package is installed",
    category=CheckCategory.ENVIRONMENT,
    severity=CheckSeverity.CRITICAL,
)
def check_shadowbotagents_package(config: DoctorConfig) -> CheckResult:
    """Check shadowbotagents package is installed."""
    try:
        import shadowbotagents
        # shadowbotagents does not expose __version__ at module level yet, so
        # prefer importlib.metadata (which reads the installed dist-info) and
        # fall back to the attribute for forward-compat.
        version = getattr(shadowbotagents, "__version__", None)
        if not version:
            try:
                from importlib.metadata import version as _pkg_version
                version = _pkg_version("shadowbotagents")
            except Exception:
                version = "unknown"
        return CheckResult(
            id="shadowbotagents_package",
            ...
            category=CheckCategory.ENVIRONMENT,
            status=CheckStatus.PASS,
            message=f"shadowbotagents {version} installed",
        ...
        return CheckResult(
            ...
            category=CheckCategory.ENVIRONMENT,
            status=CheckStatus.FAIL,
            message="shadowbotagents package not found",
            details=str(e),
            remediation="Install with: pip install shadowbotagents",
            severity=CheckSeverity.CRITICAL,
        )
    ...
    if key:
        ...
        if alternatives:
            ...
            return CheckResult(
                ...
                remediation="Run 'shadowbot setup' to configure API keys, or set OPENAI_API_KEY environment variable",
                severity=CheckSeverity.HIGH,
            )
...
def check_optional_deps(config: DoctorConfig) -> CheckResult:
    ...
    optional_packages = [
        ...
        ("shadowbotui", "aiui (Chat/Dashboard UI)"),
        ("gradio", "Gradio UI"),
...
@register_check(
    ...
    description="Check for stale shadowbotagents artifacts in site-packages",
    category=CheckCategory.ENVIRONMENT,
    severity=CheckSeverity.CRITICAL,
)
def check_stale_packages(config: DoctorConfig) -> CheckResult:
    """
    Check for stale shadowbotagents artifacts in site-packages.
    ...
    for sp in site_packages:
        shadowbot_dir = os.path.join(sp, 'shadowbotagents')
        if os.path.isdir(shadowbot_dir):
            init_path = os.path.join(shadowbot_dir, '__init__.py')
            if not os.path.exists(init_path):
                # This is a stale namespace package directory
                stale_dirs.append(shadowbot_dir)
    ...
    if stale_dirs:
        return CheckResult(
            ...
            category=CheckCategory.ENVIRONMENT,
            status=CheckStatus.FAIL,
            message=f"Found {len(stale_dirs)} stale shadowbotagents directory(ies)",
    ...
    try:
        import shadowbotagents
        if shadowbotagents.__file__ is None:
            return CheckResult(
                id="stale_packages",
                title="Stale Package Artifacts",
                category=CheckCategory.ENVIRONMENT,
                status=CheckStatus.FAIL,
                message="shadowbotagents loaded as namespace package",
                details="__file__ is None, indicating namespace package shadowing",
                remediation=(
                    "Remove stale shadowbotagents directory from site-packages:\n"
        ...
        pass  # Package not installed - handled by shadowbotagents_package check
