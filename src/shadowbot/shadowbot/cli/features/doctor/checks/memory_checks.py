...
@register_check(
    id="memory_shadowbot_module",
    title="ShadowBot Memory Module",
    description="Check ShadowBot memory module",
    category=CheckCategory.MEMORY,
    severity=CheckSeverity.LOW,
)
def check_memory_shadowbot_module(config: DoctorConfig) -> CheckResult:
    """Check ShadowBot memory module."""
    try:
        from shadowbotagents.memory import FileMemory, Memory
        return CheckResult(
            id="memory_shadowbot_module",
            title="ShadowBot Memory Module",
            category=CheckCategory.MEMORY,
