...
def check_tools_registry(config: DoctorConfig) -> CheckResult:
    ...
    try:
        ...
        return CheckResult(
            ...
            details=str(e),
            remediation="Ensure shadowbotagents is properly installed",
...
def check_tools_web_search(config: DoctorConfig) -> CheckResult:
    ...
    if os.environ.get("TAVILY_API_KEY"):
        try:
            from shadowbotagents.tools import tavily_search
    ...
    if os.environ.get("EXA_API_KEY"):
        try:
            from shadowbotagents.tools import exa_search
...
def check_tools_file_ops(config: DoctorConfig) -> CheckResult:
    ...
    try:
        from shadowbotagents.tools import read_file, write_file, list_files
        return CheckResult(
            id="tools_file_ops",
            title="File Operation Tools",
            category=CheckCategory.TOOLS,
