...
def check_serve_module(config: DoctorConfig) -> CheckResult:
    ...
    try:
        ...
        return CheckResult(
            ...
            remediation="pip install shadowbot[serve]",
        )
...
def check_endpoints_module(config: DoctorConfig) -> CheckResult:
    ...
    try:
        from shadowbot.endpoints import (
            DiscoveryDocument,
            EndpointInfo,
            ProviderInfo,
            list_provider_types,
...
def check_endpoints_cli(config: DoctorConfig) -> CheckResult:
    ...
    try:
        ...
        return CheckResult(
            ...
            remediation="pip install shadowbot",
        )
    except Exception as e:
        return CheckResult(
            id="endpoints_cli",
...
@register_check(
    ...
    description="Check if a ShadowBot server is reachable",
...
def check_server_connectivity(config: DoctorConfig) -> CheckResult:
    """Check if a ShadowBot server is reachable."""
    ...
    try:
        ...
        return CheckResult(
            ...
            remediation="Start a server: shadowbot serve unified --port 8765",
        )
    except Exception as e:
        return CheckResult(
            id="server_connectivity",
...
def check_discovery_endpoint(config: DoctorConfig) -> CheckResult:
    ...
    url = os.environ.get("PRAISONAI_ENDPOINTS_URL", "http://localhost:8765")
    discovery_url = f"{url}/__shadowbot__/discovery"
...
def check_provider_adapters(config: DoctorConfig) -> CheckResult:
    ...
    try:
        from shadowbot.endpoints.providers import (
            BaseProvider,
            RecipeProvider,
            AgentsAPIProvider,
            MCPProvider,
