"""
Bot-specific doctor checks for ShadowBot.
...
def check_bot_tokens(config: DoctorConfig) -> CheckResult:
    ...
    from shadowbot.cli._paths import resolve_bot_config_path
...
def check_bot_config(config: DoctorConfig) -> CheckResult:
    ...
    if not os.path.exists(config_path):
        return CheckResult(
            ...
            remediation="Run 'shadowbot onboard' to create one",
    ...
    try:
        ...
        return CheckResult(
            ...
            remediation="Fix errors in bot.yaml or run 'shadowbot onboard'",
            duration_ms=(time.time() - start) * 1000,
        )
