...
@register_check(
    id="performance_shadowbot_import",
    title="ShadowBot Import Time",
    description="Measure shadowbot package import time",
    category=CheckCategory.PERFORMANCE,
    severity=CheckSeverity.INFO,
)
def check_performance_shadowbot_import(config: DoctorConfig) -> CheckResult:
    """Measure shadowbot package import time."""
    budget_ms = config.budget_ms or 2000  # 2 second default budget
    
    # shadowbot is already imported, measure a fresh import of a submodule
    start = time.perf_counter()
    try:
        # Import version module (lightweight)
        from shadowbot import version
        elapsed = (time.perf_counter() - start) * 1000
...
@register_check(
    id="performance_shadowbotagents_import",
    title="ShadowBot Agents Import Time",
    description="Measure shadowbotagents package import time",
    category=CheckCategory.PERFORMANCE,
    severity=CheckSeverity.INFO,
)
def check_performance_shadowbotagents_import(config: DoctorConfig) -> CheckResult:
    """Measure shadowbotagents package import time."""
