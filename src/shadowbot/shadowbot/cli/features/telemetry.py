...
Usage: shadowbot "prompt" --telemetry
"""
...
class TelemetryHandler(FlagHandler):
    ...
    def check_dependencies(self) -> Tuple[bool, str]:
        ...
        if _load_telemetry():
            return True, ""
        return False, "Telemetry requires shadowbotagents[telemetry]. Install with: pip install shadowbotagents[telemetry]"
    ...
    def enable(self, **kwargs) -> bool:
        ...
        if not _load_telemetry():
            self.print_status(
                "Telemetry not available. Install with: pip install shadowbotagents[telemetry]",
                "warning"
            )
            return False
