"""
Diagnostics export CLI handler for ShadowBot.
...
class DiagHandler:
    ...
    def setup_subparser(subparsers) -> None:
        ...
        export_parser.add_argument(
            "-o", "--output",
            type=str,
            help="Output path (default: shadowbot-diag-{timestamp}.zip)"
        )
        export_parser.add_argument(
            "--include-logs",
            action="store_true",
    ...
    def handle_export(args: argparse.Namespace) -> int:
        ...
        output_path = args.output or f"shadowbot-diag-{timestamp}.zip"
    ...
    def _collect_system_info() -> dict:
        ...
        try:
            ...
            info["shadowbot"]["version"] = __version__
        except ImportError:
            info["shadowbot"]["version"] = "unknown"
        
        # Get shadowbotagents version
        try:
            import shadowbotagents
            info["shadowbot"]["agents_version"] = getattr(shadowbotagents, '__version__', 'unknown')
        except ImportError:
            info["shadowbot"]["agents_version"] = "not installed"
    ...
    def _collect_logs(tmppath: Path) -> List[str]:
        ...
        log_locations = [
            ...
            Path.home() / ".shadowbot" / "logs",
            Path("/var/log/shadowbot"),
        ]
    ...
    def _collect_traces(workspace: str, tmppath: Path) -> List[str]:
        ...
        trace_patterns = ["*_trace.json", "shadowbot_trace*.json", "*.trace"]
...
def run_diag_command(args: List[str]) -> int:
    ...
    parser = argparse.ArgumentParser(prog="shadowbot diag")
