...
class AgentSchedulerHandler:
    ...
    def handle_daemon_command(subcommand: str, args, unknown_args=None) -> int:
        ...
        Returns:
            Exit code
        """
        from shadowbot.scheduler.state_manager import SchedulerStateManager
        from shadowbot.scheduler.daemon_manager import DaemonManager
    ...
    def _handle_start(args, unknown_args, state_manager, daemon_manager) -> int:
        ...
        if not unknown_args:
            ...
            print("\nUsage:")
            print('  shadowbot schedule start <name> "Your task" --interval hourly')
            print('  shadowbot schedule start <name> --recipe <recipe-name> --interval hourly')
            print("\nExample:")
            print('  shadowbot schedule start news-checker "Check AI news" --interval hourly')
            print('  shadowbot schedule start news-checker --recipe news-monitor --interval hourly')
            return 1
        ...
        if existing and daemon_manager.get_status(existing.get('pid', 0))['is_alive']:
            ...
            print(f"   Use 'shadowbot schedule stop {name}' to stop it first")
            return 1
        ...
        print(f"   PID: {pid}")
        print(f"   Logs: ~/.shadowbot/logs/{name}.log")
        print(f"\nManage:")
        print(f"   shadowbot schedule list")
        print(f"   shadowbot schedule describe {name}")
        print(f"   shadowbot schedule logs {name} --follow")
        print(f"   shadowbot schedule stop {name}")
    ...
    def _handle_stop(unknown_args, state_manager, daemon_manager) -> int:
        ...
        if not unknown_args:
            ...
            print("\nUsage: shadowbot schedule stop <name>")
            return 1
        ...
        if not state:
            ...
            print("\nList schedulers with: shadowbot schedule list")
            return 1
    ...
    def _handle_logs(unknown_args, daemon_manager) -> int:
        ...
        if not unknown_args:
            print("❌ Error: Please provide scheduler name")
            print("\nUsage: shadowbot schedule logs <name> [-f]")
            return 1
    ...
    def _handle_restart(unknown_args, state_manager, daemon_manager) -> int:
        ...
        if not unknown_args:
            print("❌ Error: Please provide scheduler name")
            print("\nUsage: shadowbot schedule restart <name>")
            return 1
    ...
    def _handle_delete(unknown_args, state_manager) -> int:
        ...
        if not unknown_args:
            print("❌ Error: Please provide scheduler name")
            print("\nUsage: shadowbot schedule delete <name>")
            return 1
    ...
    def _handle_describe(unknown_args, state_manager, daemon_manager) -> int:
        ...
        if not unknown_args:
            print("❌ Error: Please provide scheduler name")
            print("\nUsage: shadowbot schedule describe <name>")
            return 1
    ...
    def _handle_save(unknown_args, state_manager) -> int:
        ...
        if not unknown_args:
            print("❌ Error: Please provide scheduler name")
            print("\nUsage: shadowbot schedule save <name> [output.yaml]")
            return 1
        ...
        try:
            ...
            print(f"\nRun with:")
            print(f"  shadowbot schedule {output_file}")
            return 0
        except Exception as e:
    ...
    def handle_schedule_command(args, unknown_args, daemon_mode=False) -> int:
        ...
        Supports two modes:
        1. YAML mode: shadowbot schedule agents.yaml
        2. Prompt mode: shadowbot schedule "Your task here" --interval hourly
        ...
        try:
            ...
            print("Error: shadowbot.scheduler or shadowbotagents module not found")
            print("Please ensure ShadowBot is properly installed")
            print("pip install shadowbot shadowbotagents")
            return 1
        ...
        if verbose:
            ...
            logging.getLogger('shadowbot.scheduler').setLevel(logging.WARNING)
        ...
        try:
            ...
            if first_arg and not is_yaml_mode:
                ...
                if ' ' not in first_arg and not first_arg.endswith('.yaml') and not first_arg.endswith('.yml'):
                    try:
                        from shadowbot.recipe.bridge import resolve
            ...
            if is_yaml_mode:
                ...
                if not first_arg:
                    ...
                    print("\nExamples:")
                    print("  shadowbot schedule agents.yaml")
                    print("  shadowbot schedule my-recipe --interval hourly")
                    print('  shadowbot schedule "Check news every hour" --interval hourly')
                    return 1
