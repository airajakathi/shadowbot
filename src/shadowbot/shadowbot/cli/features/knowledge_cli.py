...
def create_knowledge_app():
    ...
    def stats_command(
        ...
        try:
            from shadowbotagents.knowledge import Knowledge
            from shadowbotagents.knowledge.indexing import CorpusStats
        except ImportError:
            typer.echo("Error: shadowbotagents not installed", err=True)
            raise typer.Exit(1)
    ...
    def search_command(
        ...
        Examples:
            shadowbot knowledge search "What is the API key?"
            shadowbot knowledge search "config" --strategy hybrid --rerank
            shadowbot knowledge search "auth" --compress --max-context-tokens 2000
            shadowbot knowledge search "*.py" --include "*.py" --exclude "test_*"
        """
        try:
            from shadowbotagents.knowledge import Knowledge
            from shadowbotagents.rag import (
                SmartRetriever, ContextCompressor, RetrievalResult,
            )
        except ImportError as e:
            typer.echo(f"Error: shadowbotagents not installed: {e}", err=True)
            raise typer.Exit(1)
    ...
    def summarize_command(
        ...
        Examples:
            shadowbot knowledge summarize ./docs
            shadowbot knowledge summarize ./src --levels 2 --rebuild
            shadowbot knowledge summarize ./project --output ./summaries
        """
        try:
            from shadowbotagents.rag import HierarchicalSummarizer
            from shadowbotagents.knowledge.indexing import CorpusStats
        except ImportError as e:
            typer.echo(f"Error: shadowbotagents not installed: {e}", err=True)
            raise typer.Exit(1)
