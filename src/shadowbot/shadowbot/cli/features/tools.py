...
Usage: shadowbot tools list
       shadowbot tools info internet_search
"""
...
class ToolsHandler(CommandHandler):
    ...
    Example:
        shadowbot tools list
        shadowbot tools info internet_search
        shadowbot tools search "web"
    """
    ...
    def get_help_text(self) -> str:
        return """
Tools Commands:
  shadowbot tools list                   - List all available tools
  shadowbot tools info <name>            - Show tool details
  shadowbot tools search <query>         - Search tools by name/description
  shadowbot tools doctor                 - Diagnose tool availability and dependencies
  shadowbot tools doctor --json          - Output diagnosis as JSON
  shadowbot tools resolve <name>         - Resolve a tool name to its source
  shadowbot tools discover               - Discover tools from installed packages
  shadowbot tools show-sources           - Show all tool sources for a template
  shadowbot tools add <source>           - Add tools from package, file, or GitHub
  shadowbot tools add-sources <source>   - Add a tool source to persistent config
  shadowbot tools remove-sources <source> - Remove a tool source from config

Add Examples:
  shadowbot tools add pandas             - Wrap pandas functions as tools
  shadowbot tools add ./my_tools.py      - Add local tools file
  shadowbot tools add github:user/repo   - Add tools from GitHub
...
"""
    ...
    def _get_registry(self):
        ...
        if self._registry is None:
            try:
                from shadowbotagents.tools import get_registry
                self._registry = get_registry()
            except ImportError:
                self.print_status(
                    "Tools require shadowbotagents. Install with: pip install shadowbotagents",
                    "error"
                )
                return None
    ...
    def action_info(self, args: List[str], **kwargs) -> Dict[str, Any]:
        ...
        """
        if not args:
            self.print_status("Usage: shadowbot tools info <tool_name>", "error")
            return {}
    ...
    def action_search(self, args: List[str], **kwargs) -> List[str]:
        ...
        """
        if not args:
            self.print_status("Usage: shadowbot tools search <query>", "error")
            return []
    ...
    def action_doctor(self, args: List[str], **kwargs) -> Dict[str, Any]:
        ...
        try:
            from shadowbot.templates.tools_doctor import ToolsDoctor
    ...
    def action_resolve(self, args: List[str], **kwargs) -> Dict[str, Any]:
        ...
        """
        if not args:
            self.print_status("Usage: shadowbot tools resolve <tool_name> [--template <template>]", "error")
            return {}
    ...
    def action_discover(self, args: List[str], **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                pkg_tools.append("shadowbot_tools.video")
            except ImportError:
                pass
            
            # Check for tools module
            try:
                import shadowbot_tools.tools as ext_tools
            ...
            if pkg_tools:
                discovered["shadowbot_tools"] = pkg_tools
        except ImportError:
            pass
        
        # Try shadowbotagents built-in tools
        try:
            from shadowbotagents.tools import TOOL_MAPPINGS
            discovered["shadowbotagents.tools"] = list(TOOL_MAPPINGS.keys())[:20]  # Limit
        except ImportError:
            pass
    ...
    def action_show_sources(self, args: List[str], **kwargs) -> Dict[str, Any]:
        ...
        sources = {
            "built_in": "shadowbotagents.tools.TOOL_MAPPINGS",
            "package_discovery": ["shadowbot_tools"],
            "default_dirs": [
                "~/.praison/tools",
    ...
    def action_add(self, args: List[str], **kwargs) -> Dict[str, Any]:
        ...
        Examples:
            shadowbot tools add pandas
            shadowbot tools add ./my_tools.py
            shadowbot tools add github:user/repo/tools
        """
        if not args:
            self.print_status("Usage: shadowbot tools add <source>", "error")
    ...
    def action_add_sources(self, args: List[str], **kwargs) -> Dict[str, Any]:
        ...
        """
        if not args:
            self.print_status("Usage: shadowbot tools add-sources <source>", "error")
            return {"success": False, "error": "No source provided"}
    ...
    def action_remove_sources(self, args: List[str], **kwargs) -> Dict[str, Any]:
        ...
        """
        if not args:
            self.print_status("Usage: shadowbot tools remove-sources <source>", "error")
            return {"success": False, "error": "No source provided"}
