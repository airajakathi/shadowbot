"""
Jobs CLI Feature for ShadowBot.

Provides CLI commands for managing async jobs.

Commands:
- shadowbot run submit <prompt>     # Submit a new job
- shadowbot run status <job_id>     # Get job status
- shadowbot run result <job_id>     # Get job result
- shadowbot run cancel <job_id>     # Cancel a job
- shadowbot run list                # List jobs
"""
...
def handle_run_command(args: List[str], verbose: bool = False):
    ...
    Usage:
        shadowbot run submit "Your prompt here" [--wait] [--agent-file agents.yaml]
        shadowbot run status <job_id>
        shadowbot run result <job_id>
        shadowbot run cancel <job_id>
        shadowbot run list [--status <status>]
    """
    import argparse
    
    parser = argparse.ArgumentParser(prog="shadowbot run", description="Manage async jobs")
    ...
    submit_parser.add_argument("--framework", default="shadowbot", help="Framework to use")
