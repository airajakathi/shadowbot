"""
Comprehensive Benchmarking CLI for ShadowBot.

Provides deep profiling and comparison across all execution paths:
- OpenAI SDK (baseline)
- ShadowBot Agent (Python SDK)
- ShadowBot CLI (fast path)
- ShadowBot CLI with profiling
- ShadowBot Workflow (single agent)
- ShadowBot Workflow (multi-agent)
- ShadowBot via LiteLLM
- LiteLLM standalone

Usage: shadowbot benchmark profile "What is 2+2?"
"""
...
class ModuleBreakdown:
    ...
    shadowbot_modules: List[Tuple[str, float]] = field(default_factory=list)  # (file, cumulative_ms)
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            "shadowbot": [{"file": f, "cumulative_ms": t} for f, t in self.shadowbot_modules[:20]],
...
class BenchmarkHandler(FlagHandler):
    """
    Comprehensive benchmarking handler for ShadowBot.
    
    Commands:
        shadowbot benchmark profile [query]  - Run full benchmark suite
        shadowbot benchmark sdk [query]      - Benchmark OpenAI SDK only
        shadowbot benchmark agent [query]    - Benchmark Agent only
        shadowbot benchmark cli [query]      - Benchmark CLI only
        shadowbot benchmark compare [query]  - Quick comparison table
    """
    ...
    PATHS = [
        ...
        "shadowbot_agent",
        "shadowbot_cli",
        "shadowbot_cli_profile",
        "shadowbot_workflow_single",
        "shadowbot_workflow_multi",
        "shadowbot_litellm",
        "litellm_standalone",
    ]
    ...
    def _extract_module_breakdown(self, functions: List[FunctionStat]) -> ModuleBreakdown:
        ...
        shadowbot_modules: Dict[str, float] = {}
        ...
        for func in functions:
            ...
            if "shadowbot" in file_path and "agents" not in file_path:
                shadowbot_modules[func.file] = shadowbot_modules.get(func.file, 0) + cumul
            elif "shadowbotagents" in file_path or ("shadowbot" in file_path and "agents" in file_path):
        ...
        return ModuleBreakdown(
            shadowbot_modules=to_sorted_list(shadowbot_modules),
    ...
    def benchmark_shadowbot_agent_deep(self, prompt: str, iteration: int, is_cold: bool, limit: int = 30) -> BenchmarkRun:
        """Benchmark ShadowBot Agent with deep cProfile profiling (in-process)."""
        t0 = time.perf_counter()
        ...
        try:
            ...
            return BenchmarkRun(
                path_name="shadowbot_agent",
                iteration=iteration,
                is_cold=is_cold,
                timings=PhaseTimings(),
                success=False,
                error="shadowbotagents not installed"
            )
...
'''
    ...
    def benchmark_shadowbot_agent(self, prompt: str, iteration: int, is_cold: bool) -> BenchmarkRun:
        """Benchmark ShadowBot Agent (Python SDK)."""
        code = f'''
import time
import json
import sys
sys.path.insert(0, "{self._get_project_root()}/src/shadowbot")
...
'''
    ...
    def benchmark_shadowbot_cli(self, prompt: str, iteration: int, is_cold: bool, with_profile: bool = False) -> BenchmarkRun:
        """Benchmark ShadowBot CLI."""
        t0 = time.perf_counter()
        
        cmd = ["shadowbot", prompt, "--llm", self.DEFAULT_MODEL]
        if with_profile:
            cmd.append("--profile")
        ...
        return BenchmarkRun(
            path_name="shadowbot_cli_profile" if with_profile else "shadowbot_cli",
            iteration=iteration,
            is_cold=is_cold,
            timings=PhaseTimings(
                total_ms=total_ms,
    ...
    def benchmark_shadowbot_workflow_single(self, prompt: str, iteration: int, is_cold: bool) -> BenchmarkRun:
        """Benchmark ShadowBot Workflow with single agent."""
        code = f'''
import time
import json
import sys
...
'''
        ...
        return BenchmarkRun(
            path_name="shadowbot_workflow_single",
            iteration=iteration,
            is_cold=is_cold,
            timings=PhaseTimings(
                import_ms=timings.get("import_ms", 0),
    ...
    def benchmark_shadowbot_workflow_multi(self, prompt: str, iteration: int, is_cold: bool) -> BenchmarkRun:
        """Benchmark ShadowBot Workflow with multiple agents."""
        code = f'''
import time
import json
import sys
...
'''
        ...
        return BenchmarkRun(
            path_name="shadowbot_workflow_multi",
            iteration=iteration,
            is_cold=is_cold,
            timings=PhaseTimings(
                import_ms=timings.get("import_ms", 0),
    ...
    def benchmark_shadowbot_litellm(self, prompt: str, iteration: int, is_cold: bool) -> BenchmarkRun:
        """Benchmark ShadowBot Agent using LiteLLM path (openai/ prefix)."""
        code = f'''
import time
import json
import sys
...
'''
    ...
    def create_deep_profile_output(self, result: BenchmarkResult, limit: int = 30) -> str:
        ...
        if result.aggregated_module_breakdown:
            ...
            if mb.shadowbot_modules:
                lines.append("\nShadowBot CLI Modules:")
                for file, cumul in mb.shadowbot_modules[:5]:
                    short_file = "..." + file[-50:] if len(file) > 50 else file
                    lines.append(f"  {short_file:<55} {cumul:>10.2f}ms")
            
            if mb.agent_modules:
                lines.append("\nShadowBot Agent Modules:")
    ...
    def run_full_benchmark(self, prompt: str = None, iterations: int = None, 
        ...
        if verbose:
            print("=" * 70)
            print("ShadowBot Comprehensive Benchmark Suite")
            print("=" * 70)
        ...
        if deep:
            ...
            benchmark_methods = {
                ...
                "shadowbot_agent": lambda p, i, c: self.benchmark_shadowbot_agent_deep(p, i, c, limit=limit),
                "shadowbot_cli": lambda p, i, c: self.benchmark_shadowbot_cli(p, i, c, with_profile=False),
                "shadowbot_cli_profile": lambda p, i, c: self.benchmark_shadowbot_cli(p, i, c, with_profile=True),
                "shadowbot_workflow_single": self.benchmark_shadowbot_workflow_single,
                "shadowbot_workflow_multi": self.benchmark_shadowbot_workflow_multi,
                "shadowbot_litellm": self.benchmark_shadowbot_litellm,
                "litellm_standalone": self.benchmark_litellm_standalone,
            }
        else:
            benchmark_methods = {
                "openai_sdk": self.benchmark_openai_sdk,
                "shadowbot_agent": self.benchmark_shadowbot_agent,
                "shadowbot_cli": lambda p, i, c: self.benchmark_shadowbot_cli(p, i, c, with_profile=False),
                "shadowbot_cli_profile": lambda p, i, c: self.benchmark_shadowbot_cli(p, i, c, with_profile=True),
                "shadowbot_workflow_single": self.benchmark_shadowbot_workflow_single,
                "shadowbot_workflow_multi": self.benchmark_shadowbot_workflow_multi,
        ...
        for path_name in paths:
            ...
            if verbose:
                deep_indicator = " [DEEP]" if deep and path_name in ["openai_sdk", "shadowbot_agent"] else ""
    ...
    def handle(self, subcommand: str = "profile", prompt: str = None, 
        ...
        if subcommand == "profile":
            ...
            # Agent only
            report = self.run_full_benchmark(prompt, iterations, paths=["openai_sdk", "shadowbot_agent"])
            self.print_report(report)
            return report
        
        elif subcommand == "cli":
            # CLI only
            report = self.run_full_benchmark(prompt, iterations, paths=["openai_sdk", "shadowbot_cli", "shadowbot_cli_profile"])
            self.print_report(report)
            return report
            ...
            report = self.run_full_benchmark(prompt, iterations=2, 
                                            paths=["openai_sdk", "shadowbot_agent", "shadowbot_cli", "litellm_standalone"])
