"""
Background Agents CLI Feature for ShadowBot.

Provides CLI commands for managing background agent tasks.

Commands:
- shadowbot background list              # List background tasks
- shadowbot background status <id>       # Get task status
- shadowbot background cancel <id>       # Cancel a task
- shadowbot background clear             # Clear completed tasks
"""
...
class BackgroundHandler:
    ...
    async def _get_runner(self):
        ...
        if self._runner is None:
            from shadowbotagents.background import BackgroundRunner
    ...
    async def list_tasks(self, status: Optional[str] = None) -> List[dict]:
        ...
        status_enum = None
        if status:
            from shadowbotagents.background import TaskStatus
            status_map = {
...
def handle_background_command(args: List[str], verbose: bool = False):
    ...
    Usage:
        shadowbot background list [--status <status>] [--json] [--page N] [--page-size N]
        shadowbot background status <task_id> [--json]
        shadowbot background cancel <task_id> [--json]
        shadowbot background clear [--all] [--older-than SEC] [--json]
        shadowbot background submit --recipe <name> [--input JSON] [--session-id ID] [--timeout SEC] [--json]
    """
    import argparse
    import json
    
    parser = argparse.ArgumentParser(prog="shadowbot background", description="Manage background tasks")
