"""
Persistence CLI feature for ShadowBot.

Provides commands for database persistence configuration and management:
- shadowbot persistence doctor  - Validate DB connectivity
- shadowbot persistence run     - Run agent with persistence
- shadowbot persistence resume  - Resume a session
"""
...
def print_persistence_help():
    ...
    help_text = """
ShadowBot Persistence Commands
==============================

Usage: shadowbot persistence <command> [options]
...
Doctor Command:
  shadowbot persistence doctor [options]
...
Run Command:
  shadowbot persistence run [options] "prompt"
...
Resume Command:
  shadowbot persistence resume --session-id ID [options]
...
Export/Import Commands:
  shadowbot persistence export --session-id ID [options]
  shadowbot persistence import --file FILE [options]
...
Examples:
  ...
  shadowbot persistence doctor --conversation-url "postgresql://localhost/mydb"
  
  # Run agent with persistence
  shadowbot persistence run --session-id "my-session" "Hello, remember my name is Alice"
  
  # Resume a session
  shadowbot persistence resume --session-id "my-session" --show-history
  
  # Continue a session
  shadowbot persistence resume --session-id "my-session" --continue "What's my name?"
  
  # Export a session
  shadowbot persistence export --session-id "my-session" --output session.jsonl
  
  # Import a session
  shadowbot persistence import --file session.jsonl
"""
    print(help_text)
...
def handle_doctor(args: list) -> bool:
    ...
    print("=" * 50)
    print("ShadowBot Persistence Doctor")
    print("=" * 50)
...
def handle_run(args: list) -> bool:
    ...
    if not config["prompt"]:
        ...
        print("Usage: shadowbot persistence run [options] \"your prompt\"")
        return True
...
def handle_migrate(args: list) -> bool:
    ...
    try:
        from shadowbotagents import db
        from shadowbot.persistence.migrations import MigrationManager
...
def _test_conversation_store(url: str) -> tuple:
    ...
    try:
        from shadowbot.persistence.factory import create_conversation_store
...
def _test_knowledge_store(url: str) -> tuple:
    ...
    try:
        from shadowbot.persistence.factory import create_knowledge_store
...
def _test_state_store(url: str) -> tuple:
    ...
    try:
        from shadowbot.persistence.factory import create_state_store
