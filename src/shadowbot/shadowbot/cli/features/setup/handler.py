"""
Setup handler for ShadowBot CLI.
...
class SetupHandler(CommandHandler):
    """
    Handler for the 'shadowbot setup' command.
    ...
    def get_praison_home(self) -> Path:
        """Get the ShadowBot home directory."""
        home = os.getenv("PRAISONAI_HOME")
        if home:
            return Path(home)
        return Path.home() / ".shadowbot"
    ...
    def _create_directories(self, praison_home: Path, output) -> None:
        """Create the ~/.shadowbot directory structure."""
    ...
    def _run_non_interactive(
        ...
        output.console.print('Try: [cyan]shadowbot "Say hello in one sentence"[/cyan]')
    ...
    def _run_interactive(self, praison_home: Path, output) -> int:
        ...
        output.console.print("\n[bold green]🚀 ShadowBot Setup Wizard[/bold green]\n")
        output.console.print("Let's configure ShadowBot for your first agent run!\n")
        ...
        output.console.print("Anonymous usage data helps improve ShadowBot.")
        ...
        output.console.print('• Test: [cyan]shadowbot "Say hello in one sentence"[/cyan]')
        output.console.print("• Chat: [cyan]shadowbot chat[/cyan]")
        output.console.print("• Help: [cyan]shadowbot --help[/cyan]")
        output.console.print("• Docs: [cyan]shadowbot doctor[/cyan]")
    ...
    def _save_env(self, praison_home: Path, env_vars: Dict[str, str], output) -> None:
        ...
        lines = []
        lines.append("# ShadowBot Configuration")
        lines.append("# Generated by shadowbot setup")
        lines.append("")
    ...
    def _save_config(self, praison_home: Path, config: Dict[str, Any], output) -> None:
        ...
        try:
            ...
            config_file.write_text(f"# ShadowBot Configuration\n# Generated by shadowbot setup\n\n{config_content}")
    ...
    def _create_starter_yaml(self, output, model: str) -> None:
        ...
        output.console.print("Run [cyan]shadowbot workflow run --file agents.yaml[/cyan] to test it")