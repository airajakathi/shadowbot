...
Provides CLI commands for interacting with ShadowBot endpoints:
...
All commands use the canonical `shadowbot endpoints` prefix.
...
- No impact on shadowbotagents import time
...
class EndpointsHandler:
    ...
    def _print_help(self):
        """Print help message."""
        help_text = """
[bold cyan]ShadowBot Endpoints[/bold cyan]

Unified client CLI for interacting with ShadowBot endpoints.

[bold]Usage:[/bold]
  shadowbot endpoints <command> [options]
...
[bold]Examples:[/bold]
  ...
  shadowbot endpoints list --type agents-api
  shadowbot endpoints list --format json --tags audio,video
  shadowbot endpoints describe my-agent
  shadowbot endpoints describe my-agent --schema
  shadowbot endpoints invoke my-agent --input-json '{"query": "hello"}'
  shadowbot endpoints invoke my-recipe --input ./data.json --stream
  shadowbot endpoints health
  shadowbot endpoints health --url http://localhost:8000
  shadowbot endpoints types
  shadowbot endpoints discovery
"""
    ...
    def _get_provider(self, provider_type: str, url: Optional[str] = None, api_key: Optional[str] = None):
        ...
        try:
            from shadowbot.endpoints import get_provider
            return get_provider(
                provider_type,
    ...
    def _try_unified_discovery(self, url: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        result = self._make_request("GET", "/__shadowbot__/discovery", url=url)
