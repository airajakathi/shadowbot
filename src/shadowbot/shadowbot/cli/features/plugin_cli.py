...
def create_plugin_app():
    ...
    def init_command(
        ...
        Examples:
            shadowbot plugin init weather_tools
            shadowbot plugin init my_plugin --author "John Doe" --with-hook
            shadowbot plugin init custom --output ./my_plugins/
        """
        try:
            from shadowbotagents.plugins.discovery import get_plugin_template, ensure_plugin_dir
        except ImportError:
            typer.echo("Error: shadowbotagents not installed", err=True)
            raise typer.Exit(1)
        ...
        typer.echo("  from shadowbotagents import Agent")
    ...
    def list_command(
        ...
        Examples:
            shadowbot plugin list
            shadowbot plugin list --json
            shadowbot plugin list --dir ./my_plugins/
        """
        try:
            from shadowbotagents.plugins.discovery import discover_plugins, get_default_plugin_dirs
        except ImportError:
            typer.echo("Error: shadowbotagents not installed", err=True)
            raise typer.Exit(1)
        ...
        if not plugins:
            ...
            typer.echo("  shadowbot plugin init my_plugin")
            return
    ...
    def load_command(
        ...
        Examples:
            shadowbot plugin load ./my_plugin.py
            shadowbot plugin load ~/.praison/plugins/weather.py
        """
        try:
            from shadowbotagents.plugins.discovery import load_plugin
        except ImportError:
            typer.echo("Error: shadowbotagents not installed", err=True)
            raise typer.Exit(1)
    ...
    def discover_command(
        ...
        Examples:
            shadowbot plugin discover
            shadowbot plugin discover --verbose
        """
        try:
            from shadowbotagents.plugins.discovery import discover_and_load_plugins
        except ImportError:
            typer.echo("Error: shadowbotagents not installed", err=True)
            raise typer.Exit(1)
    ...
    def template_command(
        ...
        Examples:
            ...
            shadowbot plugin template > my_plugin.py
            shadowbot plugin template --with-hook
        """
        try:
            from shadowbotagents.plugins.discovery import get_plugin_template
        except ImportError:
            typer.echo("Error: shadowbotagents not installed", err=True)
            raise typer.Exit(1)
