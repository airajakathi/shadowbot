"""
n8n Integration Handler for ShadowBot CLI.

Converts ShadowBot agents.yaml workflows to n8n workflow JSON format
and opens them in the n8n UI.

Usage:
    shadowbot agents.yaml --n8n
    shadowbot agents.yaml --n8n --n8n-url http://localhost:5678
"""
...
def _default_api_url(n8n_url: str, port: int = 8005) -> str:
    """Return a host-reachable URL for ShadowBot API based on where n8n runs."""
...
class N8nHandler(FlagHandler):
    ...
    Converts ShadowBot YAML workflows to n8n-compatible JSON format
    ...
    def __init__(self, verbose: bool = False, n8n_url: str = "http://localhost:5678",
        ...
        Args:
            ...
            use_execute_command: Use Execute Command nodes (runs shadowbot directly)
                                 instead of HTTP Request nodes
            api_url: ShadowBot API URL that n8n will call (for tunnel/cloud deployments)
                    If None, uses Docker-aware default based on n8n_url
            port: ShadowBot API port (used when api_url is None)
        """
        super().__init__(verbose=verbose)
        self.n8n_url = n8n_url
        self._api_url_explicit = api_url is not None
        self._port = port
        self.shadowbot_api_url = api_url if api_url is not None else _default_api_url(n8n_url, port)
    ...
    def execute(self, yaml_path: str, n8n_url: Optional[str] = None, 
        ...
        if n8n_url:
            ...
            if not self._api_url_explicit:
                self.shadowbot_api_url = _default_api_url(self.n8n_url, self._port)
        ...
        if workflow_id:
            ...
            if self._activate_workflow(workflow_id, api_key):
                ...
                if trigger_node:
                    webhook_path = trigger_node["parameters"].get("path", "shadowbot")
    ...
    def _create_workflow_via_api(self, workflow_json: Dict[str, Any], 
        ...
        try:
            ...
            workflow_data = {
                "name": workflow_json.get("name", "ShadowBot Workflow"),
    ...
    def convert_yaml_to_n8n(self, yaml_path: str, use_webhook: bool = True) -> Dict[str, Any]:
        """
        Convert a ShadowBot YAML workflow to n8n JSON format.
        ...
        workflow_name = yaml_content.get('name', 'ShadowBot Workflow')
    ...
    def _create_trigger_node(self, use_webhook: bool = False, 
                              webhook_path: str = "shadowbot") -> Dict[str, Any]:
    ...
    def _create_per_agent_node(self, agent_id: str, agent_config: Dict,
        ...
        return {
            ...
            "parameters": {
                "method": "POST",
                "url": f"{self.shadowbot_api_url}/agents/{agent_url_id}/invoke",
                "sendBody": True,
                "specifyBody": "json",
    ...
    def _create_shadowbot_workflow_node(self, workflow_name: str,
                                         agent_names: List[str],
                                         position: List[int]) -> Dict[str, Any]:
        """
        Create a single HTTP Request node that triggers the entire ShadowBot workflow.
        
        The ShadowBot API will run all agents sequentially when called.
        ...
        return {
            "id": "shadowbot_workflow",
            "name": f"ShadowBot: {workflow_name}",
            "type": "n8n-nodes-base.httpRequest",
            "typeVersion": 4.2,
            "position": position,
            "parameters": {
                "method": "POST",
                "url": f"{self.shadowbot_api_url.rstrip('/')}/workflow/execute",
                "sendBody": True,
                "specifyBody": "json",
    ...
    def _create_http_request_node(self, agent_id: str, agent_config: Dict,
        ...
        return {
            ...
            "parameters": {
                "method": "POST",
                "url": f"{self.shadowbot_api_url.rstrip('/')}/agents/{agent_url_id}/invoke",
                "sendBody": True,
                "specifyBody": "json",
