...
Provides access to the lightweight shadowbotagents.lite subpackage.
Usage: shadowbot lite [run|info]
"""
...
class LiteHandler(FlagHandler):
    ...
    Commands:
        shadowbot lite run "prompt"     - Run a lite agent with custom LLM
        shadowbot lite info             - Show lite package info
        shadowbot lite example          - Show example usage
    """
    ...
    def check_dependencies(self) -> Tuple[bool, str]:
        ...
        if importlib.util.find_spec("shadowbotagents.lite") is not None:
            return True, ""
        return False, "Lite package requires shadowbotagents>=0.5.0"
    ...
    def get_info(self) -> Dict[str, Any]:
        ...
        """
        import importlib.util
        if importlib.util.find_spec("shadowbotagents.lite") is None:
            return {
                "available": False,
                "error": "shadowbotagents.lite not found"
            }
    ...
    def show_example(self) -> str:
        ...
        """
        example = '''
# Example: Using shadowbotagents.lite with custom LLM

from shadowbotagents.lite import LiteAgent, tool
...
from shadowbotagents.lite import create_openai_llm_fn
...
'''
    ...
    def run_lite_agent(
        ...
        """
        try:
            from shadowbotagents.lite import LiteAgent, create_openai_llm_fn, create_anthropic_llm_fn
    ...
    def handle(self, subcommand: str = "info", **kwargs) -> Any:
        ...
        if subcommand == "info":
            ...
            if info.get("available"):
                print("shadowbotagents.lite - Lightweight Agent Package")
                print("=" * 50)
