"""
Hybrid Workflow Executor for ShadowBot CLI.
...
- Multi-agent workflow steps (using shadowbotagents)

This provides a unified executor that can handle both paradigms in a single workflow.

Usage:
    shadowbot workflow run hybrid-workflow.yaml
    shadowbot workflow run hybrid-workflow.yaml --dry-run
"""
...
class HybridWorkflowExecutor:
    ...
    def _execute_workflow_step(self, step: Dict, flags: Dict) -> Dict:
        ...
        try:
            ...
            return {"ok": False, "error": "shadowbotagents not installed"}
        except Exception as e:
