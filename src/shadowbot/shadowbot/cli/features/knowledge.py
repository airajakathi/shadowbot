...
Usage:
    shadowbot knowledge add document.pdf
    shadowbot knowledge add ./docs/
    shadowbot knowledge query "How to authenticate?"
    shadowbot knowledge list
    shadowbot knowledge clear
    shadowbot knowledge stats
"""
...
class KnowledgeHandler(CommandHandler):
    ...
    Example:
        shadowbot knowledge add document.pdf
        shadowbot knowledge add ./docs/
        shadowbot knowledge query "How to authenticate?" --retrieval fusion
        shadowbot knowledge list
        shadowbot knowledge clear
        shadowbot knowledge stats
    """
    ...
    def get_help_text(self) -> str:
        return """
Knowledge Commands:
  shadowbot knowledge add <file|dir|url>   - Add document(s) to knowledge base
  shadowbot knowledge query "<question>"   - Query knowledge base with RAG
  shadowbot knowledge list                 - List indexed documents
  shadowbot knowledge clear                - Clear knowledge base
  shadowbot knowledge stats                - Show knowledge base statistics
  shadowbot knowledge export <file.json>   - Export knowledge base to JSON file
  shadowbot knowledge import <file.json>   - Import knowledge base from JSON file
  shadowbot knowledge help                 - Show this help message
...
Examples:
  shadowbot knowledge add document.pdf
  shadowbot knowledge add ./docs/
  shadowbot knowledge query "How to authenticate?" --retrieval-strategy fusion
  shadowbot knowledge query "Compare approaches" --reranker simple --query-mode sub_question
"""
    ...
    def _get_knowledge(self):
        ...
        if self._knowledge is None:
            try:
                from shadowbotagents import Knowledge
                ...
                self.print_status(
                    "Knowledge requires shadowbotagents. Install with: pip install shadowbotagents[knowledge]",
                    "error"
                )
                return None
    ...
    def action_add(self, args: List[str], **kwargs) -> bool:
        ...
        """
        if not args:
            self.print_status("Usage: shadowbot knowledge add <file|directory|url|glob>", "error")
            return False
    ...
    def action_query(self, args: List[str], **kwargs) -> List[Dict[str, Any]]:
        ...
        """
        if not args:
            self.print_status("Usage: shadowbot knowledge query <question>", "error")
            return []
    ...
    def action_import(self, args: List[str], **kwargs) -> bool:
        ...
        if not args:
            self.print_status("Usage: shadowbot knowledge import <file.json>", "error")
            return False
