"""
Evaluation CLI feature for ShadowBot.
...
class EvalHandler:
    ...
    def run_performance(
        ...
        """
        try:
            from shadowbotagents.eval import PerformanceEvaluator
            from shadowbot.agents_generator import AgentsGenerator
        except ImportError as e:
            logger.error(f"Failed to import evaluation modules: {e}")
            return {"error": str(e)}
    ...
    def run_reliability(
        ...
        """
        try:
            from shadowbotagents.eval import ReliabilityEvaluator
            from shadowbot.agents_generator import AgentsGenerator
        except ImportError as e:
            logger.error(f"Failed to import evaluation modules: {e}")
            return {"error": str(e)}
...
def handle_eval_command(args) -> int:
    ...
    if isinstance(args, list):
        parser = argparse.ArgumentParser(prog="shadowbot eval")
        ...
        if not args.eval_type:
            parser.print_help()
            print("\n[bold]Examples:[/bold]")
            print("  shadowbot eval accuracy --prompt \"What is 2+2?\" --expected \"4\"")
            print("  shadowbot eval performance --agent agents.yaml --input \"Hello\"")
            return 0
