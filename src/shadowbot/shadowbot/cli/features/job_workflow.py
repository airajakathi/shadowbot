"""
Job Workflow Executor for ShadowBot CLI.
...
Usage:
    shadowbot workflow run publish-pypi.yaml
    shadowbot workflow run publish-pypi.yaml --dry-run
    shadowbot workflow run publish-pypi.yaml --major
"""
...
class JobWorkflowExecutor:
    ...
    def _exec_agent_step(self, agent_config: Any, step: Dict, flags: Dict) -> Dict:
        """
        Execute an agent step using shadowbotagents.Agent.
        ...
        try:
            ...
            return {"ok": False, "error": "shadowbotagents not installed. Install with: pip install shadowbotagents"}
    ...
    def _exec_judge_step(self, judge_config: Any, step: Dict, flags: Dict) -> Dict:
        ...
        try:
            ...
            return {"ok": False, "error": "shadowbotagents.eval not available. Install with: pip install shadowbotagents"}
    ...
    def _exec_approve_step(self, approve_config: Any, step: Dict, flags: Dict) -> Dict:
        ...
        """
        try:
            from shadowbotagents.approval.backends import ConsoleBackend, AutoApproveBackend
            from shadowbotagents.approval.protocols import ApprovalRequest
        except ImportError:
            return {"ok": False, "error": "shadowbotagents.approval not available. Install with: pip install shadowbotagents"}
    ...
    def _resolve_agent_tools(self, tool_names: List) -> List:
        ...
        for tool in tool_names:
            if callable(tool):
                ...
                try:
                    from shadowbotagents.tools import get_tool
                    ...
                    if tool == "execute_command":
                        ...
                        try:
                            from shadowbotagents.tools import read_file
