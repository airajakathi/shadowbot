...
Usage: shadowbot "prompt" --guardrail "Ensure output is valid JSON"
"""
...
def _check_shadowbot():
    """Check if shadowbotagents is available."""
    global PRAISONAI_AVAILABLE, LLMGuardrail
    try:
        from shadowbotagents import LLMGuardrail as _LLMGuardrail
...
class GuardrailHandler(FlagHandler):
    ...
    Example:
        shadowbot "Write code" --guardrail "Ensure code is secure"
    """
    ...
    def create_guardrail(self, description: str, llm: Any = None):
        ...
        """
        if not _check_shadowbot():
            self.print_status(
                "Guardrail requires shadowbotagents. Install with: pip install shadowbotagents",
                "error"
            )
            return None
    ...
    def validate_output(self, output: str, description: str, llm: Any = None) -> Tuple[bool, str]:
        ...
        try:
            ...
            from shadowbotagents import TaskOutput
