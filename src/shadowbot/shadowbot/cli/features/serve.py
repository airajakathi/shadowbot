...
Provides CLI commands for launching ShadowBot servers:
- shadowbot serve agents - Launch agents as HTTP API
- shadowbot serve recipe - Launch recipe runner
- shadowbot serve mcp - Launch MCP server
- shadowbot serve tools - Launch tools as MCP server
- shadowbot serve a2a - Launch A2A server
- shadowbot serve a2u - Launch A2U event stream server
- shadowbot serve unified - Launch unified server with all providers

All servers include the unified discovery endpoint at /__shadowbot__/discovery.
"""
...
class ServeHandler:
    ...
    def _print_help(self):
        """Print help message."""
        help_text = """
[bold cyan]ShadowBot Serve[/bold cyan]

Launch ShadowBot servers with unified discovery support.

[bold]Usage:[/bold]
  shadowbot serve <command> [options]
...
[bold]Examples:[/bold]
  shadowbot serve agents --file agents.yaml --port 8000
  shadowbot serve recipe --port 8765
  shadowbot serve mcp --transport http --port 8080
  shadowbot serve tools --port 8081
  shadowbot serve a2a --port 8082
  shadowbot serve unified --port 8765

[bold]Discovery:[/bold]
  All servers expose /__shadowbot__/discovery for unified endpoint discovery.
  Use `shadowbot endpoints` to interact with any server.
"""
    ...
    def cmd_agents(self, args: List[str]) -> int:
        ...
        try:
            ...
            print("  Discovery: /__shadowbot__/discovery")
            ...
            print("Install with: pip install shadowbot[serve]")
            return self.EXIT_GENERAL_ERROR
    ...
    def _create_agents_app(self, config: Dict[str, Any]) -> Any:
        ...
        from shadowbot.endpoints.discovery import (
            create_discovery_document,
            EndpointInfo,
            ProviderInfo,
        )
        from shadowbot.endpoints.server import add_discovery_routes
        ...
        discovery = create_discovery_document(server_name="shadowbot-agents")
        ...
        # Create app
        app = FastAPI(
            title="ShadowBot Agents API",
            description="HTTP API for ShadowBot Agents",
        )
        ...
        try:
            from shadowbot.api import agent_invoke
        ...
        async def invoke_agents(request: Request, query_data: AgentQuery = None):
            ...
            try:
                ...
                if agent_name:
                    try:
                        from shadowbot.api.agent_invoke import get_agent
                ...
                from shadowbot.inc import LLMConfig
    ...
    def _register_agents_from_yaml(self, agents_config: Dict[str, Any], register_agent_func) -> None:
        ...
        try:
            # Try to import shadowbotagents to create Agent instances
            ...
            logging.getLogger(__name__).warning("shadowbotagents not available, skipping agent registration")
        except Exception as e:
    ...
    def cmd_recipe(self, args: List[str]) -> int:
        ...
        try:
            ...
            from shadowbotagents.gateway.adapters.recipe_adapter import RecipeBotAdapter
    ...
    def cmd_mcp(self, args: List[str]) -> int:
        """Launch MCP server (DEPRECATED - use 'shadowbot mcp serve' instead)."""
        import sys
        
        # Print deprecation warning
        print("\n[yellow]⚠ DEPRECATION WARNING:[/yellow]", file=sys.stderr)
        print("[yellow]'shadowbot serve mcp' is deprecated and will be removed in a future version.[/yellow]", file=sys.stderr)
        print("[yellow]Please use 'shadowbot mcp serve' instead.[/yellow]\n", file=sys.stderr)
        
        # Redirect to new MCP server CLI
        try:
            from shadowbot.mcp_server.cli import handle_mcp_command
    ...
    def _create_mcp_app(self, config: Dict[str, Any]) -> Any:
        ...
        # Create discovery document
        discovery = create_discovery_document(server_name="shadowbot-mcp")
        discovery.add_provider(ProviderInfo(
            type="mcp",
            name="MCP Server",
    ...
    def cmd_tools(self, args: List[str]) -> int:
        """Launch tools as MCP server (DEPRECATED - use 'shadowbot mcp serve' instead)."""
        import sys
        
        # Print deprecation warning
        print("\n[yellow]⚠ DEPRECATION WARNING:[/yellow]", file=sys.stderr)
        print("[yellow]'shadowbot serve tools' is deprecated and will be removed in a future version.[/yellow]", file=sys.stderr)
        print("[yellow]Please use 'shadowbot mcp serve' instead.[/yellow]\n", file=sys.stderr)
    ...
    def _create_tools_app(self, config: Dict[str, Any]) -> Any:
        ...
        discovery = create_discovery_document(server_name="shadowbot-tools-mcp")
        discovery.add_provider(ProviderInfo(
            type="tools-mcp",
        ...
        app = FastAPI(
            title="ShadowBot Tools MCP Server",
        ...
        async def root():
            return {
                "message": "ShadowBot Tools MCP Server",
                "discovery": "/__shadowbot__/discovery",
            }
    ...
    def _create_a2a_app(self, config: Dict[str, Any]) -> Any:
        ...
        discovery = create_discovery_document(server_name="shadowbot-a2a")
        discovery.add_provider(ProviderInfo(
            type="a2a",
            name="A2A Protocol",
        ...
        async def agent_card():
            ...
            return {
                "name": "ShadowBot Agent",
                "description": "ShadowBot Agent via A2A protocol",
                "version": "1.0.0",
        ...
        async def status():
            ...
            return {"status": "ok", "name": "ShadowBot A2A", "version": "1.0.0"}
    ...
    def _create_a2u_app(self, config: Dict[str, Any]) -> Any:
        ...
        from shadowbot.endpoints.server import add_discovery_routes
        from shadowbot.endpoints.a2u_server import create_a2u_routes
        
        discovery = create_discovery_document(server_name="shadowbot-a2u")
        discovery.add_provider(ProviderInfo(
            type="a2u",
    ...
    def _create_unified_app(self, config: Dict[str, Any]) -> Any:
        ...
        discovery = create_discovery_document(server_name="shadowbot-unified")
        ...
        app = FastAPI(
            title="ShadowBot Unified Server",
            description="Unified server with all ShadowBot providers",
        )
        ...
        async def root():
            return {
                "message": "ShadowBot Unified Server",
        ...
        async def agent_card():
            return {
                "name": "ShadowBot Unified",
                "description": "ShadowBot Unified Server",
                "version": "1.0.0",
            }
