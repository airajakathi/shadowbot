...
All commands use the canonical `shadowbot recipe` prefix.
"""
...
class RecipeHandler:
    ...
    def _print_help(self):
        """Print help message."""
        help_text = """
[bold cyan]ShadowBot Recipe[/bold cyan]

[bold]Usage:[/bold]
  shadowbot recipe <command> [options]
...
[bold]Examples:[/bold]
  shadowbot recipe list
  shadowbot recipe search video
  shadowbot recipe info transcript-generator
  shadowbot recipe validate support-reply
  shadowbot recipe run support-reply --input '{"ticket_id": "T-123"}'
  shadowbot recipe run transcript-generator ./audio.mp3 --json
  shadowbot recipe init my-recipe
  shadowbot recipe serve --port 8765
"""
    ...
    def cmd_list(self, args: List[str]) -> int:
        ...
        try:
            ...
            if parsed["registry"]:
                from shadowbot.recipe.registry import get_registry
                import os
    ...
    def _run_background(
        ...
        try:
            from shadowbot.recipe.operations import run_background
            ...
            if json_output:
                ...
                print(f"\nCheck status with: shadowbot background status {task.task_id}")
...
```bash
shadowbot recipe run {parsed["name"]} --input "your input"
```
...
'''
            ...
            print(f"  3. shadowbot recipe validate {parsed['name']}")
            print(f"  4. shadowbot recipe run {parsed['name']} --input 'test'")
    ...
    def cmd_unpack(self, args: List[str]) -> int:
        ...
        try:
            ...
            from shadowbot.recipe.registry import _safe_extractall
    ...
    def cmd_export(self, args: List[str]) -> int:
        ...
        try:
            from shadowbot.recipe.history import get_history
    ...
    def cmd_serve(self, args: List[str]) -> int:
        ...
        if parsed["config"]:
            try:
                from shadowbot.recipe.serve import load_config
        ...
        try:
            from shadowbot.recipe.serve import serve
            ...
            self._print_error("Serve dependencies not installed. Run: pip install shadowbot[serve]")
    ...
    def cmd_sbom(self, args: List[str]) -> int:
        ...
        try:
            from shadowbot.recipe.security import generate_sbom
    ...
    def cmd_audit(self, args: List[str]) -> int:
        ...
        try:
            from shadowbot.recipe.security import audit_dependencies
    ...
    def cmd_sign(self, args: List[str]) -> int:
        ...
        try:
            from shadowbot.recipe.security import sign_bundle
    ...
    def cmd_verify(self, args: List[str]) -> int:
        ...
        try:
            from shadowbot.recipe.security import verify_bundle
    ...
    def cmd_policy(self, args: List[str]) -> int:
        ...
        try:
            from shadowbot.recipe.policy import PolicyPack, get_default_policy
    ...
    def cmd_judge(self, args: List[str]) -> int:
        ...
        Usage:
            shadowbot recipe judge <trace_id> [options]
        ...
        if not parsed.get("trace_id"):
            ...
            print("\nUsage: shadowbot recipe judge <trace_id> [--memory|--knowledge|--context]")
            print("\nExamples:")
            print("  shadowbot recipe judge run-abc123")
            print("  shadowbot recipe judge run-abc123 --memory")
            print("  shadowbot recipe judge run-abc123 --knowledge")
            print("  shadowbot recipe judge run-abc123 --yaml agents.yaml --output plan.yaml")
            return self.EXIT_VALIDATION_ERROR
        ...
        try:
            from shadowbot.replay import (
                ContextTraceReader,
                ContextEffectivenessJudge,
            )
            from shadowbot.replay.judge import (
            ...
            if yaml_file:
                ...
                if not dry_run:
                    ...
                    print(f"   Run 'shadowbot recipe apply {out_file}' to apply fixes")
            else:
    ...
    def cmd_apply(self, args: List[str]) -> int:
        ...
        Usage:
            shadowbot recipe apply <plan_file> [options]
        ...
        if not parsed.get("plan_file"):
            ...
            print("\nUsage: shadowbot recipe apply <plan_file> [--confirm] [--dry-run]")
            return self.EXIT_VALIDATION_ERROR
        ...
        try:
            from shadowbot.replay.judge import JudgePlan
    ...
    def cmd_create(self, args: List[str]) -> int:
        ...
        Usage:
            shadowbot recipe create "GOAL" [options]
        ...
        if not parsed.get("goal"):
            ...
            print("\nUsage: shadowbot recipe create \"GOAL\" [options]")
            print("\nExamples:")
            print('  shadowbot recipe create "Build a web scraper for news"')
            print('  shadowbot recipe create "Research AI trends" --no-optimize')
            print('  shadowbot recipe create "Research AI" --agents "researcher:role=AI Researcher,goal=Find papers"')
            print('  shadowbot recipe create "Research AI" --tools "researcher:internet_search,arxiv"')
            return self.EXIT_VALIDATION_ERROR
        ...
        try:
            ...
            print(f"   Run with: shadowbot recipe run {recipe_path.name}")
    ...
    def cmd_optimize(self, args: List[str]) -> int:
        ...
        Usage:
            shadowbot recipe optimize <NAME> [WHAT] [options]
        ...
        if not parsed.get("name"):
            self._print_error("Recipe name required")
            print("\nUsage: shadowbot recipe optimize <NAME> [WHAT] [options]")
            print("\nExamples:")
            print('  shadowbot recipe optimize my-recipe')
            print('  shadowbot recipe optimize my-recipe "improve error handling"')
            return self.EXIT_VALIDATION_ERROR
