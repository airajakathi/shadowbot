"""
Hooks CLI Feature for ShadowBot.

Provides CLI commands for managing and testing hooks.

Commands:
- shadowbot hooks list                    # List registered hooks
- shadowbot hooks test <event> <target>   # Test hooks for an event
- shadowbot hooks validate <config>       # Validate hooks configuration
- shadowbot hooks run <command>           # Run a command hook manually
"""
...
class HooksHandler:
    ...
    def _get_registry(self):
        ...
        if self._registry is None:
            from shadowbotagents.hooks import HookRegistry
    ...
    def _get_runner(self):
        ...
        if self._runner is None:
            from shadowbotagents.hooks import HookRunner
    ...
    def test_hook(
        ...
        """
        from shadowbotagents.hooks import HookEvent
        from shadowbotagents.hooks.events import BeforeToolInput, BeforeAgentInput
...
"""
    ...
    def run_command_hook(
        ...
        Returns:
            Execution result
        """
        from shadowbotagents.hooks import HookEvent, HookRegistry, HookRunner
        from shadowbotagents.hooks.events import BeforeToolInput
        from datetime import datetime
...
def handle_hooks_command(args: List[str], verbose: bool = False):
    ...
    Usage:
        shadowbot hooks list [--format json|table]
        shadowbot hooks test <event> [target] [--input '{"key": "value"}']
        shadowbot hooks validate <config_path>
        shadowbot hooks run <command> [--event <event>] [--input <json>] [--timeout <seconds>]
        shadowbot hooks load <config_path>
    """
    handler = HooksHandler(verbose=verbose)
    
    if not args:
        print("Usage: shadowbot hooks <command> [options]")
        print("\nCommands:")
    ...
    if command == "list":
        ...
        if len(args) < 2:
            print("Usage: shadowbot hooks test <event> [target] [--input <json>]")
            return
        ...
        if len(args) < 2:
            print("Usage: shadowbot hooks validate <config_path>")
            return
        handler.validate_config(args[1])
    
    elif command == "run":
        if len(args) < 2:
            print("Usage: shadowbot hooks run <command> [--event <event>] [--input <json>] [--timeout <seconds>]")
            return
        ...
        if len(args) < 2:
            print("Usage: shadowbot hooks load <config_path>")
            return
        ...
        print("\n  shadowbot hooks test <event> [target] [--input <json>]")
