...
Usage: shadowbot workflow run research.yaml
       shadowbot workflow validate research.yaml
       shadowbot workflow list
       shadowbot workflow create --template routing
"""
...
class WorkflowHandler(CommandHandler):
    ...
    Example:
        shadowbot workflow run research.yaml --var topic="AI"
        shadowbot workflow validate research.yaml
        shadowbot workflow list
        shadowbot workflow create --template routing --output my_workflow.yaml
    """
...
"""
    ...
    def get_help_text(self) -> str:
        return """
Workflow Commands:
  shadowbot workflow run <file.yaml>           - Run a YAML workflow
  shadowbot workflow run <file.yaml> --var key=value  - Run with variables
  shadowbot workflow validate <file.yaml>      - Validate a YAML workflow
  shadowbot workflow list                      - List workflows in .praison/workflows/
  shadowbot workflow create --template <name>  - Create from template
  shadowbot workflow auto "topic" --pattern <pattern>  - Auto-generate workflow
  
Templates: simple, routing, parallel, loop, evaluator-optimizer, model-routing
Patterns: sequential, routing, parallel

Example:
  shadowbot workflow run research.yaml --var topic="AI trends"
  shadowbot workflow create --template routing --output my_workflow.yaml
  shadowbot workflow auto "Research AI trends" --pattern parallel
"""
    ...
    def _get_manager(self):
        ...
        if self._manager is None:
            try:
                ...
                self.print_status(
                    "Workflows require shadowbotagents. Install with: pip install shadowbotagents",
                    "error"
                )
                return None
    ...
    def action_run(self, args: List[str], **kwargs) -> Any:
        ...
        if not file_path:
            self.print_status("Usage: shadowbot workflow run <file.yaml>", "error")
            return None
    ...
    def action_validate(self, args: List[str], **kwargs) -> bool:
        ...
        file_path = self._get_file_from_args(args)
        if not file_path:
            self.print_status("Usage: shadowbot workflow validate <file.yaml>", "error")
            return False
    ...
    def action_create(self, args: List[str], **kwargs) -> Optional[str]:
        ...
        if not template_name:
            self.print_status("Usage: shadowbot workflow create --template <name>", "error")
        ...
        self.print_status(f"Run with: shadowbot workflow run {output_file}", "info")
    ...
    def action_auto(self, args: List[str], **kwargs) -> Optional[str]:
        ...
        if not topic:
            self.print_status('Usage: shadowbot workflow auto "topic" [--type job|agent] [--pattern <pattern>]', "error")
        ...
        try:
            if workflow_type == "job":
                ...
                from shadowbot.auto import JobWorkflowAutoGenerator
