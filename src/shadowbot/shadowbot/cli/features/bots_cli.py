"""
Bot CLI commands for ShadowBot.
...
class BotHandler:
    ...
    def start_telegram(
        ...
        try:
            from shadowbot.bots import TelegramBot
    ...
    def start_discord(
        ...
        try:
            from shadowbot.bots import DiscordBot
        except ImportError as e:
    ...
    def start_slack(
        ...
        try:
            from shadowbot.bots import SlackBot
        except ImportError as e:
    ...
    def start_whatsapp(
        ...
        try:
            from shadowbot.bots import WhatsAppBot
        except ImportError as e:
            print(f"Error: WhatsAppBot import failed. {e}")
            if mode == "web":
                print("Install with: pip install 'shadowbot[bot-whatsapp-web]'")
            else:
    ...
    def start_linear(
        ...
        try:
            from shadowbot.bots import LinearBot
        except ImportError as e:
    ...
    def start_email(
        ...
        try:
            from shadowbot.bots import EmailBot
        except ImportError as e:
    ...
    def start_agentmail(
        ...
        try:
            from shadowbot.bots import AgentMailBot
        except ImportError as e:
    ...
    def _build_tools(self, capabilities: BotCapabilities) -> List:
        ...
        try:
            from shadowbotagents.tools import execute_command, search_web
            tools.append(execute_command)
            tools.append(search_web)
            logger.info("Default tools enabled: execute_command, search_web")
        except ImportError:
            logger.warning("Default tools not available from shadowbotagents.tools")
        ...
        try:
            ...
            logger.warning("Schedule tools not available from shadowbotagents.tools")
        
        # Browser tool
        if capabilities.browser:
            try:
                from shadowbot_tools import BrowserBaseTool
                tools.append(BrowserBaseTool())
                logger.info("Browser tool enabled")
            except ImportError:
                logger.warning("Browser tool not available. Install shadowbot-tools.")
        
        # Web search (additional provider-specific tool)
        if capabilities.web_search:
            try:
                if capabilities.web_search_provider == "duckduckgo":
                    from shadowbot_tools import DuckDuckGoTool
                    tools.append(DuckDuckGoTool())
                elif capabilities.web_search_provider == "tavily":
                    from shadowbot_tools import TavilyTool
                    tools.append(TavilyTool())
                elif capabilities.web_search_provider == "serper":
                    from shadowbot_tools import SerperTool
                ...
                logger.warning("Web search tool not available. Install shadowbot-tools.")
        ...
        if capabilities.tts:
            try:
                from shadowbot.tools.audio import create_tts_tool
        ...
        if capabilities.stt:
            try:
                from shadowbot.tools.audio import create_stt_tool
    ...
    def _resolve_tool_by_name(self, name: str):
        ...
        try:
            ...
            tool_class = getattr(shadowbot_tools, name, None)
            if tool_class:
                return tool_class()
        except ImportError:
            pass
        ...
        try:
            from shadowbotagents import tools as agent_tools
...
Examples:
  ...
  # Bot with agent YAML
  shadowbot bot slack --token $SLACK_BOT_TOKEN --agent agents.yaml

  # Bot with browser and web search
  shadowbot bot discord --token $DISCORD_BOT_TOKEN --browser --web

  # Bot with tools
  shadowbot bot telegram --token $TELEGRAM_BOT_TOKEN --tools DuckDuckGoTool WikipediaTool

  # Full-featured bot
  shadowbot bot slack --token $SLACK_BOT_TOKEN --agent agents.yaml --browser --web --memory --model gpt-4o
""",
        )
    ...
    if platform == "telegram":
        ...
        print("")
        print("Usage: shadowbot bot <platform> [options]")
        print("")
        ...
        print("")
        print("Examples:")
        print("  shadowbot bot telegram --token $TELEGRAM_BOT_TOKEN")
        print("  shadowbot bot slack --token $SLACK_BOT_TOKEN --agent agents.yaml --browser --web")
        print("  shadowbot bot discord --token $DISCORD_BOT_TOKEN --tools DuckDuckGoTool --memory")
        return 1
