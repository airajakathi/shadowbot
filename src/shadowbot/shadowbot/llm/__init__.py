"""
LLM Module for ShadowBot CLI Wrapper.

This module provides LLM provider registry and utilities for the CLI wrapper.
The core LLM functionality is in shadowbotagents.llm.
"""
...
def embedding(text, model="text-embedding-3-small", **kwargs):
    ...
    .. deprecated::
        Use `from shadowbot import embed` or `from shadowbot.capabilities import embed` instead.
    ...
    Note:
        Requires litellm. Install with: pip install shadowbot[llm]
    """
    try:
        from shadowbotagents.utils.deprecation import warn_deprecated_param
        warn_deprecated_param(
            "shadowbot.llm.embedding()",
            since="1.0.0",
            removal="2.0.0",
            alternative="use 'from shadowbot import embed' or 'from shadowbot.capabilities import embed' instead",
        ...
        import warnings
        warnings.warn(
            "shadowbot.llm.embedding() is deprecated. Use 'from shadowbot import embed' instead.",
            DeprecationWarning,
            stacklevel=2
        )
    ...
    try:
        ...
        raise ImportError(
            ...
            "Install with: pip install shadowbot[llm] or pip install litellm"
        )
