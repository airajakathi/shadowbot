...
_DEFAULT_STORE_DIR = os.path.join(
    os.path.expanduser("~"), ".shadowbot", "gateway"
)
...
class PairingStore:
    ...
    Args:
        store_dir: Directory to persist pairing data (default ``~/.shadowbot/gateway``).
        ...
        with self._lock:
            ...
            for code, info in self._pending.items():
                ...
                pending_list.append({
                    ...
                    # UI-friendly aliases (used by shadowbot.ui._pairing banner)
                    "channel": ct,
                    "user_id": code,
