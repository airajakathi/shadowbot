"""
WebSocket Gateway Server for ShadowBot.
...
from shadowbotagents.gateway import (
    GatewayConfig,
    GatewayEvent,
    GatewayMessage,
    EventType,
...
class WebSocketGateway:
    ...
    def __init__(
        ...
        if hasattr(self.config, 'auth_token') and not self.config.auth_token:
            # Prefer a user-configured token (persisted by `shadowbot onboard`
            # to ~/.shadowbot/.env as GATEWAY_AUTH_TOKEN) so the dashboard
            ...
            if env_tok:
                ...
                import secrets
                from shadowbotagents.gateway.protocols import is_loopback
                ...
                if is_loopback(self.config.bind_host):
                    ...
                    # Save to ~/.shadowbot/.env for future use
                    try:
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            raise ImportError(
                ...
                "Install with: pip install shadowbot[api]"
            )
        ...
        def _check_auth(request) -> Optional[JSONResponse]:
            ...
            Accepts either:
              ...
              - ``?token=<token>`` query parameter (so the dashboard URL from
                ``shadowbot onboard`` is clickable in a browser)
              - Cookie-based authentication (shadowbot_session cookie)
            """
        ...
        async def magic_link_handler(request):
            ...
            if not nonce:
                ...
                return JSONResponse(
                    {"error": "Authentication required. Get a fresh link: shadowbot gateway mint-link"},
                    status_code=401
                )
            ...
            if not _magic_store.consume(nonce):
                return JSONResponse(
                    {"error": "This link was already used or has expired. Get a fresh one: shadowbot gateway mint-link"},
                    status_code=401
                )
    ...
    def _make_stream_relay(
        ...
        def _relay(event) -> None:
            try:
                from shadowbotagents.streaming.events import StreamEventType
    ...
    def _start_scheduler_tick(self, interval: float = 15.0) -> None:
        ...
        async def _run():
            try:
                from shadowbotagents.scheduler import (
                    ScheduleRunner,
                    FileScheduleStore,
                )
                from shadowbot.scheduler.executor import ScheduledAgentExecutor
        ...
        def _replacer(match):
            ...
            if env_val is None:
                logger.warning(
                    ...
                    f"Run `shadowbot onboard` or set it in ~/.shadowbot/.env."
                )
                return ""
            return env_val
    ...
    def _create_agents_from_config(
        ...
        *,
        ...
        tool_resolver = None
        try:
            from shadowbot.tool_resolver import ToolResolver
        ...
        for channel_name, ch_cfg in channels_cfg.items():
            ...
            if not config.allowed_users:
                logger.warning(
                    ...
                    "Re-run `shadowbot onboard` to configure.",
                    channel_name,
                )
    ...
    def _create_bot(
        ...
        from shadowbot.bots._defaults import apply_bot_smart_defaults
        ...
        if not current_tools:
            logger.warning(
                ...
                "Check that `shadowbotagents.tools` is installed and that "
    ...
    async def _start_telegram_bot_polling(self, name: str, bot: Any) -> None:
        ...
        from shadowbotagents.bots import BotUser
