"""
Auth enforcement for ShadowBot Gateway.
...
from shadowbotagents.gateway.protocols import AuthMode, is_loopback, resolve_auth_mode
from shadowbotagents.gateway.config import GatewayConfig
...
def assert_external_bind_safe(config: GatewayConfig) -> None:
    ...
    if not auth_token:
        raise GatewayStartupError(
            ...
            f"Fix:  shadowbot onboard         (30 seconds, 3 prompts)\n"
...
def save_auth_token_to_env(token: str, env_file: Optional[str] = None) -> None:
    ...
    Args:
        ...
        env_file: Path to .env file (defaults to ~/.shadowbot/.env)
    """
    import stat
    from pathlib import Path
    
    if env_file is None:
        # Default to ~/.shadowbot/.env
        home = Path.home()
        shadowbot_dir = home / ".shadowbot"
        shadowbot_dir.mkdir(exist_ok=True)
        env_file = shadowbot_dir / ".env"
