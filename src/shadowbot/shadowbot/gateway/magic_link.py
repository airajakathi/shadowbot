"""
Magic-link authentication store for ShadowBot Gateway.
...
class MagicLinkStore:
    ...
    def __init__(
        ...
        Args:
            ...
            storage_path: Path to JSON storage file (default: ~/.shadowbot/magic-links.json)
        ...
        if storage_path:
            ...
            shadowbot_home = Path(os.environ.get("PRAISONAI_HOME", Path.home() / ".shadowbot"))
            self.storage_path = shadowbot_home / "magic-links.json"
    ...
    def _get_or_create_secret(self) -> str:
        ...
        shadowbot_home = Path(os.environ.get("PRAISONAI_HOME", Path.home() / ".shadowbot"))
        secret_file = shadowbot_home / ".magic-secret"
        
        shadowbot_home.mkdir(parents=True, exist_ok=True)
