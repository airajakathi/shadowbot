"""Auto-generation module for ShadowBot agents and workflows.
...
from shadowbot._logging import get_logger
...
# --- ShadowBot Agents lazy loading ---
def _check_shadowbot_available() -> bool:
    """Check if shadowbotagents is available (cached, thread-safe)."""
    result = _load_optional("shadowbot_check", lambda: __import__("shadowbotagents"))
    return result is not None


def _get_shadowbot():
    """Lazy load shadowbotagents classes (thread-safe)."""
    def shadowbot_loader():
        from shadowbotagents import Agent as PraisonAgent, Task as PraisonTask, AgentTeam as Agents
        return (PraisonAgent, PraisonTask, Agents)
    
    return _load_optional("shadowbot_classes", shadowbot_loader)


# --- ShadowBot Tools lazy loading ---
def _check_shadowbot_tools_available() -> bool:
    """Check if shadowbot_tools is available (cached, thread-safe)."""
    result = _load_optional("shadowbot_tools_check", lambda: __import__("shadowbot_tools"))
    return result is not None


def _get_shadowbot_tools():
    """Lazy load shadowbot_tools classes (thread-safe)."""
    def tools_loader():
        from shadowbot_tools import (
    ...
    return _load_optional("shadowbot_tools_dict", tools_loader)
...
# =============================================================================
# Available Tools List (shared between generators) - Legacy for shadowbot_tools
# =============================================================================
AVAILABLE_TOOLS = [
    "CodeDocsSearchTool", "CSVSearchTool", "DirectorySearchTool", "DOCXSearchTool",
    "DirectoryReadTool", "FileReadTool", "TXTSearchTool", "JSONSearchTool",
...
# =============================================================================
# Enhanced Tool Discovery from shadowbotagents.tools
# =============================================================================

# Tool categories with their tools from shadowbotagents.tools
TOOL_CATEGORIES = {
    'web_search': [
...
class AutoGenerator(BaseAutoGenerator):
    ...
    def __init__(self, topic="Movie Story writing about AI", agent_file="test.yaml", 
        ...
        Args:
            ...
            framework: Framework to use (crewai, autogen, shadowbot)
        ...
        if requires_tools_extra and not _check_shadowbot_tools_available():
            logger.warning(f"Tools are not available for {framework}. To use tools, install:\n    {install_hint}")

        self.topic = topic
        self.agent_file = agent_file
        self.framework = framework or "shadowbot"
...
class WorkflowAutoGenerator(BaseAutoGenerator):
    ...
    def __init__(self, topic: str = "Research and write about AI", 
        ...
        Args:
            ...
            config_list: Optional LLM configuration
            framework: Framework to use (shadowbot, crewai, autogen)
