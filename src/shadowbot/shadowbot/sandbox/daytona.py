"""
Daytona Sandbox implementation for ShadowBot.
...
from shadowbotagents.sandbox import (
    SandboxResult,
    SandboxStatus,
    ResourceLimits,
)
...
class DaytonaSandbox:
    ...
    Example:
        from shadowbot.sandbox import DaytonaSandbox
    ...
    Requires: daytona package (install with pip install shadowbot[daytona])
    """
    ...
    def __init__(
        ...
        self.workspace_name = workspace_name or f"shadowbot-{uuid.uuid4().hex[:8]}"
    ...
    async def start(self) -> None:
        ...
        if not self.is_available:
            raise RuntimeError(
                "Daytona backend not available. Install with: pip install shadowbot[daytona]"
            )
