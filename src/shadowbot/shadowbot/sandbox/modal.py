"""
Modal Sandbox implementation for ShadowBot.
...
class ModalSandbox:
    ...
    Example:
        from shadowbot.sandbox import ModalSandbox
    ...
    Requires: modal package (install with pip install shadowbot[modal])
    """
    ...
    def __init__(
        ...
        self.app_name = app_name or f"shadowbot-sandbox-{uuid.uuid4().hex[:8]}"
    ...
    async def start(self) -> None:
        ...
        if not self.is_available:
            raise RuntimeError(
                "Modal backend not available. Install with: pip install shadowbot[modal]"
            )
