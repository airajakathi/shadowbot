...
Maps ShadowBot agent operations to MCP tools.
"""
...
def register_agent_tools() -> None:
    ...
    @register_tool("shadowbot.agent.chat")
    def agent_chat(
        message: str,
        model: str = "gpt-4o-mini",
        instructions: Optional[str] = None,
    ) -> str:
        """Chat with a ShadowBot agent."""
        try:
            ...
            return "Error: shadowbotagents not installed"
        except Exception as e:
            return f"Error: {e}"
    
    @register_tool("shadowbot.agent.run")
    def agent_run(
        task: str,
        model: str = "gpt-4o-mini",
        instructions: Optional[str] = None,
    ) -> str:
        """Run a task with a ShadowBot agent."""
        try:
            from shadowbotagents import Agent
    ...
    @register_tool("shadowbot.workflow.run")
    def workflow_run(
        yaml_content: str,
    ) -> str:
    ...
    @register_tool("shadowbot.workflow.run_file")
    ...
    @register_tool("shadowbot.research.run")
    def research_run(
        topic: str,
        depth: str = "medium",
    ) -> str:
