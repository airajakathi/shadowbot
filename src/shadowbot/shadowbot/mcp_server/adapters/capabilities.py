...
Maps ShadowBot capabilities (audio, images, embeddings, etc.) to MCP tools.
"""
...
def register_capability_tools() -> None:
    ...
    # Audio tools
    @register_tool("shadowbot.audio.transcribe")
    ...
    @register_tool("shadowbot.audio.speech")
    def audio_speech(
        text: str,
        model: str = "tts-1",
        voice: str = "alloy",
    ...
    # Image tools
    @register_tool("shadowbot.images.generate")
    def images_generate(
        prompt: str,
        model: str = "dall-e-3",
        size: str = "1024x1024",
    ...
    # Embedding tools
    @register_tool("shadowbot.embed.create")
    def embed_create(
        text: str,
    ...
    # Moderation tools
    @register_tool("shadowbot.moderate.check")
    ...
    # Chat completion
    @register_tool("shadowbot.chat.completion")
    def chat_completion(
        message: str,
        ...
        from shadowbot.capabilities import chat_completion
        messages = []
        if system_prompt:
    ...
    # Rerank
    @register_tool("shadowbot.rerank")
    def rerank_documents(
        ...
        from shadowbot.capabilities import rerank
        result = rerank(query=query, documents=documents, model=model, top_n=top_n)
        return str(result)
    
    # RAG
    @register_tool("shadowbot.rag.query")
    def rag_query(
        query: str,
    ...
    # Search
    @register_tool("shadowbot.search")
    def search_web(query: str) -> str:
        """Search the web."""
        from shadowbot.capabilities import search
        result = search(query=query)
        return str(result)
    
    # Guardrails
    @register_tool("shadowbot.guardrails.check")
