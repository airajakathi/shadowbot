...
Bridges shadowbotagents.tools registry/lazy TOOL_MAPPINGS into
shadowbot.mcp_server registry WITHOUT duplicating tool definitions
...
def is_bridge_available() -> bool:
    """Check if shadowbotagents.tools is available."""
    try:
        import importlib.util
        spec = importlib.util.find_spec("shadowbotagents.tools")
...
def get_tool_mappings() -> Dict[str, Any]:
    """Get TOOL_MAPPINGS from shadowbotagents.tools."""
    try:
...
def _infer_tool_hints(tool_name: str) -> Dict[str, bool]:
    ...
    Args:
        tool_name: Full tool name (e.g., "shadowbot.memory.show")
...
def _extract_category(tool_name: str) -> Optional[str]:
    ...
    if len(parts) >= 2:
        return parts[-2]  # e.g., "shadowbot.memory.show" -> "memory"
    return None


def register_shadowbot_tools(
    namespace_prefix: str = "shadowbot.agents.",
    skip_on_collision: bool = True,
) -> int:
    """
    Bridge shadowbotagents tools to MCP registry.
    ...
    if not is_bridge_available():
        logger.debug("shadowbotagents.tools not available, skipping bridge")
        return 0
    ...
    for tool_name, mapping_info in tool_mappings.items():
        ...
        try:
            ...
            tool_def = MCPToolDefinition(
                name=full_name,
                description=f"ShadowBot Agents tool: {tool_name}",
                handler=handler,
    ...
    logger.info(f"Registry bridge registered {registered_count} tools from shadowbotagents")
