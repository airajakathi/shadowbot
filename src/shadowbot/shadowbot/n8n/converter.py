...
Converts ShadowBot YAML workflows to n8n JSON format for visual editing.
"""
...
class YAMLToN8nConverter:
    """Convert ShadowBot YAML workflows to n8n JSON format."""
    ...
    def convert(self, yaml_workflow: Dict[str, Any]) -> Dict[str, Any]:
        ...
        n8n_workflow = {
            "name": yaml_workflow.get("name", "ShadowBot Workflow"),
        ...
        if yaml_workflow.get("description"):
            n8n_workflow["staticData"]["shadowbot_description"] = yaml_workflow["description"]
    ...
    def _agent_to_node(self, agent_id: str, config: Dict[str, Any]) -> N8nNode:
        """Convert ShadowBot agent to n8n node."""
        self.node_counter += 1
        
        # Use httpRequest node type that points to ShadowBot API
        node_type = "n8n-nodes-base.httpRequest"
        
        # Build HTTP request parameters for ShadowBot API
    ...
    def _convert_tools(self, tools: List[str]) -> List[Dict[str, Any]]:
        """Convert ShadowBot tools to n8n tool format."""
        converted_tools = []
