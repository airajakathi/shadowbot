...
Converts n8n JSON workflows back to ShadowBot YAML format.
"""
...
class N8nToYAMLConverter:
    """Convert n8n JSON workflows back to ShadowBot YAML format."""
    
    def __init__(self):
        self.agent_counter = 0
        
    def convert(self, n8n_workflow: Dict[str, Any]) -> Dict[str, Any]:
        """Convert n8n JSON to ShadowBot YAML format.
        
        Args:
            n8n_workflow: n8n workflow JSON dict
            
        Returns:
            ShadowBot YAML workflow dict
        """
        ...
        if static_data.get("shadowbot_description"):
            yaml_workflow["description"] = static_data["shadowbot_description"]
    ...
    def _is_agent_node(self, node: Dict[str, Any]) -> bool:
        ...
        # Check for httpRequest nodes that point to ShadowBot API
    ...
    def _node_to_agent(self, node: Dict[str, Any]) -> tuple[str, Dict[str, Any]]:
        """Convert n8n node to ShadowBot agent configuration."""
        node_name = node.get("name", f"agent_{self.agent_counter}")
        ...
        if "tools" in parameters:
            ...
            if isinstance(tool_configs, list):
                for tool_config in tool_configs:
                    tool_name = self._map_n8n_tool_to_shadowbot(tool_config)
                    if tool_name:
                        tools.append(tool_name)
        
        return tools
    
    def _map_n8n_tool_to_shadowbot(self, tool_config: Dict[str, Any]) -> Optional[str]:
        """Map n8n tool configuration to ShadowBot tool name."""
    ...
    def _connections_to_steps(self, nodes: List[Dict[str, Any]], connections: Dict[str, Any]) -> List[Any]:
        """Convert n8n connections to ShadowBot workflow steps."""
        steps = []
    ...
    def _convert_control_flow(
        ...
        """Convert control flow node to ShadowBot step."""
        for node in nodes:
            if node.get("name") == node_name:
                node_type = node.get("type", "")
