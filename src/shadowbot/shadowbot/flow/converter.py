...
Converts ShadowBot YAML workflows to/from Langflow JSON format for seamless
...
class YAMLToLangflowConverter:
    """Converts ShadowBot YAML workflows to Langflow JSON format."""
    ...
    def _convert_workflow_to_nodes(self, workflow, raw_yaml: Dict) -> Tuple[List[Dict], List[Dict]]:
        ...
        # Convert agents_data to dict format if it's a list (common ShadowBot format)
    ...
    def _create_agent_node(self, agent_id: str, agent_config: Dict) -> Dict:
        """Create a ShadowBot Agent node for Langflow."""
        node_id = f"ShadowBotAgent-{agent_id}"
        ...
        return {
            ...
            "data": {
                ...
                "node": {
                    ...
                    "description": f"ShadowBot agent: {agent_config.get('role', agent_id)}",
                }
            }
        }
    
    def _create_orchestrator_node(self, process: str, raw_yaml: Dict, agent_nodes: List[Dict]) -> Dict:
        """Create a ShadowBot Agents (orchestrator) node."""
        node_id = "ShadowBotAgents-orchestrator"
        x, y = self._get_next_position()
...
class LangflowToYAMLConverter:
    """Converts Langflow JSON flows back to ShadowBot YAML format."""
    ...
    def convert(self, langflow_json: Dict[str, Any]) -> str:
        ...
        for node in nodes:
            ...
            if node_type == "ShadowBotAgent":
                agent_nodes.append(node)
            elif node_type == "ShadowBotAgents":
    ...
    def _extract_agent_id_from_node(self, node: Dict) -> str:
        ...
        node_id = node.get("id", "")
        if node_id.startswith("ShadowBotAgent-"):
            return node_id.replace("ShadowBotAgent-", "")
...
def export_yaml_to_langflow(yaml_path: str, langflow_url: str = "http://localhost:7860") -> str:
    ...
    """
    from shadowbot.flow.client import LangflowClient
