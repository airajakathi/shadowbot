"""ShadowBot helper utilities for Langflow integration.

Provides conversion functions for tools, LLMs, and memory configuration
between Langflow and ShadowBot formats.
"""
...
if TYPE_CHECKING:
    ...
    from shadowbotagents.trace.context_events import ContextTraceEmitter
else:
...
def convert_tools(tools: list | None) -> list[Callable] | None:
    """Convert Langflow/LangChain tools to ShadowBot-compatible callables.

    ShadowBot accepts any callable, so we extract the callable portion
    ...
    Returns:
        List of callables compatible with ShadowBot, or None if no tools provided.
    """
    if not tools:
        return None
...
def convert_llm(llm: Any) -> str | None:
    """Convert Langflow LanguageModel to ShadowBot LLM string format.

    ShadowBot uses 'provider/model-name' format (e.g., 'openai/gpt-4o-mini').
...
def build_memory_config(
    *,
    ...
    """Build ShadowBot MemoryConfig from Langflow inputs.
    ...
    Returns:
        bool (False/True) or dict (MemoryConfig) for ShadowBot Agent.
    """
