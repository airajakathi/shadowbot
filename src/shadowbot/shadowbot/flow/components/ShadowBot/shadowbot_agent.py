"""ShadowBot Agent Component for Langflow.

Creates and executes a ShadowBot Agent with full tool, memory, handoffs,
...
class ShadowBotAgentComponent(Component):
    """ShadowBot Agent component for Langflow workflows.
    ...
    description: str = "Create a ShadowBot agent with tools, memory, handoffs, knowledge, and guardrails."
    documentation: str = "https://docs.praison.ai"
    icon: str = "bot"
    name: str = "ShadowBotAgent"
    ...
    def _import_agent(self):
        ...
        try:
            ...
            msg = "ShadowBot Agents is not installed. Install with: pip install shadowbotagents"
    ...
    def build_agent(self) -> Any:
        """Build and return the ShadowBot Agent instance."""
        from shadowbot.flow.helpers import build_memory_config, convert_llm, convert_tools
        ...
        # If knowledge is enabled, ShadowBot SDK relies on memory for RAG ingestion
        if knowledge and not memory_cfg:
            memory_cfg = True

        # Inject session_id natively into ShadowBot memory auto_save to persist conversation
        if memory_cfg:
    ...
    def _setup_observability(self) -> None:
        ...
        if observe == "langfuse":
            try:
                from shadowbot.observability import LangfuseSink
                from shadowbotagents.trace.context_events import (
    ...
    def _get_llm(self) -> str:
        ...
        from shadowbot.flow.helpers import convert_llm

        if self.llm_handle:
            # Convert LangChain model to ShadowBot format
    ...
    def _setup_observability(self) -> None:
        """Auto-configure observability from environment variables."""
        from shadowbot.flow.helpers import setup_langfuse_context_observability
