"""
Tool Override Loader for ShadowBot.
...
def create_tool_registry_with_overrides(
    ...
    6. Package discovery (shadowbot-tools if installed)
    7. Built-in tools
    ...
    # 6. Package discovery - try shadowbot-tools if installed
    try:
        import shadowbot_tools.tools as external_tools
        # Get all exported tools from shadowbot_tools
    ...
    for tool in tool_names:
        if callable(tool):
            ...
            if tool_name in registry:
                ...
                # They should be resolved via shadowbotagents.tools.__getattr__
                if isinstance(tool_obj, tuple):
                    # Try to import from shadowbotagents.tools instead
                    try:
            ...
            if not found:
                # Try to import from shadowbotagents.tools
                try:
                    from shadowbotagents import tools as agent_tools
                    if hasattr(agent_tools, tool_name):
                        resolved.append(getattr(agent_tools, tool_name))
