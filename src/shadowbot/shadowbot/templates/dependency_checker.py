...
class DependencyChecker:
    ...
    Supports checking:
    - Tools (from built-in registry, shadowbot-tools, custom dirs)
    ...
    TOOL_INSTALL_HINTS = {
        "youtube_tool": "pip install shadowbot-tools[video]",
        "whisper_tool": "pip install shadowbot-tools[audio]",
        "vision_tool": "pip install shadowbot-tools[vision]",
    ...
    def check_tool(self, tool_name: str) -> Dict[str, Any]:
        ...
        # Check shadowbot-tools
        source = self._check_shadowbot_tools(tool_name)
        if source:
            status = DependencyStatus(
                name=tool_name,
                available=True,
        ...
        if not install_hint:
            install_hint = "Check shadowbot-tools or define custom tool"
    ...
    def _check_shadowbot_tools(self, tool_name: str) -> Optional[str]:
        """Check if tool exists in shadowbot-tools package."""
        try:
            import shadowbot_tools
            if hasattr(shadowbot_tools, tool_name):
                return "shadowbot-tools"
            # Check for get_tool function
            if hasattr(shadowbot_tools, 'get_tool'):
                try:
                    tool = shadowbot_tools.get_tool(tool_name)
                    if tool:
                        return "shadowbot-tools"
                except Exception:
                    pass
