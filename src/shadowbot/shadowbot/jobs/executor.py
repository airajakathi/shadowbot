"""
Job Executor for ShadowBot Async Jobs API.
...
class JobExecutor:
    ...
    async def _run_agent(self, job: Job) -> Any:
        ...
        This is the core execution logic that runs the ShadowBot agent.
        ...
        # Imports are done lazily in _run_shadowbot_agents and _run_legacy_shadowbot
        
        # Determine agent configuration
        agent_file = job.agent_file or "agents.yaml"
        framework = job.framework or "shadowbot"
        ...
        try:
            # Try shadowbotagents first (preferred)
            if framework == "shadowbot":
                result = await self._run_shadowbot_agents(job, agent_file)
            else:
                # Use legacy ShadowBot for crewai/autogen
                result = await self._run_legacy_shadowbot(job, agent_file, framework)
    ...
    async def _run_recipe(self, job: Job) -> Any:
        ...
        try:
            from shadowbot.recipe.bridge import resolve, execute_resolved_recipe
        except ImportError:
            raise RuntimeError("Recipe execution requires shadowbot.recipe module")
    ...
    async def _run_shadowbot_agents(self, job: Job, agent_file: str) -> Any:
        """Run using shadowbotagents framework."""
        try:
            from shadowbotagents import Agent
        except ImportError:
            raise RuntimeError("shadowbotagents not installed")
    ...
    async def _run_legacy_shadowbot(self, job: Job, agent_file: str, framework: str) -> Any:
        """Run using legacy ShadowBot (crewai/autogen)."""
        try:
            from shadowbot import ShadowBot
        except ImportError:
            raise RuntimeError("shadowbot not installed")
        
        # Update progress
        job.update_progress(percentage=20.0, step="Creating ShadowBot instance")
        await self.store.save(job)
        await self._notify_progress(job)
        
        # Create ShadowBot instance
        shadowbot = ShadowBot(
        ...
        loop = asyncio.get_event_loop()
        result = await loop.run_in_executor(None, shadowbot.run)
