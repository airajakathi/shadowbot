"""ShadowBot Clean Chat UI — minimal chat interface.

Loaded by ``shadowbot ui`` → ``aiui run app.py``.
Clean chat without sidebar navigation.

Requires: pip install "shadowbot[ui]"
...
aiui.set_branding(title="ShadowBot Chat", logo="🤖")
aiui.set_theme(preset="blue", dark_mode=True, radius="lg")
aiui.set_pages(["chat"])

# Add external agent settings
try:
    from shadowbot.ui._external_agents import aiui_settings_entries
...
async def on_welcome():
    """Greet the user."""
    await aiui.say("👋 Hi! I'm your ShadowBot assistant. Ask me anything!")
...
def _get_agent(settings: dict | None = None):
    ...
    if cache_key not in _agents_cache:
        try:
            ...
            from shadowbot.ui._external_agents import external_agent_tools
