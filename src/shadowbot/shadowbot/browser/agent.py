...
Uses ShadowBot agents to decide browser actions based on observations.
"""

import logging
from typing import Optional, Dict, Any, List, Literal

logger = logging.getLogger("shadowbot.browser.agent")
...
class BrowserAgent:
    ...
    This is a thin wrapper that creates a ShadowBot agent with browser-specific
    ...
    def _ensure_agent(self):
        """Lazily initialize the ShadowBot agent."""
        if self._agent is not None:
            return
        
        logger.debug(f"[AGENT][CALL] _ensure_agent:agent.py → Creating ShadowBot Agent with model={self.model}")
        
        try:
            from shadowbotagents import Agent
        except ImportError:
            logger.error(f"[AGENT][ERROR] _ensure_agent:agent.py → shadowbotagents not installed")
            raise ImportError(
                "shadowbotagents is required. Install it with: pip install shadowbotagents"
            )
