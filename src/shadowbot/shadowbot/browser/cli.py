"""Browser CLI commands for ShadowBot.

Commands:
    shadowbot browser start   - Start the browser automation server
    shadowbot browser stop    - Stop the server
    shadowbot browser sessions - List active sessions
"""
...
def start_server(
    ...
    to communicate with ShadowBot agents.
    
    Example:
        shadowbot browser start --port 8765 --model gpt-4o
    """
    try:
        from shadowbot.browser import BrowserServer
...
def list_sessions(
    ...
    Example:
        shadowbot browser sessions --status running
        shadowbot browser sessions --limit 10
    """
    try:
        from shadowbot.browser.sessions import SessionManager
    except ImportError as e:
        console.print(f"[red]Error:[/red] {e}")
        raise typer.Exit(1)
...
def show_history(
    ...
    Example:
        shadowbot browser history abc12345
    """
    try:
        from shadowbot.browser.sessions import SessionManager
    except ImportError as e:
...
def clear_sessions(
    ...
    Example:
        shadowbot browser clear --status completed --yes
        shadowbot browser clear --all --yes
    """
    try:
        from shadowbot.browser.sessions import SessionManager
    except ImportError as e:
...
def _run_alternative_engine(
    ...
    if record_video and not actual_screenshot_dir:
        actual_screenshot_dir = str(Path.home() / ".shadowbot" / "browser_screenshots" / datetime.now().strftime("%Y%m%d_%H%M%S"))
...
def run_agent(
    ...
    Launches Chrome with the ShadowBot extension and executes the goal.
    ...
    Example:
        shadowbot browser run "Search for ShadowBot on Google"
        shadowbot browser run "task" --engine cdp --headless
        shadowbot browser run "task" --engine hybrid --vision
        shadowbot browser run "task" --screenshots ./screenshots
    """
    import logging
    import json
    import asyncio
...
def manage_tabs(
    ...
    Example:
        shadowbot browser tabs              # List all tabs
        shadowbot browser tabs --new https://google.com
        shadowbot browser tabs --close 12345
        shadowbot browser tabs --focus 12345
    """
    import requests
    import json
...
def list_pages(
    ...
    Example:
        shadowbot browser pages
        shadowbot browser pages --json
    """
    try:
...
def get_dom_cmd(
    ...
    Example:
        shadowbot browser pages  # Get page ID
        shadowbot browser dom <page-id>
    """
    import asyncio
    try:
...
def read_content(
    ...
    Example:
        shadowbot browser pages  # Get page ID
        shadowbot browser content <page-id>
    """
    import asyncio
    try:
...
def get_console_logs(
    ...
    Example:
        shadowbot browser pages  # Get page ID
        shadowbot browser console <page-id>
    """
    import asyncio
    try:
...
def execute_javascript(
    ...
    Example:
        shadowbot browser pages  # Get page ID
        shadowbot browser js <page-id> "document.title"
        shadowbot browser js <page-id> "document.querySelectorAll('a').length"
    """
    import asyncio
    try:
...
def navigate_tab(
    ...
    Example:
        shadowbot browser navigate "https://google.com"
        shadowbot browser navigate "https://github.com" --tab ABC123
    """
    import requests
    import json
...
def execute_script(
    ...
    Example:
        shadowbot browser execute "document.title"
        shadowbot browser execute "window.location.href"
        shadowbot browser execute "document.querySelectorAll('a').length"
    """
    import requests
    import json
...
def capture_screenshot(
    ...
    Example:
        shadowbot browser screenshot
        shadowbot browser screenshot -o page.png
        shadowbot browser screenshot --fullpage -o full.png
    """
    import requests
    import json
    import base64
...
def reload_extension(
    ...
    async def do_reload():
        ...
        try:
            ...
            # Find ShadowBot extension service worker
            sw = next((t for t in targets if t.get('type') == 'service_worker' 
                       and 'shadowbot' in t.get('url', '').lower()), None)
            ...
            if not sw:
                console.print("[yellow]⚠️ ShadowBot extension not found[/yellow]")
...
def chrome_start(
    ...
    Example:
        shadowbot browser chrome start              # Default port 9222
        shadowbot browser chrome start -p 9333      # Custom port
        shadowbot browser chrome start --headless   # Headless mode
    """
    import subprocess
    import platform
    import shutil
    ...
    try:
        ...
        for attempt in range(max_attempts):
            ...
            if asyncio.run(check_port()):
                ...
                console.print(f"  shadowbot browser chrome status -p {port}")
                console.print(f"  shadowbot browser extension load -p {port}")
                return
...
def chrome_status(
    ...
    Example:
        shadowbot browser chrome status
        shadowbot browser chrome status -p 9333
    """
    import asyncio
    ...
    async def check():
        ...
        console.print(f"[red]✗ Chrome not responding on port {port}[/red]")
        console.print(f"  Start with: shadowbot browser chrome start -p {port}")
        return False
...
def extension_load(
    ...
    """Load ShadowBot extension into Chrome.
    ...
    Example:
        shadowbot browser extension load              # Default path
        shadowbot browser extension load ~/my-ext     # Custom path
    """
    from pathlib import Path
    import json
    import webbrowser
    ...
    if path is None:
        path = str(Path.home() / "shadowbot-chrome-extension" / "dist")
    ...
    if not ext_path.exists():
        ...
        console.print("")
        console.print("Expected location: ~/shadowbot-chrome-extension/dist")
        console.print("Build the extension first:")
        console.print("  cd ~/shadowbot-chrome-extension && npm run build")
        raise typer.Exit(1)
    ...
    if not chrome_running:
        ...
        console.print(f"Start Chrome first: shadowbot browser chrome start -p {port}")
        raise typer.Exit(1)
...
def extension_validate(
    ...
    Example:
        shadowbot browser extension validate
        shadowbot browser extension validate ~/my-ext/dist
    """
    from pathlib import Path
...
def doctor_server(
    ...
    try:
        ...
        console.print("   Start with: shadowbot browser start")
        raise typer.Exit(1)
    except Exception as e:
...
def doctor_extension(
    ...
    """Check ShadowBot extension status."""
    import requests
    ...
    try:
        ...
        sw = next((t for t in targets if t.get('type') == 'service_worker' 
                   and ('shadowbot' in t.get('url', '').lower() or 
...
def doctor_test_extension(
    ...
    Examples:
        shadowbot browser doctor test-extension
        shadowbot browser doctor test-extension --goal "Search for AI"
        shadowbot browser doctor test-extension --timeout 30
    """
    import asyncio
    from rich.table import Table
    ...
    try:
        ...
        if result.get("success"):
            ...
            console.print("  1. Bridge server is running: shadowbot browser start")
...
def doctor_db():
    ...
    db_path = Path.home() / ".shadowbot" / "browser_sessions.db"
...
def doctor_bridge(
    ...
    Examples:
        shadowbot browser doctor bridge
        shadowbot browser doctor bridge --port 8766
    """
    import asyncio
...
def doctor_api_keys(
    ...
    Examples:
        shadowbot browser doctor api-keys
        shadowbot browser doctor api-keys --validate
        shadowbot browser doctor api-keys --model gemini/gemini-2.0-flash --validate
    """
...
def doctor_agent(
    ...
    Examples:
        shadowbot browser doctor agent
        shadowbot browser doctor agent --model gemini/gemini-2.0-flash
    """
    import asyncio
...
def doctor_flow(
    ...
    Examples:
        shadowbot browser doctor flow
        shadowbot browser doctor flow --json
        shadowbot browser doctor flow --skip-llm
    """
    import asyncio
...
def doctor_all(
    ...
    Examples:
        shadowbot browser doctor
        shadowbot browser doctor --json
    """
...
def launch_browser(
    ...
    temp_profile: bool = typer.Option(False, "--temp-profile", help="Use temporary profile (deleted after run). Default: ~/.shadowbot/browser_profile"),
    chrome_profile: Optional[str] = typer.Option(None, "--chrome-profile", help="Custom Chrome profile dir. Default: ~/.shadowbot/browser_profile"),
):
    """Launch Chrome with extension and optionally run a goal.
    
    This is an all-in-one command that:
    1. Finds the ShadowBot Chrome extension
    ...
    Examples:
        shadowbot browser launch                           # Just launch Chrome with extension
        shadowbot browser launch "Search for AI"           # Launch and run goal (uses extension)
        shadowbot browser launch "Search" --engine cdp     # Force CDP mode
        shadowbot browser launch "Search" --engine extension  # Force extension mode
        shadowbot browser launch --headless --no-server    # Headless mode, no server
    """
    import subprocess
    import platform
    import shutil
    ...
    log_dir = Path.home() / ".shadowbot" / "browser_logs"
    log_dir.mkdir(parents=True, exist_ok=True)
    
    # Setup screenshot directory if enabled
    screenshot_dir = None
    if screenshot or debug:
        screenshot_dir = Path.home() / ".shadowbot" / "browser_screenshots" / datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    if debug:
        ...
        # Enable DEBUG only for ShadowBot browser components
        logging.getLogger("shadowbot.browser").setLevel(logging.DEBUG)
        logging.getLogger("shadowbot.browser.server").setLevel(logging.DEBUG)
        logging.getLogger("shadowbot.browser.agent").setLevel(logging.DEBUG)
        logging.getLogger("shadowbot.browser.sessions").setLevel(logging.DEBUG)
        logging.getLogger("shadowbot.browser.cdp_agent").setLevel(logging.DEBUG)
        logging.getLogger("shadowbot.browser.diagnostics").setLevel(logging.DEBUG)
    ...
    if system == "Darwin":  # macOS
        ...
        candidates = [
            ...
            os.path.join(home, ".shadowbot/chrome-for-testing/chrome-mac-arm64/Google Chrome for Testing.app/Contents/MacOS/Google Chrome for Testing"),
            os.path.join(home, ".shadowbot/chrome-for-testing/chrome-mac-x64/Google Chrome for Testing.app/Contents/MacOS/Google Chrome for Testing"),
    ...
    if extension_path is None:
        ext_candidates = [
            Path.home() / "shadowbot-chrome-extension" / "dist",
            Path.cwd() / "shadowbot-chrome-extension" / "dist",
            Path(__file__).parent.parent.parent.parent.parent / "shadowbot-chrome-extension" / "dist",
        ]
    ...
    console.print(f"[bold blue]🚀 ShadowBot Browser Launch[/bold blue]")
    ...
    if not no_server:
        ...
        if not server_running:
            ...
            server_process = subprocess.Popen(
                [sys.executable, "-m", "shadowbot.browser.server", "--port", str(server_port)],
    ...
    DEFAULT_PROFILE_PATH = os.path.expanduser("~/.shadowbot/browser_profile")
    
    # Determine profile to use: --temp-profile = temp, else persistent (default or custom)
    if temp_profile:
        # Use temp profile (deleted after run)
        profile_dir = tempfile.mkdtemp(prefix="shadowbot_chrome_")
    ...
    async def verify_extension_loaded():
        """Verify ShadowBot extension loaded by checking CDP targets and bridge connection."""
        try:
            import aiohttp
            async with aiohttp.ClientSession() as session:
                # Get CDP targets
                async with session.get(f"http://localhost:{port}/json/list", timeout=aiohttp.ClientTimeout(total=3)) as resp:
                    if resp.status == 200:
                        ...
                        for target in targets:
                            ...
                            # Quick check for ShadowBot in title
                            if "ShadowBot" in target_title:
                        ...
                        # Check each extension's manifest to find ShadowBot
            ...
            async def run_goal():
                ...
                if selected_engine == "extension":
                    ...
                    try:
                        ...
                        async with aiohttp.ClientSession() as session:
                            async with session.get(f"http://localhost:{port}/json") as resp:
                                if resp.status == 200:
                                    targets = await resp.json()
                                    # Find the ShadowBot extension service worker or background
                                    ...
                                    for t in targets:
                                        ...
                                        if 'chrome-extension' in target_url and 'ShadowBot' not in target_url:
                                        ...
                                        if connections >= 1:
                                            ...
                                            if check_count == 1 or (check_count % 3 == 0):
                                                ...
                                                if debug and check_count == 1:
                                                    ...
                                                    console.print("[dim]   [DEBUG] Try: chrome://extensions/ -> ShadowBot -> Click 'Reload'[/dim]")
                        except Exception as e:
                            if debug:
                    ...
                    if not extension_connected:
                        ...
                        console.print("[yellow]Hint: Open Chrome DevTools -> Extensions -> ShadowBot -> Background page to check console[/yellow]")
        ...
        while True:
            try:
                ...
                if goal_input.lower() in ("help", "?"):
                    ...
                    console.print('  "Search for ShadowBot on Google"')
                ...
                try:
                    ...
                    if record_video or screenshot:
                        goal_screenshot_dir = Path.home() / ".shadowbot" / "browser_screenshots" / f"goal_{goal_count}_{datetime.now().strftime('%H%M%S')}"
