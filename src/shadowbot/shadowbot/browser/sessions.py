...
logger = logging.getLogger("shadowbot.browser.sessions")
...
class SessionManager:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        Args:
            db_path: Path to SQLite database. Defaults to ~/.shadowbot/browser_sessions.db
        """
        if db_path is None:
            home = os.path.expanduser("~")
            shadowbot_dir = os.path.join(home, ".shadowbot")
            os.makedirs(shadowbot_dir, exist_ok=True)
            db_path = os.path.join(shadowbot_dir, "browser_sessions.db")
