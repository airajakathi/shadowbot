...
Usage:
    from shadowbot.browser.diagnostics import run_all_diagnostics
...
logger = logging.getLogger("shadowbot.browser.diagnostics")
...
async def check_bridge_server(host: str = "localhost", port: int = 8765) -> DiagnosticResult:
    ...
    try:
        ...
        return DiagnosticResult(
            ...
            details={"health_url": health_url, "hint": "Run: shadowbot browser start"},
        )
    except Exception as e:
        return DiagnosticResult(
            name="bridge_server",
...
async def check_extension_loaded(port: int = 9222) -> DiagnosticResult:
    """Check if ShadowBot extension is loaded in Chrome.
    
    Looks for service worker with 'shadowbot' or known extension ID.
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=5)) as resp:
                ...
                extension_patterns = ["shadowbot", "fkmfdklcegbbpipbcimbokpfcfamhpdc"]
                ...
                if service_worker:
                    ...
                    return DiagnosticResult(
                        name="extension_loaded",
                        status=DiagnosticStatus.PASS,
                        message="ShadowBot extension loaded",
                    ...
                    return DiagnosticResult(
                        name="extension_loaded",
                        status=DiagnosticStatus.FAIL,
                        message="ShadowBot extension not found",
                        details={
                            "port": port,
