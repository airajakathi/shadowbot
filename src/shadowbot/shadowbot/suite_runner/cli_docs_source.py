...
Discovers and extracts shadowbot CLI commands from bash code blocks
...
# Pattern to extract shadowbot commands (handles multi-line with \)
...
class CLIDocsSource:
    ...
    Parses bash code blocks in .md/.mdx files and extracts shadowbot commands.
    ...
    def _extract_commands(self, doc_path: Path, group: str) -> List[RunItem]:
        ...
        for match in BASH_FENCE_PATTERN.finditer(content):
            ...
            # Extract shadowbot commands from block
            for cmd_match in PRAISONAI_CMD_PATTERN.finditer(block_content):
                ...
                if not cmd or not cmd.startswith('shadowbot'):
                    continue
    ...
    def _is_runnable(self, cmd: str) -> tuple:
        ...
        # Extract base command (shadowbot <subcommand>)
        parts = cmd.split()
        if len(parts) < 2:
    ...
    def _create_test_command(self, cmd: str) -> str:
        ...
        parts = cmd.split()
        if len(parts) >= 2:
            # shadowbot <subcommand> --help
            base_parts = [parts[0], parts[1]]
            
            # Handle nested subcommands (e.g., shadowbot batch list)
