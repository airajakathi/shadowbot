"""
Batch Source - Adapter for running local Python files with ShadowBot imports.

Discovers and prepares Python files in the current directory (or specified path)
that contain imports from shadowbotagents or shadowbot packages.
...
class BatchSource:
    """
    Discovers and prepares local Python files with ShadowBot imports.
    ...
    - # shadowbot: skip=true
    - # shadowbot: timeout=120
    - # shadowbot: require_env=KEY1,KEY2
    - # shadowbot: xfail=reason
    """
    ...
    def _has_shadowbot_import(self, content: str) -> bool:
        """
        Check if file content contains ShadowBot imports.
        
        Args:
            content: File content to check.
            
        Returns:
            True if file contains 'from shadowbot...' or 'import shadowbot...'.
        """
    ...
    def discover(self) -> List[RunItem]:
        """
        Discover all Python files with ShadowBot imports.
        
        Returns:
            List of RunItem objects for files that contain ShadowBot imports.
        """
        ...
        for path in files:
            ...
            # Check for ShadowBot imports
            if not self._has_shadowbot_import(content):
                continue
