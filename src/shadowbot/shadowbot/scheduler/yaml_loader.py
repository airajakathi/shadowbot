...
def load_agent_yaml_with_schedule(yaml_path: str) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    # Get framework (default to shadowbot)
    agent_config['framework'] = config.get('framework', 'shadowbot')
...
def create_agent_from_config(agent_config: Dict[str, Any]) -> Any:
    """
    Create a ShadowBot Agent instance from configuration.
    ...
    Raises:
        ImportError: If shadowbotagents is not installed
    """
    try:
        from shadowbotagents import Agent
    except ImportError:
        raise ImportError("shadowbotagents package is required. Install with: pip install shadowbotagents")
