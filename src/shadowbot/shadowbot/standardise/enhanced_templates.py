...
Based on analysis of existing ShadowBot documentation best practices:
...
FEATURE_TEMPLATE = '''---
title: "{name}"
description: "How to use {slug} in ShadowBot"
icon: "code"
---
...
- [API Reference](/docs/sdk/shadowbotagents/{slug}/{slug})
'''
...
```bash
shadowbot {slug} --config config.yaml
```

### Example 3: Verbose Mode

```bash
shadowbot {slug} --verbose
```
...
<Accordion title="Command not found">
  Ensure ShadowBot is installed: `pip install shadowbot`
</Accordion>
...
```python
from shadowbotagents import {class_name}
...
class EnhancedTemplateGenerator:
    ...
    def generate(self, slug: FeatureSlug, artifact_type: ArtifactType) -> str:
        ...
        return template.format(
            ...
            description=f"{name} functionality for ShadowBot agents",
        )
    ...
    def get_expected_path(self, slug: FeatureSlug, artifact_type: ArtifactType,
        ...
        if artifact_type == ArtifactType.EXAMPLE_BASIC:
            ...
            if docs_root:
                return docs_root / "sdk" / "shadowbotagents" / slug_str / f"{slug_str}.mdx"
