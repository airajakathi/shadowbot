...
class StandardiseConfig:
    ...
    def _auto_detect_paths(self):
        ...
        # Docs root detection - default: ~/ShadowBotDocs/docs
        if self.docs_root is None:
            # First check home directory for ShadowBotDocs
            home_docs = Path.home() / "ShadowBotDocs" / "docs"
            if home_docs.exists():
                self.docs_root = home_docs
            else:
                # Fallback to project-relative paths
                for candidate in ["docs", "ShadowBotDocs/docs"]:
                    path = root / candidate
                    if path.exists():
                        self.docs_root = path
                        break
                # Check parent for ShadowBotDocs
                parent_docs = root.parent / "ShadowBotDocs"
                if parent_docs.exists() and self.docs_root is None:
                    self.docs_root = parent_docs / "docs"
        
        # Examples root detection - default: ~/shadowbot-package/examples
        if self.examples_root is None:
            # First check home directory for shadowbot-package/examples
            home_examples = Path.home() / "shadowbot-package" / "examples"
        ...
        if self.sdk_root is None:
            for candidate in [
                "src/shadowbot-agents/shadowbotagents",
                "shadowbotagents",
            ]:
                path = root / candidate
                if path.exists():
