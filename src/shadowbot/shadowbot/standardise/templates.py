...
agents = ShadowBotAgentTeam(agents=[agent1, agent2], tasks=[task])
result = agents.start()
print(result)
...
'''
    ...
    def _generate_concept_doc(self, slug: FeatureSlug) -> str:
        ...
        return f'''---
title: "{name}"
description: "Understanding {slug_str} in ShadowBot"
icon: "lightbulb"
---
...
'''
    ...
    def _generate_feature_doc(self, slug: FeatureSlug) -> str:
        ...
        return f'''---
title: "{name}"
description: "How to use {slug_str} in ShadowBot"
icon: "code"
---
...
- [API Reference](/docs/sdk/shadowbotagents/{slug_str}/{slug_str})
'''
...
```bash
shadowbot "Your prompt" --{slug_str}
```
...
```bash
shadowbot "Hello" --{slug_str}
```

### With Other Flags

```bash
shadowbot "Hello" --{slug_str} --verbose
```
