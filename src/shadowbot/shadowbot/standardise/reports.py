...
class ReportGenerator:
    ...
    def _generate_text(self, report: StandardiseReport) -> str:
        ...
        # Header
        lines.append("")
        lines.append("🔍 ShadowBot Standardisation Check")
        lines.append("══════════════════════════════════")
        lines.append("")
        ...
        lines.append("───────────────")
        lines.append("1. Run `shadowbot standardise report --format markdown` for details")
        if report.duplicates:
            lines.append("2. Run `shadowbot standardise fix --feature <slug>` to consolidate")
        if report.missing_artifacts:
            lines.append("3. Run `shadowbot standardise init --feature <slug>` to create missing")
        lines.append("")
    ...
    def _generate_markdown(self, report: StandardiseReport) -> str:
        ...
        # Header
        lines.append("# ShadowBot Standardisation Report")
        lines.append("")
