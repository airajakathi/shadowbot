...
for ShadowBot Agents. Uses the official Anthropic Python SDK (v0.94+) which handles
...
class ManagedConfig:
    ...
    # ── Session fields ──
    session_title: str = "ShadowBot session"
    resources: List[Dict[str, Any]] = field(default_factory=list)
    vault_ids: List[str] = field(default_factory=list)


class AnthropicManagedAgent:
    """Anthropic Managed Agents backend for ShadowBot.
    ...
    def _ensure_environment(self) -> str:
        ...
        environment = client.beta.environments.create(
            name=c.get("env_name", "shadowbot-env"),
            config=env_config,
        )
    ...
    def _ensure_session(self) -> str:
        ...
        kwargs: Dict[str, Any] = {
            ...
            "title": c.get("session_title", "ShadowBot session"),
        }
        if c.get("resources"):
...
def map_managed_tools(managed_tools: List[str]) -> List[str]:
    """Map managed agent tool names to ShadowBot tool names."""
