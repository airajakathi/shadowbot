...
class LocalManagedConfig:
    ...
    # ── Session fields ──
    session_title: str = "ShadowBot local session"
...
class LocalManagedAgent:
    ...
    Satisfies ``ManagedBackendProtocol`` (Core SDK).  Uses ShadowBot's own
    ...
    def _resolve_tools(self) -> List:
        """Resolve tool names to actual ShadowBot tool functions.
        ...
        for name in resolved_names:
            try:
                from shadowbotagents import tools as tool_module
    ...
    def _get_session_store(self):
        ...
        if self._session_store is None:
            try:
                from shadowbotagents.session.store import get_default_session_store
    ...
    def _persist_state(self) -> None:
        ...
        try:
            ...
            if session is not None:
                ...
                from shadowbotagents.session.store import SessionData
    ...
    def _ensure_agent(self) -> Any:
        """Create or return the inner ShadowBot Agent."""
    ...
    def _ensure_session(self) -> str:
        ...
        self._session_history.append({
            "id": self._session_id,
            "status": "idle",
            "title": self._cfg.get("session_title", "ShadowBot local session"),
    ...
    def _execute_sync(self, prompt: str, stream_live: bool = False) -> str:
        """Synchronous execution using ShadowBot Agent.chat()."""
        # Get context emitter (zero-overhead when no emitter is installed)
        try:
            from shadowbotagents.trace.context_events import get_context_emitter
    ...
    def _resolve_compute(compute: Optional[Any]) -> Optional[Any]:
        ...
        if isinstance(compute, str):
            name = compute.lower()
            if name == "local":
                from shadowbot.integrations.compute.local import LocalCompute
                return LocalCompute()
            elif name == "docker":
                from shadowbot.integrations.compute.docker import DockerCompute
                return DockerCompute()
            elif name == "e2b":
                from shadowbot.integrations.compute.e2b import E2BCompute
                return E2BCompute()
            elif name == "modal":
                from shadowbot.integrations.compute.modal_compute import ModalCompute
                return ModalCompute()
            elif name == "daytona":
                from shadowbot.integrations.compute.daytona import DaytonaCompute
                return DaytonaCompute()
            elif name == "flyio":
                from shadowbot.integrations.compute.flyio import FlyioCompute
                return FlyioCompute()
            else:
    ...
    async def provision_compute(self, **kwargs) -> Any:
        ...
        from shadowbotagents.managed.protocols import ComputeConfig
