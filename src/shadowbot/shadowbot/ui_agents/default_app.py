"""ShadowBot Agents UI — YAML-defined agents dashboard.

Loaded by ``shadowbot ui agents`` → ``aiui run app.py``.
...
aiui.set_branding(title="ShadowBot Agents", logo="🤖")
...
if yaml_agents:
    try:
        from shadowbotui.features.agents import get_agent_registry
...
async def on_welcome():
    """Welcome message."""
    await aiui.say("👋 Welcome to ShadowBot Agents! This interface runs YAML-defined multi-agent workflows.")
