"""
Prompt injection defense for ShadowBot agents.

Implements a 6-check detection pipeline adapted from production
sovereign-agent security research. Works as a ShadowBot hook
...
_TRUSTED_SOURCES = frozenset([
    "trusted_tool", "internal", "system", "shadowbot_core",
])
...
def scan_text(text: str, source: str = "external") -> ScanResult:
    ...
    if triggered:
        logger.warning(
            "[shadowbot.security] Injection detected | source=%s level=%s checks=%s preview=%r",
...
class InjectionDefense:
    """
    Injection defense that integrates with ShadowBot's hook system.
    ...
    Example:
        ...
        >>> # Register with agent:
        >>> from shadowbotagents.hooks import add_hook
    ...
    def create_hook(self) -> Callable:
        """
        Create a BEFORE_TOOL hook function for use with ShadowBot's hook system.
        ...
        def _injection_hook(data: Any):
            ...
            from shadowbotagents.hooks import HookResult
            ...
            for text in strings:
                ...
                if result.blocked:
                    logger.warning(
                        "[shadowbot.security] Blocking tool=%s agent=%s checks=%s",
    ...
    def create_agent_hook(self) -> Callable:
        ...
        def _agent_injection_hook(data: Any):
            ...
            if result.blocked:
                logger.warning(
                    "[shadowbot.security] Blocking agent prompt agent=%s checks=%s",
                    getattr(data, "agent_name", "?"),
                    result.checks_triggered,
                )
                return HookResult(
