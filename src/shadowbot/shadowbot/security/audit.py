"""
Audit log hook for ShadowBot agents.

Records every tool call to an append-only JSONL file.
Integrates with ShadowBot's AFTER_TOOL hook event.
...
_DEFAULT_LOG_PATH = os.path.expanduser("~/.shadowbot/audit.jsonl")
...
class AuditLogHook:
    ...
    Example:
        >>> audit = AuditLogHook(log_path="~/.shadowbot/audit.jsonl")
    ...
    def _write(self, entry: dict) -> None:
        ...
        try:
            ...
            logger.error("[shadowbot.security.audit] Failed to write audit log: %s", e)
