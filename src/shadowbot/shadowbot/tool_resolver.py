...
2. shadowbotagents.tools.TOOL_MAPPINGS (built-in SDK tools)
3. shadowbot-tools package (external tools, optional)
4. Tool registry (plugins via entry_points)

Usage:
    from shadowbot.tool_resolver import ToolResolver, resolve_tool
...
class ToolResolver:
    ...
    to built-in tools from shadowbotagents.tools.
    ...
    def __init__(self, tools_py_path: Optional[str] = None):
        ...
        self._shadowbot_tools_available: Optional[bool] = None
    ...
    def _resolve_from_shadowbotagents(self, name: str) -> Optional[Callable]:
        """Resolve tool from shadowbotagents.tools.TOOL_MAPPINGS.
        
        Uses lazy loading via __getattr__ in shadowbotagents.tools.
        ...
        try:
            ...
            if hasattr(agent_tools, 'TOOL_MAPPINGS'):
                if name in agent_tools.TOOL_MAPPINGS:
                    ...
                    try:
                        ...
                        return None
                    if tool is not None:
                        logger.debug(f"Resolved '{name}' from shadowbotagents.tools")
                        return tool
            ...
            if tool is not None and callable(tool):
                logger.debug(f"Resolved '{name}' from shadowbotagents.tools (direct)")
                return tool
                
        except ImportError:
            logger.debug("shadowbotagents not available")
        except AttributeError:
            pass
        except Exception as e:
            logger.debug(f"Error resolving '{name}' from shadowbotagents: {e}")
        
        return None
    
    def _resolve_from_shadowbot_tools(self, name: str) -> Optional[Callable]:
        """Resolve tool from shadowbot-tools package (external).
        ...
        if self._shadowbot_tools_available is None:
            self._shadowbot_tools_available = importlib.util.find_spec("shadowbot_tools") is not None
        
        if not self._shadowbot_tools_available:
            return None
        ...
        try:
            ...
            tool = getattr(shadowbot_tools, name, None)
            if tool is not None:
                logger.debug(f"Resolved '{name}' from shadowbot-tools")
                return tool
                
        except ImportError:
            self._shadowbot_tools_available = False
        except AttributeError:
            pass
        except Exception as e:
            logger.debug(f"Error resolving '{name}' from shadowbot-tools: {e}")
    ...
    def _resolve_from_registry(self, name: str) -> Optional[Callable]:
        ...
        """
        try:
            from shadowbotagents.tools.registry import get_tool
            tool = get_tool(name)
            if tool is not None:
    ...
    def resolve(self, name: str) -> Optional[Callable]:
        ...
        2. shadowbotagents.tools.TOOL_MAPPINGS (built-in)
        3. shadowbot-tools package (external, optional)
        ...
        # 2. Check shadowbotagents.tools
        tool = self._resolve_from_shadowbotagents(name)
        if tool is not None:
            return tool
        
        # 3. Check shadowbot-tools package
        tool = self._resolve_from_shadowbot_tools(name)
        if tool is not None:
            return tool
    ...
    def list_available(self) -> Dict[str, str]:
        ...
        # 2. Add shadowbotagents.tools
        try:
            from shadowbotagents.tools import TOOL_MAPPINGS
            for name in TOOL_MAPPINGS.keys():
                if name not in available:
                    available[name] = "Built-in tool from shadowbotagents"
        except ImportError:
            pass
        
        # 3. Add shadowbot-tools (if installed)
        if self._shadowbot_tools_available is None:
            self._shadowbot_tools_available = importlib.util.find_spec("shadowbot_tools") is not None
        
        if self._shadowbot_tools_available:
            try:
                from shadowbot_tools import __all__ as shadowbot_tools_all
                for name in shadowbot_tools_all:
                    if name not in available:
                        available[name] = "External tool from shadowbot-tools"
