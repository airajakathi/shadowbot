"""
ShadowBotDB adapter - implements the DbAdapter protocol from shadowbotagents.
...
class ShadowBotDB:
    """
    Universal database adapter for ShadowBot agents.
    
    Automatically selects backends based on URL or explicit configuration.
    Implements the DbAdapter protocol expected by shadowbotagents.Agent.
    
    Example:
        from shadowbotagents import Agent
        from shadowbot.db import ShadowBotDB
        
        # Simple usage with PostgreSQL
        db = ShadowBotDB(database_url="postgresql://localhost/mydb")
    ...
    def on_agent_start(
        ...
        from shadowbotagents.db.protocol import DbMessage
        return [
            DbMessage(
                role=msg.role,
                content=msg.content,
...
class SQLiteDB(ShadowBotDB):
    """SQLite-specific database adapter."""
    
    def __init__(self, path: str = "shadowbot.db", database_url: str = None, **options):
