...
warnings.warn(
    "shadowbot.async_agent_scheduler is pending deprecation; it will be moved to "
    "shadowbot.scheduler.async_agent_scheduler in a future release. Continue "
    "importing from shadowbot.async_agent_scheduler until then.",
...
class AsyncPraisonAgentExecutor(AsyncAgentExecutorInterface):
    """Async executor for ShadowBot agents."""
    
    def __init__(self, agent):
        """
        Initialize executor with a ShadowBot agent.
...
class AsyncAgentScheduler:
    """
    Async-native scheduler for running ShadowBot agents periodically.
