"""
Windows service management for ShadowBot bots.
...
def _generate_startup_script(config_path: str) -> str:
    ...
    return f"""@echo off
REM ShadowBot Bot Startup Script
cd /d "{os.path.dirname(abs_config)}"
"{python}" -m shadowbot gateway start --config "{abs_config}"
"""
...
def get_status() -> Dict[str, Any]:
    ...
    try:
        # Look for shadowbot bot processes
        cmd = ["tasklist", "/FI", "IMAGENAME eq python.exe", "/FO", "CSV"]
        result = subprocess.run(cmd, capture_output=True, text=True, check=True)
        if "shadowbot bot" in result.stdout or "shadowbot.exe" in result.stdout:
            status["running"] = True
    except Exception:
