...
from .inc import ShadowBotModel
import inspect
...
try:
    ...
    try:
        from shadowbot.tools import BaseTool
    except ImportError:
        pass
...
class AgentsGenerator:
    ...
    def generate_crew_and_kickoff(self):
        ...
        if 'agents' in config and 'roles' not in config:
            ...
            for agent_name, agent_config in config['agents'].items():
                ...
                # Note: preserve 'instructions' key so _run_shadowbot can pass it to PraisonAgent
        ...
        if PRAISONAI_TOOLS_AVAILABLE and (CREWAI_AVAILABLE or AUTOGEN_AVAILABLE or PRAISONAI_AVAILABLE or AG2_AVAILABLE):
            try:
                ...
                # Standard shadowbot-tools tool names
                standard_tools = [
                ...
                self.logger.debug(f"Error resolving shadowbot_tools: {e}")
                tools_dict = {}
    ...
    def _run_yaml_workflow(self, config):
        ...
        if not PRAISONAI_AVAILABLE:
            raise ImportError("ShadowBot is not installed. Please install it with 'pip install shadowbotagents'")
        
        try:
            from shadowbotagents.workflows import YAMLWorkflowParser
        except ImportError:
            raise ImportError("YAMLWorkflowParser not available. Please update shadowbotagents.")
    ...
    def _run_crewai(self, config, topic, tools_dict):
        ...
        for role, details in config['roles'].items():
            ...
            if function_calling_llm_model:
                function_calling_llm = ShadowBotModel(
    ...
    def _run_shadowbot(self, config, topic, tools_dict):
        """
        Run agents using the ShadowBot framework.
        
        Tool resolution order:
        1. Local tools.py (backward compat, custom tools)
        2. YAML tools: field resolved via ToolResolver
        3. Built-in tools from shadowbotagents.tools
        """
        agents = {}
        tasks = []
        tasks_dict = {}
        ...
        if acp_enabled or lsp_enabled:
            try:
                import asyncio
                from shadowbot.cli.features.interactive_runtime import InteractiveRuntime, RuntimeConfig
                from shadowbot.cli.features.agent_tools import create_agent_centric_tools
        ...
        for role, details in config['roles'].items():
            ...
            if 'approval' in details:
                ...
                try:
                    ...
                    try:
                        from shadowbotagents.approval.protocols import ApprovalConfig
        ...
        # Create and run the ShadowBot agents
