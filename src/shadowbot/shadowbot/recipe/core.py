...
def reload_registry():
    ...
    import importlib
    try:
        from shadowbot import recipe as recipe_module
...
def list_recipes(
    ...
    """
    try:
        from shadowbot.templates import TemplateDiscovery
...
def _load_recipe(name: str, offline: bool = False) -> Optional[RecipeConfig]:
    ...
    try:
        from shadowbot.templates import TemplateDiscovery, TemplateLoader
...
def _execute_recipe(
    ...
    try:
        ...
        from shadowbot.templates.loader import TemplateConfig as LoaderTemplateConfig
        ...
        if "agents" in workflow_config and "tasks" in workflow_config:
            return _execute_shadowbot_workflow(
...
def _execute_shadowbot_workflow(
    workflow_config: Dict[str, Any],
    config: Dict[str, Any],
    tool_registry: Any,
    options: Dict[str, Any],
) -> Any:
    """Execute a ShadowBot agents/tasks workflow."""
    from shadowbotagents import Agent, Task, AgentTeam
    from shadowbot.templates.tool_override import resolve_tools
...
def _execute_steps_workflow(
    ...
    """
    Execute a steps-based workflow using shadowbotagents Workflow.
