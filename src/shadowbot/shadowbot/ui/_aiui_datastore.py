"""Bridge shadowbotagents SessionStore → aiui BaseDataStore.
...
def _build_impl_cls():
    ...
    try:
        from shadowbotui.datastore import BaseDataStore
    except ImportError as e:
        raise ImportError(
            "shadowbotui is required for ShadowBotSessionDataStore. "
            "Install with: pip install 'shadowbot[ui]'"
        ) from e
    
    try:
        from shadowbotagents.session import get_hierarchical_session_store
    except ImportError as e:
        raise ImportError(
            "shadowbotagents is required for ShadowBotSessionDataStore. "
            "Install with: pip install shadowbotagents"
        ) from e

    class _ShadowBotSessionDataStoreImpl(BaseDataStore):
    ...
    _impl_cls = _ShadowBotSessionDataStoreImpl
    return _impl_cls


class ShadowBotSessionDataStore:
    """Adapter that bridges ShadowBot SessionStoreProtocol to aiui BaseDataStore.
