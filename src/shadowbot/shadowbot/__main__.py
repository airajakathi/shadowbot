# __main__.py
"""
ShadowBot CLI — Unified Entry Point.
...
def _get_typer_commands():
    ...
    with _typer_commands_lock:
        ...
        try:
            from shadowbot.cli.app import app, register_commands
            register_commands()

            import typer.main
            import click
            click_app = typer.main.get_command(app)
            ctx = click.Context(click_app, info_name="shadowbot")
            commands = set(click_app.list_commands(ctx))
        except Exception:
            # Do NOT poison the cache on failure — let the next caller retry.
            import logging
            logging.getLogger("shadowbot.__main__").warning(
...
def main():
    ...
    if "--version" in argv or "-V" in argv:
        ...
        print(f"ShadowBot version {__version__}")
        return
