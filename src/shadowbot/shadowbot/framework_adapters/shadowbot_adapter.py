"""
ShadowBot agents framework adapter.

Provides lazy-loaded integration with the ShadowBot agents framework.
"""
...
class ShadowBotAdapter(BaseFrameworkAdapter):
    """Adapter for ShadowBot agents framework."""
    
    name = "shadowbot"
    install_hint = 'pip install shadowbotagents'
    requires_tools_extra = False
    
    def is_available(self) -> bool:
        """Check if ShadowBot agents is available for import."""
        try:
            from shadowbotagents import Agent, Task, AgentTeam  # noqa: F401
            return True
        except ImportError:
            return False
    ...
    def run(
        ...
        *,
        ...
        """
        Run ShadowBot agents with given configuration.
        
        Args:
            config: ShadowBot configuration with agents and tasks
        ...
        # Import ShadowBot components only when needed
        from shadowbotagents import Agent as PraisonAgent, Task as PraisonTask, AgentTeam
        
        logger.info("Starting ShadowBot execution...")
        ...
        response = team.start()
        result = f"### ShadowBot Output ###\n{response}" if response else "### ShadowBot Output ###\nTask completed."
        
        logger.info("ShadowBot execution completed")
        return result
