#!/usr/bin/env python3
"""
ShadowBot CLI - Performance Benchmark
...
def run_benchmark():
    ...
    print('=' * 60)
    print('ShadowBot CLI - Performance Benchmark')
    print('=' * 60)
    ...
    # Clear cache
    modules_to_remove = [k for k in sys.modules.keys() if 'shadowbot' in k]
    for mod in modules_to_remove:
        del sys.modules[mod]
    
    start = time.perf_counter()
    import shadowbot
    results['shadowbot_import_ms'] = (time.perf_counter() - start) * 1000
    print(f"   import shadowbot: {results['shadowbot_import_ms']:.2f}ms")
    ...
    from shadowbot.cli.features.message_queue import MessageQueue, StateManager
    from shadowbot.cli.features.at_mentions import FileSearchService
    ...
    # 5. ShadowBot class (lazy load)
    print("\n5. Testing ShadowBot class access (lazy load)...")
    start = time.perf_counter()
    ShadowBot = shadowbot.ShadowBot
    results['ShadowBot_lazy_ms'] = (time.perf_counter() - start) * 1000
    print(f"   ShadowBot lazy load: {results['ShadowBot_lazy_ms']:.2f}ms")
...
def print_summary(results: dict):
    ...
    import_checks = [
        ('shadowbot', results['shadowbot_import_ms'], 10),
...
def save_results(results: dict):
    ...
    with open(filepath, 'w') as f:
        f.write('# ShadowBot CLI - Benchmark Results\n\n')
        ...
        f.write(f'|-----------|----------|\n')
        f.write(f'| ShadowBot lazy load | {results["ShadowBot_lazy_ms"]:.2f} |\n')
        
        f.write('\n## How to Reproduce\n\n')
        f.write('```bash\n')
        f.write('cd shadowbot\n')
...
def main():
    parser = argparse.ArgumentParser(description='ShadowBot CLI Performance Benchmark')
