#!/usr/bin/env python3
"""
Comprehensive test runner for ShadowBot Agents
...
def run_test_suite():
    ...
    print("🧪 Starting ShadowBot Agents Test Suite")
    print("=" * 50)
...
def run_tests(pattern=None, verbose=False, coverage=False):
    ...
    if coverage:
        cmd.extend(["--cov=shadowbotagents", "--cov-report=term-missing"])
    ...
    if is_full_test:
        # Add -s flag to see real-time output from shadowbot.run()
        cmd.append("-s")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test runner for ShadowBot")
    parser.add_argument(
        "--pattern", 
        choices=[
    ...
    if "full" in args.pattern:
        ...
        print("💰💰 These tests run shadowbot.run() with ACTUAL API calls!")
