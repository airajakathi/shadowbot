...
This directory contains **real end-to-end tests** that make actual API calls to test ShadowBot framework integrations. These are fundamentally different from the mock tests in `tests/integration/`.
...
**Run with full execution (actual shadowbot.run()):**
```bash
...
**Full execution tests (with shadowbot.run()):**
```bash
...
- ✅ **Framework Integration**: ShadowBot + AutoGen/CrewAI
