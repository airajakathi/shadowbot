...
This directory contains integration tests for ShadowBot that verify functionality across different frameworks and external dependencies.
...
RUN_REAL_AGENTIC=1 pytest src/shadowbot/tests/integration/test_managed_real.py -v

# Run only Anthropic tests
RUN_REAL_AGENTIC=1 PRAISONAI_TEST_PROVIDERS=anthropic pytest src/shadowbot/tests/integration/test_managed_real.py::test_anthropic_managed_real -v

# Run only local/OpenAI tests
RUN_REAL_AGENTIC=1 PRAISONAI_TEST_PROVIDERS=openai pytest src/shadowbot/tests/integration/test_managed_real.py::test_local_managed_real_openai -v
```
...
- ✅ Basic agent creation through ShadowBot
...
def test_basic_autogen_agent_creation(self, mock_completion, mock_autogen_completion):
    """Test creating basic AutoGen agents through ShadowBot"""
    yaml_content = """
framework: autogen
...
- ✅ Basic crew creation through ShadowBot
...
### Framework Integration Tests
- **AutoGen**: Tests ShadowBot integration with Microsoft AutoGen framework
- **CrewAI**: Tests ShadowBot integration with CrewAI framework
...
**Path Issues:**
```
ModuleNotFoundError: No module named 'shadowbot'
```
