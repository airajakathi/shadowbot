...
def run_doctor(*args, timeout=60):
    ...
    env = os.environ.copy()
    # Propagate sys.path to PYTHONPATH so the subprocess can find shadowbot package
    env["PYTHONPATH"] = os.pathsep.join(sys.path)
    return subprocess.run(
        [sys.executable, "-m", "shadowbot", "doctor", *args],
        capture_output=True,
        text=True,
        timeout=timeout,
        env=env
...
class TestDoctorCLI:
    """Integration tests for shadowbot doctor CLI."""
