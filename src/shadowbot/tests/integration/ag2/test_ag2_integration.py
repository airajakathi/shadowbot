"""
AG2 Mock Integration Tests — verifies ShadowBot correctly orchestrates AG2 agents.
...
# so that tests can import shadowbot without a full installation.
...
class TestAG2SingleAgentFlow:
    ...
    @pytest.mark.integration
    def test_single_agent_yaml_initialises_shadowbot(self, single_agent_yaml, mock_ag2_classes):
        """ShadowBot initialises correctly with framework='ag2' from YAML."""
        import tempfile
        ...
        try:
            with patch("shadowbot.agents_generator.AG2_AVAILABLE", True), \
                 patch("shadowbot.agents_generator.PRAISONAI_AVAILABLE", True):
                try:
                    from shadowbot import ShadowBot
                    ai = ShadowBot(agent_file=yaml_path, framework="ag2")
                    assert ai.framework == "ag2"
                except ImportError as e:
                    pytest.skip(f"ShadowBot not available: {e}")
        finally:
            os.unlink(yaml_path)
    ...
    def test_single_agent_run_returns_ag2_output(self, mock_ag2_classes):
        ...
        m = mock_ag2_classes
        with patch("shadowbot.agents_generator.AG2_AVAILABLE", True), \
             patch("shadowbot.agents_generator.CREWAI_AVAILABLE", False), \
             patch("shadowbot.agents_generator.AUTOGEN_AVAILABLE", False), \
             patch("shadowbot.agents_generator.PRAISONAI_AVAILABLE", True):
            try:
...
class TestAG2BackwardCompatibility:
    ...
    def test_autogen_framework_still_works(self):
        ...
        with patch("shadowbot.agents_generator.AUTOGEN_AVAILABLE", True), \
             patch("shadowbot.agents_generator.AG2_AVAILABLE", False), \
             patch("shadowbot.agents_generator.CREWAI_AVAILABLE", False), \
             patch("shadowbot.agents_generator.PRAISONAI_AVAILABLE", True):
            try:
                from shadowbot.agents_generator import AgentsGenerator
