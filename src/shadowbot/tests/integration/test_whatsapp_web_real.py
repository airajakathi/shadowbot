...
PRAISONAI_SRC = os.path.join(
    os.path.dirname(__file__), "..", "..", "..", "shadowbot"
)
PYTHON = sys.executable


def run_cli(*args, timeout=15, env_extra=None):
    """Run a shadowbot CLI command as subprocess, return (returncode, stdout, stderr)."""
    env = os.environ.copy()
    ...
    cmd = [PYTHON, "-m", "shadowbot", "bot", "whatsapp"] + list(args)
    proc = subprocess.run(
        cmd,
        capture_output=True,
        text=True,
...
def run_cli_background(*args, env_extra=None):
    """Start a shadowbot CLI command as background process, return Popen."""
    env = os.environ.copy()
    env["PYTHONPATH"] = PRAISONAI_SRC + os.pathsep + env.get("PYTHONPATH", "")
    env.setdefault("OPENAI_API_KEY", "sk-test-integration-dummy")
    if env_extra:
...
class TestCLIWebModeLaunch:
    """Verify `shadowbot bot whatsapp --mode web` actually starts."""
...
class TestRealAdapter:
    ...
    def test_adapter_import(self):
        ...
        from shadowbot.bots._whatsapp_web_adapter import WhatsAppWebAdapter
...
class TestPerformance:
    ...
    def test_shadowbot_bots_import_fast(self):
        """Importing shadowbot.bots.whatsapp doesn't load neonize."""
