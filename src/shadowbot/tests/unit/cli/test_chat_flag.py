...
class TestChatFlagIntegration:
    """Integration tests with actual ShadowBot CLI parser."""
    
    def test_shadowbot_parser_has_chat_flag(self):
        """ShadowBot CLI should have --chat as alias for --chat-mode.
        
        Note: --chat-mode and --chat flags were deprecated in favor of
        'shadowbot chat' subcommand. This test verifies the parser doesn't
        ...
        try:
            sys.argv = ["shadowbot", "test prompt"]
    ...
    def test_shadowbot_chat_subcommand_still_works(self):
        """'shadowbot chat' subcommand should still launch Chainlit.
        ...
        """
        # Test the parser structure directly without going through ShadowBot
        # which has test environment detection
        parser = argparse.ArgumentParser()
        parser.add_argument("--chat-mode", "--chat", action="store_true", dest="chat_mode")
        parser.add_argument("command", nargs="?")
        
        # Simulate 'shadowbot chat' - should set command='chat', not chat_mode
        args, _ = parser.parse_known_args(["chat"])
        assert args.command == "chat"
        assert args.chat_mode is False  # subcommand, not flag
