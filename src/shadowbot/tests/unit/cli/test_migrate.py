...
class TestPatternDetection:
    ...
    def test_detect_agent_class_pattern_with_role_goal_backstory(self):
        ...
        from shadowbot.cli.features.migrate import PatternDetector
...
class TestCodeConversion:
    """Test code conversion to ShadowBot format."""
    
    def test_convert_agent_pattern_to_shadowbot(self):
        """Convert detected agent pattern to ShadowBot Agent."""
        from shadowbot.cli.features.migrate import CodeConverter
...
'''
        ...
        assert "from shadowbotagents import Agent" in result.converted_code
...
class TestYAMLMigration:
    ...
    def test_yaml_migration_still_works(self):
        ...
        from shadowbot.cli.features.migrate import ConfigMigrator
...
class TestCLIIntegration:
    ...
    def test_migrate_help_is_generic(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "shadowbot", "migrate", "--help"],
            capture_output=True,
            text=True,
            timeout=30
        )
        ...
        # Skip if run failed due to missing shadowbotagents (integration env issue, not test issue)
        if "shadowbotagents" in help_text.lower() and "not installed" in help_text.lower():
            import pytest
            pytest.skip("shadowbotagents not installed - cannot test migrate CLI")
    ...
    def test_migrate_dry_run_flag(self):
        ...
        from shadowbot.cli.features.migrate import handle_migrate_command
        import argparse
