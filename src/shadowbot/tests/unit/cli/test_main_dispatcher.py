"""Unit tests for the unified CLI dispatcher in `shadowbot.__main__`.
...
import shadowbot.__main__ as dispatcher
...
class TestGetTyperCommandsCache(unittest.TestCase):
    ...
    def test_failure_does_not_poison_cache(self):
        ...
        with mock.patch(
            "shadowbot.cli.app.register_commands",
...
class TestVersionShortCircuit(unittest.TestCase):
    ...
    def test_long_flag_prints_version(self):
        sys.argv = ["shadowbot", "--version"]
        with mock.patch("builtins.print") as mock_print:
            dispatcher.main()
        printed = " ".join(str(c.args[0]) for c in mock_print.call_args_list)
        self.assertIn("ShadowBot version", printed)

    def test_short_flag_prints_version(self):
        sys.argv = ["shadowbot", "-V"]
        with mock.patch("builtins.print") as mock_print:
            dispatcher.main()
        printed = " ".join(str(c.args[0]) for c in mock_print.call_args_list)
        self.assertIn("ShadowBot version", printed)
    ...
    def test_version_does_not_import_typer_or_legacy(self):
        ...
        We deliberately avoid ``mock.patch("shadowbot.cli.app.register_commands")``
        ...
        ``shadowbot.cli.*`` modules from ``sys.modules`` before invoking
        ``main()`` and assert they remain absent afterwards.
        """
        sys.argv = ["shadowbot", "--version"]
        cli_mods = [m for m in list(sys.modules) if m.startswith("shadowbot.cli")]
        saved = {m: sys.modules.pop(m) for m in cli_mods}
        try:
            with mock.patch("builtins.print"):
                dispatcher.main()
            still_loaded = [m for m in sys.modules if m.startswith("shadowbot.cli")]
            self.assertEqual(
                still_loaded, [],
                f"--version must not import shadowbot.cli.*, but loaded: {still_loaded}",
            )
        finally:
...
class TestMainRouting(unittest.TestCase):
    ...
    def test_known_typer_command_routes_to_typer(self):
        sys.argv = ["shadowbot", "fake-cmd", "--opt"]
        with mock.patch.object(
    ...
    def test_freetext_prompt_routes_to_legacy(self):
        sys.argv = ["shadowbot", "Create a weather app"]
        with mock.patch.object(
    ...
    def test_yaml_path_routes_to_legacy(self):
        ...
        sys.argv = ["shadowbot", "agents.yaml"]
        with mock.patch.object(
            dispatcher, "_get_typer_commands", return_value={"chat", "ui"}
        ), mock.patch.object(dispatcher, "_run_typer") as run_typer, \
             mock.patch.object(dispatcher, "_run_legacy") as run_legacy:
    ...
    def test_unknown_command_routes_to_legacy(self):
        sys.argv = ["shadowbot", "totally-unknown"]
        with mock.patch.object(
            dispatcher, "_get_typer_commands", return_value={"chat"}
        ), mock.patch.object(dispatcher, "_run_typer") as run_typer, \
             mock.patch.object(dispatcher, "_run_legacy") as run_legacy:
...
class TestRunLegacyArgvRestoration(unittest.TestCase):
    ...
    def test_argv_restored_after_normal_exit(self):
        # NOTE: argv[0] differs from the dispatcher's rewrite ("shadowbot")
        # so the assertion has discriminating power: if the ``finally``
        # clause were missing, ``sys.argv[0]`` would still be "shadowbot"
        ...
        with mock.patch("shadowbot.cli.main.ShadowBot", return_value=fake), \
             self.assertRaises(SystemExit) as cm:
            dispatcher._run_legacy(["agents.yaml"])

        self.assertEqual(cm.exception.code, 0)
        self.assertEqual(sys.argv, original)
        self.assertNotEqual(sys.argv[0], "shadowbot")  # invariant pin
    ...
    def test_argv_restored_after_systemexit(self):
        ...
        self.assertEqual(sys.argv, original)
        self.assertNotEqual(sys.argv[0], "shadowbot")
...
class TestRunTyperArgvRestoration(unittest.TestCase):
    ...
    def test_argv_restored_after_systemexit(self):
        ...
        sys.argv = list(original)
        with mock.patch("shadowbot.cli.app.register_commands"), \
             mock.patch("shadowbot.cli.app.app", side_effect=SystemExit(0)), \
