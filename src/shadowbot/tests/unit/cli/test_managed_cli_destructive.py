...
from shadowbot.cli.commands.managed import (
    sessions_app,
    agents_app, 
    envs_app,
    _get_client,
...
def mock_anthropic_client():
    ...
    with patch('shadowbot.cli.commands.managed._get_client') as mock_get_client:
        mock_client = Mock()
...
class TestGetClient:
    ...
    def test_get_client_with_anthropic_api_key(self):
        ...
        with patch.dict(sys.modules, {'anthropic': mock_anthropic}):
            from shadowbot.cli.commands.managed import _get_client
            client = _get_client()
