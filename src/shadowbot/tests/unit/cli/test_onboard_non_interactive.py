"""Unit tests for non-interactive mode in ``shadowbot onboard``.
...
def _import_onboard():
    ...
    from shadowbot.cli.features import onboard
    return onboard
...
def test_wizard_run_does_not_block_in_non_interactive(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "shadowbot.cli.features.onboard.OnboardWizard._install_daemon_with_feedback",
    ...
    async def _fake_probe(self, platform):
        ...
        return _R()
    monkeypatch.setattr(
        "shadowbot.cli.features.onboard.OnboardWizard._probe", _fake_probe
    )
