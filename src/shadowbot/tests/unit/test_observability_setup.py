...
from shadowbot.cli.app import _setup_langfuse_observability
from shadowbot.flow import helpers as flow_helpers
...
def test_flow_langfuse_context_observability_reuses_single_emitter(monkeypatch):
    ...
    fake_langfuse_module = types.ModuleType("shadowbot.observability.langfuse")
    fake_langfuse_module.LangfuseSink = FakeSink

    fake_context_events_module = types.ModuleType("shadowbotagents.trace.context_events")
    ...
    monkeypatch.setitem(sys.modules, "shadowbot.observability.langfuse", fake_langfuse_module)
    monkeypatch.setitem(sys.modules, "shadowbotagents.trace.context_events", fake_context_events_module)
...
def test_setup_langfuse_observability_verbose_logs_warning(monkeypatch, capsys):
    ...
    fake_protocol_module = types.ModuleType("shadowbotagents.trace.protocol")
    ...
    monkeypatch.setitem(sys.modules, "shadowbot.observability.langfuse", fake_langfuse_module)
    monkeypatch.setitem(sys.modules, "shadowbotagents.trace.protocol", fake_protocol_module)
    monkeypatch.setitem(sys.modules, "shadowbotagents.trace.context_events", fake_context_events_module)
