...
class TestURLDetection(unittest.TestCase):
    ...
    def _detect(self, url):
        from shadowbot.db.adapter import ShadowBotDB
        db = ShadowBotDB.__new__(ShadowBotDB)
    ...
    def test_standard_postgres_still_works(self):
        url = "postgresql://postgres:pass@localhost:5432/shadowbot"
        self.assertEqual(self._detect(url), "postgres")
...
class TestRetryLogic(unittest.TestCase):
    ...
    @patch('shadowbot.persistence.conversation.postgres.psycopg2', create=True)
...
class TestConvenienceClasses(unittest.TestCase):
    ...
    def test_neondb_class_exists(self):
        from shadowbot.db.adapter import NeonDB
        self.assertTrue(callable(NeonDB))

    def test_cockroachdb_class_exists(self):
        from shadowbot.db.adapter import CockroachDB as CockroachDBClass
        self.assertTrue(callable(CockroachDBClass))

    def test_xatadb_class_exists(self):
        from shadowbot.db.adapter import XataDB
        self.assertTrue(callable(XataDB))

    def test_tursodb_class_exists(self):
        from shadowbot.db.adapter import TursoDB
        self.assertTrue(callable(TursoDB))

    def test_neondb_inherits_shadowbotdb(self):
        from shadowbot.db.adapter import NeonDB, ShadowBotDB
        self.assertTrue(issubclass(NeonDB, ShadowBotDB))

    def test_cockroachdb_inherits_shadowbotdb(self):
        from shadowbot.db.adapter import CockroachDB as CockroachDBClass, ShadowBotDB
        self.assertTrue(issubclass(CockroachDBClass, ShadowBotDB))
...
class TestConfigEnvVars(unittest.TestCase):
    ...
    def test_neon_env_var_exists(self):
        from shadowbot.persistence.config import ENV_VARS
    ...
    def test_turso_in_conversation_backends(self):
        from shadowbot.persistence.config import CONVERSATION_BACKENDS
