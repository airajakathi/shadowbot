...
class TestGapG2ProviderDefaultTools:
    """Test Gap G2: ShadowBotUI provider default tools."""

    def test_provider_agent_kwargs_include_reflection(self):
        """Provider should set reflection=True by default."""
        # This tests the pattern, not the actual provider (which is in ShadowBotUI)
        default_kwargs = {
            "name": "Assistant",
...
class TestGapG3ChannelBotTools:
    ...
    def test_channel_bot_agent_pattern_includes_tools(self):
        ...
        # This tests the pattern, not the actual channel bot (which is in ShadowBotUI)
        from shadowbot.tool_resolver import ToolResolver
