...
1. New simplified imports work: from shadowbotagents import db
2. New simplified imports work: from shadowbot import db
3. Old imports emit DeprecationWarning: from shadowbot.db import PraisonDB
"""
...
class TestNewDbImports:
    ...
    def test_import_db_from_shadowbotagents(self):
        """Test: from shadowbotagents import db"""
        import sys
        ...
        if isinstance(db, type(sys)):
            from shadowbotagents.db import db as db_proxy
            db = db_proxy
        assert hasattr(db, 'PraisonDB')
        assert callable(db)  # db(...) should work as shortcut
    
    def test_import_db_from_shadowbot(self):
        """Test: from shadowbot import db
        
        Note: This returns the shadowbot.db module (for backwards compat).
        The recommended pattern is: from shadowbotagents import db
        """
        import sys
        ...
        if isinstance(db, type(sys)) and not hasattr(db, 'PraisonDB'):
            from shadowbotagents.db import db as db_proxy
            db = db_proxy
        # shadowbot.db is a module with lazy-loaded classes
        assert hasattr(db, 'PraisonDB')
    ...
    def test_import_agent_and_db_together(self):
        """Test: from shadowbotagents import Agent, db"""
        from shadowbotagents import Agent, db
        assert Agent is not None
        assert db is not None
    
    def test_import_all_common_symbols(self):
        """Test: from shadowbotagents import Agent, Agents, Task, tool, db"""
        from shadowbotagents import Agent, Agents, Task, tool, db
        assert Agent is not None
...
class TestDeprecationWarnings:
    ...
    def test_praisondb_import_warns(self):
        """Test: from shadowbot.db import PraisonDB emits warning"""
        with warnings.catch_warnings(record=True) as w:
            warnings.simplefilter("always")
            from shadowbot.db import PraisonDB
            ...
            assert "shadowbotagents" in msg or "shadowbot" in msg
    
    def test_postgresdb_import_warns(self):
        """Test: from shadowbot.db import PostgresDB emits warning"""
        with warnings.catch_warnings(record=True) as w:
            warnings.simplefilter("always")
            from shadowbot.db import PostgresDB
    ...
    def test_sqlitedb_import_warns(self):
        """Test: from shadowbot.db import SQLiteDB emits warning"""
        with warnings.catch_warnings(record=True) as w:
            warnings.simplefilter("always")
            from shadowbot.db import SQLiteDB
    ...
    def test_redisdb_import_warns(self):
        """Test: from shadowbot.db import RedisDB emits warning"""
        with warnings.catch_warnings(record=True) as w:
            warnings.simplefilter("always")
            from shadowbot.db import RedisDB
