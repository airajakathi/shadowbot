...
class TestStepTypeDetection:
    ...
    def test_detect_agent_step(self):
        ...
        from shadowbot.cli.features.job_workflow import JobWorkflowExecutor
...
class TestAgentStepExecution:
    ...
    @patch('shadowbot.cli.features.job_workflow.JobWorkflowExecutor._resolve_agent_tools')
    ...
    def test_agent_step_import_error(self):
        """Agent step should handle missing shadowbotagents gracefully."""
        from shadowbot.cli.features.job_workflow import JobWorkflowExecutor
        
        executor = JobWorkflowExecutor({"steps": []}, "test.yaml")
        
        with patch.dict('sys.modules', {'shadowbotagents': None}):
            with patch('builtins.__import__', side_effect=ImportError("No module")):
                ...
                assert "shadowbotagents not installed" in result["error"]
...
class TestJudgeStepExecution:
    ...
    def test_judge_step_with_input_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch('shadowbotagents.eval.Judge') as MockJudge:
                mock_judge = Mock()
                mock_result = Mock()
...
class TestApproveStepExecution:
    ...
    def test_approve_step_auto_approve(self):
        ...
        with patch('shadowbotagents.approval.backends.AutoApproveBackend') as MockBackend:
            mock_backend = Mock()
            mock_decision = Mock()
