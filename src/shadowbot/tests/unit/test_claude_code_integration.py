#!/usr/bin/env python3
"""
Test script for Claude Code integration with ShadowBot Agents
...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), 'src', 'shadowbot'))

def test_claude_code_tool_import():
    """Test that we can import the Claude Code tool function"""
    try:
        from shadowbot.ui.code import claude_code_tool
...
def test_shadowbot_agents_import():
    """Test shadowbotagents import availability"""
    try:
        from shadowbotagents import Agent
        print("✅ ShadowBot Agents is available")
        return True
    except ImportError as e:
        print(f"⚠️  ShadowBot Agents not available (fallback to litellm): {e}")
        return False
...
async def run_tests():
    """Run all tests"""
    print("🧪 Testing Claude Code Integration with ShadowBot Agents")
    print("=" * 60)
    
    tests = [
        ("Import Claude Code Tool", test_claude_code_tool_import),
        ("Import ShadowBot Agents", test_shadowbot_agents_import),
