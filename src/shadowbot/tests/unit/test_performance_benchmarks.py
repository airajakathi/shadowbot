"""
Performance benchmark tests for ShadowBot.
...
# Get the paths for shadowbot and shadowbotagents
PRAISONAI_PATH = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
PRAISONAI_AGENTS_PATH = os.path.join(os.path.dirname(PRAISONAI_PATH), "shadowbot-agents")
...
class TestImportTimeBenchmarks:
    ...
    def test_shadowbot_wrapper_import_time(self):
        """Test that shadowbot wrapper imports quickly (lazy loading)."""
        code = '''
import time
t = time.time()
import shadowbot
...
'''
        ...
        assert elapsed_ms < PRAISONAI_WRAPPER_IMPORT_TIME_MS, \
            f"shadowbot import took {elapsed_ms:.0f}ms, expected < {PRAISONAI_WRAPPER_IMPORT_TIME_MS}ms"
...
heavy_modules = ['shadowbot.auto', 'shadowbot.agents_generator', 'litellm', 'instructor']
...
'''
    ...
    def test_shadowbotagents_not_imported_at_cli_startup(self):
        """Verify shadowbotagents is not imported when CLI starts."""
        code = '''
import sys
import os
os.environ['LOGLEVEL'] = 'WARNING'
from shadowbot.cli.main import ShadowBot
# Check if shadowbotagents is NOT loaded
if 'shadowbotagents' in sys.modules:
    print("FAIL: shadowbotagents loaded")
else:
    print("OK")
'''
        result = run_python_code(code)
        assert result.returncode == 0, f"Test failed: {result.stderr}"
        output = result.stdout.strip()
        assert output == "OK", f"shadowbotagents loaded at startup: {output}"
...
import threading
import time
from shadowbotagents.tools.registry import ToolRegistry
...
class TestSchemaVersioning:
    ...
    def test_schema_version_constant_exists(self):
        ...
        code = '''
from shadowbotagents.db.protocol import SCHEMA_VERSION
