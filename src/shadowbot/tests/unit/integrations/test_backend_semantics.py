...
def test_hosted_agent_imports():
    ...
    from shadowbot.integrations import HostedAgent, HostedAgentConfig
    assert HostedAgent is not None
    assert HostedAgentConfig is not None
    
    # Also test top-level imports
    from shadowbot import HostedAgent as TopLevelHostedAgent
    from shadowbot import HostedAgentConfig as TopLevelHostedAgentConfig
...
def test_local_agent_imports():
    ...
    from shadowbot.integrations import LocalAgent, LocalAgentConfig
    assert LocalAgent is not None
    assert LocalAgentConfig is not None
    
    # Also test top-level imports  
    from shadowbot import LocalAgent as TopLevelLocalAgent
    from shadowbot import LocalAgentConfig as TopLevelLocalAgentConfig
...
def test_managed_agent_deprecation_warnings():
    ...
    from shadowbot.integrations.managed_agents import ManagedAgent
    
    # LLM routing providers should emit deprecation warning
    with warnings.catch_warnings(record=True) as w:
        warnings.simplefilter("always")
        
        with patch('shadowbot.integrations.managed_local.LocalManagedAgent'):
...
def test_managed_agent_compute_provider_warnings():
    ...
    from shadowbot.integrations.managed_agents import ManagedAgent
    from shadowbot.integrations.managed_local import LocalManagedAgent
...
def test_managed_agent_anthropic_passthrough():
    ...
    from shadowbot.integrations.managed_agents import ManagedAgent, AnthropicManagedAgent
    
    with patch('shadowbot.integrations.managed_agents.AnthropicManagedAgent') as mock_anthropic:
...
def test_backward_compatibility_all_old_names():
    ...
    from shadowbot.integrations.managed_agents import (
        ManagedAgent, ManagedConfig, AnthropicManagedAgent,
        ManagedAgentIntegration, ManagedBackendConfig
    )
    from shadowbot.integrations.managed_local import (
        LocalManagedAgent, LocalManagedConfig
    )
    from shadowbot.integrations.sandboxed_agent import (
        SandboxedAgent, SandboxedAgentConfig
    )
    
    # Top-level imports
    from shadowbot import (
...
def test_config_aliases():
    ...
    from shadowbot.integrations import (
        HostedAgent, HostedAgentConfig, LocalAgent, LocalAgentConfig
    )
    from shadowbot.integrations.managed_agents import ManagedConfig
    from shadowbot.integrations.managed_local import LocalManagedConfig
