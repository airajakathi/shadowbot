...
Tests the bridge between shadowbotagents.tools and shadowbot.mcp_server registry.
"""

from unittest.mock import patch, MagicMock
from shadowbot.mcp_server.adapters.tools_bridge import (
    is_bridge_available,
    _infer_tool_hints,
    _extract_category,
...
class TestBridgeAvailability:
    ...
    def test_is_bridge_available_when_installed(self):
        """Test detection when shadowbotagents is installed."""
    ...
    def test_is_bridge_available_when_not_installed(self):
        """Test detection when shadowbotagents is not installed."""
        with patch("importlib.util.find_spec") as mock_find_spec:
...
class TestCategoryExtraction:
    ...
    def test_extract_category_standard(self):
        ...
        assert _extract_category("shadowbot.memory.show") == "memory"
        assert _extract_category("shadowbot.web.search") == "web"
        assert _extract_category("shadowbot.file.read") == "file"
...
class TestBridgeRegistration:
    ...
    @patch("shadowbot.mcp_server.adapters.tools_bridge.is_bridge_available")
    def test_register_skips_when_unavailable(self, mock_available):
        """Test registration skips when shadowbotagents not available."""
        from shadowbot.mcp_server.adapters.tools_bridge import register_shadowbot_tools
        
        mock_available.return_value = False
        count = register_shadowbot_tools()
        assert count == 0
    
    @patch("shadowbot.mcp_server.adapters.tools_bridge.is_bridge_available")
    @patch("shadowbot.mcp_server.adapters.tools_bridge.get_tool_mappings")
    @patch("shadowbot.mcp_server.registry.get_tool_registry")
    def test_register_tools_from_mappings(self, mock_get_registry, mock_mappings, mock_available):
        ...
        assert count == 1
        assert "shadowbot.agents.test.tool" in mock_registry._tools
