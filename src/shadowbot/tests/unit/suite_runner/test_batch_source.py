"""
Tests for BatchSource - discovers Python files with ShadowBot imports.
...
class TestBatchSourceImportDetection:
    ...
    def test_detects_shadowbotagents_import(self):
        """Should detect 'from shadowbotagents import ...'"""
        from shadowbot.suite_runner.batch_source import BatchSource
...
"""
        ...
        assert source._has_shadowbot_import(content) is True
    
    def test_detects_shadowbot_import(self):
        """Should detect 'from shadowbot import ...'"""
        from shadowbot.suite_runner.batch_source import BatchSource
...
"""
    ...
    def test_detects_import_shadowbotagents(self):
        """Should detect 'import shadowbotagents'"""
        from shadowbot.suite_runner.batch_source import BatchSource
        
        content = """
import shadowbotagents

agent = shadowbotagents.Agent()
"""
        source = BatchSource(root=Path("."))
        assert source._has_shadowbot_import(content) is True
    
    def test_rejects_non_shadowbot_file(self):
        """Should reject files without ShadowBot imports."""
        from shadowbot.suite_runner.batch_source import BatchSource
...
"""
        source = BatchSource(root=Path("."))
        assert source._has_shadowbot_import(content) is False
...
class TestBatchSourceDiscovery:
    ...
    def test_discovers_files_in_current_directory(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Create a file with shadowbot import (not a test file name)
            (root / "my_agent.py").write_text("from shadowbotagents import Agent\n")
            
            # Create a file without shadowbot import
            (root / "other.py").write_text("print('hello')\n")
            
            # Create a subdirectory with a file (should not be found without recursive)
            subdir = root / "subdir"
            subdir.mkdir()
            (subdir / "nested.py").write_text("from shadowbotagents import Agent\n")
            
            source = BatchSource(root=root, recursive=False)
            items = source.discover()
            
            # Should only find my_agent.py (has shadowbot import, in root)
            assert len(items) == 1
    ...
    def test_discovers_files_recursively(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Create files
            (root / "root_agent.py").write_text("from shadowbotagents import Agent\n")
            
            subdir = root / "subdir"
            subdir.mkdir()
            (subdir / "nested_agent.py").write_text("from shadowbot import Agent\n")
    ...
    def test_respects_depth_limit(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            (root / "level0.py").write_text("from shadowbotagents import Agent\n")
            
            level1 = root / "level1"
            level1.mkdir()
            (level1 / "level1.py").write_text("from shadowbotagents import Agent\n")
            
            level2 = level1 / "level2"
            level2.mkdir()
            (level2 / "level2.py").write_text("from shadowbotagents import Agent\n")
    ...
    def test_excludes_test_files_by_default(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Create test files
            (root / "test_agent.py").write_text("from shadowbotagents import Agent\n")
            (root / "agent_test.py").write_text("from shadowbotagents import Agent\n")
            (root / "conftest.py").write_text("from shadowbotagents import Agent\n")
...
""")
    ...
    def test_detects_agents_usage(self):
        """Should detect Agents/ShadowBotAgents class usage."""
        from shadowbot.suite_runner.batch_source import BatchSource
...
class TestBatchSourceGroups:
    ...
    def test_assigns_root_group_for_top_level_files(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            (root / "agent.py").write_text("from shadowbotagents import Agent\n")
    ...
    def test_assigns_directory_group_for_nested_files(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            subdir.mkdir()
            (subdir / "agent.py").write_text("from shadowbotagents import Agent\n")
    ...
    def test_get_groups_returns_all_groups(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            (root / "root.py").write_text("from shadowbotagents import Agent\n")
            
            dir1 = root / "agents"
            dir1.mkdir()
            (dir1 / "a.py").write_text("from shadowbotagents import Agent\n")
            
            dir2 = root / "tools"
            dir2.mkdir()
            (dir2 / "b.py").write_text("from shadowbotagents import Agent\n")
...
""")
    ...
    def test_filter_agents_only(self):
        """Should filter to only scripts using Agents/ShadowBotAgents class."""
        from shadowbot.suite_runner.batch_source import BatchSource
...
from shadowbotagents import Agent, Workflow
