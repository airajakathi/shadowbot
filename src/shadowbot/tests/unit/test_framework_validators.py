"""
Unit tests for shadowbot.framework_adapters.validators.assert_framework_available().
...
try:
    ...
    from shadowbot.framework_adapters.registry import FrameworkAdapterRegistry
...
class TestAssertFrameworkAvailableRaises:
    ...
    def test_crewai_hint_mentions_shadowbot_extra(self):
        with patch.object(FrameworkAdapterRegistry, "get_instance") as mock_get:
            mock_registry = MagicMock()
            mock_registry.is_available.return_value = False
            mock_get.return_value = mock_registry
            ...
            assert "shadowbot[crewai]" in error_msg or "pip install crewai" in error_msg
...
class TestAssertFrameworkAvailableSucceeds:
    ...
    def test_returns_none_for_available_framework(self):
        with patch.object(FrameworkAdapterRegistry, "get_instance") as mock_get:
            ...
            result = assert_framework_available("shadowbot")
