...
in ``ShadowBot.parse_args``) rather than internal helpers. Each subprocess
...
def _build_env():
    ...
    short-circuit argparse (see ``ShadowBot.parse_args`` in-test-env check).
    """
    env = {
    ...
    pythonpath_parts = [
        os.path.join(REPO_ROOT, "src", "shadowbot-agents"),
        os.path.join(REPO_ROOT, "src", "shadowbot"),
    ]
...
def _run_cli(*args, timeout=30):
    """Invoke the shadowbot CLI with the given argv and return CompletedProcess."""
    return subprocess.run(
        [sys.executable, "-m", "shadowbot.cli.main", *args],
        capture_output=True,
        text=True,
        timeout=timeout,
        cwd=REPO_ROOT,
...
def test_backends_list_subcommand():
    """``shadowbot backends list`` prints the registered backend ids."""
...
def test_backends_bare_subcommand_lists():
    """``shadowbot backends`` (no sub-sub-arg) defaults to listing.

    NOTE: this assertion is coupled to the ``backends`` handler in
    ``shadowbot.cli.main`` which treats a missing/None subcommand as ``list``.
...
def test_backends_unknown_subcommand_reports_error():
    """``shadowbot backends bogus`` prints an error message.

    NOTE: the error string comes from the ``backends`` handler in
    ``shadowbot.cli.main`` (``[red]Unknown backends subcommand: ...[/red]``),
