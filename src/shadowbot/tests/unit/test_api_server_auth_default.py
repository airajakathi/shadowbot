...
def import_api_server(monkeypatch):
    # Stub the shadowbot package import so the server module loads in
    # isolation against a controlled ShadowBot workflow class.
    stub = types.ModuleType("shadowbot")

    class _DummyShadowBot:
        def __init__(self, agent_file="agents.yaml"):
            self.agent_file = agent_file
        def run(self):
            return {"ran": True, "agent_file": self.agent_file}

    stub.ShadowBot = _DummyShadowBot
    monkeypatch.setitem(sys.modules, "shadowbot", stub)
