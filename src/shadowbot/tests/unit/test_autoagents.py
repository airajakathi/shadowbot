...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', '..', '..', 'shadowbot-agents'))

try:
    from shadowbotagents.agents.autoagents import AutoAgents, AutoAgentsConfig, AgentConfig, TaskConfig
    from shadowbotagents.llm import OpenAIClient, LLM
...
class TestAutoAgents:
    ...
    @patch('shadowbotagents.llm.supports_structured_outputs')
    @patch('shadowbotagents.agents.autoagents.get_openai_client')
    def test_generate_config_openai_success(self, mock_get_client, mock_supports_structured, sample_valid_config, mock_tools):
        ...
        with patch.object(AutoAgents, '_create_agents_and_tasks') as mock_create, \
             ...
             patch('shadowbotagents.agents.agents.Agents.__init__') as mock_super_init:
    ...
    @patch('shadowbotagents.llm.supports_structured_outputs')
    @patch('shadowbotagents.agents.autoagents.LLM')
