...
class TestPythonFeatureExtractor:
    ...
    def test_extract_lazy_imports(self):
        ...
        from shadowbot._dev.parity.python_extractor import PythonFeatureExtractor
        
        # Create a mock __init__.py with _LAZY_IMPORTS
        mock_init = '''
_LAZY_IMPORTS = {
    'Agent': ('shadowbotagents.agent.agent', 'Agent'),
    'Tool': ('shadowbotagents.tools.base', 'Tool'),
}
'''
        with tempfile.TemporaryDirectory() as tmpdir:
            pkg_dir = Path(tmpdir) / "src" / "shadowbot-agents" / "shadowbotagents"
            pkg_dir.mkdir(parents=True)
            init_file = pkg_dir / "__init__.py"
            init_file.write_text(mock_init)
            
            # Also create wrapper structure
            wrapper_dir = Path(tmpdir) / "src" / "shadowbot" / "shadowbot" / "cli" / "features"
            ...
            assert lazy_imports['Agent'] == ('shadowbotagents.agent.agent', 'Agent')
...
'''
    ...
    def test_categorize_module_path(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert extractor._categorize('shadowbotagents.agent.agent') == 'agent'
            assert extractor._categorize('shadowbotagents.tools.base') == 'tools'
            assert extractor._categorize('shadowbotagents.memory.memory') == 'memory'
            assert extractor._categorize('shadowbotagents.workflows') == 'workflows'
            assert extractor._categorize('shadowbotagents.unknown') == 'other'
    ...
    def test_extract_cli_features(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            features_dir = Path(tmpdir) / "src" / "shadowbot" / "shadowbot" / "cli" / "features"
...
class TestTypeScriptFeatureExtractor:
    ...
    def test_extract_exports(self):
        ...
        from shadowbot._dev.parity.typescript_extractor import TypeScriptFeatureExtractor
...
'''
        with tempfile.TemporaryDirectory() as tmpdir:
            ts_dir = Path(tmpdir) / "src" / "shadowbot-ts" / "src"
...
class TestParityTrackerGenerator:
    ...
    def test_generate_structure(self):
        ...
        from shadowbot._dev.parity.generator import ParityTrackerGenerator
...
''')
    ...
    def test_write_rust_creates_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Create rust dir
            rust_dir = Path(tmpdir) / "src" / "shadowbot-rust"
...
class TestGenerateParityTracker:
    ...
    def test_stdout_mode(self, capsys):
        ...
        from shadowbot._dev.parity.generator import generate_parity_tracker
...
class TestRealExtraction:
    """Integration tests using the real ShadowBot repository."""
    
    @pytest.mark.skipif(
        not Path("/Users/praison/shadowbot-package").exists(),
