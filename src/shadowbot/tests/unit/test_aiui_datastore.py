"""Tests for ShadowBot → aiui datastore adapter."""
...
class TestShadowBotSessionDataStore:
    """Test the datastore adapter that bridges ShadowBot sessions to aiui."""

    def test_init_with_default_store(self):
        """Test initialization with default hierarchical store."""
        with patch('shadowbot.ui._aiui_datastore.get_hierarchical_session_store') as mock_get_store:
            mock_store = Mock()
            mock_get_store.return_value = mock_store
            
            adapter = ShadowBotSessionDataStore()
    ...
    def test_init_with_custom_store(self):
        ...
        mock_store = Mock()
        adapter = ShadowBotSessionDataStore(store=mock_store)
