...
from shadowbot.templates.tool_override import (
...
PAYLOAD = textwrap.dedent(
    """
    import os, tempfile
    _MARKER = os.path.join(tempfile.gettempdir(), 'shadowbot_autoload_marker.txt')
...
def _clean_marker(tmp_path: Path):
    import tempfile
    marker = Path(tempfile.gettempdir()) / "shadowbot_autoload_marker.txt"
    if marker.exists():
        marker.unlink()
    yield
    if marker.exists():
...
def _marker_path() -> Path:
    import tempfile
    return Path(tempfile.gettempdir()) / "shadowbot_autoload_marker.txt"
