#!/usr/bin/env python3
"""
Live Smoke Tests for ShadowBot Recipe System
...
"""
    ...
    def test_cli_list_command(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "shadowbot.cli.main", "recipe", "list", "--json"],
            capture_output=True,
            text=True,
            timeout=30,
        )
    ...
    def test_cli_help_command(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "shadowbot.cli.main", "recipe", "help"],
            capture_output=True,
            text=True,
            timeout=10,
        )
        ...
        assert result.returncode == 0
        assert "shadowbot recipe" in result.stdout.lower() or "shadowbot recipe" in result.stdout
...
class TestLiveSecurity:
    ...
    def test_pii_redaction(self, tmp_path):
        """Test PII redaction."""
        from shadowbot.recipe.security import redact_pii, detect_pii
...
class TestLivePolicy:
    ...
    def test_policy_workflow(self, tmp_path):
        ...
        from shadowbot.recipe.policy import PolicyPack, get_default_policy, PolicyDeniedError
...
def run_smoke_tests():
    ...
    print("=" * 60)
    print("ShadowBot Live Smoke Tests")
    print("=" * 60)
