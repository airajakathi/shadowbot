...
This runner is CLI-FIRST: all scenarios execute via `shadowbot code` or `shadowbot chat`
...
PRAISONAI_PATH = Path(__file__).parent.parent.parent.parent / "shadowbot"
AGENTS_PATH = Path(__file__).parent.parent.parent.parent / "shadowbot-agents"
...
class Scenario:
    ...
    use_code_command: bool = True  # Use `shadowbot code` vs `shadowbot chat`
...
def create_working_dir() -> Path:
    ...
    work_dir = Path(tempfile.mkdtemp(prefix="shadowbot_code_editor_"))
...
def build_cli_env(extra_env: Optional[dict] = None) -> dict:
    ...
    # Set PYTHONPATH to include shadowbot and shadowbot-agents
    paths = []
    if PRAISONAI_PATH.exists():
        paths.append(str(PRAISONAI_PATH.parent))  # Parent of shadowbot package
...
def run_shadowbot_cli(
    ...
    """
    Run shadowbot CLI command and return (exit_code, stdout, stderr, cli_command).
    
    This is the CLI-FIRST approach: we invoke `python -m shadowbot code` or 
    `python -m shadowbot chat` as a subprocess, exactly as a user would.
    
    We use `python -m shadowbot` instead of the `shadowbot` entry point to ensure
    ...
    Args:
        ...
        use_code: Use `shadowbot code` (True) or `shadowbot chat` (False)
    ...
    cmd = [sys.executable, "-m", "shadowbot", "code" if use_code else "chat"]
    ...
    # Human-readable command for logging (using shadowbot directly)
    cli_command = f"shadowbot {'code' if use_code else 'chat'} -m {effective_model} -w {workspace} \"{prompt[:50]}...\""
...
def run_scenario(scenario: Scenario, work_dir: Path) -> ScenarioResult:
    ...
    This executes `shadowbot code` or `shadowbot chat` as a subprocess,
    ...
    # Run the CLI command
    exit_code, stdout, stderr, cli_command = run_shadowbot_cli(
