...
class TestShadowBotFramework(unittest.TestCase):
    @pytest.mark.real
    def test_main_with_agents_advanced(self):
        shadowbot = ShadowBot(agent_file="tests/agents-advanced.yaml")
    ...
    def test_main_with_autogen_framework(self):
        shadowbot = ShadowBot(agent_file="tests/autogen-agents.yaml")
        result = shadowbot.run()
        print(f"Result: {result}")
        self.assertIsNotNone(result)
    ...
    def test_main_with_custom_framework(self):
        shadowbot = ShadowBot(agent_file="tests/crewai-agents.yaml")
        result = shadowbot.run()
        print(f"Result: {result}")
        self.assertIsNotNone(result)
    ...
    def test_main_with_internet_search_tool(self):
        shadowbot = ShadowBot(agent_file="tests/search-tool-agents.yaml")
        result = shadowbot.run()
        print(f"Result: {result}")
        self.assertIsNotNone(result)
    ...
    def test_main_with_built_in_tool(self):
        shadowbot = ShadowBot(agent_file="tests/inbuilt-tool-agents.yaml")
        result = shadowbot.run()
        print(f"Result: {result}")
        self.assertIsNotNone(result)
    

class TestShadowBotCommandLine(unittest.TestCase):
    ...
    @pytest.mark.real
    def test_shadowbot_command(self):
        # Test basic shadowbot command
        command = "shadowbot --framework autogen --auto \"create a 2-agent team to write a simple python game\""
        result = self.run_command(command)
        print(f"Result: {result}")
        self.assertIn('TERMINATE', result)

    @pytest.mark.real
    def test_shadowbot_init_command(self):
        # Test shadowbot --init command
        # This command primarily creates files, but let's ensure it uses the real key if any underlying ShadowBot init involves API calls
        command = "shadowbot --framework autogen --init \"create a 2-agent team to write a simple python game\""
        result = self.run_command(command)
        print(f"Result: {result}")
