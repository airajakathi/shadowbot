# Claude Code Integration with ShadowBot Agents

This document describes the integration of Claude Code with ShadowBot Agents, allowing AI agents to intelligently decide when to use Claude Code for file modifications and coding tasks.

## Overview

The refactored implementation replaces direct `litellm` usage with `shadowbotagents`, making Claude Code a custom tool that the AI agent can choose to use based on the user's request. This provides better intelligence and flexibility while maintaining backward compatibility.
...
- Claude Code is implemented as a tool function for ShadowBot Agents
...
In the ShadowBot Code UI, you can:
...
- If `shadowbotagents` unavailable → Falls back to `litellm`
...
### File Structure
```
src/shadowbot/shadowbot/
├── cli.py                 # Added --claudecode flag
└── ui/
    └── code.py           # Refactored to use shadowbotagents
```

### Key Functions

1. **`handle_with_shadowbot_agents()`** - Main agent execution handler
...
**Required:**
- `shadowbotagents` - Core agent framework
...
1. Ensure `shadowbotagents` is installed
2. Update ShadowBot to include the refactored code
...
- Ensure `shadowbotagents` is installed
...
**Fallback to litellm:**
- Install `shadowbotagents`: `pip install shadowbotagents`
...
### Planned Features
- Real-time streaming from ShadowBot Agents
...
For technical support or questions, please refer to the main ShadowBot documentation or submit issues to the repository.